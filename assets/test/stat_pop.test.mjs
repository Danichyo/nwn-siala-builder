...
function mountTrigger(extra = {}) {
  ...
  if (extra.extraTitle) el.dataset.popExtraTitle = extra.extraTitle;
  el.dataset.popExtraTerms = JSON.stringify(extra.extraTerms || []);
...
// Задача 3.197 часть B («против заклинаний» у Fort/Ref/Will) — второй,
// ОТДЕЛЬНО подписанный блок под тем же разбором. Регрессия для этого места
// нужна ровно потому, что `Phoenix.LiveViewTest` хук не исполняет вовсе
// (AGENTS.md): LiveView-тест проверяет только атрибуты `data-pop-extra-*`
// на триггере, а рендер их в панель — код этого хука, и без JS-теста он
// никем не проверяется.
test("«против заклинаний»: второй блок рисуется ПОД разделителем, своим заголовком", () => {
  dom.window.happyDOM.setViewport({ width: BREAKPOINT_PX + 1 });
  const t = mountTrigger({
    extraTitle: "против заклинаний",
    extraTerms: [{ label: "Spellcraft (43 ранга)", value: "+8" }]
  });

  click(t.el);
  const panel = panelEl();

  const title = panel.querySelector(".stat-pop-extra-title");
  assert.ok(title, "заголовок второго блока обязан появиться");
  assert.equal(title.textContent, "против заклинаний");

  const extraRows = panel.querySelectorAll(".stat-pop-term-extra");
  assert.equal(extraRows.length, 1);
  assert.ok(extraRows[0].textContent.includes("Spellcraft"));

  // Основной терм (BAB) остаётся первым и не помечен как «extra» — два
  // списка не перепутаны местами и не слиты в один.
  const mainRows = panel.querySelectorAll(".stat-pop-term:not(.stat-pop-term-extra)");
  assert.equal(mainRows.length, 1);
  assert.ok(mainRows[0].textContent.includes("BAB"));

  unmount(t);
});

test("без extra_terms второго блока нет вовсе — заголовок не рисуется в пустоту", () => {
  dom.window.happyDOM.setViewport({ width: BREAKPOINT_PX + 1 });
  const t = mountTrigger(); // data-pop-extra-terms="[]", без extraTitle

  click(t.el);
  const panel = panelEl();

  assert.equal(panel.querySelector(".stat-pop-extra-title"), null);
  assert.equal(panel.querySelectorAll(".stat-pop-term-extra").length, 0);
