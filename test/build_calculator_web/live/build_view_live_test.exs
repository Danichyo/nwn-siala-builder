defmodule BuildCalculatorWeb.BuildViewLiveTest do
  ...
  describe "totals show the arithmetic behind them" do
    ...
    # Задача 3.197 часть B (замер `AQ1`) — та же строка «против заклинаний»,
    # что и в конструкторе, тем же вызовом `Summary.saves_vs_spells_note/1`.
    # ⚠️ Фикстура — соркерер 40 с 43 рангами Spellcraft, тот же билд, каким
    # это уже проверяется в конструкторе (`builder_live_test.exs`), — числа
    # обязаны совпасть на обоих экранах, иначе разбор одного и того же билда
    # расходится по адресу.
    test "«против заклинаний» видна и строкой, и подразделом разбора", %{
      conn: conn,
      ruleset: ruleset
    } do
      build =
      ...
      # ⚠️ **ЗДЕСЬ СТОЯЛО «+8»** — до задачи 3.202, пока прибавка считалась
      # от рангов. Теперь она от полного значения навыка: 43 ранга +
      # модификатор INT +2 = 45 → **+9**.
      note = render(element(view, "#view-saves-vs-spells"))
      assert note =~ "против заклинаний"
      assert note =~ "Fort +9"
      assert note =~ "Ref +9"
      assert note =~ "Will +9"

      # Подраздел разбора — своим блоком, а не термом внутри «+22»:
      # `v-terms` печатает арифметику листа, `v-terms-extra` — справку рядом.
      #
      # ✅ 3.202 B: подпись печатает значение навыка (45 → +9) и ранги в скобках.
      # Здесь стояло «Spellcraft (43 ранга)» — долг веб-слоя из отчёта 3.202, закрыт.
      fort_html = render(element(view, "#view-stat-fort"))
      assert fort_html =~ "против заклинаний"
      assert fort_html =~ "Spellcraft 45 (43 ранга)"
    end

    # Положительный контроль: без единого ранга Spellcraft строке и
    # подразделу взяться неоткуда — билд из шапки файла (воин/мастер оружия)
    # ничего не покупал в Spellcraft.
    test "без рангов Spellcraft строки «против заклинаний» нет", %{conn: conn, code: code} do
      {:ok, view, _html} = live(conn, ~p"/b/#{code}")

      refute has_element?(view, "#view-saves-vs-spells")
      refute render(element(view, "#view-stat-fort")) =~ "против заклинаний"
    end
    ...
    # флажок Адры и НЕ даёт Сагры, а флажок помечен — правило чистоты Адры
    # не процитировано. Взято с реальной страницы «Паладин Адры» (revid 19670):
    # паладин 38 / монах 1 / чемпион Торма 1.
    #
    # ⚠️ Здесь стояло «правило чистоты Адры не описано никем» — устарело
    # 11.09.2026 (задача 3.182): его называют серверные скрипты шарда, но
    # не про саму Адру, а про родственную функцию, поэтому пометка осталась,
    # а её ТЕКСТ сменился («выведено из разбора…»). Сам текст проверяется
    # в `summary_test.exs`, здесь — что пометка доезжает до разметки.
    test "билд Адры получает свой флажок с пометкой", %{conn: conn, ruleset: ruleset} do
      adra =
        Build.new(
          ruleset_version: ruleset.version,
          race: :elf,
  ...
  describe "Резисты (поглощение стихийного урона) — задачи 3.211, 3.216" do
    # Гном-сагровик 40 с полуторным мечом (не топором!) — числа сверены
    # `Rules.Resistances`'s own test suite (`resistances_test.exs`).
    defp dwarf_sagra_40(ruleset, gear_extra \\ []) do
      Build.new(
        ruleset_version: ruleset.version,
        race: :dwarf,
        levels: List.duplicate(:fighter, 40),
        base_abilities: %{str: 10, dex: 10, con: 10, int: 10, wis: 10, cha: 10},
        gear: Gear.new([weapon: :longsword, feats: [:siala_blade_proficiency]] ++ gear_extra)
      )
    end

    # id всех карточек `.v-stats` в порядке документа — для проверок места
    # секции (3.216: «в самый конец её за скилл поинты поставить»).
    defp stat_card_ids(view) do
      view
      |> render()
      |> LazyHTML.from_fragment()
      |> LazyHTML.query(".v-stats .v-stat")
      |> LazyHTML.attribute("id")
    end

    defp resist_row_ids(view) do
      view
      |> element("#view-stat-resistances")
      |> render()
      |> LazyHTML.from_fragment()
      |> LazyHTML.query(".v-stat-group")
      |> LazyHTML.attribute("id")
    end

    defp popup_terms(builder, id) do
      builder
      |> element("##{id}")
      |> render()
      |> LazyHTML.from_fragment()
      |> LazyHTML.query("##{id} .stat-pop-trigger")
      |> LazyHTML.attribute("data-pop-terms")
      |> hd()
      |> Jason.decode!()
    end

    test "билд без поглощения — секции резистов нет вовсе", %{conn: conn, ruleset: ruleset} do
      {:ok, view, _html} = live(conn, ~p"/b/#{Encoding.encode(monk_up_to(ruleset, 1))}")

      refute has_element?(view, "#view-stat-resistances")
      refute "view-stat-resistances" in stat_card_ids(view)
    end

    # 🔴 Задача 3.216 (слово Dan 18.09.2026: «все резисты в одну секцию надо
    # объединить… слагаемые было бы здорово получить, давай оставим их»):
    # семь карточек-на-стихию (3.211) стали ОДНОЙ секцией, у каждой стихии
    # своя строка с итогом и свой разбор под ней.
    test "расовый эффект Гнома-сагровика — одна секция, строка на стихию, разбор у каждой", %{
      conn: conn,
      ruleset: ruleset
    } do
      {:ok, view, _html} = live(conn, ~p"/b/#{Encoding.encode(dwarf_sagra_40(ruleset))}")

      assert has_element?(view, "#view-stat-resistances")

      for %{id: id} <- ruleset.resistance_energy_types do
        row = caption_text(render(element(view, "#view-resist-#{id}")))
        assert row =~ "18"
        assert row =~ "Гном +18"

        # Прежних карточек по стихии больше нет — секция одна.
        refute has_element?(view, "#view-stat-resist_#{id}")
      end
    end

    test "секция стоит в самом конце сетки, сразу после «Скилл-поинтов»", %{
      conn: conn,
      ruleset: ruleset
    } do
      {:ok, view, _html} = live(conn, ~p"/b/#{Encoding.encode(dwarf_sagra_40(ruleset))}")

      ids = stat_card_ids(view)
      assert List.last(ids) == "view-stat-resistances"
      assert Enum.at(ids, -2) == "view-stat-skill_points"
    end

    test "стихии идут в порядке словаря ruleset'а", %{conn: conn, ruleset: ruleset} do
      {:ok, view, _html} = live(conn, ~p"/b/#{Encoding.encode(dwarf_sagra_40(ruleset))}")

      assert resist_row_ids(view) ==
               Enum.map(ruleset.resistance_energy_types, &"view-resist-#{&1.id}")
    end

    # Общего итога у резистов нет — большого числа-заголовка у секции нет тоже
    # (выдумывать агрегат нельзя): `.v` не печатается, вместо него строки стихий.
    test "у секции нет числа-заголовка", %{conn: conn, ruleset: ruleset} do
      {:ok, view, _html} = live(conn, ~p"/b/#{Encoding.encode(dwarf_sagra_40(ruleset))}")

      refute has_element?(view, "#view-stat-resistances > .v")
      assert has_element?(view, "#view-stat-resistances[data-groups=\"1\"]")
    end

    # 🔴 Задача 3.211: разбор одинаков на ДВУХ экранах — оба читают
    # `Summary.resistance_terms`, и разъехаться они могут молча (та же
    # ловушка, что у SR выше). Список один и тот же ВСЕГДА (3.218):
    # проигравшая сторона в него не входит ни там, ни там — на просмотре её
    # нет вовсе (3.216), в конструкторе она во втором блоке поп-апа.
    test "разбор на просмотре и в конструкторе — один и тот же список", %{
      conn: conn,
      ruleset: ruleset
    } do
      code = Encoding.encode(dwarf_sagra_40(ruleset))

      {:ok, viewer, _html} = live(conn, ~p"/b/#{code}")
      {:ok, builder, _html} = live(conn, ~p"/?b=#{code}")

      caption = caption_text(render(element(viewer, "#view-resist-fire")))
      terms = popup_terms(builder, "stat-resist-fire")

      assert %{"label" => "Гном", "value" => "+18"} in terms
      sentence = Enum.map_join(terms, " ", &"#{&1["label"]} #{&1["value"]}")
      assert caption =~ sentence
    end

    # 🔴 Задача 3.216, слово Dan: «то, что не засчитано, я бы убрал, чтобы
    # не загромождать UI». Вещь на огонь 15 проигрывает эффекту 18
    # (наибольшее, не сумма): на просмотре строки о ней НЕТ, а сумма
    # слагаемых равна итогу стихии; в поп-апе конструктора она названа —
    # с 3.218 отдельным блоком «не вошло», а не строкой среди слагаемых.
    test "проигравшая вещь: на просмотре её нет, в поп-апе конструктора — блоком «не вошло»", %{
      conn: conn,
      ruleset: ruleset
    } do
      code = Encoding.encode(dwarf_sagra_40(ruleset, resistances: %{fire: 15}))

      {:ok, viewer, _html} = live(conn, ~p"/b/#{code}")
      {:ok, builder, _html} = live(conn, ~p"/?b=#{code}")

      fire = render(element(viewer, "#view-resist-fire"))
      refute fire =~ "не вошла"
      refute fire =~ "+15"

      doc = LazyHTML.from_fragment(fire)

      assert LazyHTML.query(doc, ".v-stat-group-value") |> LazyHTML.text() |> String.trim() ==
               "18"

      term_sum =
        doc
        |> LazyHTML.query(".tv")
        |> Enum.map(&(&1 |> LazyHTML.text() |> String.trim() |> String.to_integer()))
        |> Enum.sum()

      assert term_sum == 18

      # Основной список поп-апа — тот же, что на просмотре, и тоже сходится
      # с итогом; проигравшая вещь — во втором блоке под своим заголовком.
      assert popup_terms(builder, "stat-resist-fire") == [%{"label" => "Гном", "value" => "+18"}]

      trigger =
        builder
        |> element("#stat-resist-fire")
        |> render()
        |> LazyHTML.from_fragment()
        |> LazyHTML.query("#stat-resist-fire .stat-pop-trigger")

      assert LazyHTML.attribute(trigger, "data-pop-extra-title") == ["не вошло — берётся большее"]

      assert trigger |> LazyHTML.attribute("data-pop-extra-terms") |> hd() |> Jason.decode!() ==
               [%{"label" => "вещь", "value" => "+15"}]
    end

    # Круглая ссылка: то, что игрок вписал в «Вещах» конструктора, доезжает
    # до просмотра тем же числом (постановка 3.211 — «ссылка с резистами
    # открывается на экране просмотра тем же числом»). У человека эффекта
    # нет, вещь единственное слагаемое — и печатается как «вещь».
    test "ссылка с вписанным поглощением открывается на просмотре тем же числом", %{
      conn: conn,
      ruleset: ruleset
    } do
      build =
        Build.new(
          ruleset_version: ruleset.version,
          race: :human,
          levels: List.duplicate(:fighter, 1),
          base_abilities: %{str: 10, dex: 10, con: 10, int: 10, wis: 10, cha: 10},
          gear: Gear.new(resistances: %{fire: 15, cold: 15})
        )

      {:ok, view, _html} = live(conn, ~p"/b/#{Encoding.encode(build)}")

      assert resist_row_ids(view) == ["view-resist-cold", "view-resist-fire"]
      assert caption_text(render(element(view, "#view-resist-fire"))) =~ "вещь +15"
      assert caption_text(render(element(view, "#view-resist-cold"))) =~ "15"
    end
  end
  ...
  describe "the way out" do
    ...
    # Задача 3.179 — «скачать .txt» стоит РЯДОМ со «скопировать текст», не
    # в диалоге `#text-dialog`: Dan, «мне важно, чтобы обе кнопки стояли
    # в одном месте, а не одна в диалоге, вторая снаружи».
    #
    # ⚠️ Задача 3.180 развела их по ИСТОЧНИКУ (Dan, 10.09.2026: «выводить
    # ссылку в попапе смысла нет, она на этот момент доступна и так»):
    # «скопировать» переехал на `#view-export-text` (уже существующий
    # `<pre>` диалога «показать текст» — он и так всегда в DOM), «скачать»
    # остался на `#export-source`, который теперь несёт не просто
    # `@export_text`, а `Export.file_text/5` — тот же текст плюс ссылка.
    test "download and copy sit next to each other in #view-actions, but now read different sources",
         %{conn: conn, code: code} do
      {:ok, view, _html} = live(conn, ~p"/b/#{code}")

      assert has_element?(
               view,
               "#view-actions button#copy-text[data-target='#view-export-text']"
             )

      assert has_element?(
               view,
               "#view-actions button#view-download[data-target='#export-source']"
             )

      # Фикстура — Fighter 20 / Weapon Master 21 (модуля `setup`), капнутый
      # билд ровно на 41.
      assert has_element?(
               view,
               "button#view-download[data-filename='lvl41_fighter_20_weapon_master_21.txt']"
             )

      # Ссылка в файле — тот же адрес, на котором стоит сам экран
      # (постановка 3.180: «на экране просмотра это текущий адрес»).
      file = render(element(view, "#export-source"))
      assert file =~ "Собран в конструкторе:"
      assert file =~ "/b/#{code}"

      # А попап и «скопировать» её по-прежнему не несут (тот же довод, что
      # у конструктора) — прямой вызов, а не догадка по подстроке "Собран".
      refute render(element(view, "#view-export-text")) =~ "Собран в конструкторе"
    end

    # Задача 3.180: сохранённый билд открывают и по `/b/:code`, и по
    # `/builds/:id` — но ссылка в файле обязана называть форму, которая
    # работает для ЛЮБОГО получателя без аккаунта и видимости
    # (CLAUDE.md, «Длинная ссылка остаётся канонической»), а не адрес,
    # с которого САМ читатель сейчас смотрит: `/builds/:id` завязан на
    # видимость (`Library.fetch_build/2`), приватный билд по нему не
    # откроется никому, кроме своей группы.
    test "a saved build's file still names the /b/:code link, not /builds/:id", %{conn: conn} do
      build = build_fixture(user_scope_fixture(), %{visibility: :public})

      {:ok, view, _html} = live(conn, ~p"/builds/#{build}")

      file = render(element(view, "#export-source"))
      assert file =~ "/b/#{build.code}"
      refute file =~ "/builds/#{build.id}"
    end
  end
