defmodule BuildCalculatorWeb.Builder.SummaryTest do
  ...
  describe "save_terms/3" do
    ...
    # 🔴 **ЗДЕСЬ БЫЛО «вклад Spellcraft назван поимённо»**, и задача 3.197
    # (часть A) перевернула утверждение: прибавка Spellcraft — **условная**
    # (против заклинаний), и в самом спасе её нет (замер `AQ1`, Dan
    # 12.09.2026). В разборе ЭТОГО числа её быть не может: слагаемое, которого
    # нет в значении, ломает единственное правило разбора — сходиться
    # с числом, которое он объясняет.
    #
    # 📋 Показывать её по-прежнему обязательно (CLAUDE.md §3 — «заметный вклад,
    # который игроки не считают»), но СВОЕЙ строкой «против заклинаний»:
    # это часть B задачи 3.197. Здесь проверяется, что ядро её отдаёт, а разбор
    # листа — не печатает.
    test "вклад Spellcraft из разбора спаса УШЁЛ — он условный", %{ruleset: ruleset} do
      # Sorcerer со Spellcraft: +1 ко всем трём сейвам за каждые 5 рангов
      # против заклинаний.
      stats = spellcraft_build(ruleset)

      for save <- [:fort, :ref, :will] do
        refute Enum.any?(Summary.save_terms(ruleset, stats, save), &(&1.label == "Spellcraft"))
        refute caption(ruleset, stats, Atom.to_string(save)) =~ "Spellcraft"
      end

      # Положительный контроль: число не потеряно, оно лежит рядом и ждёт
      # части B.
      assert stats.skill_save_bonus > 0
      assert stats.conditional_save_bonus.fort > 0
      assert Enum.map(stats.conditional_save_terms, & &1.scope) == ["spells"]
    end

    # ⚠️ **ЗДЕСЬ ПРОВЕРЯЛСЯ СРЕЗ ОБЩЕГО ПОТОЛКА вещей и Spellcraft** — до задачи
    # 3.197. В самом спасе этих двух источников больше не встречается: в нём
    # только вещи, и срезать их нечем, пока игрок не вписал больше +20. Сам
    # общий потолок никуда не делся — он виден на условном числе, и это
    # проверяется в ядре (`save_bonuses_test.exs`), а не здесь.
    #
    # Осталось то, что принадлежит разбору: **сумма слагаемых равна числу**,
    # и срез появляется тогда и только тогда, когда потолок кусает.
    test "срез печатается ровно тогда, когда потолок кусает", %{ruleset: ruleset} do
      exact = spellcraft_build(ruleset, %Gear{saves: 20})
      over_cap = spellcraft_build(ruleset, %Gear{saves: 26})

      # Ровно в потолок — среза нет…
      terms = Summary.save_terms(ruleset, exact, :will)
      refute Enum.any?(terms, &(&1.label == "сверх капа"))

      assert Enum.sum(Enum.map(terms, &(&1.value |> Integer.parse() |> elem(0)))) ==
               exact.save_bonus.will

      # …а сверх потолка — есть, и сумма по-прежнему сходится с числом.
      terms = Summary.save_terms(ruleset, over_cap, :will)
      ...
      assert Enum.sum(Enum.map(terms, &(&1.value |> Integer.parse() |> elem(0)))) ==
               over_cap.save_bonus.will

      assert caption(ruleset, over_cap, "will") =~ "сверх капа #{over}"
    end
    ...
    # Задача 3.187 (часть B): у Хнюпиуса вещи бьют в Стойкость ДВАЖДЫ —
    # `Universal +16` и `Fortitude +12` разом (`test/fixtures/game_logs_plus/
    # hnyupius.log`), и разбор обязан печатать ОДНУ строку «вещи» с их
    # суммой, а не полагаться на universal-only `gear_save_bonus`, иначе
    # арифметика под числом не сходится с самим числом.
    test "раздельная прибавка складывается с universal в ОДНОЙ строке «вещи»", %{
      ruleset: ruleset,
      build: %Build{} = build
    } do
      gear = %Gear{saves: 5, saves_specific: %{fort: 4}}
      stats = Rules.compute(%Build{build | gear: gear}, ruleset)

      assert Summary.save_terms(ruleset, stats, :fort) == [%{label: "вещи", value: "+9"}]

      # А у Reflect'а и Воли раздельной прибавки нет — там по-прежнему только
      # universal, и число совпадает с тем, что ушло бы туда и раньше.
      assert Summary.save_terms(ruleset, stats, :ref) == [%{label: "вещи", value: "+5"}]
      assert caption(ruleset, stats, "fort") =~ "вещи +9"
    end

    # Живое число задачи: у Хнюпиуса вещевая Стойкость 16 + 12 = 28 при
    # потолке 20 — разбор обязан показать И сумму слагаемых, И срез, а их
    # сумма обязана сойтись с тем, что ядро реально применило
    # (`stats.save_bonus.fort`), а не с 28.
    test "потолок режет сумму СЛАГАЕМЫХ вещей, а не одно из двух полей", %{
      ruleset: ruleset,
      build: %Build{} = build
    } do
      gear = %Gear{saves: 16, saves_specific: %{fort: 12}}
      stats = Rules.compute(%Build{build | gear: gear}, ruleset)

      terms = Summary.save_terms(ruleset, stats, :fort)

      assert terms == [%{label: "вещи", value: "+28"}, %{label: "сверх капа", value: "-8"}]

      assert Enum.sum(Enum.map(terms, &(&1.value |> Integer.parse() |> elem(0)))) ==
               stats.save_bonus.fort

      assert stats.save_bonus.fort == 20
    end

    # Групповая подпись (панель итогов, не попап конкретного сейва) остаётся
    # universal-only и НЕ обязана видеть раздельную прибавку — печатать её
    # означало бы соврать про Ref и Will, у которых своей прибавки нет.
    test "групповая подпись читает только universal", %{ruleset: ruleset, build: %Build{} = build} do
      gear = %Gear{saves: 16, saves_specific: %{fort: 12}}
      stats = Rules.compute(%Build{build | gear: gear}, ruleset)

      assert Summary.save_extras_terms(stats) == [%{label: "вещи", value: "+16"}]
    end
    ...
    test "новое слагаемое ядра появляется без правки разметки", %{
      ...
      # ⚠️ Пример сменился с `skill_save_bonus` на `gear_save_bonus` (задача
      # 3.197): первый ушёл из разбора спаса вместе с самой прибавкой, а
      # контракт «поле читается по имени, а его отсутствие — ноль» проверяется
      # любым слагаемым этого списка.
      stats = Rules.compute(build, ruleset)

      appeared = %{
        stats
        | gear_save_bonus: 3,
          gear_save_bonuses: %{fort: 3, ref: 3, will: 3},
          save_bonus: %{fort: 3, ref: 3, will: 3}
      }

      assert Summary.save_terms(ruleset, stats, :fort) == []
      assert Summary.save_terms(ruleset, appeared, :fort) == [%{label: "вещи", value: "+3"}]

      # Поля вовсе нет — читается нулём, а не роняет разбор.
      assert Summary.save_terms(
               ruleset,
               appeared |> Map.delete(:gear_save_bonus) |> Map.delete(:gear_save_bonuses),
               :fort
             ) == []
    end
  end
  ...
  describe "ab_terms/2" do
    ...
    # 🔴 **Здесь стоял контроль «на 39-м расового терма нет вовсе» — переписан
    # 11.09.2026 (задача 3.181).** Довод был верен: величина ниже 40-го
    # не известна никому, и «раса +0» утверждала бы, что бонуса нет. Теперь
    # величина известна (разбор серверных скриптов шарда), и на 39-м уровне
    # терм обязан стоять — со своим числом ступени, а не с сорокового.
    #
    # 39-й — тир 5, билд из воинов, то есть сагровик: floor(5 × 3 / 2) = 7.
    test "на 39-м уровне расовый терм свой, а не сороковой", %{ruleset: ruleset} do
      %Build{} = build = half_elf_40(ruleset)
  ...
  describe "ac_naked_terms/2 и ac_geared_terms/2" do
    ...
    # 🔴 Задача 3.184: усиление мини-сетами — ОТДЕЛЬНАЯ строка разбора, а не
    # прибавка внутрь расового или оружейного терма. Иначе разбор приписал бы
    # усиление одному из двух, тогда как считается оно от их суммы.
    #
    # ⚠️ Подпись — «Мини-сеты», а не сырой id: у терма `source` свой
    # (`{:mini_sets, :mini_sets}`), и общая ветка напечатала бы `mini_sets`.
    test "усиление мини-сетами названо своей строкой", %{ruleset: ruleset} do
      gnome = fn sets ->
        Build.new(
          race: :gnome,
          levels: List.duplicate(:fighter, 39) ++ [:bard],
          gear: Gear.new(weapon: :longsword, feats: [:siala_blade_proficiency], mini_sets: sets)
        )
      end

      bare = Rules.compute(gnome.([]), ruleset)
      with_sets = Rules.compute(gnome.([10]), ruleset)

      refute Enum.any?(Summary.ac_geared_terms(ruleset, bare), &(&1.label == "Мини-сеты"))

      terms = Summary.ac_geared_terms(ruleset, with_sets)

      # Расовый и оружейный термы не сдвинулись — прибавка стоит рядом с ними.
      assert %{label: "Карлик", value: "+6"} in terms
      assert %{label: "Longsword", value: "+6"} in terms
      assert %{label: "Мини-сеты", value: "+6"} in terms

      # ...и разбор сходится со своим же итогом, как и у всех термов рядом.
      assert sum_terms(terms) == with_sets.ac_geared
    end
  ...
  describe "racial_bonus_note/2 (задача 3.102)" do
    ...
    test "у непосчитанного справки нет — говорит гэп", %{ruleset: ruleset} do
      ...
      # 🔴 Третье «нет» было про уровень ниже 40-го — снято 11.09.2026 (задача
      # 3.181): величина известна на любом уровне, бонус там ПОСЧИТАН, и
      # справка обязана быть, иначе игрок не узнает, какой из четырёх вариантов
      # взят. Осталось на своём месте у снапшота БЕЗ таблицы тиров — там
      # величины и правда не знает никто.
      #
      # ⚠️ Ruleset синтетический, не живая запись (урок 3.85).
      without = put_in(ruleset.racial_bonuses.scaling, nil)
      below = compute(without, List.duplicate(:fighter, 39), race: :half_elf, weapon: :club)

      assert Summary.racial_bonus_note(without, below) == nil
      assert {:missing_data, {:racial_bonus_level, :half_elf}} in below.gaps
    end

    # ⚠️ Вид бонуса без получателя (урон Могучего человека) молчит по той же
    # причине, по которой молчит гэп: калькулятор про него не отвечает вовсе
    # (CLAUDE.md §9, решение Dan 16.08.2026).
    #
    # 🔴 **Гном из этого кейса выбыл 18.09.2026 (задача 3.210), и это НЕ правка
    # решения, а снятие его посылки:** у поглощения стихийного урона появился
    # получатель — секция «Резисты» (`DerivedStats.resistances`), — значит его
    # 12/18 посчитаны, и справка у них обязана быть ровно та же, что у трёх
    # остальных рас: игрок не узнает иначе, какой из четырёх вариантов взят.
    # Положительный контроль на это — следом.
    test "вид бонуса, которого мы не считаем, справки не печатает", %{ruleset: ruleset} do
      for race <- [:half_orc] do
        assert Summary.racial_bonus_note(ruleset, armed_40(ruleset, race)) == nil, "#{race}"
      end
    end

    # Положительный контроль к выбытию Гнома: без него кейс выше зеленел бы
    # и у правки, которая просто убрала расу из списка.
    test "поглощение Гнома справку ПЕЧАТАЕТ — получатель появился", %{ruleset: ruleset} do
      note = Summary.racial_bonus_note(ruleset, armed_40(ruleset, :dwarf))
  ...
  describe "skill_rows/3" do
    ...
    # 🔴 БИЛДОВ ЗДЕСЬ ЧЕТЫРЕ, А НЕ ОДИН (задача 3.220). До неё цикл гонялся по
    # одному эльфу, у которого и бонус за тип оружия, и усиление кусками равны
    # нулю, — и потому молчал ДВАЖДЫ подряд, пока подпись экрана просмотра
    # теряла сначала `weapon_type_bonus` (3.35), потом `mini_set_bonus` (3.185).
    # Ненулевыми их делают древковое у Человека, магический посох (три навыка
    # сразу, 3.183) и куски мини-сетов.
    #
    # ⚠️ Каждый билд несёт СВОЙ положительный контроль ниже по файлу: цикл
    # «сумма сходится» зеленеет и там, где проверяемого слагаемого нет вовсе,
    # ровно этим он и обманул нас два раза.
    test "разбор сходится с числом, которое он объясняет", %{ruleset: ruleset} do
      ...
      builds = [
        {"эльф", elf_build(ruleset)},
        {"Человек с древковым", polearm_human(ruleset)},
        {"магический посох", magic_staff_human(ruleset)},
        {"куски мини-сетов", mini_set_human(ruleset, [5, 5])}
      ]

      for {name, build} <- builds,
          row <- Summary.skill_rows(ruleset, build, stats(ruleset, build)) do
        assert sum(row.from) == row.value, "#{name}, #{row.name}: «#{row.from}» ≠ #{row.value}"
      end
    end

    # ============ бонус за тип оружия в подписи (задача 3.220) ===============
    #
    # 🔴 Живой баг, воспроизведённый вызовом 18.09.2026: Дисциплина 43, подпись
    # «4 р. · STR +3 · Человек +18» — восемнадцати очков в ней нет. Число ядро
    # считало верно с задачи 3.35; врала только подпись экрана просмотра, то есть
    # шаренная ссылка у чужого читателя.
    test "бонус за тип оружия назван в подписи, а не только в поп-апе", %{ruleset: ruleset} do
      build = polearm_human(ruleset)
      value = stats(ruleset, build).skill_values.discipline
      row = row(ruleset, build, :discipline)

      # Положительный контроль: терм действительно ненулевой, иначе проверка
      # ниже зеленела бы, ничего не проверив.
      assert value.weapon_type_bonus == 18

      assert row.value == 40
      assert row.from == "4 р. · STR +0 · Человек +18 · тип оружия +18"
      assert sum(row.from) == row.value

      # ⚠️ Подпись и поп-ап — один список, а не два похожих: разойтись им
      # теперь негде, и это проверяется сравнением самих термов.
      assert Summary.skill_value_terms(ruleset, value) == [
               %{label: "ранги", value: "4"},
               %{label: "STR", value: "+0"},
               %{label: "Человек", value: "+18"},
               %{label: "тип оружия", value: "+18"}
             ]
    end

    # Магический посох (задача 3.183) — восьмой тип оружия у движка: он бьёт
    # сразу в ТРИ навыка и не даёт Дисциплине ничего. Поэтому им проверяются
    # три подписи разом и четвёртая — отрицательным контролем.
    test "бонус магического посоха стоит в подписи всех трёх своих навыков", %{ruleset: ruleset} do
      build = magic_staff_human(ruleset)
      ...
      for {skill, ability} <- [
        ...
        row = row(ruleset, build, skill)

        assert stats.skill_values[skill].weapon_type_bonus == 18
        assert row.value == 19
        assert row.from == "1 р. · #{ability} +0 · тип оружия +18", "#{skill}: «#{row.from}»"
        assert sum(row.from) == row.value
      end

      # 🔴 Отрицательный контроль: Дисциплина от посоха не получает НИЧЕГО —
      # это древковое оружие даёт дисциплину, а посох свой, восьмой тип
      # (`CLAUDE.md` §3, «не путать с „Посохами“ сводной таблицы»). Расовый
      # бонус Человека при этом на месте: оружие в руках его включает любое.
      discipline = row(ruleset, build, :discipline)

      assert stats.skill_values.discipline.weapon_type_bonus == 0
      refute discipline.from =~ "тип оружия"
      assert discipline.from == "1 р. · STR +0 · Человек +18"
      assert sum(discipline.from) == discipline.value
    end

    # ============ усиление кусками в подписи (задача 3.220) ==================
    #
    # Второй недовезённый терм, тем же маршрутом и с тем же следствием.
    test "усиление кусками мини-сетов названо в подписи", %{ruleset: ruleset} do
      build = mini_set_human(ruleset, [5, 5])
      value = stats(ruleset, build).skill_values.discipline
      row = row(ruleset, build, :discipline)
      ...
      assert row.value == 40
      assert row.from == "4 р. · STR +0 · Человек +12 · тип оружия +12 · Мини-сеты +12"
      assert sum(row.from) == row.value
    end

    # ⚠️ Положительный контроль обратной стороны: у билда БЕЗ кусков подпись
    # ровно та, что была до задачи, — ни одной лишней строки. Терм с нулём
    # не печатается, и именно поэтому правка не сдвинула ни один разбор
    # у билдов без древкового и без мини-сетов.
    test "без кусков подпись остаётся прежней", %{ruleset: ruleset} do
      row = row(ruleset, mini_set_human(ruleset, []), :discipline)

      assert row.from == "4 р. · STR +0 · Человек +12 · тип оружия +12"
      refute row.from =~ "Мини-сеты"
      assert sum(row.from) == row.value
    end
    ...
    test "вклад Spellcraft в сейвы не подмешивается в значение навыка", %{ruleset: ruleset} do
      ...
      # Положительный контроль: прибавка к сейвам существует и лежит на своей
      # стороне. ⚠️ ЗДЕСЬ СТОЯЛ счёт строк «Spellcraft» в разборе спаса — с
      # задачи 3.197 её там нет вовсе (прибавка условная, `AQ1`), а число
      # отдаётся своим полем.
      assert stats.skill_save_bonus == 8
      assert stats.conditional_save_bonus == %{fort: 8, ref: 8, will: 8}
    ...
    # 🔴 **Здесь стоял контроль «на 39-м терма нет» — переписан 11.09.2026
    # (задача 3.181).** Терм есть и на 39-м, просто со своим числом: тир 5,
    # единица Дисциплины вдвое больше тира, билд воинский (сагровик) —
    # floor(10 × 3 / 2) = 15.
    test "на 39-м уровне расовый терм у навыка свой, а не сороковой", %{ruleset: ruleset} do
      ...
      assert row.value == 19
      assert row.from == "4 р. · STR +0 · Человек +15"
    end
  ...
  describe "усиление кусками мини-сетов в разборах (задача 3.185)" do
    # 🔴 Разбор, который не складывается в стоящее рядом значение, хуже
    # отсутствующего — правило этого файла с первой его строки. До задачи
    # 3.185 терм усиления не входил НИ в разбор AB, ни в разбор значения
    # навыка, и найдено это было прогоном: у Светлого эльфа 40 с луком
    # и четырьмя кусками сумма термов AB давала 46 при AB 50, а Дисциплина
    # у Человека с древковым и десятью — 30 при значении 42.
    defp mini_set_human(ruleset, counts) do
      %Build{gear: %Gear{} = gear} = build = human_discipline_40(ruleset)

      # Один уровень барда выводит билд из группы Сагры — иначе он упирается
      # в кап исполнителя (36 у Дисциплины) ещё до кусков, и усиление даёт
      # ровно ноль. Это законный ответ модели, но проверять им сходимость
      # разбора нельзя: нулевой терм не печатается вовсе.
      levels = List.duplicate(:fighter, 39) ++ [:bard]
    ...
    test "терм стоит своей строкой и разбор сходится с итогом", %{ruleset: ruleset} do
      build = mini_set_human(ruleset, [5, 5])
      value = stats(ruleset, build).skill_values.discipline
      ...
      assert %{label: "Мини-сеты", value: "+12"} in terms

      assert terms
             |> Enum.map(fn %{value: v} ->
               v |> String.replace("+", "") |> String.to_integer()
             end)
             |> Enum.sum() == value.total
    end

    # ⚠️ Отдельной строкой, а не внутри «раса»/«тип оружия», и это не вкус:
    # усиление считается от их СУММЫ (в движке это один эффект,
    # `Rules.MiniSets`), поэтому приписать его любому из двух было бы
    # неправдой в разборе.
    test "усиление не сливается ни с расовым бонусом, ни с типом оружия",
         %{ruleset: ruleset} do
      terms =
        Summary.skill_value_terms(
          ruleset,
          stats(ruleset, mini_set_human(ruleset, [5, 5])).skill_values.discipline
        )

      assert %{label: "Человек", value: "+12"} in terms
      assert %{label: "тип оружия", value: "+12"} in terms
    end

    # Положительный контроль: без кусков разбор ровно тот же, что был до
    # задачи, — ни одной лишней строки. Ровно это и есть критерий «билд без
    # мини-сетов не изменился нигде».
    test "без кусков строки нет вовсе", %{ruleset: ruleset} do
      terms =
        Summary.skill_value_terms(
          ruleset,
          stats(ruleset, mini_set_human(ruleset, [])).skill_values.discipline
        )

      refute Enum.any?(terms, &(&1.label == "Мини-сеты"))
    end

    # ⚠️ Одинокий кусок не даёт ничего вовсе — главная ловушка правила,
    # и в разборе это выглядит как отсутствие строки, а не как «+0».
    test "одинокие куски строки не заводят", %{ruleset: ruleset} do
      terms =
        Summary.skill_value_terms(
          ruleset,
          stats(ruleset, mini_set_human(ruleset, [1, 1])).skill_values.discipline
        )
  ...
  describe "class_group_flags/2 — цитата, вывод и молчание (задача 3.182)" do
    # 🔴 Флажок группы отвечает не на «сколько у меня BAB», а на «сагровик я или
    # нет», и рядом с ним обязано стоять КАЧЕСТВО факта, из которого он выведен.
    # Качеств теперь три, и у каждого своя фраза.
    #
    # ⚠️ Механизм проверяется СИНТЕТИЧЕСКИМИ членствами, а не живыми данными:
    # живая запись назавтра получает правку и молча перестаёт что-либо
    # проверять (уроки задач 3.85, 3.90, 3.93).
    test "процитированное правило подписи не несёт", %{ruleset: ruleset} do
      [flag] = Summary.class_group_flags(ruleset, stats_with(purity_status: "verified"))

      refute flag.assumed?
      refute flag.title =~ "⚠"
      assert flag.title =~ "Все уровни билда — из этих классов"
    end

    test "выведенное правило названо выводом, а не цитатой и не молчанием", %{ruleset: ruleset} do
      [flag] = Summary.class_group_flags(ruleset, stats_with(purity_status: "derived"))

      assert flag.assumed?
      assert flag.title =~ "выведено из разбора серверных скриптов шарда"

      # ⚠️ И ровно не наоборот: «применяем правило Сагры» было бы неправдой —
      # правило названо источником, а не взято по аналогии.
      refute flag.title =~ "применяем правило Сагры"
    end

    test "неназванное правило по-прежнему называет себя допущением", %{ruleset: ruleset} do
      stats = stats_with(purity_required?: true, purity_stated?: false, purity_status: nil)
      [flag] = Summary.class_group_flags(ruleset, stats)

      assert flag.assumed?
      assert flag.title =~ "применяем правило Сагры"
      refute flag.title =~ "серверных скриптов"
    end

    # 🔴 Умолчание — В СТОРОНУ ПОКАЗА: статус, которого подпись не знает, это не
    # цитата, а молчание читается игроком как цитата. Сегодня таких записей нет
    # ни одной, и проверяется поэтому механизм, а не живая запись.
    test "статус, который не цитата и не вывод, всё равно помечен", %{ruleset: ruleset} do
      for status <- ["unclear", "custom", nil] do
        [flag] = Summary.class_group_flags(ruleset, stats_with(purity_status: status))

        assert flag.assumed?, "status #{inspect(status)}: подпись выдала вывод за цитату"
        assert flag.title =~ "не процитировано"
      end
    end

    # 🔴 Значок и текст, объясняющий значок, считает одна функция — иначе они
    # разойдутся молча, и игрок получит «?» без причины либо причину без «?».
    test "значок и подпись не могут разойтись", %{ruleset: ruleset} do
      for status <- ["verified", "derived", "unclear", nil],
          stated? <- [true, false] do
        stats = stats_with(purity_status: status, purity_stated?: stated?)
        [flag] = Summary.class_group_flags(ruleset, stats)

        assert flag.assumed? == String.contains?(flag.title, "⚠"),
               "status #{inspect(status)}/#{stated?}: значок и подпись разошлись"
      end
    end

    # И живая половина: сегодня у Адры стоит именно `derived`, то есть фраза
    # выше доезжает до настоящего билда, а не только до синтетики.
    test "у настоящего билда Адры подпись называет скрипты", %{ruleset: ruleset} do
      stats =
        Build.new(
          ruleset_version: ruleset.version,
          levels: List.duplicate(:paladin, 20),
          base_abilities: %{str: 10, dex: 10, con: 10, int: 10, wis: 10, cha: 10}
        )
        |> Rules.compute(ruleset)

      assert [%{id: :adra_warriors} = flag] = Summary.class_group_flags(ruleset, stats)
      assert flag.assumed?
      assert flag.title =~ "выведено из разбора серверных скриптов шарда"
    end
  end

  # Одно синтетическое членство — ровно те поля, которые читает подпись.
  defp stats_with(opts) do
    %{
      class_groups: [
        %{
          id: :test_group,
          ...
          purity_stated?: Keyword.get(opts, :purity_stated?, true),
          purity_status: Keyword.get(opts, :purity_status)
        }
      ]
    }
  end
  ...
  # Тот же Человек 40, но с ДРЕВКОВЫМ в руках: у Дисциплины к расовому бонусу
  # шарда прибавляется второй, свой терм — бонус за тип оружия (задача 3.35).
  # ⚠️ Фит владения обязателен: без него оружие «в руках» не считается вовсе
  # (замер Dan 15.08.2026, задача 3.36), и оба бонуса были бы нулём — то есть
  # кейс зеленел бы, не проверив ничего.
  defp polearm_human(ruleset) do
    %Build{} = build = human_discipline_40(ruleset)

    %Build{build | gear: Gear.new(weapon: :halberd, feats: [:siala_polearm_proficiency])}
  end

  # Магический посох (задача 3.183) — восьмой тип оружия у движка. Он здесь
  # не «ещё одно оружие»: фита владения не требует вовсе, а бонус кладёт сразу
  # в ТРИ навыка, то есть одним предметом проверяются три подписи.
  #
  # По одному рангу на навык — они кросс-классовые для воина (потолок 2 на 1-м
  # уровне, цена 2 очка): 6 очков из восьми, билд законный.
  defp magic_staff_human(ruleset) do
    Build.new(
      ruleset_version: ruleset.version,
      race: :human,
      levels: List.duplicate(:fighter, 40),
      base_abilities: %{str: 10, dex: 10, con: 10, int: 10, wis: 10, cha: 10},
      skills: %{1 => %{spellcraft: 1, concentration: 1, animal_empathy: 1, discipline: 1}},
      gear: Gear.new(weapon: :magic_staff)
    )
  end
