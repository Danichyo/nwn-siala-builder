defmodule BuildCalculatorWeb.BuilderLive do
  ...
  def mount(params, _session, socket) do
    ...
    socket =
      ...
      |> assign(:export_text, "")
      # Задача 3.180 — тот же попап-текст плюс ссылка на билд одной строкой
      # внизу (Dan, 10.09.2026: «выводить ссылку в попапе смысла нет, она
      # на этот момент доступна и так»); читает `#export-download`,
      # `#export-text`/«скопировать» её по-прежнему не несут. Пересчитывается
      # в тех же местах, что `:export_text` (`"export"`,
      # `"toggle_export_granted"` ниже).
      |> assign(:export_file_text, "")
      # Задача 3.179 — имя, которое `download_text/1` кладёт в `download`.
      # Пересчитывается вместе с `:export_text`, в тех же двух местах
      # (`"export"`, ниже); плейсхолдер здесь не виден никому — диалог
      # скрыт, пока `export_open?` не станет `true`.
      |> assign(:export_filename, "build.txt")
      ...
      # Состояние интерфейса блока «Вещи» — ТОЙ ЖЕ функцией, которой оно
      # забывается при замене билда целиком (задача 3.222). Свежий конструктор
      # и конструктор, которому только что подсунули чужой билд, обязаны
      # начинать блок «Вещи» с одного и того же места; пока эти десять ассайнов
      # стояли здесь списком руками, а в `forget_gear_ui/1` — списком ещё раз,
      # два списка могли разойтись молча (ровно форма бага 1.2, о которой
      # говорит сама `forget_gear_ui/1`). ⚠️ `:gear_open?` в неё НЕ входит
      # и стоит отдельной строкой выше: открыт блок или нет — это не то, что
      # забывается вместе с билдом.
      |> forget_gear_ui()
  ...
  def handle_params(params, uri, socket) do
    socket = assign(socket, :base_url, host_root(uri))
  ...
  defp load_code(socket, code, params) do
    case Encoding.decode(code) do
      {:ok, %{ruleset: ruleset, build: build, dropped: dropped}} ->
        ...
        socket
        |> assign(:ruleset, ruleset)
        |> show_loaded_build(build, code, active)
        |> warn_dropped(dropped)

      # 🔴 Задача 3.228: экран обязан совпасть с собственным сообщением. Все
      # пять текстов `Labels.decode_error/1` кончаются фразой «открыт пустой
      # конструктор» — и на свежей загрузке страницы это выходило само (билд
      # из `mount/3` и так пуст), а в живой сессии не выходило: прежний билд,
      # правила и всё UI-состояние оставались на экране, и только флеш
      # утверждал обратное. Перезагрузка той же страницы давала уже ТРЕТЬЕ
      # состояние — пустой конструктор, — то есть экран не переживал сам себя.
      #
      # Направление выбрано в пользу сообщения, а не наоборот («оставить билд
      # и переписать текст»): источник истины у конструктора — адрес, и
      # состояние «в адресе мусор, на экране чужой билд» не переживает
      # перезагрузку. Прежний билд при этом не теряется — его код лежит
      # в предыдущей записи истории браузера.
      #
      # ⚠️ Достижимость измерена, а не предположена (см. модульдок
      # `builder_broken_link_test.exs`): сегодня в интерфейсе НЕТ пути,
      # которым игрок приводит сюда битый код без перемонтирования —
      # `<.link patch>` в этот LiveView не ведёт ниоткуда, `/s/:key` и переходы
      # из библиотеки делают полную загрузку, а свои `push_patch` всегда несут
      # валидный `@code`. То есть это страховка, а не починка бага прода: цена
      # её — одна строка, а цена обратного — экран, спорящий со своим
      # сообщением, в тот день, когда путь появится.
      {:error, reason} ->
        socket
        |> put_flash(:error, Labels.decode_error(reason))
        |> show_loaded_build(empty_build(socket.assigns.ruleset), nil, 1)
        |> patch_to_own_code()
    end
  end

  # Билд, приехавший ИЗ АДРЕСА, встаёт на экран одним путём — и когда код
  # прочитался, и когда он битый (тогда билд пустой, задача 3.228). `code`
  # — прочитанная строка либо `nil`: битую в `:code` оставлять нельзя, из неё
  # строится `#share-link`, а `refresh/1` ниже пересчитает её из самого билда.
  #
  # 🔴 Почему НЕ `put_build/2`, хотя кнопка «сбросить» открывает тот же пустой
  # билд именно им: **его нельзя звать из `handle_params/3`**, и это измерено
  # падением, а не выведено. `mount/3` сознательно не делает `refresh/1`, когда
  # в адресе есть `b` (задача 3.66 — два `refresh` без рендера между ними
  # СКЛАДЫВАЮТ стримы фитов, а не заменяют), поэтому на первом проходе
  # `handle_params/3` ассайнов сцены (`:slots`, `:granted`, `:budget`,
  # `:class_choice`…) ещё не существует — а `put_build/2` первым же делом
  # снимает ими снимок цели гида (`first_guide_target/1` → `nav_sections/1`)
  # и падает `KeyError`. Позвать здесь сначала `refresh/1`, а потом
  # `put_build/2` — это ровно те два `refresh` подряд, которые запрещает 3.66.
  #
  # Поэтому «одна функция, а не вторая копия сброса» здесь выполнена иначе, чем
  # просила постановка, но по существу: ОДНО забывание UI (`forget_build_ui/1`
  # — общее с `reset` и с обоими импортами), ОДНО определение пустого билда
  # (`empty_build/1` — общее с `reset`) и ОДНА установка билда из адреса (эта
  # функция — общая у обеих ветвей `load_code/3`). Расходятся входы ровно там,
  # где они и так расходились до задачи: билд из ссылки идёт мимо воронки
  # `put_build/2` (решение 1.7).
  defp show_loaded_build(socket, %Build{} = build, code, active) do
    socket
    |> assign(:build, build)
    |> assign(:ladder_issues, ladder_issues(socket.assigns.ruleset, build))
    |> assign(:code, code)
    ...
    |> assign(:point_buy_reset, nil)
    # 🔴 Задача 3.226: ТА ЖЕ функция, что у импорта (`apply_imported/5`),
    # а не свой список ассайнов. Раньше список стоял здесь руками и был
    # НЕПОЛНЫМ — `:feat_slot` и `:preview` он не гасил, хотя их гасят все
    # прочие пути замены билда. Цена измерена
    # (`builder_load_code_reset_test.exs`): «Назад»/«Вперёд» браузера или
    # переход по другой ссылке `?b=` в той же вкладке оставляли на экране
    # подсвеченный чип слота ПРЕЖНЕГО билда (вместе с подсказкой «показаны
    # только фиты, которые примет этот слот» — указывающей на слот,
    # которого на новом уровне может не быть вовсе) и живой блок Δ
    # с призрачными `+N` от прежнего наведения.
    |> forget_build_ui()
    |> refresh()
  end

  # Задача 3.228, пункт 3: адрес обязан описывать то, что на экране. Битый код
  # после сброса остался бы в `?b=` до первой правки билда — и «Скопировать
  # ссылку» отдавала бы правильную ссылку (она строится из `@code`, а тот уже
  # пересчитан из пустого билда), а адресная строка и перезагрузка — прежний
  # мусор. Патчим на свой собственный код, тем же адресом `?b=…&l=…`, каким
  # его пишет `put_build/2`.
  #
  # 🔴 «Ровно ОДИН `push_patch` на событие» (грабли 3.163) соблюдено с запасом:
  # на этом проходе `handle_params/3` его не толкал никто — сюда приходят
  # только из ветки отказа `load_code/3`, а `put_build/2` из `handle_params/3`
  # не зовётся вовсе (см. `show_loaded_build/4`). Патч, вернувшись, попадёт
  # в клаузу `code == socket.assigns.code` и ничего не сделает второй раз.
  #
  # ⚠️ Только на живом сокете, и это не осторожность, а разные механизмы:
  # на МЁРТВОМ рендере (первая загрузка страницы) отложенный `push_patch`
  # `Phoenix.LiveView.Static` превращает в HTTP-редирект
  # (`Controller.live_render/3`, ветка `{:stop, %Socket{redirected: {:live, …}}}`),
  # то есть битая ссылка отвечала бы 302 вместо страницы — а мёртвый рендер
  # с битым кодом это ещё и предмет регрессии 3.66 («не удваивает список
  # фитов»), у которой тогда исчез бы сам объект проверки. Адрес от этого
  # кривым не остаётся: через миг тот же `handle_params/3` пройдёт ещё раз,
  # уже подключённым, и патч уйдёт отсюда же.
  defp patch_to_own_code(socket) do
    if connected?(socket) do
      push_patch(socket,
        to: ~p"/?b=#{socket.assigns.code}&l=#{socket.assigns.active}",
        replace: true
      )
    else
      socket
  ...
  defp go_to_level(socket, n) do
    ...
    # ⚠️ Задача 3.226: `forget_level_ui/1` — та же пятёрка ассайнов, которой
    # забывают уровневое состояние замена билда ссылкой и импорт, только без
    # блока «Вещи»: он принадлежит БИЛДУ, а здесь билд тот же, сменился лишь
    # уровень (см. `forget_build_ui/1` рядом с `forget_gear_ui/1`).
    socket =
      socket
      |> assign(:active, active)
      |> forget_level_ui()
  ...
  def handle_event("gear", params, socket) do
    ...
    gear = %Gear{
      ...
      | abilities: gear_numbers(ruleset, :abilities, params["ability"]),
        ...
        saves_specific: gear_saves_specific(params["saves_specific"]),
        resistances: gear_resistances(ruleset, params["resist"])
    }
  ...
  def handle_event("toggle_gear_feat", %{"feat" => feat} = params, socket) do
    ...
    with {:ok, id} <- Ids.fetch(ruleset, :feats, id_or_nil(feat)),
      ...
      entry = gear_feat_entry(id, choice)
  ...
  # ---- фиты с вещи: повторные взятия (задача 3.204, часть B) ----
  #
  # Стакающийся фит без выбора (`Rules.GearFeats.stackable?/2`, 16 штук на
  # siala_41 — `Epic toughness`, шесть `Great …`, `Improved spell
  # resistance`…) кладётся в `gear.feats` НЕСКОЛЬКО раз, и это уже НЕ
  # переключатель (`Gear.toggle_feat/3`), а счётчик — `Gear.add_feat/3`
  # добавляет ровно одну запись, `Gear.remove_feat/3` снимает ровно одну.
  # Тот же приём, что у кусков мини-сета (`add_mini_set`/`mini_set_more`/
  # `mini_set_less`): одна пара «+»/«−», воронка правки — `put_build/2`.
  #
  # 🔴 Задача 3.224: то же событие шлёт и степпер ЗНАЧЕНИЯ у фита, чья пара
  # законно повторяется (`Epic energy resistance (Fire) ×2`), — тогда приходит
  # ещё и `phx-value-choice`, и добавляется ПАРА, а не голый id. Разбор
  # значения — `gear_feat_toggle_choice/3`, та же функция и тот же вид ответа,
  # что у «×» в `toggle_gear_feat` выше: событие без `choice` осталось байт
  # в байт тем же, каким было у 3.204.
  #
  # ⚠️ Три проверки перед добавлением, а не одна — иначе руками собранная
  # ссылка на это же событие могла бы продублировать НЕстакающийся фит
  # (`Gear.add_feat/3`, в отличие от `toggle_feat/3`, дубли не гасит сам):
  #
  #   * фит существует и не отключён шардом (и значение легально) — та же
  #     проверка, что у `toggle_gear_feat` для голого добавления, только
  #     спрошенная про целую ЗАПИСЬ: `Rules.validate_gear_feat/2` её
  #     и принимает;
  #   * эта ЗАПИСЬ вообще может стоять в списке ещё раз
  #     (`GearFeats.repeats?/2` — у голой это «фит стакуется», у пары «каждое
  #     объявление есть взятие»). Кнопка степпера шлёт событие только там,
  #     где ядро сказало «да», но событие само по себе записи не проверяет;
  #   * потолок ОБЪЯВЛЕНИЙ не достигнут (`GearFeats.add_reasons/3`,
  #     `{:max_takes, id, n}`, у пары — по паре) — та же функция, что красит
  #     кнопку «+» заблокированной в разметке, спрошена ещё раз здесь: между
  #     отрисовкой и кликом билд мог поменяться.
  def handle_event("gear_feat_more", %{"feat" => feat} = params, socket) do
    %{ruleset: ruleset, build: %Build{gear: %Gear{} = gear} = build} = socket.assigns

    with {:ok, id} <- Ids.fetch(ruleset, :feats, feat),
         {:ok, choice} <- gear_feat_toggle_choice(ruleset, id, params["choice"]),
         entry = gear_feat_entry(id, choice),
         true <- GearFeats.repeats?(entry, ruleset),
         :ok <- Rules.validate_gear_feat(entry, ruleset),
         [] <- GearFeats.add_reasons(gear, ruleset, entry) do
      {:noreply, put_build(socket, %Build{build | gear: Gear.add_feat(gear, id, choice)})}
    else
      _ -> {:noreply, socket}
    end
  end

  # «−» снимает ОДНУ запись, никогда не все разом — как «−» у мини-сета.
  # Проверки нет: снятие законно всегда, той же безусловностью, что у
  # голого снятия в `toggle_gear_feat` выше (объявление, которое ядро
  # отбивает, обязано сниматься, иначе билд из старой ссылки не починить).
  #
  # ⚠️ С `choice` снимается ровно эта ПАРА (3.224): у билда с двумя стихиями
  # «−» на одной не имеет права тронуть другую. Последнее снятие убирает
  # значение с экрана целиком — второй кнопки («×») у такой строки нет.
  def handle_event("gear_feat_less", %{"feat" => feat} = params, socket) do
    %{ruleset: ruleset, build: %Build{gear: %Gear{} = gear} = build} = socket.assigns

    with {:ok, id} <- Ids.fetch(ruleset, :feats, feat),
         {:ok, choice} <- gear_feat_toggle_choice(ruleset, id, params["choice"]) do
      {:noreply, put_build(socket, %Build{build | gear: Gear.remove_feat(gear, id, choice)})}
    else
      _ -> {:noreply, socket}
    end
  end
  ...
  # ---- мини-сеты: сколько кусков каждого набора надето (задача 3.185) ----
  #
  ...
  #
  # 🔴 **Степпер, а не поле ввода, и это не вкус.** Числа здесь маленькие
  # (в игре слотов одиннадцать), а строка обязана существовать при любом
  # своём значении: её ПОЗИЦИЯ — это подпись на экране («Набор 1», «Набор 2»),
  # и набор без числа значит «набор не надет», то есть не строку, а её
  # отсутствие. С числовым полем пришлось бы держать второй, черновой список
  # рядом с билдом — только чтобы не терять строку, пока игрок стирает цифру,
  # чтобы набрать другую (та же беда, что поймала задача 3.63 на мобильном:
  # реальный «0» в поле, и набранное приписывается к нему). Степпер снимает
  # вопрос целиком: `mini_sets` в билде — это ровно то, что игрок видит,
  # второго источника правды нет. Тот же приём, что у поинт-бая и у рангов
  # навыков (CLAUDE.md §6: «степперы с потолками, а не поиск по списку»).
  #
  # ⚠️ Правок мини-сетов ЧЕТЫРЕ вида, а тело одно: все четыре — это функция
  # `[count] -> [count]`, и разница между ними в одной строке. Второй копии
  # «прочитать индекс, поправить список, положить в билд» не заводим — форма
  # бага 1.2 (CLAUDE.md §8).
  def handle_event("add_mini_set", _params, socket) do
    %{build: %Build{gear: %Gear{} = gear} = build} = socket.assigns

    # Новая строка начинается с ОДНОГО куска, а не с двух, и это сознательно:
    # один кусок — законное состояние («набор надет, но собран не полностью»),
    # и строка сразу называет причину, по которой он не в счёт. Так блок
    # заодно учит главной ловушке правила, вместо того чтобы прятать её за
    # умолчанием, которое уже «правильное».
    {:noreply, put_mini_sets(socket, build, gear, gear.mini_sets ++ [1])}
  end

  def handle_event("mini_set_more", %{"index" => index}, socket),
    do: update_mini_set(socket, index, &min(&1 + 1, gear_input_limit()))

  # Пол — один кусок, а не ноль: убрать строку — это «×», отдельное действие
  # с отдельной причиной. Ноль был бы третьим состоянием, неотличимым
  # от «строки нет» ни в билде, ни в ссылке.
  def handle_event("mini_set_less", %{"index" => index}, socket),
    do: update_mini_set(socket, index, &max(&1 - 1, 1))

  def handle_event("drop_mini_set", %{"index" => index}, socket) do
    %{build: %Build{gear: %Gear{} = gear} = build} = socket.assigns

    case mini_set_index(gear, index) do
      nil -> {:noreply, socket}
      i -> {:noreply, put_mini_sets(socket, build, gear, List.delete_at(gear.mini_sets, i))}
    end
  end

  # Крафтовые (именные) и уникальные вещи (задача 3.186) — второй вход одной
  # таблицы процентов к HP с кусками мини-сетов выше, но СВОЁ поле (`Gear.
  # named_items`, одно число), а не строка степпера: количество вещей, а не
  # позиция набора, и второй строки на экране под него заводить незачем.
  #
  # ⚠️ `gear_nonneg_number/1`, НЕ `gear_number/1` соседних полей вещей:
  # `named_items :: non_neg_integer()` (`Rules.Gear`) — у штрафа здесь нет
  # игрового смысла («минус одна крафтовая вещь» никто не вводит), в отличие
  # от характеристик и оружия, где отрицательное реально (CLAUDE.md §6, тот
  # же довод, что развёл `mini_set_less/2`'s пол в единицу и снятие набора).
  def handle_event("gear_named_items", params, socket) do
    %{build: %Build{gear: %Gear{} = gear} = build} = socket.assigns
    count = gear_nonneg_number(params["count"])

    {:noreply, put_build(socket, %Build{build | gear: %Gear{gear | named_items: count}})}
  end
  ...
  def handle_event("export", _params, socket) do
    {:noreply,
     ...
     |> assign(:export_text, export_text(socket))
     |> assign(:export_file_text, export_file_text(socket))
     |> assign(:export_filename, export_filename(socket))}
  end
  ...
  # события. Задача 3.180 добавила второй читатель того же переключателя —
  # `:export_file_text` — и он обновляется здесь же, а не отдельным
  # событием: иначе кнопка «скачать» отдавала бы устаревший файл одним
  # кликом позже, чем видно на экране.
  ...
  def handle_event("import_apply", _params, socket) do
    case socket.assigns.import_report do
      %{result: %{build: %Build{} = build, title: title, issues: issues}} ->
        {:noreply,
         apply_imported(socket, build, title, :import_open?, ImportPanel.import_flash(issues))}
  ...
  # ⚠️ Та же воронка, что у текстового импорта (`apply_imported/5` выше) —
  # различий у двух окон ровно три, и все три стоят аргументами: что закрыть,
  # откуда взялся отчёт и чьим форматтером говорит флеш.
      ...
      %{result: %{build: %Build{} = build, title: title, issues: issues}} ->
        {:noreply,
         apply_imported(
           ...
           build,
           title,
           :game_log_import_open?,
           GameLogImportPanel.flash(issues)
         )}
  ...
  # ⚠️ Задача 3.228: список сброса отсюда УЕХАЛ в `forget_build_ui/1` целиком —
  # включая три ассайна поиска (`:feat_query`, `:skill_add?`, `:skill_query`),
  # которые до задачи чистились ТОЛЬКО здесь, а у ссылки и обоих импортов
  # оставались от прежнего билда (репро — `builder_load_code_reset_test.exs`).
  # Осталось то, что у «сбросить» своё: пустой билд начинается с 1-го уровня
  # и не имеет имени.
  #
  # ⚠️ `put_build/2` здесь остался, хотя ветка отказа `load_code/3` открывает
  # ТОТ ЖЕ пустой билд другим путём. Причина не во вкусе: из `handle_params/3`
  # эту воронку звать нельзя (падает до первого `refresh/1`) — разбор
  # в `show_loaded_build/4`. Общими у двух входов остаются `forget_build_ui/1`
  # и `empty_build/1`, то есть всё, что можно рассинхронизировать молча.
  def handle_event("reset", _params, socket) do
    {:noreply,
     ...
     |> put_build(empty_build(socket.assigns.ruleset))}
  end

  # 🔴 ОДНА воронка на оба импорта — задача 3.221. Клаузы `"import_apply"`
  # и `"game_log_import_apply"` были близнецами: различались ассайном отчёта,
  # флагом закрытия и форматтером флеша, остальное буква в букву. И разошлись
  # они не в различиях, а в ОБЩЕЙ части — `:feat_choice` не гасил ни один,
  # хотя его гасят все прочие пути замены билда (`mount`, `load_code`,
  # `go_to_level`, `reset`, `put_feat`).
  #
  # 🔴 Цена измерена, а не предположена (репро в `builder_live_import_test.exs`):
  # игрок на 3-м уровне Волшебника открывает второй шаг у `Spell focus`,
  # вставляет чужой текст `Fighter(2)` и жмёт «применить» — импорт приземляет
  # его на 3-й уровень НОВОГО билда, а над списком фитов остаётся панель
  # мёртвого выбора со всеми восемью школами «недоступно: на уровне Fighter
  # этот фит не выбрать». Список фитов при этом СПРЯТАН (панель занимает его
  # место), то есть уровень нечем заполнить, пока игрок не заметит «отмена».
  # Панель сама закрывается только когда `active` разошёлся с её уровнем
  # (`assign_choice_panel/1` сверяет `level: ^active`), а импорт приземляется
  # ровно на `taken + 1` — и совпадение это не редкость, а обычный случай.
  #
  # ⚠️ Сброс — тот же, что у `load_code/3`: уровень, `hold`, секции, слот,
  # второй шаг, превью и весь UI «Вещей». Восемь прочих мест с `assign(:sections,
  # %{})` сюда НЕ сводятся (решение 3.221): у `pick_class` и `drop_level` состав
  # честно свой.
  #
  # 🔴 Задача 3.226: «тот же» стало буквальным — `forget_build_ui/1`, одна
  # функция на оба места (уровень и заголовок остались здесь: они у двух путей
  # честно разные, см. её же комментарий). Пока два списка стояли руками, они
  # и разошлись: у `load_code/3` не хватало `:feat_slot` и `:preview` — форма
  # бага 1.2 на том же наборе, что 3.221 поймала на `:feat_choice`, только
  # с другой стороны.
  defp apply_imported(socket, %Build{} = build, title, close_key, flash) do
    taken = Build.character_level(build)

    socket
    |> assign(close_key, false)
    |> assign(:build_title, title)
    |> assign(
      :active,
      clamp_level(taken + 1, level_ceiling(build, taken), socket.assigns.ruleset.level_cap)
    )
    |> forget_build_ui()
    |> put_flash(:info, flash)
    # ⚠️ В отличие от ссылки (решение 1.7), импорт УЖЕ шёл через эту воронку
    # и до задачи 3.17 (ради `enforce_floor/2`) — значит и сброс распределения
    # применится к нему точно так же, если чужой текст принёс нелегальную
    # покупку. `put_build/2` сам решит и сам выставит `:point_buy_reset`,
    # ничего доопределять здесь не нужно.
    |> put_build(build)
  end
  ...
  # Задача 3.180 — то же самое, только `Export.file_text/5`: попап-текст плюс
  # ссылка одной строкой внизу. `view_url/1` — та же функция, что рисует
  # `#share-link` рядом в этой же сцене, а не второй расчёт «какая у билда
  # ссылка» (CLAUDE.md §9): их совпадение — структурный факт, а не то, что
  # нужно сверять тестом отдельно от того, что оба читают один код.
  defp export_file_text(socket) do
    Export.file_text(
      socket.assigns.build,
      socket.assigns.ruleset,
      socket.assigns.stats,
      view_url(socket.assigns),
      show_granted_feats: socket.assigns.export_show_granted?
    )
  end

  # Задача 3.179 — читает свежие `build`/`stats` в момент открытия диалога,
  # тем же приёмом, что `export_text/1` рядом: не хранится пересчитанным на
  # каждую правку билда, потому что диалог блокирует правки, пока открыт
  # (`.scrim` поверх сцены), так что имя не может протухнуть у игрока на
  # глазах — оно просто снова читается свежим при следующем открытии.
  defp export_filename(socket) do
    Export.file_name(socket.assigns.build, socket.assigns.ruleset, socket.assigns.stats)
  end
  ...
  # Запись фита с вещи: голый id или пара — ровно та форма, в которой её
  # хранит `Gear` и которую принимают `Rules.validate_gear_feat/2` и
  # `GearFeats.repeats?/2`/`add_reasons/3`. Одно написание на три события
  # (`toggle_gear_feat`, `gear_feat_more`, `gear_feat_less`), чтобы «пара
  # или не пара» не решалась в каждом по-своему.
  defp gear_feat_entry(id, nil), do: id
  defp gear_feat_entry(id, choice), do: {id, choice}

  # Одна выборка чисел из параметров формы на все три зоны «Вещей», которые
  # присылают `%{"<имя>" => "<число>"}`: характеристики/AC/навыки, раздельные
  # спасы и поглощение стихий (задача 3.222, сведено из трёх копий). Различие
  # у них ровно одно — чем резолвится имя, — и оно едет аргументом.
  #
  # Общее у всех трёх и потому написано один раз: имя проходит белый список
  # и НИКОГДА не становится атомом руками (AGENTS.md); ноль в билд не пишется
  # («вписал и стёр» и «не вписывал» — одно состояние, иначе у одного билда
  # было бы два разных кода ссылки); потолки — дело ядра, форма отказывает
  # только бессмыслице; `nil` вместо карты (форма, поля которой в этом
  # `params` не пришли вовсе) даёт пустой набор, а не падение.
  defp gear_numbers_by(params, resolve) when is_map(params) do
  ...
  defp gear_numbers_by(_params, _resolve), do: %{}

  defp gear_numbers(ruleset, kind, params),
    do: gear_numbers_by(params, &Ids.fetch(ruleset, kind, &1))

  # Раздельные спасы (задача 3.187, часть B): `%{"fort" => "12", "will" => "3"}`.
  # Резолвер свой, не `Ids.fetch/3` — у сейвов нет своей записи в справочнике
  # ruleset'а (это не игровая сущность с id, именем и source, а фиксированная
  # тройка ядра), поэтому список из трёх имён закрыт прямо здесь же — тот же
  # приём, что у `@save_extras` в `Summary` и у `@saves` в `Rules.GearImport`,
  # не второй словарь, а то же самое закрытое множество, названное там, где
  # оно нужно.
  defp gear_saves_specific(params), do: gear_numbers_by(params, &gear_save_id/1)

  defp gear_save_id("fort"), do: {:ok, :fort}
  defp gear_save_id("ref"), do: {:ok, :ref}
  defp gear_save_id("will"), do: {:ok, :will}
  defp gear_save_id(_other), do: :error

  # Поглощение стихий с вещей (задача 3.211): `%{"fire" => "15", "cold" =>
  # "15"}`. Белый список — `ruleset.resistance_energy_types` через
  # `Ids.fetch_energy_type/2`, ту же функцию, которой `Encoding` проверяет
  # стихию из URL-кода (`resist|<стихия>|<n>`): до задачи 3.222 этот поиск был
  # написан здесь и там дважды, посимвольно.
  defp gear_resistances(ruleset, params),
    do: gear_numbers_by(params, &Ids.fetch_energy_type(ruleset, &1))
  ...
  # Уровневое UI-состояние — ровно то, что описывает РЕШЕНИЯ ОДНОГО уровня и
  # потому не имеет смысла ни на другом уровне, ни у другого билда: зажатая
  # кнопка перехода, свёрнутые секции, выбранный чип слота, незакрытый второй
  # шаг выбора и наведение (призрачные `+N` в панели итогов).
  #
  # 🔴 Задача 3.226. Пятёрка стояла руками в ТРЁХ местах (`go_to_level/2`,
  # `load_code/3`, `apply_imported/5`), и один из списков был неполон: ссылка
  # не гасила ни `:feat_slot`, ни `:preview`. Ровно форма бага 1.2 (CLAUDE.md
  # §8) — и уже второй раз на этом же наборе: 3.221 свела сюда `:feat_choice`,
  # которого не гасил ни один из двух импортов. Дальше список живёт в одном
  # месте, и следующий ассайн этого рода прибавляется тоже в одном.
  #
  # ⚠️ Блока «Вещи» здесь НЕТ, и это не забывчивость: `:gear_*` принадлежит
  # билду, а не уровню, — переход с 5-го уровня на 6-й не имеет права стирать
  # набранный запрос в списке фитов с вещи. Замену билда целиком обслуживает
  # `forget_build_ui/1` ниже, она и складывает оба забывания.
  #
  # ⚠️ `mount/3` эти же пять перечисляет у себя и сюда НЕ сводится намеренно:
  # он обязан объявить КАЖДЫЙ ассайн модуля, а не только уровневые, и своим
  # списком он же и сторожит — ассайн, забытый в `mount/3`, валит первый же
  # рендер, а не ждёт нужного сценария. Здесь риск обратный (ассайн есть,
  # но его никто не гасит), и он тихий — потому и сведён.
  defp forget_level_ui(socket) do
    socket
    |> assign(:hold, nil)
    |> assign(:sections, %{})
    |> assign(:feat_slot, nil)
  ...
  # Билд заменился ЦЕЛИКОМ — ссылкой (`load_code/3`) или применённым импортом
  # (`apply_imported/5`). Тогда забывается и уровневое состояние, и состояние
  # блока «Вещи»: от прежнего билда на экране не остаётся ничего, кроме того,
  # что принадлежит читателю (тема, режим гида, открыт ли сам блок «Вещи»).
  #
  # ⚠️ Уровень (`:active`) считают сами вызывающие, и по-разному: ссылка умеет
  # `?l=`, импорт всегда приземляется на `taken + 1`. `:build_title` тоже их
  # собственный — у ссылки его нет, у импорта он пришёл из текста. Сюда сведено
  # только то, что у обоих совпадает ДОСЛОВНО.
  #
  # 🔴 Задача 3.228, хвост 3.221/3.226: поиск фитов и открытый список
  # «+ добавить навык» — тоже здесь, и это ИЗМЕРЕНО, а не приведено к порядку
  # ради симметрии. `reset` чистил эти три ассайна с самого начала, а ссылка
  # и оба импорта — нет, и через импорт это видно игроку:
  # `builder_load_code_reset_test.exs` воспроизводит обе половины (набранный
  # запрос фитов переживает смену билда; открытый список «+ добавить навык»
  # остаётся раскрытым после применённого лога). Запрос от ПРЕЖНЕГО билда
  # молча режет список фитов нового — ровно та же форма, что чип слота в 3.226,
  # только фильтрует не слотом, а строкой.
  #
  # ⚠️ В `forget_level_ui/1` они СОЗНАТЕЛЬНО не поехали: переход между уровнями
  # одного билда их не трогал и не должен (`go_to_level/2` — соседние уровни
  # одного персонажа, там набранный запрос как раз помогает), а замена билда
  # целиком — трогает. Это и есть граница между двумя функциями.
  #
  # 🔴 Задача 3.229: сюда же — чип типа фитов (`:feat_type`) и поиск заклинаний
  # (`:spell_query`). Их не чистил НИКТО, включая кнопку «Сброс»: «Эпические»,
  # выбранные у прежнего билда, оставляли новому пустой список фитов на
  # неэпическом уровне, а запрос заклинаний резал каталог чужого кастера.
  # Воспроизведено тремя тестами (`builder_load_code_reset_test.exs`), все три
  # падали до правки. Граница та же: между уровнями ОДНОГО билда оба живут
  # намеренно — чип подсвечен над списком, то есть фильтр виден.
  # ⚠️ `:spell_circles` сюда НЕ идёт, хотя стоял в том же списке кандидата: это
  # не состояние игрока, а производная — `Builder.LevelPicks` пересобирает его
  # из каталога активного уровня на каждой перерисовке.
  defp forget_build_ui(socket) do
    socket
    |> forget_level_ui()
    |> forget_gear_ui()
    |> assign(:feat_query, "")
  ...
  # одних и тех же ассайнов разошлись бы — ровно форма бага 1.2. ⚠️ С задачи
  # 3.222 мест ЧЕТЫРЕ: `mount/3` зовёт её же вместо четвёртого списка руками —
  # начальное состояние блока и есть «ничего не помним».
  defp forget_gear_ui(socket) do
    ...
    # и своя строка поиска, как у главной руки, но не смешиваются: у каждой
    # руки свой пул отказов. Забывание — то же, что у главной.
    |> assign(:gear_off_weapon_add?, false)
    |> assign(:gear_off_weapon_query, "")
  end

  # Одна воронка на все четыре правки мини-сетов: `+`, `−` и «×» отличаются
  # ровно функцией над числом, а «прочитать индекс — поправить — положить»
  # у них общее.
  #
  # ⚠️ Индекс из браузера проходит ту же проверку, что любой id: строка вне
  # диапазона — не ветка ошибки, а «ничего не делаем». Такое приходит
  # от клика по строке, которую только что унёс другой клик (или от правленого
  # руками события), и валить на этом LiveView нельзя.
  defp update_mini_set(socket, index, fun) do
    %{build: %Build{gear: %Gear{} = gear} = build} = socket.assigns
    ...
    case mini_set_index(gear, index) do
      ...
      i ->
        counts = List.update_at(gear.mini_sets, i, fun)
        {:noreply, put_mini_sets(socket, build, gear, counts)}
    end
  end

  defp put_mini_sets(socket, %Build{} = build, %Gear{} = gear, counts),
    do: put_build(socket, %Build{build | gear: %Gear{gear | mini_sets: counts}})

  # `nil` — «такой строки нет». Мусор в параметре и индекс за границей списка
  # отвечают одинаково, как и у `requested_level/1` выше: здесь только формат
  # и диапазон, решение — у вызывающего.
  defp mini_set_index(%Gear{mini_sets: counts}, index) when is_binary(index) do
    with {i, ""} <- Integer.parse(index),
         true <- i >= 0 and i < length(counts) do
      i
    else
      _ -> nil
    end
  ...
  defp mini_set_index(_gear, _index), do: nil
  ...
  # Число вещей (задача 3.186, `gear_named_items`), а не характеристика или
  # оружие — `Rules.Gear.named_items :: non_neg_integer()`, и отрицательного
  # здесь нет по игровому смыслу (§6 «не выдумывай число», в обе стороны:
  # не додумывать и отрицательный вход тоже). ⚠️ Тот же предел формы, что
  # у `gear_number/1` — граница ввода одна на все числа «Вещей», удвоение её
  # под каждое поле развело бы два разных потолка молча.
  defp gear_nonneg_number(value) when is_binary(value) do
    case Integer.parse(String.trim(value)) do
      {number, _rest} -> number |> max(0) |> min(@gear_input_limit)
      :error -> 0
    end
  end

  defp gear_nonneg_number(_value), do: 0
  ...
  defp put_build(socket, %Build{} = build) do
    ...
    # (`forget_level_ui/1` — с задачи 3.226 сам список живёт там одним местом,
    # а не перечислением в комментариях).
    socket =
    ...
    #
    # ⚠️ Задача 3.228: второй патч на событие по-прежнему невозможен и с новой
    # стороны — ветка отказа `load_code/3` патчит адрес сама
    # (`patch_to_own_code/1`), но эта воронка из `handle_params/3` не
    # вызывается вовсе (почему — в комментарии `show_loaded_build/4`).
  ...
  defp skill_row({id, skill}, ruleset, build, level, budget, here) do
    ...
    why = rank_block(ruleset, build, level, row, headroom, budget.free)

    row
    |> Map.put(:why, why)
    |> Map.put(:max_hint, max_hint(row, why))
  end

  # Подсказка кнопки «макс». До задачи 3.225 она печаталась ОДНОЙ формулой на
  # оба состояния, и у выключенной кнопки выходила арифметика с минусом:
  # «Добавить -1 р. за -2 очков» (замер CDP, `#skill-max-open_lock`, билд
  # `wide_4class` на 21-м уровне — рангов куплено больше, чем позволяет потолок
  # ЭТОГО уровня, что законно; CLAUDE.md §6, «потолок ограничивает покупку
  # на уровне, а не итог»). `room` там отрицателен по построению — это
  # `min(headroom, affordable)`, а `headroom` у перекупленного навыка < 0, — и
  # умножать его на цену ранга бессмысленно: кнопка ничего не добавит.
  #
  # Выключенная кнопка называет ПРИЧИНУ, и причина берётся оттуда же, откуда
  # её берёт видимая строка `.sk-why` (`rank_block/6`), слово в слово: второго
  # источника правды здесь не заводится, игровых условий в вебе тем более.
  # Дословно, а не «своими словами» ещё и потому, что строка и подсказка стоят
  # в трёх сантиметрах друг от друга — разойдясь хоть частицей, они выглядели
  # бы как два разных правила.
  #
  # `free == 0` — единственный случай, где `rank_block/6` молчит СОЗНАТЕЛЬНО
  # («0 очков свободно» стоит крупно над списком, см. его комментарий); для
  # подсказки этот довод не работает — она не занимает на экране ничего, пока
  # на неё не навели, — поэтому у неё свой хвост.
  defp max_hint(%{can_raise?: true} = row, _why) do
    "Добавить #{Labels.plural(row.room, "ранг", "ранга", "рангов")} " <>
      "за #{Labels.plural(row.room * row.cost, "очко", "очка", "очков")}"
  end

  defp max_hint(_row, nil), do: "свободных очков не осталось"
  defp max_hint(_row, why), do: why
  ...
  @doc false
  # "This host, at the root, no query" — what `view_url/1` below needs to
  # build a `/b/:code` link out of a request `uri`. This LiveView's own route
  # is already `/` (`handle_params/3` above), so stripping the query alone
  # used to be enough here; forcing the path to `/` is a no-op for THIS
  # caller and stays a no-op if a future route ever mounts this LiveView
  # somewhere else, so it is written the way the OTHER caller genuinely
  # needs it, not the narrower way this one happens to get away with.
  #
  # `BuildCalculatorWeb.BuildViewLive`'s own `handle_params/3` calls this too
  # (task 3.180) — its routes are `/b/:code` and `/builds/:id`, neither one
  # `/`, so it cannot skip the path reset the way a copy-pasted one-liner
  # would tempt it to. One function, not two module bodies computing "the
  # host this build's link should name" and drifting the day only one of
  # them gets fixed (CLAUDE.md §9's own recurring lesson) — a downloaded
  # file's link and this screen's own `#share-link` are two independent
  # readings of the same request today only because they call the same code.
  def host_root(uri),
    do: uri |> URI.parse() |> Map.merge(%{path: "/", query: nil}) |> URI.to_string()
