defmodule BuildCalculatorWeb.BuilderGearFeatsTest do
  ...
  describe "показ объявленного" do
    ...
    # ⚠️ Задача 3.191 сняла с шапки ПЕРЕЧИСЛЕНИЕ, а не сам сигнал: она по-прежнему
    # отвечает на «задана ли экипировка», и объявленный фит — это «да».
    test "свёрнутая шапка не говорит «ещё не задана», когда объявление есть", %{
      conn: conn,
      build: build
    } do
    ...
    test "объявление со значением показывается и не роняет страницу", %{
      ...
      # ⚠️ Здесь стояло `assert #gear-summary =~ "фиты: 1"` — шапка считала
      # объявления. Задача 3.191 счётчик сняла вместе со всем перечислением;
      # «пара — это ОДНА запись, а не две и не ноль» доказывает сам список.
      assert has_element?(view, "#gear-feat-entry-skill_focus-discipline")
      refute has_element?(view, "#gear-feat-entry-skill_focus-spot")
    end
  ...
  describe "второй шаг: значение фита с вещи" do
    ...
    test "один фит объявляется дважды с разными значениями — обе записи видны и обе считаются",
      ...
      # Две записи, а не одна: уникальность у объявления по ПАРЕ, а не по имени
      # фита. ⚠️ Раньше это же доказывал счётчик свёрнутой шапки («фиты: 2»),
      # снятый задачей 3.191 вместе с перечислением; сами строки доказывают то
      # же самое и не зависят от того, что шапка решит показывать.
      assert render(element(view, "#gear-feat-entry-skill_focus-discipline")) =~ "Discipline"
