defmodule BuildCalculator.Encoding do
  ...
  **старой** сборкой, потеряет строку видимо — `{:unknown_slot, "granted"}`, —
  ...
  ## Куски мини-сетов — тем же приёмом, ДЕВЯТЫЙ раз (задача 3.185)

  В вещах игрок перечисляет, сколько кусков каждого надетого мини-сета на нём
  (`Rules.Gear` → `mini_sets`, задача 3.184; Dan 11.09.2026: «надо будет
  указать, что из минисета 1 у нас 2 куска, из минисета 2 у нас 3 куска,
  из минисета 3 у нас 2 куска, etc»). Куски усиливают оружейные и расовые
  бонусы (`B += floor(B · Nmini / 10)`), то есть двигают AB, AC и значения
  навыков — часть билда, и ехать в ссылке обязаны.

  Новая секция в хвосте v2 снова нельзя, и довод тот же в девятый раз: лишний
  счётчик в хвосте — это лишний байт **у КАЖДОГО** билда, включая те, где
  никаких мини-сетов нет, а версия входит в код первым байтом и в префикс
  строки. Поэтому куски едут **одной строкой таблицы фитов**, уровень `0`,
  своим псевдо-слотом:

      <<0>>  "miniset|2,3,2"  <<0::16>>

  🔴 **Строкой ЧИСЕЛ, а не именами, и это свойство правила, а не экономия.**
  В `Nmini` входит только мультимножество счётчиков — какие именно наборы их
  дали, арифметика не спрашивает вовсе (`Rules.MiniSets`), — поэтому и
  кодировать нечего, кроме чисел. Имена приедут с импортом игрового лога
  (задача 3.187) и **тогда** потребуют своей строки, не раньше.

  ⚠️ **Порядок сохраняется, а не сортируется**, в отличие от фитов с вещи
  и надетого. Для арифметики он не значит ничего, а для игрока значит:
  наборы безымянные и пронумерованы по порядку ввода («Набор 1», «Набор 2»),
  то есть позиция в списке — это подпись строки на экране. Отсортируй мы
  список — у игрока, набравшего «3, 2», строки молча поменялись бы местами.
  Код у одного и того же билда при этом один и тот же: билд хранит порядок,
  а не забывает его, как забывает порядок кликов по фитам.

  Числа — десятичным текстом через запятую, ровно как значения выбора класса
  (`"cleric|air,war"`): запятая в id не встречается, а до base64 разделитель
  не доживает — payload length-prefixed.

  ⚠️ Границы ЧТЕНИЯ и ЗАПИСИ одни и те же и обе не игровые: не больше
  `@mini_set_rows` строк и не больше `@mini_set_count_limit` кусков в строке.
  Это защита формата (ключ слота пишется как `<<byte_size::8>>`, то есть длиннее
  255 байт он молча обрежется и код станет битым), а не потолок игры: слотов
  в игре одиннадцать, и сумму сверх них клипает **ядро**
  (`Rules.MiniSets.pieces/2`), а интерфейс об этом предупреждает, не блокируя.
  Одни и те же границы у кодера и декодера — ради того, чтобы
  `decode(encode(b)) == b` держалось и у правленой руками ссылки.

  ⚠️ Нечитаемое и неположительное (`"0"`, `"-2"`, `"x"`) теряется ПОИМЁННО,
  строкой целиком: `{:unknown_mini_sets, "miniset|2,x"}`. Не нулём, как числа
  оружия, — там ноль означает «предмет без прибавки», законное состояние,
  а здесь строка означает «набор надет», и ноль кусков надетого набора
  не значит ничего вовсе: молча подставленный `0` завёл бы в билд строку,
  которой игрок не вводил.

  ⚠️ Плата названа, как у всех восьми приёмов до этого: код с мини-сетами,
  открытый **старой** сборкой, потеряет строку видимо — `Ids.fetch_slot/2`
  ключа `"miniset"` не знает и отдаст `:error`, то есть игрок увидит «из
  ссылки выпало: unknown_slot miniset|2,3,2». Билд при этом откроется целиком,
  только без усиления.

  ## Крафтовые (именные) вещи — тем же приёмом, ДЕСЯТЫЙ раз (задача 3.186)

  В вещах игрок вписывает, сколько крафтовых (именных) и уникальных вещей на
  нём надето (`Rules.Gear` → `named_items`) — второй вход ТОЙ ЖЕ таблицы
  процентов к HP, что и куски мини-сетов выше (`Ctotal = Cgear + Nmini`,
  `Rules.GearHitPoints`), но **одним числом**, а не списком: скрипт шарда
  спрашивает у слота только «именная ли она», а не какая. Двигает HP, то есть
  часть билда, и ехать в ссылке обязана.

  Новая секция в хвосте v2 снова нельзя, тем же доводом в десятый раз: лишний
  байт **у КАЖДОГО** билда, включая те, где именных вещей нет, а версия входит
  в код первым байтом и в префикс строки. Поэтому число едет **одной строкой**
  таблицы фитов, уровень `0`, своим псевдо-слотом:

      <<0>>  "nameditems|3"  <<0::16>>

  Ни одной строки у билда без единой именной вещи (`named_items == 0`) —
  ровно поэтому уже расшаренная ссылка кодируется побайтово как раньше.
  Число — десятичным текстом, тем же приёмом, что у чисел оружия и у кусков
  мини-сетов: `String.to_integer/1`, а не позиционный байт.

  ⚠️ **`0` — легальное значение при ЧТЕНИИ, в отличие от кусков мини-сетов.**
  У мини-сета «набор надет, но в нём 0 кусков» не значит ничего — набора без
  кусков не бывает. У именных вещей «вещей нет» — это ровно умолчание
  (`Gear.named_items` по умолчанию `0`), и правленая руками ссылка с
  `"nameditems|0"` не более чем избыточна, а не бессмысленна: она открывается
  и даёт тот же билд, что и полное отсутствие строки. Наш кодер такую строку
  не пишет (см. `named_item_entries/1`), но декодер её принимает.

  ⚠️ Нечитаемое и отрицательное теряется ПОИМЁННО, строкой целиком —
  `{:unknown_named_items, "nameditems|x"}` — не нулём: тихая подмена
  «испорченного» на «вещей нет» здесь ничем не лучше тихой подмены у кусков,
  а игрок, вписавший число и получивший от битой ссылки ноль молча, не узнал
  бы, что ссылка вообще была битой.

  ⚠️ Граница ЧТЕНИЯ — защита формата (`@named_items_limit`), не игры: потолок
  слотов (10 у Сиалы) применяет **ядро** (`Rules.GearHitPoints.
  named_item_slots/1`), а правленая руками ссылка с «+999» обязана открыться
  и получить ответ от расчёта, не от кодека.

  ⚠️ Плата названа, как у девяти приёмов до этого: код с именными вещами,
  открытый **старой** сборкой, потеряет строку видимо — `Ids.fetch_slot/2`
  ключа `"nameditems"` не знает и отдаст `:error`, то есть игрок увидит «из
  ссылки выпало: unknown_slot nameditems|3». Билд при этом откроется целиком,
  только без прибавки к HP от этих вещей.

  ## Поглощение стихий с вещей — тем же приёмом, ОДИННАДЦАТЫЙ раз (задача 3.210)

  В вещах игрок вписывает поглощение стихийного урона по стихиям
  (`Rules.Gear` → `resistances`): `Damage Resistance (Fire) 15` с предмета — это
  число, которое СКЛАДЫВАЕТСЯ с прибавкой от фитов и КОНКУРИРУЕТ с эффектом
  расы Гнома и группы топоров (`Rules.Resistances`). Двигает печатаемое число,
  то есть часть билда, и ехать в ссылке обязано.

  Новая секция в хвосте v2 снова нельзя, тем же доводом в одиннадцатый раз:
  лишние байты **у КАЖДОГО** билда, включая те, где поглощения нет. Поэтому
  стихии едут строками таблицы фитов, уровень `0`, своим псевдо-слотом —
  **по строке на стихию**, как у надетого:

      <<0>>  "resist|cold|15"  <<0::16>>
      <<0>>  "resist|fire|15"  <<0::16>>

  🔴 **По строке на стихию, а НЕ одной строкой списком, как у кусков
  мини-сетов** — и это свойство правила, а не вкус. У кусков в арифметику
  входит только мультимножество чисел (какой набор их дал, не спрашивает
  никто), поэтому там имён нет вовсе; здесь число БЕЗ имени стихии не значит
  ничего — стихии независимы, у каждой свой фит и своя строка в итогах.
  Позиционная тройка (как `"savespec|F,R,W"`) тоже не годится: стихий семь,
  и билд с одним поглощением платил бы за шесть нулей.

  Отсортировано по id стихии: билд не хранит порядок ввода, а код обязан быть
  одинаковым у одинакового билда. **Ни одной строки у билда без поглощения
  с вещей** — ровно поэтому уже расшаренная ссылка кодируется байт в байт.

  ⚠️ Стихия проходит **белый список** ruleset'а (`resistance_energy_types`,
  семь), и обязана: `"resist|magical|15"` из правленой руками ссылки означает
  вид урона, который не поглощается вовсе, а `"resist|bludgeoning|10"` —
  физическое поглощение, которое эта секция не показывает по решению владельца.
  Молча положить их в мапу значило бы завести в билд стихию, которой у него нет.
  Теряется ПОИМЁННО, строкой целиком: `{:unknown_resistances, "resist|magical|15"}`.

  ⚠️ Число — десятичным текстом, тем же приёмом, что у чисел оружия и у кусков:
  `Integer.parse/1`, а не позиционный байт. **Ноль читается** (как у именных
  вещей и раздельных спасов: «эта стихия без поглощения» — ровно умолчание
  `Gear.resistances`), нечитаемое теряется строкой целиком, а не нулём;
  отрицательное тоже читается — предмет со штрафом мы не встречали, но
  «вписал минус» это ввод игрока, и у соседних полей вещей минус проезжает.

  ⚠️ Граница ЧТЕНИЯ — защита формата (`@resistance_limit`), не игры: своего
  потолка поглощению не называет ни один источник, и выдумывать его здесь
  нельзя тем более, чем в ядре.

  ⚠️ Плата названа, как у десяти приёмов до этого: код с поглощением, открытый
  **старой** сборкой, потеряет строки видимо — `Ids.fetch_slot/2` ключа
  `"resist"` не знает и отдаст `:error`, то есть игрок увидит «из ссылки
  выпало: unknown_slot resist|fire|15». Билд при этом откроется целиком,
  только без поглощения с вещей.
  ...
  # Шестой псевдо-слот той же таблицы — куски мини-сетов (задача 3.185,
  # см. moduledoc). Одна строка на весь список: в `Nmini` входит только
  # мультимножество счётчиков, и разделять их по строкам было бы восемью
  # лишними байтами на билд ни за что.
  @mini_set_slot_key "miniset"

  # Границы мини-сетов при чтении И записи. ⚠️ Не игровые: слотов в игре
  # одиннадцать, и сумму сверх них клипает ядро (`Rules.MiniSets.pieces/2`),
  # а интерфейс предупреждает, не блокируя (задача 3.185). Это защита самого
  # формата — ключ слота пишется как `<<byte_size::8>>`, то есть строка длиннее
  # 255 байт молча обрезалась бы и код становился битым. 32 строки по 3 цифры
  # с запятыми — 128 байт плюс тег, с запасом вдвое.
  @mini_set_rows 32
  @mini_set_count_limit 255

  # Седьмой псевдо-слот той же таблицы — крафтовые (именные) и уникальные
  # вещи, одним числом (задача 3.186, см. moduledoc).
  @named_items_slot_key "nameditems"

  # Граница числа именных вещей при ЧТЕНИИ. Не игровая: потолок слотов
  # (сегодня 10) применяет ядро (`Rules.GearHitPoints.named_item_slots/1`),
  # это только защита от кода, просящего сложить что-то огромное — та же роль,
  # что у `@weapon_bonus_limit` рядом.
  @named_items_limit 9999

  # Восьмой псевдо-слот той же таблицы — раздельные спасы с вещей
  # (задача 3.187, часть B, `Rules.Gear` → `saves_specific`; см. moduledoc).
  @save_spec_slot_key "savespec"

  # Фиксированный порядок трёх полей строки `"savespec|F,R,W"` — ядро уже
  # говорит этой же тройкой (`Rules.GearImport`'s `@saves`, `Rules.Gear`'s
  # own moduledoc), одно и то же множество, не второе.
  @saves_specific_order [:fort, :ref, :will]

  # Граница числа при ЧТЕНИИ — защита формата, не игры: потолок +20 на сейв
  # применяет ядро (`Rules.Caps`), и правленая руками ссылка с «+999» обязана
  # открыться, как открывается такое же число в поле «ко всем».
  @save_spec_limit 999

  # Девятый псевдо-слот той же таблицы — поглощение стихийного урона с вещей
  # (задача 3.210, `Rules.Gear` → `resistances`; см. moduledoc). По строке
  # на стихию: число без имени стихии не значит ничего, стихии независимы.
  @resistance_slot_key "resist"

  # Граница числа при ЧТЕНИИ — защита формата, не игры: своего потолка
  # поглощению не называет НИ ОДИН источник (загрузчик запрещает `cap`
  # у записей разметки этого стата), так что здесь его выдумывать нельзя
  # тем более. То же число и та же роль, что у `@save_spec_limit` рядом.
  @resistance_limit 999
  ...
  defp common_payload(%Build{} = build) do
    ...
    feat_entries =
      feat_entries(build.feats) ++
        ...
        mini_set_entries(build.gear) ++
        named_item_entries(build.gear) ++
        saves_specific_entries(build.gear) ++
        resistance_entries(build.gear) ++
  ...
  # Куски мини-сетов — ОДНА строка той же таблицы, уровень `0`, псевдо-слот
  # `"miniset|2,3,2"` (задача 3.185, см. moduledoc). Ни одной строки у билда
  # без мини-сетов — ровно поэтому уже расшаренная ссылка кодируется байт
  # в байт как раньше.
  #
  # ⚠️ Порядок НЕ сортируется, в отличие от `worn_entries/1` и
  # `gear_feat_entries/1` рядом: позиция в списке — это подпись строки
  # на экране («Набор 1», «Набор 2»), то есть билд её помнит, а не забывает,
  # как забывает порядок кликов по фитам. Детерминированность от этого
  # не страдает: у одного и того же билда список один и тот же.
  #
  # ⚠️ Клипается границами ФОРМАТА (см. `@mini_set_rows`), а не игры: сумму
  # сверх одиннадцати слотов режет ядро, а интерфейс об этом предупреждает.
  # Те же границы применяет декодер, иначе `decode(encode(b)) == b` держалось
  # бы не всегда.
  defp mini_set_entries(%Gear{mini_sets: []}), do: []

  defp mini_set_entries(%Gear{mini_sets: counts}) do
    case mini_set_counts(counts) do
      [] -> []
      counts -> [{0, {:mini_sets, counts}, nil}]
    end
  end

  # Что вообще может попасть в строку: целые, положительные, не больше предела
  # формата, и не больше `@mini_set_rows` штук. Одна функция на кодер и декодер
  # — иначе круговой обход разошёлся бы ровно на правленой руками ссылке.
  defp mini_set_counts(counts) do
    counts
    |> Enum.filter(&(is_integer(&1) and &1 >= 1))
    |> Enum.map(&min(&1, @mini_set_count_limit))
    |> Enum.take(@mini_set_rows)
  end

  # Крафтовые (именные) вещи — ОДНА строка той же таблицы, уровень `0`,
  # псевдо-слот `"nameditems|3"` (задача 3.186, см. moduledoc). Ни одной
  # строки у билда без них (`named_items == 0`) — ровно поэтому уже
  # расшаренная ссылка кодируется побайтово как раньше.
  defp named_item_entries(%Gear{named_items: 0}), do: []

  defp named_item_entries(%Gear{named_items: count}) do
    [{0, {:named_items, min(count, @named_items_limit)}, nil}]
  end

  # Раздельные спасы (задача 3.187, часть B) — ОДНА строка, уровень `0`,
  # псевдо-слот `"savespec|F,R,W"`, всегда в порядке Fort/Ref/Will: не
  # мультимножество, как у кусков мини-сетов, а фиксированная тройка полей —
  # позиция в строке и называет сейв, отдельного имени внутри не нужно.
  # Ни одной строки у билда без единой раздельной прибавки — ровно поэтому
  # каждая уже расшаренная ссылка кодируется побайтово как раньше.
  defp saves_specific_entries(%Gear{saves_specific: specific}) do
    case non_zero(specific) do
      empty when map_size(empty) == 0 ->
        []

      _non_empty ->
        clip = fn n -> n |> max(-@save_spec_limit) |> min(@save_spec_limit) end
        triple = for save <- @saves_specific_order, do: clip.(Map.get(specific, save, 0))
        [{0, {:saves_specific, triple}, nil}]
    end
  end

  # Поглощение стихий с вещей (задача 3.210) — по строке на стихию, уровень `0`,
  # псевдо-слот `"resist|<стихия>|<число>"` (см. moduledoc). Отсортировано
  # по id стихии: билд не хранит порядок ввода, а код обязан быть одинаковым
  # у одинакового билда. Ни одной строки у билда без поглощения с вещей —
  # ровно поэтому уже расшаренная ссылка кодируется байт в байт.
  #
  # ⚠️ Нули отбрасываются, как у характеристик и AC: «вписал и стёр» и
  # «не вписывал» — один ответ у всех полей `Rules.Gear`.
  defp resistance_entries(%Gear{resistances: resistances}) do
    for {type, value} <- Enum.sort(non_zero(resistances)),
        do: {0, {:resistances, type, clamp_resistance(value)}, nil}
  end
  ...
  # `"miniset|2,3,2"` — запятая внутри одной ячейки, ровно как у значений
  # выбора класса (`class_key/2`): в id она не встречается, а числа с ней
  # не спутать.
  defp slot_key({:mini_sets, counts}) do
    @mini_set_slot_key <> @choice_separator <> Enum.map_join(counts, ",", &Integer.to_string/1)
  end

  # `"nameditems|3"` — крафтовые (именные) вещи (задача 3.186), десятичным
  # текстом, той же причиной, что и у кусков мини-сетов рядом.
  defp slot_key({:named_items, count}) do
    @named_items_slot_key <> @choice_separator <> Integer.to_string(count)
  end

  # `"savespec|-3,0,12"` — раздельные спасы (задача 3.187), десятичным
  # текстом Fort/Ref/Will, той же причиной, что у чисел оружия: минус
  # проезжает целиком.
  defp slot_key({:saves_specific, triple}) do
    @save_spec_slot_key <> @choice_separator <> Enum.map_join(triple, ",", &Integer.to_string/1)
  end

  # `"resist|fire|15"` — поглощение стихии с вещей (задача 3.210), формой
  # `"<имя>|<число>"`, ровно как у надетого: имя проходит белый список
  # ruleset'а, число едет десятичным текстом, минус проезжает целиком.
  defp slot_key({:resistances, type, value}) do
    Enum.join(
      [@resistance_slot_key, Atom.to_string(type), value],
      @choice_separator
    )
  end
  ...
  defp to_gear(ruleset, tail, nested, gear_skills) do
    ...
    Gear.new(
      ...
      # Куски мини-сетов (задача 3.185). Уже отфильтрованы формой
      # (`mini_set_slot/1`), а вот сумму сверх одиннадцати слотов здесь
      # НЕ режем — это дело ядра (`Rules.MiniSets.pieces/2`), ровно как
      # у всех остальных чисел вещей: правленая руками ссылка открывается,
      # а во что она упёрлась, говорит расчёт и интерфейс.
      mini_sets: nested.mini_sets,
      # Крафтовые (именные) вещи (задача 3.186) — тем же путём: код
      # декодируется как записан, а потолок слотов (10 у Сиалы) — дело ядра
      # (`Rules.GearHitPoints.named_item_slots/1`), не декодера.
      named_items: nested.named_items,
      # Раздельные спасы (задача 3.187, часть B) — тем же путём: код
      # декодируется как записан, а потолок +20 на СУММУ с universal — дело
      # ядра (`Rules.compute/2`), не декодера.
      saves_specific: nested.saves_specific,
      # Поглощение стихий с вещей (задача 3.210) — тем же путём: стихия уже
      # прошла белый список словаря (`resistance_slot/2`), а число едет как
      # записано. Потолка у поглощения не называет ни один источник, так что
      # и клипать здесь нечего — кроме границы самого формата.
      resistances: nested.resistances,
  ...
  @nothing_nested %{
    ...
    worn: %{},
    # Куски мини-сетов (задача 3.185). Пусто у КАЖДОЙ ссылки, записанной до
    # той задачи, — и это ответ на «как читается старая ссылка»: кусков
    # в ней нет, значит `Nmini` ноль, значит усиления нет и ни одно число
    # билда не сдвинулось.
    mini_sets: [],
    # Крафтовые (именные) вещи (задача 3.186). `0` у КАЖДОЙ ссылки, записанной
    # до той задачи, — ровно умолчание `Rules.Gear`, ни одно число билда
    # не сдвинулось.
    named_items: 0,
    # Раздельные спасы (задача 3.187, часть B). `%{}` у КАЖДОЙ ссылки,
    # записанной до этой задачи — ровно умолчание `Rules.Gear`, ни одно
    # число билда не сдвинулось.
    saves_specific: %{},
    # Поглощение стихий с вещей (задача 3.210). `%{}` у КАЖДОЙ ссылки,
    # записанной до этой задачи — ровно умолчание `Rules.Gear`, ни одно
    # число билда не сдвинулось.
    resistances: %{},
    granted: %{},
  ...
  #
  # ⚠️ Мапой, а не кортежем, с задачи 3.41: псевдо-слотов стало семь
  # (3.132 добавила пятый, 3.185 — шестой, 3.186 — седьмой), и кортеж
  # читается позиционно — то есть новый вид записи добавляется правкой каждой
  # ветки `case`, а перепутанные местами два поля одного типа компилятор
  # не ловит.
  #
  # ⚠️ Все семь псевдо-слотов — `"gear"`, `"weapon|…"`, `"offweapon|…"`,
  # `"worn|…"`, `"granted"`, `"miniset|…"` и `"nameditems|…"` — разбираются
  # ЗДЕСЬ, до `Ids.fetch_slot/2`, и это не оптимизация: попади они в белый
  # список слотов, «gear» стал бы валидным слотом и для клика из браузера
  # (см. moduledoc).
  defp nest_feats(ruleset, raw, feats) do
    nested =
      Enum.reduce(raw, @nothing_nested, fn {level, slot_key, ix}, acc ->
        case {slot_or_gear(ruleset, slot_key), Map.get(feats, ix)} do
          ...
          {{:mini_sets, counts}, _pick} ->
            %{acc | mini_sets: counts}

          {{:named_items, count}, _pick} ->
            %{acc | named_items: count}

          {{:saves_specific, specific}, _pick} ->
            %{acc | saves_specific: specific}

          # ⚠️ Повторы одной стихии СКЛАДЫВАЮТСЯ, как складываются повторы
          # ранга навыка: две строки про одну стихию бывают только в правленой
          # руками ссылке, и сумма там честнее, чем «победила последняя».
          {{:resistances, type, value}, _pick} ->
            Map.update!(acc, :resistances, &Map.update(&1, type, value, fn n -> n + value end))
    ...
    # ⚠️ Сортировка — да, `Enum.uniq/1` — НЕТ (задача 3.204): две одинаковые
    # строки «фит с вещи» в коде означают два взятия у повторяемого фита без
    # выбора («Epic toughness» дважды = 40 хитов), и схлопнуть их здесь значило
    # бы потерять ввод игрока прямо в ссылке. Что из повторов считается, решает
    # ядро (`Rules.GearFeats.takes/3`) — декодер кладёт записанное как
    # записано, ровно как и с фитом, которого ruleset не знает.
    #
    # ⚠️ Уже расшаренные ссылки кодируются байт в байт: повторов в них нет
    # вовсе, а порядок задаёт та же сортировка, что и раньше.
    %{nested | gear: Enum.sort(nested.gear)}
  end
  ...
  # Куски мини-сетов (задача 3.185) — белого списка у них нет вовсе: это
  # числа, а не id, и справочник о них ничего не знает. Поэтому проверяется
  # только форма.
  defp slot_or_gear(_ruleset, @mini_set_slot_key <> @choice_separator <> rest),
    do: mini_set_slot(rest)

  # Крафтовые (именные) вещи (задача 3.186) — тоже число, не id, белого
  # списка нет вовсе, проверяется только форма.
  defp slot_or_gear(_ruleset, @named_items_slot_key <> @choice_separator <> rest),
    do: named_items_slot(rest)

  # Раздельные спасы (задача 3.187, часть B) — три числа, не id, белого
  # списка нет, проверяется только форма.
  defp slot_or_gear(_ruleset, @save_spec_slot_key <> @choice_separator <> rest),
    do: saves_specific_slot(rest)

  # Поглощение стихий (задача 3.210) — имя ПРОХОДИТ белый список ruleset'а,
  # в отличие от кусков и именных вещей: это id, а не число, и словарь
  # о нём знает (`resistance_energy_types`).
  defp slot_or_gear(ruleset, @resistance_slot_key <> @choice_separator <> rest),
    do: resistance_slot(ruleset, rest)
  ...
  # `"2,3,2"` — куски мини-сетов (задача 3.185). Читаемые числа выживают,
  # нечитаемое и неположительное теряется, и тогда строка называется ЦЕЛИКОМ
  # — `{:unknown_mini_sets, "miniset|2,x"}`.
  #
  # ⚠️ Не нулём, как битые числа оружия: там ноль означает «предмет без
  # прибавки», законное состояние билда, а здесь строка означает «набор
  # надет», и «надет ноль кусков» не значит ничего вовсе. Подставленный
  # молча `0` завёл бы в билд строку, которой игрок не вводил, — и она
  # приехала бы обратно в код при следующей же правке.
  #
  # ⚠️ Пустой список — тоже потеря: строка в коде была, а в билде её нет.
  # `nest_feats/3` кладёт `[]` и роняет причину в `dropped` — молчать здесь
  # нельзя, иначе `"miniset|"` выглядит как «мини-сетов не было».
  defp mini_set_slot(rest) do
    parsed = String.split(rest, ",")
    counts = for text <- parsed, count = mini_set_count(text), do: count
    kept = mini_set_counts(counts)

    if length(kept) == length(parsed),
      do: {:mini_sets, kept},
      else: {:unknown, {:unknown_mini_sets, @mini_set_slot_key <> @choice_separator <> rest}}
  end

  # `nil` — «это не количество кусков»: не число, ноль, отрицательное. Ноль
  # отбрасывается ЗДЕСЬ, а не считается: см. ⚠️ выше.
  defp mini_set_count(text) do
  ...
  # `"3"` — крафтовые (именные) вещи (задача 3.186). В отличие от кусков
  # мини-сетов, `0` здесь ЧИТАЕТСЯ (см. moduledoc — «вещей нет» это ровно
  # умолчание, а не бессмысленное состояние); отрицательное и нечитаемое
  # теряется строкой целиком, а не нулём.
  defp named_items_slot(rest) do
    case Integer.parse(rest) do
      {count, ""} when count >= 0 ->
        {:named_items, min(count, @named_items_limit)}

      _not_a_count ->
        {:unknown, {:unknown_named_items, @named_items_slot_key <> @choice_separator <> rest}}
    end
  end

  # `"-3,0,12"` — раздельные спасы (задача 3.187), всегда ровно три числа
  # в порядке Fort/Ref/Will. ⚠️ Теряется строкой ЦЕЛИКОМ при любом
  # расхождении (не три числа, любое из трёх нечитаемо) — та же честность,
  # что у кусков мини-сетов: «два поля из трёх» не значит ничего, молчать
  # про третье было бы решить за игрока, что оно ноль.
  #
  # `0` в отдельной позиции при этом ЧИТАЕТСЯ, как у именных вещей: «этот
  # сейв без раздельной прибавки» — ровно умолчание `Gear.saves_specific`,
  # а не бессмысленное состояние, и `non_zero/1` ниже сводит его к
  # отсутствующему ключу молча и законно.
  defp saves_specific_slot(rest) do
    with [f, r, w] <- String.split(rest, ","),
         {fort, ""} <- Integer.parse(f),
         {ref, ""} <- Integer.parse(r),
         {will, ""} <- Integer.parse(w) do
      clip = fn n -> n |> max(-@save_spec_limit) |> min(@save_spec_limit) end

      specific =
        non_zero(%{fort: clip.(fort), ref: clip.(ref), will: clip.(will)})

      {:saves_specific, specific}
    else
      _malformed ->
        {:unknown, {:unknown_saves_specific, @save_spec_slot_key <> @choice_separator <> rest}}
    end
  end

  # `"fire|15"` — поглощение одной стихии с вещей (задача 3.210). Стихия
  # обязана быть одной из семи, которые печатает секция «Резисты»; всё
  # остальное теряется ПОИМЁННО, строкой целиком.
  #
  # 🔴 Белый список здесь не формальность: `"resist|magical|15"` означает вид
  # урона, который не поглощается вовсе, а `"resist|bludgeoning|10"` —
  # физическое поглощение, которого эта секция не показывает по решению
  # владельца. Молча положить их в мапу значило бы завести в билд стихию,
  # которой у него нет, — и она приехала бы обратно в код при следующей правке.
  #
  # ⚠️ Ноль ЧИТАЕТСЯ («эта стихия без поглощения» — ровно умолчание
  # `Gear.resistances`), нечитаемое теряется строкой целиком, а не нулём:
  # молчаливая подмена битой ссылки на «поглощения не было» не лучше здесь,
  # чем у кусков мини-сетов.
  defp resistance_slot(ruleset, rest) do
    with [name, number] <- String.split(rest, @choice_separator),
         {:ok, type} <- Ids.fetch_energy_type(ruleset, name),
         {value, ""} <- Integer.parse(number) do
      {:resistances, type, clamp_resistance(value)}
    else
      _no ->
        {:unknown, {:unknown_resistances, @resistance_slot_key <> @choice_separator <> rest}}
    end
  end

  defp clamp_resistance(value),
    do: value |> max(-@resistance_limit) |> min(@resistance_limit)
