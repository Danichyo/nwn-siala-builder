defmodule BuildCalculatorWeb.BuilderGearWeaponTest do
  ...
  describe "ссылка" do
    ...
    test "«Сбросить экипировку» убирает и оружие", %{conn: conn, build: %Build{} = build} do
      gear = Gear.new(weapon: :scimitar, weapon_attack: 5)
      {:ok, view, _html} = open(conn, %Build{build | gear: gear})
    ...
    # 🔴 Задача 3.191, попутная починка расхождения ДВУХ ответов на один вопрос.
    # Свёрнутая шапка «Вещей» считала пустоту сама (характеристики, надетое, AC,
    # спасы, навыки, фиты) — и оружия в этом списке не было вовсе, поэтому билд
    # с выбранным оружием она звала «не задано», а экран просмотра тем временем
    # считал его экипированным и подсказку `#view-gear-hint` не показывал.
    # Теперь оба спрашивают одну функцию, `Gear.any?/1`.
    #
    # ⚠️ Контроль обязателен именно ЗДЕСЬ, а не в тесте про пустой билд: без
    # положительного контроля утверждение зеленело бы и на шапке, которая
    # вообще разучилась говорить «ещё не задана».
    test "выбранное оружие — это уже заданная экипировка", %{
      conn: conn,
      build: %Build{} = build
    } do
      {:ok, bare, _html} = open(conn, build)
      assert render(element(bare, "#gear-summary")) =~ "ещё не задана"

      {:ok, armed, _html} = open(conn, %Build{build | gear: Gear.new(weapon: :scimitar)})
      refute render(element(armed, "#gear-summary")) =~ "ещё не задана"
    end
  end
