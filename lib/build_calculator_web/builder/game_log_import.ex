defmodule BuildCalculatorWeb.Builder.GameLogImport do
  ...
  *that* source cannot be placed), so there is nothing to reconstruct; they
  ...
  @type result :: %{
          ...
          read: map(),
          issues: [issue()],
          gear_report: GearImport.report() | nil
        }
  ...
  def from_log(%GameLog{} = log, ruleset) do
    ...
    # `.билд+` only (task 3.187) — `log.equipment` is `[]` on every plain
    # `.билд` dump, sixteen of which are this module's own regression, so
    # `gear_report` is `nil` there and `build.gear` stays whatever
    # `Build.new/1` already defaulted it to, byte for byte.
    {build, gear_report} =
      with_equipment(build, ruleset, log.equipment, log.equipment_counts, choices)
  ...
  # -------------------------------------------------------------- equipment --
  #
  # `Rules.GearImport.sum/2`'s own moduledoc names this module — by name — as
  # the caller that hands it already-resolved values: an ability's or a
  # skill's own atom, a save, a bare feat id or a `{feat_id, choice}` pair.
  # `GameLog.equipment` gets every one of those RIGHT except a `Bonus Feat`'s
  # own free-text argument (`GameLog.equip_property/0`'s own typedoc): that
  # needs `ChoiceIndex`, and `ChoiceIndex` is a web-layer module a CORE one
  # (`BuildCalculator.GameLog`) may not call. Finishing that one piece — and
  # only that piece — is this whole section's job, one property at a time,
  # before the fully-resolved list ever reaches `GearImport.sum/2`.

  defp with_equipment(%Build{} = build, _ruleset, [], _counts, _choices), do: {build, nil}

  # `counts` — две суммы из шапки раздела второго поколения (задача 3.206,
  # `GameLog.equipment_counts/0`), пустая мапа у первого; что каждая значит
  # и как сверяется с предметами, решает `GearImport.sum/3`, не этот слой.
  defp with_equipment(%Build{} = build, ruleset, equipment, counts, choices) do
    items =
      for item <- equipment do
        %{
          item
          | properties:
              Enum.map(item.properties, &resolve_equipment_property(&1, ruleset, choices))
        }
      end

    {gear, report} = GearImport.sum(items, ruleset, counts)
    {%Build{build | gear: gear}, report}
  end

  # `{:pending_choice, id, argument}` only ever comes off a feat whose
  # `Rules.feat_choice_domain/2` is NOT `nil` — `GameLog.equip_feat_with_id/4`
  # already turned the domain-less shape (a bare grant, or an argument that
  # names nothing a domain-less feat can record) into its own final answer,
  # so this clause never has to ask "does this feat even take a value" again.
  defp resolve_equipment_property(
         %{kind: :bonus_feat, param: {:pending_choice, id, argument}} = property,
         ruleset,
         choices
       ) do
    ...
    param =
      case ChoiceIndex.resolve(Map.get(choices, domain, %{}), argument) do
        {:ok, value} -> {id, value}
        # Same honesty `GameLog` already chose for a domain-less argument: a
        # value we cannot place is not the same fact as no value at all, so
        # it stays named rather than becoming a silent bare grant.
        _not_resolved -> {:unresolved, "#{Labels.feat_name(ruleset, id)} (#{argument})"}
      end

    %{property | param: param}
  end

  defp resolve_equipment_property(property, _ruleset, _choices), do: property
  ...
  # Раздел `=== Equipped` (задача 3.187; шапка счётчиков — 3.206). Четыре
  # формы, и ни одна не роняет разбор: слот, которого нет в таблице, строка
  # свойства раньше первого слота, строка не по форме — и счётчик в шапке,
  # чьё имя мы не знаем (сервер завтра напечатает третий — игрок должен
  # услышать, что мы его видели и не прочли, а не «строка не по форме»).
  def issue_text({:unresolved_equipment_slot, token}, _ruleset),
    do: "экипировка: слот «#{token}» не распознан, предмет в нём пропущен"

  def issue_text({:equipment_property_before_slot, line}, _ruleset),
    do: "экипировка: свойство до первого слота — «#{line}»"

  def issue_text({:unrecognized_equipment_line, line}, _ruleset),
    do: "экипировка: строка не по форме — «#{line}»"

  def issue_text({:unknown_equipment_count, label}, _ruleset),
    do: "экипировка: счётчик «#{label}» в шапке раздела не прочитан — мы такого не знаем"
  ...
  def issue_kind({:unresolved_equipment_slot, _token}), do: "Не перенесено"
  def issue_kind({:equipment_property_before_slot, _line}), do: "Пропущенные строки"
  def issue_kind({:unrecognized_equipment_line, _line}), do: "Пропущенные строки"
  def issue_kind({:unknown_equipment_count, _label}), do: "Пропущенные строки"
  ...
  def issue_forms do
    [
      ...
      {:unresolved_equipment_slot, "WAIST"},
      {:equipment_property_before_slot, "[1] AC Bonus (0) 3"},
      {:unrecognized_equipment_line, "что-то совсем не по форме"},
      {:unknown_equipment_count, "ARTIFACT SET PIECES"}
    ]
  end
end
