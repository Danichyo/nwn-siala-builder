defmodule BuildCalculatorWeb.BuildViewLive do
  ...
  # Задача 3.180 — единственное место, где этот экран узнаёт `uri` запроса:
  # `mount/3` его не получает вовсе (ни разу, ни для одного из двух своих
  # кложей), а скачиваемому файлу нужен хост, которым игрок реально
  # пользуется, чтобы назвать в себе ссылку на билд (Dan, 10.09.2026:
  # «внутрь файла на самом деле добавить ссылку было бы неплохо»).
  # `host_root/1` — та же функция, что строит `BuilderLive`'s собственный
  # `#share-link`, не второй расчёт «какой у этого билда хост» (CLAUDE.md §9).
  #
  # ⚠️ `:export_file_text` пересчитывается здесь, а не в `load/3` рядом
  # с `:export_text`: `load/3` вызывается из ОБОИХ кложей `mount/3`, ДО того
  # как `handle_params/3` вообще получил `uri` — раньше в жизненном цикле
  # `base_url` попросту неоткуда взять. `handle_params/3` при этом всегда
  # отрабатывает до первого рендера (тот же порядок, на который уже
  # полагается `BuilderLive`'s собственный `:base_url`), так что плейсхолдер
  # игрок не увидит.
  #
  # ⚠️ Ветка `@error` — билд не открылся (`open/2` его не нашёл или
  # `Library.fetch_build/2` отказал), и `:build`/`:ruleset`/`:stats`
  # в сокете нет вовсе: звать `file_text/1` в этом случае значит упасть
  # `KeyError`, а не показать пустую ссылку — шаблон и так не рисует ни
  # кнопку «скачать», ни источник для нее на странице с ошибкой.
  @impl true
  def handle_params(_params, uri, socket) do
    socket = assign(socket, :base_url, BuilderLive.host_root(uri))

    {:noreply,
     if socket.assigns.error do
       socket
     else
       assign(socket, :export_file_text, file_text(socket))
     end}
  end
  ...
  defp load(socket, ruleset, %Build{} = build) do
    ...
    socket =
      ...
      # «Против заклинаний» под самими Fort/Ref/Will (задача 3.197 часть B,
      # замер `AQ1`) — та же строка, тем же вызовом, что у панели итогов
      # конструктора: пришедший по ссылке не знает, что Spellcraft вообще
      # что-то добавляет, если это не названо прямо на этом экране.
      ...
      # Задача 3.179 — считается один раз при загрузке, не при каждом
      # переключателе: этот билд не меняется под игроком (в отличие от
      # конструктора), так что пересчитывать при `"toggle_view_granted"`
      # незачем — имя не зависит от того, показаны ли выданные фиты.
      |> assign(:export_filename, Export.file_name(build, ruleset, stats))
  ...
  # Задача 3.180 — тот же приём, тот же переключатель, а зовёт
  # `Export.file_text/5`: `export_text/1`'s блок плюс ссылка одной строкой
  # внизу. `BuilderLive.view_url/1` — та же функция, что рисует
  # `#share-link` в конструкторе; `assigns.base_url`/`.code` там читаются
  # ровно так же, как там, поэтому файл с этого экрана и файл, скачанный
  # из конструктора того же билда, не могут назвать два разных адреса
  # (CLAUDE.md §9 про две копии одного правила).
  #
  # Два читателя, оба обязаны звать именно это, а не собирать
  # `Export.file_text/5` заново: `handle_params/3` (один раз при открытии,
  # там, где впервые появляется `base_url`) и `"toggle_view_granted"` ниже
  # (на каждый клик).
  defp file_text(socket) do
    Export.file_text(
      socket.assigns.build,
      socket.assigns.ruleset,
      socket.assigns.stats,
      BuilderLive.view_url(socket.assigns),
      title: socket.assigns.title,
      show_granted_feats: socket.assigns.view_show_granted?
    )
  end
  ...
  # Задача 3.180 добавила третий читатель того же клика — `:export_file_text`
  # (скачиваемый файл) — и он двигается здесь же: иначе кнопка «скачать»
  # на секунду отдавала бы файл, не совпадающий с тем, что показал гид.
  def handle_event("toggle_view_granted", _params, socket) do
    ...
    {:noreply,
     ...
     |> assign(:export_file_text, file_text(socket))}
  end
