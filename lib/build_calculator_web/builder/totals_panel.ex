defmodule BuildCalculatorWeb.Builder.TotalsPanel do
  ...
  def assign_panel(socket) do
    ...
    rows =
      Map.new(panel_fields(stats), fn {key, label, value, signed?, cap_key} ->
        # «Против заклинаний» (задача 3.197 часть B) — второй, НЕ суммируемый
        # список у Fort/Ref/Will и только у них. Оба поля из ОДНОГО вызова,
        # а не порознь: заголовок обязан появляться ровно тогда, когда под
        # ним есть хоть одна строка, иначе DOM нёс бы `data-pop-extra-title`
        # без единого терма — честный пустой список печатать нечем, а
        # заголовок без содержимого выглядел бы как забытая строка.
        {extra_title, extra_terms} = panel_extra(ruleset, stats, key)
        ...
        {key,
         %{
           ...
           # Передаётся `<.stat_pop>` отдельными атрибутами, а не дописан
           # в `terms` — сумма `terms` обязана сходиться со значением строки,
           # а условная прибавка в это значение не входит вовсе (замер `AQ1`).
    ...
    # «Против заклинаний» под тройкой Fort/Ref/Will (задача 3.197 часть B) —
    # тем же приёмом, что у `racial_bonus_note` выше (`%{text:, group:}`,
    # рендерится в группе, чей ключ совпал). `nil`, пока билду нечего сказать
    # (`stats.conditional_save_terms` пуст).
    |> assign(:saves_vs_spells_note, Summary.saves_vs_spells_note(stats))
    ...
    # Резисты (задача 3.211) — своя строка на стихию, ВНЕ `@panel_groups`:
    # число строк меняется от билда к билду (0..7), а `@panel_layout` —
    # фиксированный список ключей, тем же приёмом, что `@skill_totals`
    # выше уже не пытается втиснуться в него.
    |> assign(:resistance_rows, resistance_rows_with_ghost(ruleset, stats, preview))
  ...
  defp panel_fields(stats) do
    [
      # Пара «HP голым / HP в экипировке» (задача 3.186) — тем же приёмом, что
      # у AC ниже: `hp_naked` считается вторым проходом по пустому `%Gear{}`
      # (`Rules.compute/2`, `hp_naked`), «в экипировке» — тот же ключ `:hp`,
      # что был здесь ДО этой задачи, — его DOM-id (`#stat-hp`) и его разбор
      # (`Summary.hp_terms/2`) не тронуты ни на бит, только ярлык уточнён:
      # раньше «HP» значило «в экипировке» молча, теперь названо прямо, как
      # у AC.
      {:hp_naked, "HP голым", stats.hp_naked, false, nil},
      {:hp, "HP в экипировке", stats.hp, false, nil},
  ...
  # «Против заклинаний» — только у трёх сейвов (задача 3.197 часть B).
  # Заголовок — константа модуля `Summary`, не литерал здесь: панель, экран
  # просмотра и попап обязаны называть подраздел одним словом.
  defp panel_extra(ruleset, stats, key) when key in [:fort, :ref, :will] do
    case Summary.vs_spells_terms(ruleset, stats, key) do
      [] -> {nil, []}
      terms -> {Summary.vs_spells_title(), terms}
    end
  end

  defp panel_extra(_ruleset, _stats, _key), do: {nil, []}
  ...
  @panel_layout [
    {"vital", "живучесть",
     [
       # Пара, а не одна строка (задача 3.186) — тем же доводом, что у AC:
       # мини-сеты и крафтовые (именные) вещи домножают итог до +105 %, и
       # одно число перестало отвечать сразу на «сколько без вещей» и
       # «сколько в бою». ⚠️ Ключ пары — `"hp_pair"`, НЕ `"hp"`: часть `:hp`
       # (в экипировке) сохраняет свой DOM-id `#stat-hp` байт в байт — на нём
       # стоит уже добрый десяток тестов, — а у самой пары id ещё не читал
       # никто, поэтому она вольна называться иначе, лишь бы не тем же словом,
       # что и её собственная часть (иначе два элемента претендовали бы на
       # один `id`).
       {:pair, "hp_pair", "HP", "голым / в экипировке", [:hp_naked, :hp]},
  ...
  # Тот же приём, что у навыков выше: `Summary.resistance_entries/2` уже
  # решает, какие стихии показывать и в каком порядке (ОДНО чтение фильтра
  # на конструктор и просмотр — см. её же доку), здесь только разбор термами
  # и призрачная дельта наведения.
  #
  # ⚠️ Строки, которых у ТЕКУЩЕГО билда ещё нет (стихия появится только
  # после выбора кандидата, например `Epic energy resistance` на новую
  # стихию), сюда не попадают — тот же предел, что уже принят у
  # `@skill_totals` («показываем только вложенные навыки»): дельта видна
  # там, где строка уже есть, а не заводит новую на лету.
  defp resistance_rows_with_ghost(ruleset, stats, preview) do
    for %{id: id, en: en, entry: entry} <- Summary.resistance_entries(ruleset, stats) do
      # Проигравшая сторона сравнения эффект/вещь — ВТОРЫМ, не суммируемым
      # блоком поп-апа под своим заголовком (задача 3.218), а не строкой
      # среди слагаемых: под «Fire 45» стояли `+15 / +30 / +15`, и разбор
      # спорил со своим итогом. Заголовок — только когда блоку есть что сказать.
      lost = Summary.resistance_lost_terms(ruleset, stats, entry)

      %{
        id: id,
        label: en,
        value: entry.counted,
        terms: Summary.resistance_terms(ruleset, stats, entry),
        extra_title: if(lost != [], do: Summary.resistance_lost_title()),
        extra_terms: lost,
        ghost: GearPanel.diff(entry.counted, preview_resistance_total(preview, id))
      }
    end
  end

  defp preview_resistance_total(nil, _id), do: nil

  defp preview_resistance_total(preview, id) do
    case Map.get(preview.resistances, id) do
