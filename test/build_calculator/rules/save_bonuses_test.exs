defmodule BuildCalculator.Rules.SaveBonusesTest do
  ...
  describe "потолок +20: один клип на сейв, и он покрывает НЕ ВСЕ прибавки" do
    ...
    #
    # 🔴 **ЗДЕСЬ СТОЯЛО `save_bonus == 20` и срез −2** — до задачи 3.197, пока
    # прибавка Spellcraft лежала в самом сейве. С 12.09.2026 (замер `AQ1`) она
    # в лист персонажа не идёт, поэтому в листе остаются одни вещи (14, без
    # среза), а сумма и её потолок наблюдаются в условном числе: восемь рангов
    # предлагают +8, а доезжает из них **+6**. Свойство то же, место другое.
    test "клип считает СУММУ вещей и Spellcraft, а не каждое по отдельности", %{ruleset: ruleset} do
      ...
      # Лист: одни вещи, потолок не тронут.
      assert stats.save_bonus == %{fort: 14, ref: 14, will: 14}
      assert stats.save_cap_clipped == %{fort: 0, ref: 0, will: 0}
      assert Enum.filter(stats.capped, &(&1 in [:fort_save, :ref_save, :will_save])) == []

      # Против заклинаний: 14 + 8 = 22 → 20, то есть прибавка даёт +6, а не +8.
      # Клип «по частям» дал бы здесь полные +8 и сумму 22.
      assert stats.conditional_save_bonus == %{fort: 6, ref: 6, will: 6}
      assert Enum.map(stats.conditional_save_terms, & &1.bonus) == [8]
    ...
    test "Spellcraft внутри капа: 6 + 14 = ровно 20, 6 + 20 = срез", %{ruleset: ruleset} do
      ...
      # ⚠️ ЗДЕСЬ СТОЯЛО `save_bonus == 20`: до задачи 3.197 обе прибавки лежали
      # в одном числе. Теперь «ровно 20» — это число ПРОТИВ ЗАКЛИНАНИЙ (14 вещей
      # плюс все шесть от рангов), а в листе стоят одни вещи.
      assert exact.save_bonus == %{fort: 14, ref: 14, will: 14}
      assert exact.conditional_save_bonus == %{fort: 6, ref: 6, will: 6}
      ...
      # Вторая половина: вещи +20 занимают потолок целиком, и шести рангам
      # доехать некуда — условная прибавка **ноль при полных рангах**. Ровно
      # это и объясняет Мокси, у которой лист сходится с игрой при Spellcraft 30
      # (`GearImportEngineTest`).
      assert over.save_bonus == %{fort: 20, ref: 20, will: 20}
      assert over.conditional_save_bonus == %{fort: 0, ref: 0, will: 0}
      assert over.saves_conditional == %{fort: over.fort, ref: over.ref, will: over.will}

      # ⚠️ ЗДЕСЬ СТОЯЛ срез −6 и флаг капа: их ставила прибавка Spellcraft,
      # которой в листе больше нет. Вещи ровно в потолок — это не срез
      # (`Caps.clamp/3` режет строго больше), и флага нет тоже.
      assert over.save_cap_clipped == %{fort: 0, ref: 0, will: 0}
      assert Enum.filter(over.capped, &(&1 in [:fort_save, :ref_save, :will_save])) == []
    end
  ...
  # 🔴 Условная прибавка на КАПЕ УРОВНЕЙ Сиалы и на обеих сторонах потолка
  # прибавок — две точки одной таблицы (задача 3.197).
  describe "условная прибавка на 41-м уровне" do
    alias BuildCalculator.Rules.Gear

    # | вещи | лист | против заклинаний | прибавка |
    # |------|------|-------------------|----------|
    # |    0 |   22 |                30 |       +8 |
    # |  +20 |   42 |                42 |        0 |
    #
    # ⚠️ Вторая строка — то, что выглядит багом и им не является: сорок рангов
    # не дают НИЧЕГО, потому что вещи заняли потолок целиком. Ровно так себя
    # ведёт лист Мокси в игре (`GearImportEngineTest`).
    test "40 рангов без вещей дают все восемь, с вещами +20 — ни одного", %{ruleset: ruleset} do
      levels = List.duplicate(:fighter, 41)
      ranks = %{41 => %{spellcraft: 40}}

      bare = Rules.compute(build(levels, skills: ranks), ruleset)
      geared = Rules.compute(build(levels, skills: ranks, gear: Gear.new(saves: 20)), ruleset)

      assert bare.skill_save_bonus == 8
      assert geared.skill_save_bonus == 8

      assert {bare.fort, bare.ref, bare.will} == {22, 16, 16}
      assert bare.conditional_save_bonus == %{fort: 8, ref: 8, will: 8}

      assert {bare.saves_conditional.fort, bare.saves_conditional.ref,
              bare.saves_conditional.will} ==
               {30, 24, 24}

      assert {geared.fort, geared.ref, geared.will} == {42, 36, 36}
      assert geared.conditional_save_bonus == %{fort: 0, ref: 0, will: 0}
      assert geared.saves_conditional == %{fort: 42, ref: 36, will: 36}

      # ⚠️ И «голое» число, и число требования у обоих одно и то же: ни вещей,
      # ни условной прибавки в них нет.
      for stats <- [bare, geared] do
        assert stats.saves_naked.fort == 22
        assert stats.saves_for_prereqs.fort == 22
      end
    end
  end
  ...
  describe "двойной учёт: старые источники не сдвинулись" do
    ...
    test "вещи и Spellcraft остаются одним числом на все три сейва", %{ruleset: ruleset} do
      ...
      # ⚠️ ЗДЕСЬ СТОЯЛО `%{fort: 9, …}` — сумма обеих прибавок. С задачи 3.197
      # в листе только вещи, а четыре от рангов стоят рядом, «против
      # заклинаний»; save-agnostic обе по-прежнему.
      assert stats.save_bonus == %{fort: 5, ref: 5, will: 5}
      assert stats.conditional_save_bonus == %{fort: 4, ref: 4, will: 4}
    end
  ...
  describe "saves_naked — сейв без блока «Вещи»" do
    ...
    # ⚠️ Своё остаётся своим: фит из слота — это не вещь, снять его нельзя,
    # и в голом числе он обязан быть. Без этой половины правка ушла бы в другую
    # крайность: «голым» стало бы значить «без всего».
    #
    # 🔴 **ЗДЕСЬ СТОЯЛО `spellcraft.saves_naked.fort == 8`, и Spellcraft был
    # во второй половине этого же теста** — до задачи 3.197. Замер `AQ1` (Dan,
    # 12.09.2026: «спасы от Spellcraft в листе персонажа не видна») развёл эти
    # две прибавки: «голым» значит «без вещей», а прибавка от рангов не вещь —
    # она **условная**, и её нет в самом листе, ни голом, ни одетом. Поэтому
    # у Spellcraft проверяется другое поле, а не другое число.
    test "фит из слота в голое число входит, а условная прибавка — рядом", %{ruleset: ruleset} do
      levels = List.duplicate(:fighter, 11)
      ...
      # Лист не сдвинулся ни голый, ни одетый — вещей у билда нет вовсе…
      assert spellcraft.saves_naked.fort == 7
      assert spellcraft.saves_naked.fort == spellcraft.fort
      assert slot.saves_naked.fort == slot.fort

      # …а сама прибавка на месте, своим числом и со своим условием.
      assert spellcraft.conditional_save_bonus == %{fort: 1, ref: 1, will: 1}
      assert spellcraft.saves_conditional.fort == 8

      assert spellcraft.conditional_save_terms == [
               %{skill: :spellcraft, scope: "spells", bonus: 1}
             ]

      # …и у фита из слота её нет вовсе: он безусловный.
      assert slot.conditional_save_bonus == %{fort: 0, ref: 0, will: 0}
      assert slot.conditional_save_terms == []
    end

    # 🔴 Главное, ради чего голое число считается ВТОРЫМ ПРОХОДОМ, а не
    # вычитанием: потолок +20 стоит внутри формулы. Здесь `Sacred defense` даёт
    # +5, вещи +20, оба под капом — после клипа остаётся ровно 20, и «итог минус
    # вещи» напечатало бы, что своего у билда нет вообще.
    #
    # ⚠️ **ЗДЕСЬ СТОЯЛ ДРУГОЙ ПРИМЕР — вор 41 с 40 рангами Spellcraft**, и
    # задача 3.197 его обезвредила: прибавки от рангов в листе больше нет, у
    # того билда внутри капа остались одни вещи, и «итог минус вещи» случайно
    # сошлось бы с голым числом. Свойство проверять надо тем, что в листе ЕСТЬ
    # и лежит под капом, а такая прибавка билда сегодня ровно одна —
    # `Sacred defense` Чемпиона Торма.
    test "голое число — не «итог минус вещи», когда кап кусает", %{ruleset: ruleset} do
      ...
      # Своих +5 никуда не делось: их съел не потолок, а вещи, которых
      # у голого персонажа нет.
      assert stats.fort - stats.saves_naked.fort == 15
  ...
  describe "saves_for_prereqs — что засчитывает требование" do
    ...
    # Взяты три формы разметки — плоский фит, модификатор характеристики
    # и таблица по уровню класса, — и обе стороны капа.
    #
    # ⚠️ **ЗДЕСЬ БЫЛА ЧЕТВЁРТАЯ СТРОКА — `spellcraft`**, и задача 3.197 её
    # убрала: прибавки от рангов нет в листе персонажа (замер `AQ1`), значит,
    # её не видит и требование. ✅ С 13.09.2026 это **измерение, а не
    # допущение** — замер `AR1` (Dan: «AR1 провел, resist energy не доступен,
    # 5 очков spellcraft не играют роли»); проверяется соседним тестом.
    test "остальные прибавки засчитываются: фит, классовое умение, таблица", %{
      ruleset: ruleset
    } do
    ...
    # 🔴 А УСЛОВНАЯ ПРИБАВКА ТРЕБОВАНИЕ НЕ ВЫПОЛНЯЕТ — задача 3.197. Довод был:
    # числа нет в листе персонажа (`AQ1`), а требование, скорее всего, смотрит
    # на то же число, что печатает лист. До правки здесь была ложная
    # ЛЕГАЛЬНОСТЬ — калькулятор открывал `Resist energy` персонажу, которому
    # игра его не даёт.
    #
    # ✅ **ЗАМЕРЕНО 13.09.2026, кейс `AR1`** (Dan: «AR1 провел, resist energy
    # не доступен, 5 очков spellcraft не играют роли») — воин со Стойкостью 7
    # и пятью рангами, купленными раньше. Здесь стояло «**допущение
    # с названным направлением**, а не замер» и «ответ „фит виден“ вернёт
    # `counts: true`»: ответ пришёл обратный, поле не тронуто, статус в данных
    # поднят до `verified`. Ни одно число не сдвинулось.
    test "условная прибавка Spellcraft требование НЕ выполняет", %{ruleset: ruleset} do
      stats =
        Rules.compute(
          build(List.duplicate(:fighter, 9), skills: %{9 => %{spellcraft: 5}}),
          ruleset
        )

      # Прибавка настоящая: против заклинаний она есть…
      assert stats.conditional_save_bonus.fort == 1
      assert stats.saves_conditional.fort == stats.fort + 1

      # …а в листе её нет, и требование видит ровно лист.
      assert stats.saves_for_prereqs.fort == stats.fort
      assert stats.saves_for_prereqs == stats.saves_naked

      # ⚠️ Контроль, отделяющий это правило от соседнего (S3, `Luck of heroes`):
      # там прибавка ЕСТЬ в листе и вычтена только из требования, здесь её нет
      # ни там, ни там. Числа обязаны различаться, иначе два разных правила
      # проверялись бы одним наблюдением.
      luck = Rules.compute(with_luck(List.duplicate(:fighter, 9)), ruleset)

      assert luck.fort - luck.saves_for_prereqs.fort == 1
      assert stats.fort - stats.saves_for_prereqs.fort == 0
    end
