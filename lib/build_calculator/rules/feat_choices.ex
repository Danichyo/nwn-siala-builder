defmodule BuildCalculator.Rules.FeatChoices do
  ...
  @doc """
  Whether the data lets this feat be taken again **for the same thing**.

  Two ways that happens, and the data states both: the feat names no value at
  all (`Epic toughness`), or it names one and the takes need not differ
  (`repeatable.distinct? == false`). Everywhere else the domain **is** the
  ceiling — every take must name something new — and a second take of one value
  is not a take but a duplicate.

  ⚠ The question is deliberately wider than `Rules.GearFeats.stackable?/2`,
  which answers «может ли одно ИМЯ стоять в списке несколько раз» and therefore
  still wants `domain == nil`: the interface draws a counter off that one and a
  per-value row off its negation. This one answers «повторяется ли одно и то
  же», which is what a *take count* hangs on — and today those two answers
  differ for exactly one feat, `Epic energy resistance` (задача 3.210).

  ⚠ **Nothing here names a feat**, and that is the whole point of reading
  `distinct?`: «ровно на него» comes out of the data, not out of a list
  somebody has to remember to extend.

  Private under a different name since the max-takes gap was written
  (`repeats_the_same?/1`, task 1.x); public since 18.09.2026, because the
  resistance stat needs the same sentence and a second copy of it could only
  ever drift.
  """
  @spec repeats_same_value?(atom(), map()) :: boolean()
  def repeats_same_value?(feat_id, ruleset) do
    case Map.get(ruleset.feats, feat_id) do
      %{} = feat ->
        case repeatable(feat) do
          nil -> false
          repeatable -> repeats_the_same?(repeatable)
        end
  ...
  # ⚠ Задача 3.210 добавила СЕДЬМОГО — `Rules.Resistances`, — и это ровно тот же
  # случай, что у пятого и шестого: `Resist energy` и `Epic energy resistance`
  # стали носителями этой оговорки 18.09.2026 задачей 3.209 (им дали нашего
  # получателя, `damage_resistance`), а считать их начала задача 3.210. Без
  # строки ниже у каждого билда с ними рядом с посчитанной секцией «Резисты»
  # висело бы «прибавку от фита в статы не считаем». Признание снято не
  # вычёркиванием: у записи, чей `effect_coverage` перестанет объявлять
  # полноту, оговорка вернётся сама.
