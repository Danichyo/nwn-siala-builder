defmodule BuildCalculator.Rules.GearFeats do
  ...
  **gap**, never a refusal — `{:not_modelled, {:gear_feat_choice, id}}`, owed
  ...
  ⚠ **And it was one take per feat until 13.09.2026, whatever the items said** —
  task 3.204. Слово Dan: «на фитах с вещи нам необходимо добавить возможность
  настакивать те фиты, которые можно взять несколько раз… Там не только epic
  toughness». Его же билд и измерил цену: игра печатает 1503 хита, модель
  печатала 1470 с одним объявленным `Epic toughness` и 1437 без единого, а
  сапоги с поясом лендят два. So `takes/3` counts declarations for a feat the
  data marks repeatable **with no value to name**, and `stackable?/2` is that
  test — read from `repeatable.choice`, never from a list of feat ids.
  ...
  @doc """
  Whether the same feat may be declared off items **more than once**.

  The data's answer and nothing else: a feat is repeatable and names no value
  (`repeatable.choice == nil`), so every declaration is another take of the one
  thing. Sixteen on `siala_41` (прогон 13.09.2026) — `Epic toughness`, the six
  `Great …`, `Improved spell resistance`, `Improved sneak attack`, `Great
  smiting`, `Improved stunning fist`, `Self concealment`, `Epic damage
  reduction` and the three `Automatic … spell`. There is no list of them here
  and there must never be one (`Rules.FeatChoices`'s own rule).

  ⚠ A feat that repeats **with a value** answers `false`, and that is the
  remaining half of AGENT_QUEUE.md 3.29: what repeats there is the pair, and an
  item cannot say which copy of `Epic energy resistance (fire)` it lends. The
  pair therefore stays unique in `Gear.feats` and this answers `false` so the
  interface offers a chip rather than a counter.
  """
  @spec stackable?(atom(), map()) :: boolean()
  def stackable?(feat_id, ruleset) when is_atom(feat_id) do
    FeatChoices.repeatable?(feat_id, ruleset) and
      is_nil(FeatChoices.domain(feat_id, ruleset))
  end

  @doc """
  Whether **each declaration** of this feat is a take of its own — the question
  `takes/3,4` is decided by, and not the same one as `stackable?/2`.

  Two halves, both read from the data and neither from a list here:

    * the snapshot says declarations count at all
      (`ruleset.gear.feat_takes == :per_declaration`, задача 3.204);
    * the feat repeats **for the same thing**
      (`Rules.FeatChoices.repeats_same_value?/2`) — no value to name, or a value
      the source lets a second take repeat.

  🔴 **Wider than `stackable?/2` by exactly one feat today**, and the difference
  is not cosmetic. `stackable?/2` asks «может ли одно ИМЯ стоять в списке
  несколько раз с одной и той же (пустой) припиской» — the interface draws a
  counter off it and a row-per-value off its negation, and widening it would
  send `Epic energy resistance` down the counter branch and take the element
  picker away (`BuildCalculatorWeb.Builder.GearPanel`'s `gear_feat_group/5`
  says so in as many words). This one asks «сколько ВЗЯТИЙ стоит за записями»,
  and for a feat with a value the unit is the **pair** — which is why the
  counting function that reads it takes the value too (`takes/4`).

  Слово Dan 18.09.2026 («подтверждаю твое предложение» — считать
  `Epic energy resistance` с предмета так же, как `Epic toughness` в 3.204:
  римская ступень есть номер взятия, одна ступень на двух предметах — одно
  взятие); правило — в данных, `overrides.json` → `gear.feats.takes.applies_to`.
  """
  @spec takes_per_declaration?(atom(), map()) :: boolean()
  def takes_per_declaration?(feat_id, ruleset) when is_atom(feat_id) do
    case scope(ruleset) do
      :without_choice -> stackable?(feat_id, ruleset)
      :same_value -> FeatChoices.repeats_same_value?(feat_id, ruleset)
    end
  end

  # Какие фиты режим «каждая запись — взятие» накрывает. `nil` в данных —
  # прежнее, узкое чтение: снапшот, который про расширение молчит, обязан
  # считать так, как считал до 18.09.2026. Направление умолчания — занижение.
  defp scope(ruleset) do
    case ruleset do
      %{gear: %{feat_takes_applies_to: :same_value}} -> :same_value
      _narrow_or_absent -> :without_choice
    end
  end

  @doc """
  How many takes of `feat_id` the items lend — the gear half of
  `Build.feat_takes_owned/4`.

  One per accepted declaration for a feat that may repeat (`stackable?/2`), and
  `1`/`0` for every other — an item lends such a feat, not a number of copies of
  it, and two declarations of one `Skill focus` value are still one feat with
  one value.

  Слово Dan 13.09.2026, and it is the whole of task 3.204: «на фитах с вещи нам
  необходимо добавить возможность настакивать те фиты, которые можно взять
  несколько раз… Там не только epic toughness». Measured before it on his own
  build (Карлик, воин 10 / защитник 23 / мастер оружия 7): the game prints 1503
  hit points, the model printed 1437 with nothing declared and 1470 with one
  `Epic toughness` — the two the boots and belt actually lend come to 1503.

  ⚠ **A ceiling is not applied here.** The stated `repeatable.max_takes` governs
  what may be *declared* (`add_reasons/3`), and the ceiling on the **effect**
  lives with the effect (`max_total`, `Rules.FeatBonuses`) — Dan 14.08.2026:
  «как максимум для УЧЁТА там всё равно будет только 10 раз, учитывая СУММУ
  того, что взяли в билде, и того, что набрали с вещей». Clamping here as well
  would be the second half-clip that once let a build carry +40 on a save.

  ⚠ Which mode this counts in is **read from the ruleset**
  (`ruleset.gear.feat_takes`): a snapshot that does not state the rule gets the
  narrower answer this module gave before 13.09.2026 — one take however many
  declarations. The narrower side is the one that understates rather than
  invents, and a build made against such a snapshot has to keep opening as the
  build it described.
  """
  @spec takes(Gear.t(), map(), atom()) :: non_neg_integer()
  def takes(%Gear{} = gear, ruleset, feat_id) when is_atom(feat_id) do
    if per_declaration?(ruleset) and stackable?(feat_id, ruleset) do
      Enum.count(gear.feats, &(Build.feat_id(&1) == feat_id and validate(&1, ruleset) == :ok))
    else
      if MapSet.member?(held(gear, ruleset), feat_id), do: 1, else: 0
    end
  end

  @doc """
  How many takes the items lend **of this exact pair** — the same question one
  arity down, asked about a feat that names a value.

  Задача 3.210. The unit that repeats there is the pair, never the name:
  boots lending `Epic energy resistance (Fire)` and a belt lending
  `(Cold)` are one take of fire and one of cold, not two of either. So the
  count is over entries equal to this pair, and `takes/3` is deliberately
  **not** expressed through this one: it answers «сколько взятий этого ИМЕНИ»,
  which for a feat with a value would have to conflate the values.

  ⚠ `choice: nil` is a value like any other and counts the **bare**
  declarations — the ones every link written before task 3.97 carries. A bare
  `Epic energy resistance` therefore adds nothing to any element, and the build
  says so (`{:not_modelled, {:gear_feat_choice, id}}`) rather than guessing an
  element for it.

  ⚠ Repeats count only where the data says they may
  (`takes_per_declaration?/2`); everywhere else two declarations of one pair are
  one take, exactly as `takes/3` answers for a feat with a value.
  """
  @spec takes(Gear.t(), map(), atom(), atom() | nil) :: non_neg_integer()
  def takes(%Gear{} = gear, ruleset, feat_id, choice)
      when is_atom(feat_id) and is_atom(choice) do
    declared =
      Enum.count(gear.feats, fn entry ->
        Build.feat_id(entry) == feat_id and Build.feat_choice(entry) == choice and
          validate(entry, ruleset) == :ok
      end)

    cond do
      declared == 0 -> 0
      takes_per_declaration?(feat_id, ruleset) -> declared
      true -> 1
    end
  end

  defp per_declaration?(ruleset) do
    case ruleset do
      %{gear: %{feat_takes: mode}} -> mode == :per_declaration
      _no_section -> false
    end
  end

  @doc """
  Why one **more** declaration of this entry may not be added — `[]` when it may.

  Asked by the interface before it adds a take, the way
  `Rules.validate_gear_feat/2` is asked before it records an entry at all. Two
  answers, both of them forms the web layer already words:

    * `{:max_takes, id, n}` — the data states a ceiling on takes and the items
      already lend that many. ⚠ **Counted off the items alone**, never off the
      slots too, and that is the same split the slot side keeps in the other
      direction: `Rules.FeatChoices` counts slots only, «брать эти фиты в билде
      также можно вплоть до 10 раз (если фит уже взят с вещи его можно взять
      при левел апе этот же фит)» (замер Dan 14.08.2026, `GAME_CHECKS.md` H8).
      Two ceilings on two counts; what stops the sum running away is the ceiling
      on the **effect**, which is applied where the effect is;
    * `{:already_taken, id}` — the feat does not repeat at all, so a second
      declaration would be the same statement twice. The interface offers a chip
      there and never reaches this, but a hand-edited link does.

  ⚠ Where the data states **no** ceiling the answer is `[]`, and the build says
  so elsewhere (`{:missing_data, {:feat_max_takes, …}}`): inventing ten from
  «to a maximum of 200 hit points» would be arithmetic on a number nobody wrote
  down (`Rules.FeatChoices.over_max_takes/3`'s own rule, kept identical here).

  🔴 **Both answers are about the WHOLE entry, not about its name** (задача
  3.224). A feat that repeats *for the same value* (`repeats?/2`) is asked its
  ceiling **per pair**, exactly the way the slot side asks it — `twice for fire
  and once for electricity is two takes and one, never three` (Дан, 02.08.2026,
  `Rules.FeatChoices.repeat_reasons/6`). Before that this counted `takes/3`, the
  count of the *name*, and answered `{:already_taken, :epic_energy_resistance}`
  to «can Fire be taken once more» — a refusal the data does not state, and one
  that made the counter in «Вещи» un-addable.
  """
  @spec add_reasons(Gear.t(), map(), Gear.feat_entry()) :: [tuple()]
  def add_reasons(%Gear{} = gear, ruleset, entry) do
    feat_id = Build.feat_id(entry)

    cond do
      not repeats?(entry, ruleset) and MapSet.member?(held(gear, ruleset), feat_id) ->
        [{:already_taken, feat_id}]

      over_ceiling?(gear, ruleset, entry) ->
        [{:max_takes, feat_id, max_takes(feat_id, ruleset)}]
  ...
  @doc """
  Whether **this entry** may stand in the list more than once — the question a
  counter hangs on, asked about a declaration rather than about a name.

  Two forms, two readings, and the split is the one the rest of this module
  already keeps:

    * a **bare** declaration repeats when the feat names no value at all
      (`stackable?/2`). A second bare entry of a feat that *does* name one is
      not a second take but the same unsaid thing twice — `Epic energy
      resistance` with nothing chosen lends no element either way;
    * a declaration **with a value** repeats when every declaration is a take
      (`takes_per_declaration?/2`), i.e. the data lets the same value be taken
      again (`Rules.FeatChoices.repeats_same_value?/2`) and the snapshot counts
      declarations at all.

  Задача 3.224: public because two readers need the very same sentence — the
  counter's «+» (`BuilderCalculatorWeb`'s gear panel and its `gear_feat_more`
  event) and `add_reasons/3` right above. A second copy of it in the web layer
  would be a game rule living outside the core, and it would drift.
  """
  @spec repeats?(Gear.feat_entry(), map()) :: boolean()
  def repeats?(entry, ruleset) do
    feat_id = Build.feat_id(entry)

    case Build.feat_choice(entry) do
      nil -> stackable?(feat_id, ruleset)
      _value -> takes_per_declaration?(feat_id, ruleset)
    end
  end

  # ⚠ Считается той же единицей, какой считается взятие (задача 3.224): у голой
  # записи — имя (`takes/3`, как было), у записи со значением — ПАРА (`takes/4`,
  # та самая функция, ради которой у неё есть четвёртая арность). Иначе потолок
  # «десять на каждый вид урона» превращался бы в «десять на фит целиком».
  defp over_ceiling?(gear, ruleset, entry) do
    feat_id = Build.feat_id(entry)

    case max_takes(feat_id, ruleset) do
      nil ->
        false

      max ->
        case Build.feat_choice(entry) do
          nil -> takes(gear, ruleset, feat_id) >= max
          value -> takes(gear, ruleset, feat_id, value) >= max
        end
    end
  end

  @doc """
  The ceiling on takes this ruleset states for `feat_id`, or `nil` when it
  states none.

  The same read `Rules.FeatChoices` does for a slot pick, exposed so the
  interface can size a counter without a second copy of the field's name.
  """
  @spec max_takes(atom(), map()) :: pos_integer() | nil
  def max_takes(feat_id, ruleset) when is_atom(feat_id) do
    with %{} = feat <- Map.get(ruleset.feats, feat_id),
         %{} = repeatable <- Map.get(feat, :repeatable),
         %{value: value} when is_integer(value) <- Map.get(repeatable, :max_takes) do
      value
    else
      _no_stated_ceiling -> nil
    end
  end
  ...
  ⚠ **Not a count of takes.** Two entries here are two skills getting +3; the
  count of takes is `takes/3`, and it answers `1` for exactly the feats this
  list is about — a feat with a value does not stack off items
  (`stackable?/2`). The two lists are the same length only for the feats that
  have no value to name, and even then they answer different questions.
  """
