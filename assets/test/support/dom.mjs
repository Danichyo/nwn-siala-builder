...
export function installDom({ width = 1440, height = 900 } = {}) {
  ...
  // Node 21+ already defines a global `navigator` of its own (a getter-only
  // accessor with no `.clipboard` at all) — `.CopyText`'s hook
  // (`copy_text/1`, task 3.179) reads `navigator.clipboard.writeText` as a
  // free identifier, exactly like `document`/`matchMedia` above, and a plain
  // `globalThis.navigator = …` throws in strict-mode ESM against a
  // getter-only property. `defineProperty` replaces it outright with happy-
  // dom's own `window.navigator`, which DOES carry a `clipboard` stub.
...
export function uninstallDom() {
  for (const key of ["window", "document", "matchMedia", "navigator"]) {
