defmodule BuildCalculator.Rules.ComputeTest do
  ...
  describe "hp_breakdown" do
    test "by-class dice, the CON term and a zero floor adjustment sum to hp", %{ruleset: ruleset} do
      ...
      # Пятый терм — процент шарда за надетое (задача 3.186). У этого билда
      # вещей нет, значит `nil`, и разбор сходится ровно как сходился. Тот же
      # инвариант с НЕнулевым термом — в `gear_hit_points_test.exs`.
    ...
    test "an empty build breaks down to all zeroes, not nil and not a crash", %{ruleset: ruleset} do
      ...
      # ⚠️ `gear_bonus` тоже `nil`, и по своей причине: процент шарда за надетое
      # выбирается по числу вещей, а их у пустого билда ноль (задача 3.186).
  ...
  describe "saves" do
    ...
    # ranks **against spells**. CLAUDE.md §3 calls this "заметный вклад, который
    # игроки не считают", which is why it is a term of its own rather than
    # folded into the totals.
    #
    # 🔴 **ЗДЕСЬ СТОЯЛО, ЧТО ПРИБАВКА ДВИГАЕТ `fort`/`ref`/`will`** — до задачи
    # 3.197. Замер `AQ1` (Dan, 12.09.2026): «спасы от Spellcraft в листе
    # персонажа не видна», то есть условие из цитаты источника («against
    # spells») видно и в самой игре: базовые три числа считаются без неё.
    # Число не изменилось ни на единицу, изменилось поле, в котором оно стоит.
    test "Spellcraft ranks stay beside the three saves, not inside them", %{ruleset: ruleset} do
      ...
      assert after_.save_bonus == %{fort: 0, ref: 0, will: 0}
      assert after_.conditional_save_bonus == %{fort: 4, ref: 4, will: 4}

      # Лист не сдвинулся вовсе…
      assert {after_.fort - before.fort, after_.ref - before.ref, after_.will - before.will} ==
               {0, 0, 0}

      # …а число «против заклинаний» сдвинулось на все четыре, и условие названо.
      assert {after_.saves_conditional.fort - before.fort,
              after_.saves_conditional.ref - before.ref,
              after_.saves_conditional.will - before.will} == {4, 4, 4}

      assert after_.conditional_save_terms == [
               %{skill: :spellcraft, scope: "spells", bonus: 4}
             ]

      assert before.conditional_save_terms == []
    ...
    test "gear and Spellcraft share one +20 ceiling", %{ruleset: ruleset} do
      ...
      # 🔴 **ЗДЕСЬ СТОЯЛО `save_bonus == 20` и три флага капа** — до задачи
      # 3.197. Общий потолок никуда не делся, он просто виден там, где обе
      # прибавки встречаются: в листе одни вещи (+15), а против заклинаний
      # 15 + 8 = 23 → **20**, то есть прибавка доносит +5 из восьми.
      assert stats.save_bonus == %{fort: 15, ref: 15, will: 15}
      assert stats.conditional_save_bonus == %{fort: 5, ref: 5, will: 5}
      assert stats.saves_conditional.will == stats.base_will + 20

      # ⚠️ Флага капа у листа нет: пятнадцать вещей потолка не касаются.
      # Информация о том, что потолок кусает условное число, не потеряна —
      # её несёт разница «предложено 8 / доехало 5».
