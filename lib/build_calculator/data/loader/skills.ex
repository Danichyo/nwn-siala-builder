defmodule BuildCalculator.Data.Loader.Skills do
  ...
  # rule (`skill_rules/2`); часть его области действия не моделируется (на Сиале
  # — AOE), и эта оговорка сообщается один раз и точно, как `{:not_modelled,
  # {:save_bonus_scope, skill}}`. Letting it also sit in `siala_unapplied` would
  # state the same caveat twice under two names, and a list with duplicates in
  # it is a list people learn to skim.
  ...
  defp save_bonus_rules(ov) do
    for rule <- dig(ov, ["_vanilla_constants_confirmed", "skill_save_bonus", "rules"]) || [],
      ...
      %{
        ...
        # ОТ ЧЕГО берётся `div(…, per_ranks)` — от купленных рангов или от
        # полного значения навыка со всеми прибавками (задача 3.202).
        # `fandom:Spellcraft` (revid 68572, Notes): «the saving throw bonus is
        # based on fully modified ranks (not base ranks)».
        #
        # 🔴 **Умолчания у этого вопроса НЕТ, и его отсутствие роняет сборку.**
        # Два чтения дают разные числа у любого билда с ненулевым модификатором
        # характеристики (Брунна: 8 против 12), и выбрать молча — значит с равной
        # вероятностью показать игроку не то число. Ровно этим правило и было
        # до 13.09.2026: ключа не существовало, а ядро считало от рангов.
        based_on: based_on!(rule),
        # Нужен ли хоть один КУПЛЕННЫЙ ранг, чтобы прибавка вообще появилась.
        # Цитата там же: «since spellcraft requires training, the penalty will
        # only occur if the player has at least one rank in spellcraft».
        #
        # ⚠ Имя ключа в ruleset'е НЕ `requires_…`: этот префикс в проекте занят
        # причинами отказа (`{:requires_bab, 4}`), и одноимённая пара в коде
        # читается сторожем словаря как незарегистрированная форма. В данных
        # ключ называется словами источника (`requires_training`), в ядре —
        # `training_gate`; расхождение имён намеренное и стоит дешевле, чем
        # ложный носитель в словаре форм.
        #
        # ⚠ Значение `from_skill_trained_only` не несёт признака само — оно
        # говорит, ГДЕ его спросить: у самого навыка (`trained_only?`
        # в `ruleset.skills`). Второй экземпляр того же факта разошёлся бы
        # молча с первым, и это уже случалось (CLAUDE.md §9, ложный гэп про
        # бонусный пул фитов).
        training_gate: training!(rule),
        ...
        # spells, Implosion excepted).
        #
        # ⚠ С 12.09.2026 первая половина УЧАСТВУЕТ В РАСЧЁТЕ: наличие `scope`
        # означает «это число живёт отдельно от листа персонажа», и рядом
        # обязан стоять `in_character_sheet` (см. `character_sheet!/1`). Вторая
        # половина (сиальское сужение) по-прежнему не моделируется — решение,
        # а не недосмотр, поэтому она переносится и печатается оговоркой.
        # `nil` значит «прибавка безусловна», и это другое утверждение.
        scope: dig(rule, ["scope", "value"]),
        scope_excludes: dig(rule, ["scope", "siala_excludes"]),
        # Идёт ли прибавка в БАЗОВОЕ число листа персонажа, или она живёт
        # отдельным числом «когда условие выполнено» (задача 3.197). Замер
        # `AQ1`, Dan 12.09.2026: «спасы от Spellcraft в листе персонажа
        # не видна» — то есть для Spellcraft ответ «нет», и все три спаса
        # Брунны сходятся с игрой ровно после того, как терм из них убран.
        #
        # 🔴 **Спрашивается у КАЖДОГО правила с областью действия, а умолчания
        # у этого вопроса нет.** Правило, которое называет условие и молчит
        # о том, где его число печатается, — это выбор между двумя разными
        # числами на экране, и сделать его молча значит с равной вероятностью
        # показать игроку то, чего нет в его листе. Ровно этой ошибкой и была
        # прибавка Spellcraft до 12.09.2026.
        in_character_sheet: character_sheet!(rule),
        # Засчитывает ли эту прибавку ТРЕБОВАНИЕ фита по сейву. Умолчание
        # «да» — то же, что у разметки прибавок (`feat_save_bonuses.json`
        # → `_prerequisite_decision`): собственная прибавка билда требование
        # выполняет, пока источник не сказал обратного.
        counts_toward_prerequisites: prerequisite_counts!(rule)
      }
    end
  end

  # ⚠ Умолчание есть ровно у одного случая: правило БЕЗ области действия
  # печатается в листе, потому что печататься ему больше негде — «работает
  # всегда» и «видно всегда» тут одно и то же. У правила с областью умолчания
  # нет, и его отсутствие роняет сборку.
  defp character_sheet!(%{"scope" => scope} = rule) when not is_nil(scope) do
    case dig(rule, ["in_character_sheet", "value"]) do
      value when is_boolean(value) ->
        value

      _absent ->
        raise "overrides.json: skill_save_bonus rule for #{rule["skill"]} names a `scope` " <>
                "and does not say whether `in_character_sheet`. A conditional bonus is either " <>
                "in the sheet's Fort/Ref/Will or beside them, and guessing prints a number " <>
                "the player cannot find in the game (GAME_CHECKS.md AQ1)"
    end
  end

  defp character_sheet!(_rule), do: true

  # ⚠ Сторож, а не умолчание: правило без `based_on` роняет сборку. Оба ответа
  # правдоподобны, разница между ними — весь модификатор характеристики и все
  # прибавки с вещей, и увидеть ошибку в готовом числе нечем: `+8` и `+12`
  # выглядят одинаково правильно.
  defp based_on!(rule) do
    case dig(rule, ["based_on", "value"]) do
      "skill_value" ->
        :skill_value
      ...
      other ->
        raise "overrides.json: skill_save_bonus rule for #{rule["skill"]} says " <>
                "`based_on` is #{inspect(other)}; expected \"skill_value\" (полное " <>
                "значение навыка со всеми прибавками) or \"ranks\" (только купленные). " <>
                "Умолчания у этого вопроса нет: два чтения дают разные числа " <>
                "(fandom:Spellcraft revid 68572 — «based on fully modified ranks»)"
    end
  end

  # ⚠ Спрашивается только у правила, считающего от ЗНАЧЕНИЯ: у рангового
  # вопрос не встаёт вовсе — без рангов там и значения нет, ворота ничего
  # не меняют, и требовать ответ значило бы заводить поле без читателя.
  defp training!(%{"based_on" => %{"value" => "skill_value"}} = rule) do
    case dig(rule, ["based_on", "requires_training", "value"]) do
      "from_skill_trained_only" ->
        :skill_trained_only
      ...
      other ->
        raise "overrides.json: skill_save_bonus rule for #{rule["skill"]} counts from the " <>
                "skill's value and says `based_on.requires_training` is #{inspect(other)}; " <>
                "expected \"from_skill_trained_only\" or \"none\". Вещь, поднявшая значение " <>
                "нетренированного навыка, либо даёт прибавку, либо нет, и угадывать это " <>
                "нельзя (fandom:Spellcraft revid 68572 — «only occur if the player has at " <>
                "least one rank»)"
    end
  end

  defp training!(_rule), do: nil

  defp prerequisite_counts!(rule) do
    case dig(rule, ["prerequisite", "counts"]) do
      value when is_boolean(value) -> value
      nil -> true
      other -> raise "overrides.json: skill_save_bonus `prerequisite.counts` is #{inspect(other)}"
    end
  end
