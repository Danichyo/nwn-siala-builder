defmodule BuildCalculator.Ids do
  ...
  @doc """
  Resolves an energy type — the element a resistance is written against.

  Same whitelist rule as `fetch/3`, and the same reason to be strict: `"magical"`
  names damage that is not absorbed at all and `"bludgeoning"` names physical
  absorption the section does not show, so letting either through would put an
  element into a build that has none (`Encoding`'s moduledoc, task 3.210).

  ⚠️ Its own function rather than a `kind` of `fetch/3` because the dictionary is
  a **list of records** (`ruleset.resistance_energy_types`, `%{id:, en:, …}`),
  not a map keyed by id nor a list of atoms — the two shapes `fetch/3` reads.
  Until task 3.222 this lookup was written twice, character for character, in
  `Encoding` and in `BuilderLive`, each justifying the copy by "only one caller"
  while there were two.
  """
  @spec fetch_energy_type(map(), term()) :: {:ok, atom()} | :error
  def fetch_energy_type(_ruleset, value) when not is_binary(value), do: :error

  def fetch_energy_type(ruleset, value) do
    find(for(%{id: id} <- Map.get(ruleset, :resistance_energy_types) || [], do: id), value)
  end
