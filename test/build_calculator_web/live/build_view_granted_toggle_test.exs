defmodule BuildCalculatorWeb.BuildViewGrantedToggleTest do
  ...
  describe "дефолт — скрыто" do
    ...
    test "диалог «показать текст» и источник для копирования тоже без ○", %{
      ...
      # `#export-source` — вход `Export.file_text/5` (задача 3.180): тот же
      # текст, что в диалоге, плюс ссылка на билд внизу. «Скопировать
      # текст» с той же задачи переехал на `#view-export-text` (проверено
      # `dialog` выше — тот самый узел); этот источник кормит только
      # «Скачать .txt», и переключатель обязан двигать его так же, как
      # диалог — иначе скачанный файл разойдётся с тем, что игрок читал.
