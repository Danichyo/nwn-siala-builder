defmodule BuildCalculator.Rules.Vocabulary do
  ...
  @gaps [
    ...
    # 🔴 КАТЕГОРИЯ ОРУЖИЯ ВО ВТОРОЙ РУКЕ, КОТОРОЙ НЕ ЗНАЕТ НИКТО (задача 3.198).
    # Удвоение за расовое оружие снимается оружием ДРУГОЙ категории в другой
    # руке (замер `AN1` + разбор скриптов), а у рукопашного удара категории
    # не называет ни один источник: «Система оружия» описывает бонус перчаток
    # и ни одного оружия к этому типу не приписывает.
    #
    # ⚠ Форма нужна именно потому, что обе молчаливые ветки неверны: оставить
    # удвоение — завысить, снять — занизить. Ядро считает как раньше и говорит
    # об этом (`Rules.RacialBonus.gaps/2`).
    {:missing_data, {:weapon_category, :unarmed_strike}},
    # ------------------------------------- мини-сеты (задачи 3.184 и 3.186) --
    # Третья сторона той же системы: надетые куски комплектов усиливают оба
    # бонуса выше (`B += floor(B · Nmini / 10)`) и поднимают HP процентом
    # по нелинейной таблице. Две формы, и обе про НЕДОСТАЧУ ДАННЫХ:
    #
    #   * `{:missing_data, :mini_set_counting}` — куски заявлены, а снапшот про
    #     счёт кусков не знает: `Nmini` считать нечем, усиления нет, и число
    #     меньше игрового. Умолчание направлено в сторону оговорки, ровно как
    #     у самой таблицы тиров;
    #   * `{:missing_data, :mini_set_hp_table}` — вещи заявлены (куски или
    #     именные), а таблицы процентов к HP у снапшота нет: прибавку считать
    #     нечем, и HP меньше игрового — в пределе вдвое.
    #
    # 🔴 **Здесь была третья форма — `{:not_modelled, :mini_set_hp_bonus}`**
    # («в игре куски дают ещё и +15…+105 % HP, а мы их не прибавляем»). Снята
    # 11.09.2026 задачей 3.186: процент считается (`Rules.GearHitPoints`),
    # и оставить признание значило бы пугать игрока тем, что уже сосчитано
    # (CLAUDE.md §6, «ложная неопределённость наоборот»). ⚠ Снята не
    # вычёркиванием, а заменой на форму, которая вернётся сама: снапшот Сиалы
    # без таблицы процентов получает `mini_set_hp_table` и к HP не прибавляет
    # ничего. Живой её держит СИНТЕТИЧЕСКИЙ ruleset (`MiniSetsTest`), а не
    # живая запись, — контроль на живой назавтра получает данные и молча
    # перестаёт что-либо проверять (урок задачи 3.85).
    #
    # ⚠ **Прибавка к СКОРОСТИ формы не получает, и это не пропуск:** скорость
    # бега калькулятор не считает вовсе, получателя у неё нет, и по CLAUDE.md §9
    # дыры там нет. Та же линия, которой закрыты урон и поглощение у бонусов
    # за тип оружия.
    #
    # 🔴 **И «верхом» с «раненому» формы тоже не получают** — но по ДРУГОЙ
    # причине, и её стоит назвать, потому что постановка задачи 3.184 ждала
    # обратного. Оба состояния разбор скриптов называет внутри HP-ветки
    # (`ApplyMaxHPBonus`: верхом прибавка HP делится пополам, раненому
    # не применяется вовсе), а в арифметике восьми исполнителей (раздел 6
    # документа) ни одно из них не упоминается ни разу. То есть это не «мы
    # не считаем боевое состояние у оружейного бонуса», а «у оружейного бонуса
    # такого правила нет». К HP оба относятся, записаны в данных рядом
    # с таблицей процентов (`systems.json` →
    # `mini_sets.hp_percent_combat_states_not_counted`) — и формы не получают
    # и там: боевое состояние калькулятор не отвечает вовсе, ввести его негде,
    # а гэп — дырка в нашем ОТВЕТЕ, а не в знаниях.
    {:missing_data, :mini_set_counting},
    {:missing_data, :mini_set_hp_table},
    ...
    # ------------------------------ поглощение стихийного урона (задача 3.210) --
    # Три формы, и они про три разные вещи.
    #
    # Первая — та же, что у шести статов выше: источник поглощения, который
    # персонаж держит, а модель считать отказывается. ⚠ Сегодня её не производит
    # ни один билд: в `vanilla/feat_resistance_bonuses.json` нет ни одной записи
    # с вердиктом `not_modelled` — сплошной проход нашёл ровно два фита, и оба
    # посчитаны. Форма стоит на тех же правах, что `{:not_modelled,
    # {:spell_resistance, …}}` рядом: механизм жив, и запись, добавленная
    # в файл завтра, придёт к игроку подписанной, а не сырым таплом.
    {:not_modelled, {:resistance_bonus, :resist_energy}},
    #
    # 🔴 Вторая — про ЗАМЕЩЕНИЕ, и она сиальская. «Epic energy resistance
    # supersedes resist energy; they do not stack» процитировано дословно
    # ОБЕИМИ страницами Fandom, а ни одна из пяти сиальских страниц
    # `Epic energy resistance (…)` слова `Resist energy` не содержит вовсе.
    # То есть на Сиале правило ПЕРЕНЕСЕНО, а не прочитано, и разница стоит
    # +5 к стихии, у которой взяты оба фита. Статус лежит в данных по
    # ruleset'ам (`superseded_by.status_siala` плюс `…_applies_to_ruleset`),
    # поэтому у ванили оговорки нет, а у Сиалы есть.
    #
    # ⚠ Только у билда, у которого замещение ДЕЙСТВИТЕЛЬНО сработало (оба фита
    # на одной стихии): оговорка про вопрос, который не возникает, — шум, тот же
    # критерий, что у `Bonuses.cap_side_gaps/5` и у SR с вещей.
    {:assumed, :resist_energy_superseded_by_epic},
    #
    # Третья — про СВЕДЕНИЕ эффекта расы/топоров с числом, вписанным в «Вещах»:
    # берётся наибольшее, а не сумма. ⚠ **Носителей у неё сегодня НОЛЬ**, и это
    # результат работы, а не дыра: правило `verified` — слово Dan 13.09.2026
    # («эффект расы и топора с вещью не складывается, а берётся наибольшее»)
    # плюс замер `AV1` 18.09.2026, где у Хнюпиуса холод 30 при вещи 15 (сумма
    # дала бы 45). 🔴 Живой форму держит СИНТЕТИКА — снапшот без правила или
    # с непроверенным правилом, — а не живая запись: контроль на живой записи
    # назавтра получает отметку и молча перестаёт что-либо проверять (урок
    # задачи 3.85, пять контролей подряд). Ядро в таком снапшоте берёт
    # **максимум**, потому что максимум есть нижняя граница обоих чтений.
    {:assumed, :resistance_effect_vs_gear},
