defmodule BuildCalculatorWeb.Builder.Labels do
  ...
  @doc """
  Russian numeric agreement: `«1 очко» / «3 очка» / «5 очков»`.

  Grammar, not a string catalogue — the three word forms come from the caller,
  so nothing about *which* noun is being counted lives here. Added by 3.225 for
  the one place that had a number glued to a fixed form («Добавить 3 р. за 3
  очков»); it takes the forms as arguments precisely so the next caller does not
  have to copy the `rem` arithmetic, which is where this kind of bug is born.

  ⚠️ The 11..14 clause is not an optimisation — it is the rule: `11`, `12`, `13`
  and `14` take the *many* form even though they end in 1..4.
  """
  @spec plural(integer(), String.t(), String.t(), String.t()) :: String.t()
  def plural(n, one, few, many), do: "#{n} #{plural_word(n, one, few, many)}"

  @doc "The bare word form of `plural/4` — for layouts that print the number separately."
  @spec plural_word(integer(), String.t(), String.t(), String.t()) :: String.t()
  def plural_word(n, one, few, many) do
    abs_n = abs(n)
  ...
  *does* and named the one thing it must not repeat: «я не хочу показывать
  ...
  @doc """
  Подпись терма усиления мини-сетами (задача 3.184).

  Подпись интерфейса, а не имя игровой сущности: набор шард никак не назвал
  (в предмете лежит только номер группы), а имена наборов и не нужны расчёту
  вовсе — в `Nmini` входит мультимножество счётчиков и больше ничего
  (`Rules.MiniSets`). Поэтому перевод здесь законен по §4, в отличие от имён
  классов, фитов и оружия рядом.

  ⚠ Написание НЕ викиевское: страница называется «Мини Сэты», Dan пишет «мини
  сеты». Взято третье, дефисное, — потому что это подпись интерфейса, а не имя
  сущности, и менять её можно правкой одного `msgstr`. Появится у наборов
  настоящее имя (импорт игрового лога, задача 3.187) — оно приедет из данных
  и в эту функцию не попадёт.
  """
  @spec mini_sets_name() :: String.t()
  def mini_sets_name, do: gettext("Mini-sets")

  @doc """
  Подпись одной строки ввода кусков (задача 3.185) — «Набор 1», «Набор 2».

  Наборы **безымянные и пронумерованы по порядку ввода**: идентичность группы
  арифметике не нужна вовсе (`Rules.MiniSets`), а каталог из 80 групп заводить
  незачем. Номер — позиция строки, а не номер группы шарда; настоящие имена
  приедут с импортом игрового лога (задача 3.187) и заменят номера, не раньше.
  """
  @spec mini_set_row_name(pos_integer()) :: String.t()
  def mini_set_row_name(number), do: gettext("Set %{number}", number: number)

  @doc """
  Почему строка с одним куском не идёт в счёт (задача 3.185).

  🔴 Главная ловушка правила: `[1, 1]` даёт `Nmini = 0`, хотя надето две вещи.
  Прятать это нельзя — CLAUDE.md §6 велит показывать недоступное с причиной,
  и без неё игрок решит, что калькулятор потерял его ввод.

  ⚠️ Минимум приходит **из ядра** (`Rules.MiniSets.minimum_group/1`), а не
  написан здесь двойкой: это число скрипта шарда, и веб-слой игровых чисел
  не выдумывает (CLAUDE.md §5). `nil` — снапшот правила не знает, судить нечем,
  и тогда этой строки не бывает вовсе.
  """
  @spec mini_set_singleton(pos_integer() | nil) :: String.t()
  def mini_set_singleton(minimum),
    do:
      gettext("one piece does not count at all — a set needs %{minimum}+",
        minimum: minimum
      )

  @doc """
  Кусков набрано больше, чем в игре есть слотов (задача 3.185).

  ⚠️ **Предупреждение, а не запрет.** Расшаренная ссылка может нести что
  угодно и обязана открываться (та же линия, что у щита со вторым оружием,
  задача 3.132), поэтому ввод не блокируется; срезает сумму ядро
  (`MiniSets.pieces/2`), а подпись только называет оба числа — набранное
  и дошедшее.
  """
  @spec mini_set_over_slots(non_neg_integer(), pos_integer()) :: String.t()
  def mini_set_over_slots(entered, slots),
    do:
      gettext(
        "%{name}: %{entered} pieces entered, but the character has %{slots} item slots — %{slots} go into the count",
        name: mini_sets_name(),
        entered: entered,
        slots: slots
      )
  ...
  @doc """
  Усиление срезано внутренним капом исполнителя (задача 3.185).

  🔴 Это и есть «максимальному билду мини-сеты не дают ничего»: он упирается
  в кап (18 / 18 / 36) уже без них. Игрок, надевший десять кусков и не увидевший
  роста, обязан прочитать причину, а не решить, что расчёт врёт.

  ⚠️ Число капа в подпись не пишется отдельно — им и является `total`: клип
  ставит итог ровно на потолок, и второе число рядом было бы тем же самым,
  написанным дважды.
  """
  @spec mini_set_capped(String.t(), integer()) :: String.t()
  def mini_set_capped(label, total),
    do:
      gettext("%{label}: the mini-set amplification is clipped to the ceiling of %{total}",
        label: label,
        total: total
      )

  @doc """
  Общее имя ВТОРОГО эффекта мини-сетов — процента к HP (задача 3.186), и его
  второго входа, крафтовых (именных) и уникальных вещей.

  Один терм, а не два слова врозь: у HP это не «мини-сеты» и не «крафт»
  по отдельности, а их СУММА (`Ctotal = Cgear + Nmini`, `Rules.GearHitPoints`),
  и подпись обязана называть обоих. Используется и в разборе HP
  (`Summary.hp_terms/2`), и у поля ввода, и в сводке предупреждений «Вещей»
  (`#gear-issues`) — одно имя на все три места, а не три текста, которые
  могут разойтись.

  ⚠️ Не путать с `mini_sets_name/0` — та называет ТОЛЬКО куски комплектов,
  зону ввода блока «Вещи», где именные вещи не участвуют вовсе.
  """
  @spec mini_set_hp_bonus_name() :: String.t()
  def mini_set_hp_bonus_name, do: gettext("Mini-sets and crafted items")

  @doc """
  Подпись терма прибавки к HP от мини-сетов и крафтовых вещей (задача 3.186,
  `Rules.GearHitPoints.term/3`) — «Мини-сеты и крафтовые вещи ×5 (+45%)».

  `count` — `Ctotal`, число, по которому выбрана строка таблицы; `percent` —
  сама эта строка. Оба уже посчитаны ядром, здесь только слова вокруг чисел.
  """
  @spec mini_set_hp_bonus(pos_integer(), non_neg_integer()) :: String.t()
  def mini_set_hp_bonus(count, percent),
    do:
      gettext("%{name} ×%{count} (+%{percent}%)",
        name: mini_set_hp_bonus_name(),
        count: count,
        percent: percent
      )

  @doc """
  Та же подпись на счёте, где прибавка ОБРЫВАЕТСЯ в ноль (задача 3.186,
  `Rules.GearHitPoints`, «на одиннадцатой вещи прибавка обрывается в ноль» —
  не опечатка источника, так написан `switch`).

  🔴 Строка не молчит и не исчезает: разбор HP обязан сходиться со своим
  итогом (CLAUDE.md §6), а игрок обязан увидеть, куда делись сотни очков,
  а не решить, что калькулятор их потерял.

  `cut_off_at` — первое число ЗА таблицей (`GearHitPoints.cut_off_at/1`),
  а не написанное здесь: игрового числа веб-слой не выдумывает.

  ⚠️ Формулировка правлена задачей 3.193 — старая («from %{cut_off_at}
  pieces the hit-point bonus breaks to zero») звучала неуклюже вдобавок
  к слову «pieces», которое здесь неточно: `cut_off_at` считает `Ctotal`,
  сумму кусков мини-сетов И крафтовых вещей вместе, а «pieces» в остальном
  файле означает только первое (`mini_set_singleton/1`,
  `mini_set_over_slots/2`). Новая формулировка называет то, что реально
  считается, — счёт (`count`), а не единицу измерения. Правка `msgid`
  ставит перевод `fuzzy` (CLAUDE.md §4) — русский `msgstr` обновлён тем же
  коммитом, а не оставлен фаззи-меткой на потом.
  """
  @spec mini_set_hp_bonus_cut_off(pos_integer(), pos_integer()) :: String.t()
  def mini_set_hp_bonus_cut_off(count, cut_off_at),
    do:
      gettext(
        "%{name} ×%{count} — once the count reaches %{cut_off_at}, the hit-point bonus breaks to zero",
        name: mini_set_hp_bonus_name(),
        count: count,
  ...
  **only** such record (посчитано обходом `granted_feat_ranks` на обоих
  ...
  @doc """
  English name of one of the elements the «Резисты» section prints (задача
  3.211) — read off `ruleset.resistance_energy_types` rather than
  hard-coded, the same rule that keeps every other game entity's name out of
  the code (§4: `Fire`, `Cold` — a game entity, English even in an
  otherwise-Russian interface).

  A type nobody named falls back to its id, the same honesty `ac_type/2`
  above uses — a snapshot without the record is a gap somewhere else
  already, not a reason to invent a Russian or English word here.
  """
  @spec resistance_energy_type_name(map(), atom()) :: String.t()
  def resistance_energy_type_name(ruleset, id) do
    ruleset
    |> Map.get(:resistance_energy_types, [])
    |> Enum.find_value(Atom.to_string(id), fn record ->
      if record.id == id, do: record.en
    end)
  end

  @doc """
  The «Резисты» section's own title — the "Вещи" zone kicker, the totals
  panel's group eyebrow, the game log import dialog's section heading and
  the «Мини-сеты» zone's effect label all name the same one word, and a
  second, independently written copy of it would drift the moment the
  wording changes in one place and not the other.
  """
  @spec resistances_zone_title() :: String.t()
  def resistances_zone_title, do: gettext("Resistances")

  @doc """
  What happens when two worn items name the same element (задача 3.211) —
  one full sentence, not a fragment glued onto a shared prefix (this
  module's own rule above: «Concatenated fragments do not translate»).

  Reads `ruleset.resistance_stacking.gear_vs_gear` instead of stating the
  rule as a literal: `Rules.GearImport` and `Rules.Resistances` both default
  a snapshot without the record to `:max` (the lower of the two possible
  readings), so a snapshot that ever states `:sum` gets its own honest
  sentence rather than this one printed over a different arithmetic.
  """
  @spec resistance_stacking_hint(map()) :: String.t()
  def resistance_stacking_hint(ruleset) do
    case Map.get(ruleset, :resistance_stacking) do
      %{gear_vs_gear: :sum} -> gettext("added together across worn items")
      _max_or_absent -> gettext("the largest across worn items counts, not the sum")
    end
  end
  ...
  *is* chosen and merely wrong for the class keeps the per-level replay as-is:
  ...
  # Поглощение СТИХИЙНОГО урона (задача 3.210), три подписи.
  #
  # ⚠️ Первая, как и у SR строкой выше, сегодня не печатается ни на одном
  # билде: записей с вердиктом `not_modelled` в файле разметки нет вовсе —
  # сплошной проход нашёл ровно два фита, и оба посчитаны. Свойство ДАННЫХ,
  # а не кода.
  def gap({:not_modelled, {:resistance_bonus, id}}, ruleset),
    do:
      gettext("%{source}'s damage resistance bonus is not counted",
        source: ac_bonus_name(ruleset, id)
      )

  # 🔴 Вторая — про ЗАМЕЩЕНИЕ, и стоит она пяти очков поглощения. «Epic energy
  # resistance supersedes resist energy; they do not stack» процитировано
  # дословно обеими страницами Fandom, а ни одна из пяти сиальских страниц
  # `Epic energy resistance (…)` слова `Resist energy` не содержит вовсе —
  # то есть на Сиале правило перенесено с ванили, а не прочитано. Печатается
  # только у билда, у которого замещение действительно сработало (оба фита
  # на одной стихии).
  def gap({:assumed, :resist_energy_superseded_by_epic}, _ruleset),
    do:
      gettext(
        "on the same element the epic feat supersedes Resist energy rather than adding to it — Siala's own pages do not mention it, the rule comes from vanilla"
      )

  # Третья — про сведение эффекта расы и топоров с числом, вписанным в «Вещах».
  # ⚠️ Носителей сегодня нет: правило подтверждено словом Dan и замером `AV1`,
  # и форму держит снапшот без правила. Там ядро берёт МАКСИМУМ — нижнюю
  # границу обоих чтений — и говорит об этом.
  def gap({:assumed, :resistance_effect_vs_gear}, _ruleset),
    do:
      gettext(
        "the racial and weapon absorption is counted as the larger of it and the number from items, not as their sum — the snapshot does not state the rule"
      )
  ...
  def gap({:assumed, {:racial_bonus_variant, race, variant}}, ruleset),
    do: racial_bonus_variants_note(ruleset, race, variant, racial_bonus_stated(ruleset, race))
  ...
  # 🔴 Категория оружия, которой не знает ни один источник (задача 3.198).
  # Удвоение за расовое оружие снимает оружие ДРУГОЙ категории в другой руке,
  # а у рукопашного удара категории не назвал никто — считаем по-прежнему
  # и говорим об этом. Подпись обязана назвать ОБЕ стороны: и что число могло
  # бы быть меньше, и почему мы его не уменьшили.
  def gap({:missing_data, {:weapon_category, weapon}}, ruleset),
    do:
      gettext(
        "no source names the weapon category of %{weapon}, and the racial bonus is doubled only while the other hand holds nothing of a different category — the doubling is left in place, so this number may be too high",
        weapon: weapon_name(ruleset, weapon)
      )

  # ---- мини-сеты (задачи 3.184 и 3.186) ---------------------------------------

  # Снапшот без правила мини-сетов: считать `Nmini` нечем, усиления нет,
  # и число меньше игрового. На поставляемых данных не производится.
  def gap({:missing_data, :mini_set_counting}, _ruleset),
    do:
      gettext(
        "the rules snapshot says nothing about how mini-set pieces are counted, so the pieces entered amplify nothing — in the game ten of them double the weapon and racial bonuses"
      )

  # Снапшот без таблицы процентов к HP: прибавку считать нечем, и HP меньше
  # игрового — в пределе вдвое. На поставляемых данных не производится.
  # ⚠️ Здесь стояла подпись к `{:not_modelled, :mini_set_hp_bonus}` — «в игре
  # куски дают ещё и +15…+105 % HP, а мы их не считаем». Снята вместе с самой
  # формой (задача 3.186): процент считается, и продолжать признаваться значило
  # бы пугать игрока тем, что уже сосчитано.
  def gap({:missing_data, :mini_set_hp_table}, _ruleset),
    do:
      gettext(
        "the rules snapshot carries no table of the hit-point percentage for worn mini-set pieces and named items, so none of them add hit points here — in the game ten of them are worth +105 %%"
      )
  ...
  # ⚠️ Форма про группу, чьё правило чистоты не назвал НИКТО: мы применили
  # правило Сагры, и это допущение, а не прочитанное — подпись обязана назвать
  # его так, иначе два разных по качеству факта на экране выглядят одинаково.
  #
  # ⚠️ Носителей у формы нет ни одного с 25.08.2026 (решение Dan, задача 3.100),
  # и примером здесь стояла Адра — «правило чистоты у Адры не описано НИГДЕ…
  # адровец выведен по аналогии». С 11.09.2026 (задача 3.182) Адра примером
  # быть перестала: её правило называют серверные скрипты шарда, факт стал
  # `derived`, и разницу «цитата или вывод» несёт теперь подпись флажка
  # (`Summary.class_group_flags`), а не эта оговорка. Сам msgid не тронут — он
  # по-прежнему верен для группы, про которую не сказал никто ничего.
  ...
  # ⚠️ **ЗДЕСЬ СТОЯЛО «показана плоской»** — до задачи 3.197 (замер `AQ1`,
  # 12.09.2026). Ванильное условие («против заклинаний») теперь МОДЕЛИРУЕТСЯ:
  # прибавки нет в самом спасе, она отдана отдельным числом. Не моделируется
  # ровно то, что осталось, — сиальское сужение: против AOE она не работает
  # (кроме имплозии), и эти два случая мы не различаем. Гэп остался тот же,
  # потому что оговорка та же; изменилось, о чём именно она.
  def gap({:not_modelled, {:save_bonus_scope, :spellcraft}}, _ruleset),
    do:
      gettext(
        "the Spellcraft save bonus is shown as one number «against spells»: on Siala it does not work against AOE, and we do not tell the two apart"
      )
  ...
  def racial_bonus_note(ruleset, %{race: race, counted: counted, variant: variant} = bonus)
      when is_integer(counted) and not is_nil(variant) do
    ruleset
    |> racial_bonus_variants_note(race, variant, bonus.variants)
    |> with_racial_double_note(ruleset, Map.get(bonus, :weapon_match))
  end

  def racial_bonus_note(_ruleset, _bonus), do: nil

  # 🔴 ВТОРОЕ ПРЕДЛОЖЕНИЕ, А НЕ ХВОСТ К ПЕРВОМУ (задача 3.198). Оно печатается
  # ровно там, где игрок видит число, которое иначе выглядит багом: взял
  # во вторую руку дубину — и AC упал на три очка (на 40-м на девять), хотя
  # дубина ничего не отняла, а сняла УДВОЕНИЕ за расовое оружие (замер `AN1`,
  # Dan 12.09.2026).
  #
  # ⚠️ Склейка здесь законна, в отличие от той, что разбирали заходом 3.83: это
  # два ЦЕЛЫХ предложения, каждое со своим `gettext/2`, а не общий хвост,
  # приклеенный к четырём разным веткам. Порядок слов внутри каждого
  # переводчик видит целиком.
  #
  # ⚠️ Рука не называется: нарушителем может быть и главная рука (расовое
  # оружие во второй), а «в руках есть» верно в обоих случаях.
  defp with_racial_double_note(nil, _ruleset, _match), do: nil

  defp with_racial_double_note(text, ruleset, {:broken, weapon}),
    do:
      text <>
        " " <>
        gettext(
          "The doubling for the race's own weapon is off: the hands also hold %{weapon}, a weapon of a different category.",
          weapon: weapon_name(ruleset, weapon)
        )

  defp with_racial_double_note(text, _ruleset, _match), do: text
  ...
  #
  # Числа берутся у ядра, а не пишутся здесь второй раз: их четыре, и второе
  # написание разошлось бы с первым.
  #
  # 🔴 **И берутся они у ЗАПИСИ БИЛДА, а не у ruleset'а — правка 11.09.2026
  # (задача 3.181).** До неё оба источника давали одно и то же: бонус считался
  # только на том уровне, для которого числа написаны, поэтому «числа записи»
  # и «числа файла» совпадали всегда. С ростом по уровням они расходятся —
  # у 17-го уровня это +4 против +9, — и ruleset здесь напечатал бы число,
  # которого у этого билда нет.
  #
  # ⚠️ Ветка `nil` (справки нет вовсе) осталась и нужна: гэповый путь ниже
  # числа билда не несёт, ему нечего сюда передать.
  # Числа, написанные в файле, — для гэповой ветки, у которой чисел билда нет.
  # ⚠️ Сегодня ими печатается ровно один пример: зарегистрированная форма
  # `{:assumed, {:racial_bonus_variant, :gnome, :base}}`, на которой стоит
  # сторож подписей. Живая ветка (`variant: nil`) чисел больше не называет.
  defp racial_bonus_stated(ruleset, race) do
  ...
  defp racial_bonus_variants_note(ruleset, race, variant, variants) do
    case racial_bonus_record(ruleset, race) do
      %{} = record ->
        racial_bonus_variant_sentence(ruleset, race, record, variants, variant)
  ...
  #
  # 🔴 **ДВУХ ЧИСЕЛ В КОНЦЕ ЭТОЙ ФРАЗЫ БОЛЬШЕ НЕТ — правка 11.09.2026 (задача
  # 3.181), и это не сокращение ради краткости.** Она говорила «С оружием +6,
  # у воина Сагры +9», и до 3.181 это было верно всегда: третье состояние
  # наступало только там, где числа известны, то есть на 40-м и 41-м уровне.
  # С ростом по уровням оно наступает на ЛЮБОМ — бонус известен везде, — а сюда
  # по-прежнему приезжает только тапл гэпа, в котором чисел этого билда нет.
  # Напечатать здесь числа ruleset'а значило бы обещать 17-му уровню +9 вместо
  # его настоящих +4.
  #
  # ⚠️ Действенная половина фразы не тронута и она же главная: бонус включает
  # оружие в руках, и игрок в одном движении от него. Само число он увидит там,
  # где оно появится, — строкой у посчитанного бонуса (`racial_bonus_note/2`),
  # и это будет число ЕГО уровня.
  defp racial_bonus_variant_sentence(ruleset, race, record, _v, nil) do
    gettext(
      "race %{race} bonus %{target}: not counted — the bonus is activated by a weapon in hand, and the hands hold either nothing or a weapon that does not activate it.",
      race: race_ru(ruleset, race),
      target: racial_bonus_target(ruleset, record)
    )
  end
  ...
  @receiver_ids ~w(
    ...
    feat_availability class_availability class_group damage_resistance
  ...
  # Задача 3.209 — «Резисты»: поглощение СТИХИЙНОГО урона (`resist_energy`/
  # `epic_energy_resistance`), не физическое `damage reduction` (та механика
  # у нас не наша и в этом списке не появляется вовсе).
  defp receiver_name("damage_resistance"), do: gettext("elemental damage resistance")
  ...
  # ⚠️ Задача 3.228: пятая клауза больше не молчит про пустой конструктор.
  # Поведение у всех пяти ОДНО (`BuilderLive.load_code/3` зовёт
  # `open_empty_builder/1` на любую причину), а фраза стояла у четырёх — то есть
  # незнакомая причина отказа обещала меньше, чем делает. Обратное направление
  # («убрать фразу у всех») отвергнуто самим обещанием: его же даёт
  # `.claude/agents/dev-web.md`, и оно теперь верно буквально.
  def decode_error(other),
    do:
      gettext("Could not open the link (%{reason}). An empty builder is open.",
  ...
  def dropped(entries) do
    ...
    |> Enum.map(fn
      ...
      # Куски мини-сетов (задача 3.185): строка была, а прочиталась не вся —
      # нечитаемое или неположительное число. Названа ЦЕЛИКОМ, потому что
      # выпадает она тоже целиком: частично прочитанный список — это список,
      # которого в коде не было (та же линия, что у фита с выбором выше).
      {:unknown_mini_sets, name} -> gettext("mini-set pieces %{name}", name: name)
      other -> inspect(other)
    end)
    |> Enum.join(", ")
  end
