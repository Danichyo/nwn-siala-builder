defmodule BuildCalculatorWeb.BuilderLiveTest do
  ...
  # Второй, не суммируемый блок того же поп-апа: `{заголовок | nil, термы}`.
  # Заголовка нет вовсе (атрибут не печатается), когда блоку нечего сказать.
  defp pop_extra(html, dom_id) do
    trigger =
      html
      |> LazyHTML.from_fragment()
      |> LazyHTML.query("##{dom_id} .stat-pop-trigger")

    [raw] = LazyHTML.attribute(trigger, "data-pop-extra-terms")
    {List.first(LazyHTML.attribute(trigger, "data-pop-extra-title")), Jason.decode!(raw)}
  end
  ...
  describe "панель итогов разложена по смыслу" do
    ...
    # правило чистоты Сагры прочитано дословно, у Адры — ВЫВЕДЕНО из разбора
    # серверных скриптов шарда (задача 3.182; здесь стояло «не сформулировал
    # никто» — верно до 11.09.2026). Флажок Адры помечен, флажок Сагры — нет.
    test "флажок Адры помечен, флажок Сагры нет", %{view: view} do
    ...
    # 🔴 **§3 ПО-ПРЕЖНЕМУ ТРЕБУЕТ ПОКАЗЫВАТЬ ВКЛАД Spellcraft** — восемь пунктов
    # у чистого заклинателя игроки не считают сами. Показывать его **внутри**
    # спаса нельзя: прибавка условная и в лист персонажа не входит (замер
    # `AQ1`, Dan 12.09.2026, задача 3.197). Часть B даёт ей два адреса —
    # компактную строку `#stat-saves-vs-spells` под тройкой Fort/Ref/Will
    # и подраздел «против заклинаний» в разборе КАЖДОГО из трёх.
    #
    # ⚠️ **Здесь стоял тест на ПРОМЕЖУТОЧНОЕ состояние** (ядро число отдаёт,
    # разбор листа его не печатает, отдельной строки ещё нет), и часть B
    # обязана была уронить его вслух — уронила: `#stat-will` теперь несёт
    # «Spellcraft» СНОВА, но в своём собственном, не суммируемом подразделе
    # попапа, а не термом основного числа.
    test "«против заклинаний» — своя строка и свой подраздел разбора, а не терм листа", %{
      conn: conn,
      ruleset: ruleset
    } do
      build = spellcraft_sorcerer(ruleset)
      {:ok, view, _html} = live(conn, ~p"/?b=#{Encoding.encode(build)}")

      # Групповой заметки «вещи» по-прежнему нет — у этого билда их нет.
      refute has_element?(view, "#stat-note-save")

      # Компактная строка под тройкой — сама прибавка (+N), не итог.
      # ⚠️ **ЗДЕСЬ СТОЯЛО «+8»**: до задачи 3.202 прибавка считалась
      # от 43 рангов, теперь — от полного значения навыка (43 + модификатор
      # INT +2 = 45), то есть **+9**.
      assert has_element?(view, "#stat-saves-vs-spells")
      note = render(element(view, "#stat-saves-vs-spells"))
      assert note =~ "против заклинаний"
      assert note =~ "Fort +9"
      assert note =~ "Ref +9"
      assert note =~ "Will +9"

      # Подраздел разбора — Spellcraft снова упоминается у КАЖДОГО из трёх,
      # но своим, отдельным от `data-pop-terms` атрибутом.
      #
      # ✅ 3.202 B: подпись печатает ЗНАЧЕНИЕ навыка, от которого считается
      # прибавка (45 → +9, делится на пять), а ранги — в скобках. Здесь стояло
      # «43 ранга» без значения — долг, заведённый отчётом 3.202, закрыт.
      will_html = render(element(view, "#stat-will"))
      assert will_html =~ "data-pop-extra-title=\"против заклинаний\""
      assert will_html =~ "Spellcraft 45 (43 ранга)"

      # Основной разбор (сумма которого обязана сходиться с +22) Spellcraft
      # по-прежнему не несёт — только `data-pop-terms`, до атрибута extra.
      main_terms = will_html |> String.split("data-pop-terms=\"") |> Enum.at(1)
      refute main_terms |> String.split("\"") |> hd() =~ "Spellcraft"

      stats = BuildCalculator.Rules.compute(build, ruleset)
      assert stats.conditional_save_bonus.will > 0
      assert Enum.map(stats.conditional_save_terms, & &1.scope) == ["spells"]
    end

    test "строки «против заклинаний» нет без единого ранга Spellcraft", %{
      conn: conn,
      ruleset: ruleset
    } do
      {:ok, view, _html} = live(conn, ~p"/?b=#{Encoding.encode(fighter_41(ruleset))}")

      refute has_element?(view, "#stat-saves-vs-spells")
      refute render(element(view, "#stat-fort")) =~ "против заклинаний"
    end

    # Кейс «под потолком бывает нулём» (Мокси, §3): экипировка сама уже в
    # потолке +20, и Spellcraft добавить в этот сейв уже нечего — компактная
    # строка обязана сказать это словом, а не молчать напротив Fort.
    test "прибавка «против заклинаний», упёршаяся в потолок, названа словом «потолок»", %{
      conn: conn,
      ruleset: ruleset
    } do
      ...
      # Вещи одни уже на потолке — Spellcraft (+8 предложено) добавить некуда.
      view |> form("#gear-form", %{"saves" => "22"}) |> render_change()

      note = render(element(view, "#stat-saves-vs-spells"))
      assert note =~ "Fort +0 (потолок)"
      assert note =~ "Ref +0 (потолок)"
      assert note =~ "Will +0 (потолок)"

      # И тот же билд объясняет это в разборе: предложено +8, доехало +0.
      fort_html = render(element(view, "#stat-fort"))
      assert fort_html =~ "до потолка доехало"
    end

    # Частичный клип (гвардовый случай Брунны): вещи под потолком, а вместе со
    # Spellcraft — уже нет, и доезжает МЕНЬШЕ предложенного, а не ноль.
    test "частичный срез «против заклинаний» назван в разборе — предложено больше, чем доехало",
         %{conn: conn, ruleset: ruleset} do
      {:ok, view, _html} = live(conn, ~p"/?b=#{Encoding.encode(spellcraft_sorcerer(ruleset))}")

      view |> element("#gear-toggle") |> render_click()
      # 19 (под потолком) + 8 (Spellcraft) = 27 > 20 → доехал ровно +1.
      view |> form("#gear-form", %{"saves" => "19"}) |> render_change()

      refute has_element?(view, "#stat-fort[data-capped='1']")

      note = render(element(view, "#stat-saves-vs-spells"))
      assert note =~ "Fort +1"
      refute note =~ "Fort +1 (потолок)"

      fort_html = render(element(view, "#stat-fort"))
      assert fort_html =~ "Spellcraft 45 (43 ранга)"
      assert fort_html =~ "до потолка доехало"
    end

    # ⚠️ **ЗДЕСЬ ВВОДИЛОСЬ РОВНО 20, и срез ставила прибавка Spellcraft** —
    # до задачи 3.197. Теперь в самом спасе её нет, и двадцать вещей ровно
    # в потолок ничего не режут; чтобы увидеть срез листа, надо вписать больше
    # потолка. 📋 Подпись `#gear-saves-capped` по-прежнему называет Spellcraft
    # (число `save_from_skill` живо) — формулировку «вместе со Spellcraft
    # выходит больше потолка» правит часть B вместе со строкой «против
    # заклинаний»: общий пул у них остался, но виден он теперь на условном
    # числе.
    test "срез потолка сейвов назван и в панели, и в разборе", %{conn: conn, ruleset: ruleset} do
      {:ok, view, _html} = live(conn, ~p"/?b=#{Encoding.encode(spellcraft_sorcerer(ruleset))}")
      ...
      # Сверх потолка — есть, и разбор сходится с числом рядом.
      view |> form("#gear-form", %{"saves" => "26"}) |> render_change()

      assert has_element?(view, "#stat-fort[data-capped='1']")
      capped_note = render(element(view, "#gear-saves-capped"))
      assert capped_note =~ "Spellcraft"
      assert render(element(view, "#stat-fort")) =~ "сверх капа"

      # Задача 3.197 часть B: с гвардейского числа лист (гвардия+вещи) капнут
      # ОТ ОДНИХ ВЕЩЕЙ — Spellcraft в него не входит вовсе. Тот же общий пул
      # виден на СОСЕДНЕМ, условном числе: он капнут тоже (доехало 0 из
      # предложенных +8), и подпись обязана сказать это, а не повторить
      # старую формулировку «вместе со Spellcraft выходит больше потолка»,
      # которая после части A описывала бы несуществующую арифметику.
      assert capped_note =~ "против заклинаний"
      assert render(element(view, "#stat-saves-vs-spells")) =~ "Fort +0 (потолок)"
    end

    # Задача 3.187 (часть B): раздельные спасы рядом с universal, каждый
    # в своём поле, а не вместо общего.
    test "раздельная прибавка к одному спасу видна только у него", %{conn: conn, ruleset: ruleset} do
      {:ok, view, _html} = live(conn, ~p"/?b=#{Encoding.encode(fighter_41(ruleset))}")

      view |> element("#gear-toggle") |> render_click()

      for save <- ~w(fort ref will) do
        assert has_element?(view, "#gear-save-#{save}")
        assert has_element?(view, "#gear-save-#{save}-input")
      end

      view
      |> form("#gear-form", %{"saves_specific" => %{"fort" => "4"}})
      |> render_change()

      assert render(element(view, "#stat-fort")) =~ "+4"
      refute render(element(view, "#stat-ref")) =~ "вещи"
      refute render(element(view, "#stat-will")) =~ "вещи"
    end

    # И живое число задачи: universal и раздельная режутся ОДНИМ клипом
    # поверх суммы — 16 + 12 = 28 просят, 20 дают, той же формулировкой,
    # что у общего потолка выше.
    test "потолок общий на universal и раздельную вместе", %{conn: conn, ruleset: ruleset} do
      {:ok, view, _html} = live(conn, ~p"/?b=#{Encoding.encode(fighter_41(ruleset))}")

      view |> element("#gear-toggle") |> render_click()

      view
      |> form("#gear-form", %{"saves" => "16", "saves_specific" => %{"fort" => "12"}})
      |> render_change()
  ...
  describe "Резисты (поглощение стихийного урона) — задача 3.211" do
    # Гном-сагровик 40 с полуторным мечом (не топором!) — числа сверены
    # `Rules.Resistances`'s own test suite (`resistances_test.exs`,
    # `race: :dwarf, levels: pure(40), weapon: :longsword`):
    # `stats.racial_bonus.counted == 18`, ровно расовый эффект без топора
    # и без мини-сетов, одинаковый на все семь стихий.
    defp dwarf_sagra_40(ruleset) do
      Build.new(
        ruleset_version: ruleset.version,
        race: :dwarf,
    ...
    test "зона «Резисты» в «Вещах» — семь полей, английские имена", %{
      conn: conn,
      ruleset: ruleset
    } do
      {:ok, view, _html} = live(conn, ~p"/")
      ...
      assert has_element?(view, "#gear-zone-resistances")

      for %{id: id, en: en} <- ruleset.resistance_energy_types do
        assert has_element?(view, "#gear-resist-#{id}")
        assert has_element?(view, "#gear-resist-input-#{id}")
        assert render(element(view, "#gear-resist-#{id}")) =~ en
      end
    end

    test "вписанное число доезжает до итогов и обратно в поле после круга по ссылке", %{
      conn: conn
    } do
      {:ok, view, _html} = live(conn, ~p"/")
      ...
      view
      |> form("#gear-form", %{"resist" => %{"fire" => "15"}})
      |> render_change()

      html = render(element(view, "#stat-resist-fire"))
      doc = LazyHTML.from_fragment(html)
      assert doc |> LazyHTML.query(".v") |> LazyHTML.text() |> String.trim() == "15"
      assert pop_terms(html, "stat-resist-fire") == [%{"label" => "вещь", "value" => "+15"}]

      # Билд без единого источника поглощения — секции нет вовсе, слот пуст
      # (постоянный слот `#totals-resistances-slot` остаётся в DOM).
      assert has_element?(view, "#totals-resistances-slot")
      refute has_element?(view, "#stat-resist-acid")

      # Круг по ссылке — как и у прочих полей «Вещей» (задача 3.185/187/…):
      # правка бьёт в URL через `push_patch`, и повторное открытие того же
      # адреса восстанавливает то же самое число в поле ввода.
      path = assert_patch(view)
      {:ok, reopened, _html} = live(conn, path)
      reopened |> element("#gear-toggle") |> render_click()
      assert render(element(reopened, "#gear-resist-input-fire")) =~ ~s(value="15")
    end

    # Задача 3.217 (слово Dan 18.09.2026: «навыки — резисты — навыки… давай
    # резисты вниз перенесем»): слот стоит ПОСЛЕДНИМ ребёнком `#stat-rows`,
    # под значениями навыков, а не между бюджетом скилл-поинтов и значениями.
    test "секция резистов стоит под значениями навыков, последней в #stat-rows", %{
      conn: conn,
      ruleset: ruleset
    } do
      build =
        Build.new(
          ...
          base_abilities: %{str: 10, dex: 10, con: 10, int: 10, wis: 20, cha: 10},
          skills: %{1 => %{spot: 1}},
          gear: Gear.new(resistances: %{fire: 15})
        )

      {:ok, view, _html} = live(conn, ~p"/?b=#{Encoding.encode(build)}")

      assert has_element?(view, "#stat-group-skills")
      assert has_element?(view, "#totals-resistances")
      ...
      ids =
        ...
        |> render()
        |> LazyHTML.from_fragment()
        |> LazyHTML.query("#stat-rows > *")
        |> LazyHTML.attribute("id")

      assert List.last(ids) == "totals-resistances-slot"

      assert Enum.find_index(ids, &(&1 == "stat-group-skills")) <
               Enum.find_index(ids, &(&1 == "totals-resistances-slot"))
    end

    test "билд без поглощения — ни строки, ни секции", %{conn: conn} do
      {:ok, view, _html} = live(conn, ~p"/")

      assert has_element?(view, "#totals-resistances-slot")
      refute has_element?(view, "#totals-resistances")
    end

    # Расовый эффект Гнома-сагровика — одна и та же прибавка на все семь
    # стихий, разбор называет расу по-русски (CLAUDE.md §4 — имя расы
    # у Сиалы главное).
    test "расовый эффект Гнома-сагровика виден на всех семи стихиях", %{
      conn: conn,
      ruleset: ruleset
    } do
      {:ok, view, _html} = live(conn, ~p"/?b=#{Encoding.encode(dwarf_sagra_40(ruleset))}")

      for %{id: id} <- ruleset.resistance_energy_types do
        html = render(element(view, "#stat-resist-#{id}"))
        doc = LazyHTML.from_fragment(html)
        visible = doc |> LazyHTML.query(".v") |> LazyHTML.text() |> String.trim()

        assert visible == "18", "#{id}"
        assert pop_terms(html, "stat-resist-#{id}") == [%{"label" => "Гном", "value" => "+18"}]

        # Сравнивать не с чем — блока «не вошло» нет вовсе, без заголовка.
        assert pop_extra(html, "stat-resist-#{id}") == {nil, []}
      end
    end

    # Вещь, вписанная сверх расового эффекта, конкурирует с ним — берётся
    # наибольшее, и проигравшая сторона называется вслух (постановка 3.211).
    #
    # 🔴 Задача 3.218 (Dan 18.09.2026, скриншот `Fire 45` со строками
    # `+15 / +30 / +15`): проигравшая сторона — НЕ строка среди слагаемых,
    # а второй блок поп-апа под своим заголовком. Основной список обязан
    # сходиться с итогом; «вещь +5» в сумму 18 не входит и стоять в нём не может.
    test "вещь МЕНЬШЕ эффекта — не входит, и это сказано отдельным блоком разбора", %{
      conn: conn,
      ruleset: ruleset
    } do
      {:ok, view, _html} = live(conn, ~p"/?b=#{Encoding.encode(dwarf_sagra_40(ruleset))}")
      ...
      view |> element("#gear-toggle") |> render_click()
      view |> form("#gear-form", %{"resist" => %{"fire" => "5"}}) |> render_change()

      html = render(element(view, "#stat-resist-fire"))
      doc = LazyHTML.from_fragment(html)
      assert doc |> LazyHTML.query(".v") |> LazyHTML.text() |> String.trim() == "18"

      assert pop_terms(html, "stat-resist-fire") == [%{"label" => "Гном", "value" => "+18"}]

      assert pop_extra(html, "stat-resist-fire") ==
               {"не вошло — берётся большее", [%{"label" => "вещь", "value" => "+5"}]}
    end

    # Обратная сторона того же сравнения: вещь БОЛЬШЕ эффекта — в итог идёт
    # вещь, а в блок «не вошло» уходит эффект, под тем же ярлыком, каким он
    # назывался бы в основном списке.
    test "вещь БОЛЬШЕ эффекта — в блок «не вошло» уходит эффект", %{
      conn: conn,
      ruleset: ruleset
    } do
      {:ok, view, _html} = live(conn, ~p"/?b=#{Encoding.encode(dwarf_sagra_40(ruleset))}")
      ...
      view |> element("#gear-toggle") |> render_click()
      view |> form("#gear-form", %{"resist" => %{"fire" => "40"}}) |> render_change()

      html = render(element(view, "#stat-resist-fire"))
      doc = LazyHTML.from_fragment(html)
      assert doc |> LazyHTML.query(".v") |> LazyHTML.text() |> String.trim() == "40"

      assert pop_terms(html, "stat-resist-fire") == [%{"label" => "вещь", "value" => "+40"}]

      assert pop_extra(html, "stat-resist-fire") ==
               {"не вошло — берётся большее", [%{"label" => "Гном", "value" => "+18"}]}
    end
    ...
    # 🔴 Находка агента: с тех пор, как `Rules.RacialBonus` считает
    # `:damage_resistance` (задача 3.210), этот же получатель ЛЕЖИТ
    # в `stats.mini_sets` — общем списке, который зона «Мини-сеты» (задача
    # 3.185, ДРУГАЯ зона, соседняя с «Резистами») печатает построчно
    # («AB 18 → 30», «AC (щит) …»). До правки этой задачи запись без
    # клаузы в `mini_set_effect_label/2` падала в catch-all и печатала
    # СЫРОЙ атом `damage_resistance` прямо в русской подписи — ровно то,
    # что увидел бы игрок на билде Хнюпиуса (Гном, куски мини-сетов).
    test "получатель поглощения в зоне «Мини-сеты» назван словом, а не сырым атомом", %{
      conn: conn,
      ruleset: ruleset
    } do
      %Build{gear: %Gear{} = gear} = base = dwarf_sagra_40(ruleset)
      build = %Build{base | gear: %Gear{gear | mini_sets: [7]}}

      {:ok, view, _html} = live(conn, ~p"/?b=#{Encoding.encode(build)}")

      view |> element("#gear-toggle") |> render_click()

      # DOM-id закономерно несёт имя вида (`mini_set_effect_id/1` строит его
      # из `to_string(kind)`) — сырым атомом обязана НЕ быть только ВИДИМАЯ
      # подпись строки, а не её адрес.
      assert has_element?(view, "#gear-mini-set-effect-damage_resistance")
      row = render(element(view, "#gear-mini-set-effect-damage_resistance"))
      doc = LazyHTML.from_fragment(row)
      label = doc |> LazyHTML.query(".k") |> LazyHTML.text()

      # Положительный контроль: строка называет получателя тем же словом,
      # что зона «Резисты» и её карточка в итогах (`Labels.
      # resistances_zone_title/0`, одно имя на четыре места).
      assert label =~ "Резисты"
      refute label =~ "damage_resistance"
    end
  end
  ...
  describe "«из чего собрано» доехало до AB/AC/сейвов/HP (задача 3.6)" do
    ...
    # ⚠️ **ЗДЕСЬ В СПИСКЕ БЫЛ «Spellcraft»** — до задачи 3.197 (замер `AQ1`):
    # прибавка условная, в самом спасе её нет, и в разборе ЭТОГО числа её
    # быть не может. 📋 Своей строкой «против заклинаний» её печатает часть B.
    test "сейв называет классы и характеристику — тем же приёмом, что и у навыков", %{
      conn: conn,
      ruleset: ruleset
    } do
      {:ok, view, _html} = live(conn, ~p"/?b=#{Encoding.encode(spellcraft_sorcerer(ruleset))}")
  ...
  describe "gear" do
    ...
    #
    # ⚠️ Задача 3.191 сняла и вторую половину доказательства: шапка больше
    # не называет предмет ПО ИМЕНИ (перечисление обрывалось на второй позиции
    # из семи), она отвечает только на «задана ли экипировка». Утверждение
    # теста от этого не ослабло — его доказывают переход шапки из «ещё
    # не задана» и само число, выросшее на базу лат.
    test "выбранный предмет без единого числа считается надетым", %{view: view} do
      assert render(element(view, "#gear-summary")) =~ "ещё не задана"

      view |> form("#gear-form", %{"worn" => %{"armor" => "full_plate"}}) |> render_change()

      refute render(element(view, "#gear-summary")) =~ "ещё не задана"
    ...
    # 🔴 Задача 3.191: шапка больше НЕ говорит, что именно набрано — только
    # набрано ли что-нибудь. Здесь стояло `assert summary =~ "CON +12"` и
    # `=~ "спасы +5"`; перечисление снято целиком, потому что с полного билда
    # обрывалось на второй позиции из семи, а сами эти числа никуда не делись —
    # они в своих полях и в разборах панели итогов.
    test "the collapsed block says only whether gear is set at all", %{view: view} do
      assert render(element(view, "#gear-summary")) =~ "ещё не задана"
      ...
      refute summary =~ "ещё не задана"

      # И ничего не перечисляет: ни характеристику, ни спасы.
      refute summary =~ "CON"
      refute summary =~ "спасы"
    end
    ...
    test "the penalty is not silently swallowed by the form", %{view: view} do
      ...
      # …и минус доехал до расчёта: шапка «Вещей» с задачи 3.191 ничего
      # не перечисляет (раньше проверялось `summary =~ "CON -4"` и отсутствие
      # `+-4`), поэтому знак ловится там, где он двигает число.
      assert render(element(view, "#stat-hp")) =~ "389"
  ...
  describe "export" do
    ...
    test "переключатель прячет автоматические фиты по умолчанию и возвращает их по клику", %{
      ...
      view |> element("#export-button") |> render_click()
      text = render(element(view, "#export-text"))
      file = render(element(view, "#export-file-text"))
      ...
      # Задача 3.180 — второй читатель того же переключателя: файл не
      # отстаёт от попапа ни на один клик.
      refute file =~ "○"
    ...
    # Задача 3.179 — "скачать .txt" рядом со "скопировать текст", общие
    # компоненты с экраном просмотра (`BuilderComponents.download_text/1`/
    # `copy_text/1`).
    #
    # ⚠️ Задача 3.180 развела их по ИСТОЧНИКУ: «скопировать» остаётся на
    # `#export-text` (попап — Dan: «выводить ссылку в попапе смысла нет»),
    # «скачать» переехал на `#export-file-text` (тот же текст плюс ссылка).
    # Строка в файле обязана совпасть с тем, что показывает `#share-link`
    # рядом (постановка 3.180) — не переписанная руками строка, а тот же
    # код, вытащенный оттуда же, откуда его берёт другой тест этого файла
    # («выбор доезжает до ссылки и открывается обратно»).
    test "«скачать» и «скопировать» читают разные источники, и файл несёт ссылку #share-link",
         %{conn: conn} do
      ruleset = Data.ruleset!()
      build = Build.new(ruleset_version: ruleset.version, levels: [:fighter])
      ...
      assert has_element?(view, "button#export-copy[data-target='#export-text']")
      assert has_element?(view, "button#export-download[data-target='#export-file-text']")
      assert has_element?(view, "button#export-download[data-filename='lvl1_fighter_1.txt']")

      [_, code] = Regex.run(~r/b\/([^"]+)"/, render(element(view, "#share-link")))

      file = render(element(view, "#export-file-text"))
      assert file =~ "Собран в конструкторе:"
      assert file =~ "/b/#{code}"

      refute render(element(view, "#export-text")) =~ "Собран в конструкторе"
    end

    # Имя файла едет за билдом само — не хранится с прошлого открытия, а
    # читается заново в тот же момент, что и `#export-text` (см.
    # `BuilderLive.export_filename/1`).
    #
    # ⚠️ Раса и мировоззрение обязательны в фикстуре, не только класс: без
    # них потолок лестницы держит игрока на 1-м уровне (задача 3.69,
    # describe "потолок лестницы без расы и мировоззрения" — `#level-2`
    # молча не двигает лестницу дальше без них).
    test "имя файла для скачивания пересчитывается после следующего левелапа", %{conn: conn} do
      ruleset = Data.ruleset!()
      ...
      assert has_element?(view, "button#export-download[data-filename='lvl2_fighter_2.txt']")
    end
  end

  # Диалог импорта — вынесен в `BuilderLiveImportTest` задачей 3.194
  # (ускорение `mix test`): `setup` этого блока трогал глобальный
  # `Application.put_env/3`, и держать его здесь означало бы держать
  # синхронным весь файл ради одного describe из 346 тестов.
  ...
  describe "reset" do
    ...
    test "reset button carries a data-confirm naming the loss and irreversibility", %{
      ...
      assert reset_button =~ "Уровни, характеристики и экипировка"
