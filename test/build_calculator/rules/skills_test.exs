defmodule BuildCalculator.Rules.SkillsTest do
  ...
  describe "Spellcraft's contribution to saving throws" do
    # 🔴 **ЗДЕСЬ СТОЯЛА ТА ЖЕ ТАБЛИЦА ПО РАНГАМ** — `{4, 0}, {9, 1}, {43, 8}`, —
    # и до задачи 3.202 она была верна: прибавка считалась от купленных рангов.
    # Теперь она считается от ПОЛНОГО значения навыка («the saving throw bonus
    # is based on fully modified ranks (not base ranks)», fandom:Spellcraft
    # revid 68572), а у этого волшебника INT 14, то есть модификатор +2 —
    # он и сдвинул три строки из шести.
    #
    # Средняя колонка — значение навыка, и оно проверяется рядом: без неё
    # таблица утверждала бы арифметику, не назвав числа, от которого считает.
    test "одно очко за каждые 5 полного значения, с усечением вниз", %{ruleset: ruleset} do
      for {ranks, value, bonus} <- [
            {0, 2, 0},
            {3, 5, 1},
            {4, 6, 1},
            {5, 7, 1},
        ...
        assert Skills.value(build, ruleset, :spellcraft, 40).total == value,
               "#{ranks} рангов + мод INT +2 должны дать значение #{value}"

        assert Skills.save_bonus(build, ruleset, 40) == {bonus, from},
               "значение #{value} должно дать +#{bonus}"
      end
    end

    # 🔴 Первая строка таблицы выше — не «ноль от нуля», а ВОРОТА ТРЕНИРОВКИ:
    # значение навыка там 2, а не 0 (модификатор характеристики есть у всех),
    # и без ворот арифметика дала бы тот же ноль по другой причине. Здесь
    # то же правило на числе, где эти два объяснения расходятся.
    #
    # source: fandom:Spellcraft revid 68572 — «since spellcraft requires
    # training, the penalty will only occur if the player has at least one rank
    # in spellcraft».
    test "без единого купленного ранга прибавки нет, даже если вещь дала +10",
         %{ruleset: ruleset} do
      gear = Gear.new(skills: %{spellcraft: 10})
      untrained = build_with(:wizard, 40, int: 14, gear: gear)
      trained = build_with(:wizard, 40, int: 14, gear: gear, skills: %{40 => %{spellcraft: 1}})

      # Значение навыка у обоих есть, и оно велико — прибавку отбивают
      # не арифметика и не потолок, а ровно отсутствие рангов.
      assert Skills.value(untrained, ruleset, :spellcraft, 40).total == 12
      assert Skills.save_bonus(untrained, ruleset, 40) == {0, []}

      # Положительный контроль: один-единственный ранг открывает ворота,
      # и прибавка сразу считается от всего значения, а не от этого ранга.
      assert Skills.value(trained, ruleset, :spellcraft, 40).total == 13
      assert Skills.save_bonus(trained, ruleset, 40) == {2, [:spellcraft]}
    end

    # 🔴 **СОБСТВЕННЫЙ ПРИМЕР ИСТОЧНИКА, слово в слово:** «Hence [[epic skill
    # focus]] in spellcraft grants a +2 bonus to all saving throws against
    # spells» (`fandom:Spellcraft`, revid 68572, Notes). Это и есть проверка
    # самой правки 3.202: фит даёт навыку +10, десять делится на пять, и
    # прибавка к спасам растёт ровно на два. От рангов такого не получить
    # никак — ранги фит не двигает.
    #
    # ⚠️ Третья строка объясняет, почему источник назвал ЭПИЧЕСКИЙ фокус,
    # а не обычный: +3 внутри одной пятёрки и сам по себе прибавку не двигает.
    # Это не «обычный фокус не считается» — он считается, просто не виден
    # на этом числе рангов.
    test "Epic skill focus (Spellcraft) даёт +2 к прибавке — пример источника",
         %{ruleset: ruleset} do
      cases = [
        {"без фита", %{}, 40, 8},
        {"epic skill focus", %{21 => %{{:general, 1} => {:epic_skill_focus, :spellcraft}}}, 50,
         10},
        {"skill focus", %{21 => %{{:general, 1} => {:skill_focus, :spellcraft}}}, 43, 8}
      ]

      for {label, feats, value, bonus} <- cases do
        build =
          build_with(:fighter, 41,
            race: :human,
            skills: %{41 => %{spellcraft: 40}},
            feats: feats
          )

        assert Skills.value(build, ruleset, :spellcraft, 41).total == value, label
        assert Skills.save_bonus(build, ruleset, 41) == {bonus, [:spellcraft]}, label
      end
    end

    # То же самое вещью, а не фитом: вписанные `+10` к Spellcraft — это `+2`
    # к прибавке. Слово Dan 13.09.2026 («вроде spellcraft с вещей работает»)
    # ровно про эту строку; ⚠️ оно неуверенное, и на Сиале не замерено —
    # правку разрешает цитата Fandom, а слово владельца с ней согласуется.
    test "вещь +10 к Spellcraft — это +2 к прибавке", %{ruleset: ruleset} do
      plain = build_with(:fighter, 41, race: :human, skills: %{41 => %{spellcraft: 40}})
      ...
      assert Skills.save_bonus(plain, ruleset, 41) == {8, [:spellcraft]}
      assert Skills.save_bonus(geared, ruleset, 41) == {10, [:spellcraft]}

      # ⚠️ И лист персонажа от вещи не двигается вовсе: прибавка условная,
      # а вторая её половина — «в лист не идёт» — правкой не тронута.
      sheet = Rules.compute(plain, ruleset)
      sheet_geared = Rules.compute(geared, ruleset)

      assert {sheet.fort, sheet.ref, sheet.will} ==
               {sheet_geared.fort, sheet_geared.ref, sheet_geared.will}

      assert sheet_geared.conditional_save_bonus == %{fort: 10, ref: 10, will: 10}
    end
    ...
    test "vanilla carries it too" do
      ...
      # 20 рангов + мод INT +2 = 22 → +4.
      assert Skills.save_bonus(build, vanilla, 40) == {4, [:spellcraft]}
    end

    # ⚠️ Правило читается из данных целиком: ruleset, у которого оно считает
    # от рангов, считает от рангов. Порча синтетическая — живой записи
    # с `based_on: :ranks` нет ни в одном ruleset'е, и держать ветку обязана
    # именно она (CLAUDE.md §9: живой контроль назавтра перестаёт проверять).
    test "ruleset с `based_on: :ranks` считает по-старому", %{ruleset: ruleset} do
      by_ranks =
        put_in(ruleset.skill_rules.save_bonus, [
          %{
            skill: :spellcraft,
            bonus: 1,
            per_ranks: 5,
            applies_to: [:fort, :ref, :will],
            based_on: :ranks,
            training_gate: nil,
            counts_toward_cap: :saving_throw_bonus,
            scope: "spells",
            scope_excludes: "AOE",
            in_character_sheet: false,
            counts_toward_prerequisites: false
          }
        ])

      build = build_with(:wizard, 40, int: 14, skills: %{40 => %{spellcraft: 43}})

      assert Skills.save_bonus(build, by_ranks, 40) == {8, [:spellcraft]}
      assert Skills.save_bonus(build, ruleset, 40) == {9, [:spellcraft]}
    end
  end
  ...
  # 🔴 **ЗДЕСЬ СТОЯЛО «вписанные +50 к Spellcraft на сейвы не влияют вообще»**,
  # и заголовок describe объявлял это «самым вероятным местом незаметной
  # поломки после 3.20». Поломка и была — только с другой стороны: прибавка
  # считалась от купленных рангов, а `fandom:Spellcraft` (revid 68572, Notes)
  # говорит «based on **fully modified ranks (not base ranks)**». Задача 3.202.
  #
  # ⚠️ Слово Dan 09.08.2026 («30 рангов дают +6, значит с вещей до потолка
  # добирается ровно +14»), на котором держалось прежнее чтение, НЕ отменено:
  # по первоисточнику (`GAME_CHECKS.md`, кейс F6) оно отвечало на вопрос про
  # ОДИН кап +20 на два источника, а не про состав числа под `div`. Оба теста
  # про кап ниже стоят как стояли и зеленеют по-прежнему.
  describe "прибавка к навыку с вещей и бонус Spellcraft к сейвам" do
    test "вписанные +50 к Spellcraft доезжают до прибавки", %{ruleset: ruleset} do
      plain = spellcraft_wizard(30, %Gear{})
      typed = spellcraft_wizard(30, Gear.new(skills: %{spellcraft: 50}))

      # У волшебника INT 10, то есть модификатор 0: 30 рангов — это ровно 30.
      assert Skills.save_bonus(plain, ruleset, 40) == {6, [:spellcraft]}

      # …а с вещью — 80, и прибавка растёт с шести до шестнадцати.
      assert Skills.value(typed, ruleset, :spellcraft, 40).total == 80
      assert Skills.save_bonus(typed, ruleset, 40) == {16, [:spellcraft]}
    end

    # 🔴 Потолок ПУЛА прибавок к навыку (+50) кусает прибавку к спасам сам,
    # и второго потолка для этого не завели: число берётся у `value/4`, где
    # клип уже стоял. Контроль — вписанные 60 дают ту же прибавку, что 50.
    test "потолок пула навыка +50 режет и прибавку к спасам", %{ruleset: ruleset} do
      fifty = spellcraft_wizard(30, Gear.new(skills: %{spellcraft: 50}))
      sixty = spellcraft_wizard(30, Gear.new(skills: %{spellcraft: 60}))

      assert Skills.value(sixty, ruleset, :spellcraft, 40).bonus_clipped == -10
      assert Skills.value(sixty, ruleset, :spellcraft, 40).total == 80

      assert Skills.save_bonus(sixty, ruleset, 40) == Skills.save_bonus(fifty, ruleset, 40)
      assert Skills.save_bonus(sixty, ruleset, 40) == {16, [:spellcraft]}
    end

    # 🔴 **ЗДЕСЬ ОБА ЧИСЛА СТОЯЛИ В `save_bonus`** — до задачи 3.197. С замера
    # `AQ1` (Dan, 12.09.2026) прибавка от рангов в лист персонажа не идёт:
    # в листе одни вещи, а «6 от рангов + 14 с вещей = ровно 20» наблюдается
    # в условном числе. Свойство то же — один потолок на оба источника, — просто
    # видно оно теперь на другом поле.
    test "6 от рангов + 14 с вещей = 20 против заклинаний, без флага капа", %{ruleset: ruleset} do
      stats = Rules.compute(spellcraft_wizard(30, Gear.new(saves: 14)), ruleset)

      assert stats.skill_save_bonus == 6
      assert stats.save_bonus == %{fort: 14, ref: 14, will: 14}
      assert stats.conditional_save_bonus == %{fort: 6, ref: 6, will: 6}
      assert stats.capped == []

      for save <- [:fort, :ref, :will] do
        assert Map.fetch!(stats.saves_conditional, save) - Map.fetch!(stats, save) == 6
      end
    end

    test "6 от рангов + 20 с вещей = прибавка съедена потолком", %{ruleset: ruleset} do
      stats = Rules.compute(spellcraft_wizard(30, Gear.new(saves: 20)), ruleset)

      assert stats.skill_save_bonus == 6
      assert stats.save_bonus == %{fort: 20, ref: 20, will: 20}

      # ⚠️ ЗДЕСЬ СТОЯЛ срез −6 и три флага капа: их ставила прибавка от рангов,
      # которой в листе больше нет. Вещи ровно в потолок — не срез.
      assert stats.save_cap_clipped == %{fort: 0, ref: 0, will: 0}
      assert stats.capped == []

      # А сама прибавка предлагает шесть и не доносит ни одной — ровно это
      # и значит «внутри капа» для условного числа.
      assert Enum.map(stats.conditional_save_terms, & &1.bonus) == [6]
      assert stats.conditional_save_bonus == %{fort: 0, ref: 0, will: 0}
    end

    # ⚠️ **ЗДЕСЬ СТОЯЛО «прибавка к навыку не создаёт и не снимает срез
    # сейвов»** — с числом 6 и доводом «она не добавляет к сейвам ни единицы».
    # После 3.202 она добавляет: предложено 16 вместо 6. А вот ДОЕХАЛО
    # по-прежнему 6, и это не совпадение, а тот же один кап +20 на вещи
    # и навык вместе: 14 вещевых оставляют ровно шесть свободных очков,
    # сколько бы навык ни предлагал.
    test "прибавка к навыку растит предложение, а доезжает всё те же 6",
         %{ruleset: ruleset} do
      gear = Gear.new(saves: 14, skills: %{spellcraft: 50})
      both = Rules.compute(spellcraft_wizard(30, gear), ruleset)

      assert both.skill_save_bonus == 16
      assert both.capped == []
      assert both.save_bonus == %{fort: 14, ref: 14, will: 14}
      assert both.conditional_save_bonus == %{fort: 6, ref: 6, will: 6}

      # Лист персонажа от этого не двигается вовсе — прибавка условная.
      without_skill_gear = Rules.compute(spellcraft_wizard(30, Gear.new(saves: 14)), ruleset)

      assert {both.fort, both.ref, both.will} ==
               {without_skill_gear.fort, without_skill_gear.ref, without_skill_gear.will}
    end
  end
