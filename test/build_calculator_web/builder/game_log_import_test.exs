defmodule BuildCalculatorWeb.Builder.GameLogImportTest do
  ...
  describe "four gearless fixtures: Rules.compute/2 matches COMBAT STATS exactly (task 3.131)" do
    ...
    # 🔴 **ДВА ЛОГА С РАНГАМИ Spellcraft — И ОНИ ПОДТВЕРЖДАЮТ ЗАМЕР `AQ1`
    # НЕЗАВИСИМО ОТ БРУННЫ** (задача 3.197, 12.09.2026). Кейс `AQ1` писал
    # «свидетель ровно один» — это было верно ровно до тех пор, пока никто
    # не сверил со спасами логи, лежавшие в фикстурах с самого начала.
    #
    # | лог | ранги Spellcraft | мод INT | значение | предложено | лист игры | наш лист |
    # |---|---|---|---|---|---|---|
    # | `aley` | 5 | +2 | 7 | +1 | 12/13/10 | 12/13/10 |
    # | `frah_hall` | 32 | +6 | 38 | +7 | 16/15/21 | 16/15/21 |
    #
    # ⚠️ **ЗДЕСЬ СТОЯЛО «+6» У `frah_hall`**, и это было верно, пока прибавка
    # считалась от купленных рангов. С задачи 3.202 она считается от полного
    # значения навыка (`fandom:Spellcraft`: «based on fully modified ranks»),
    # и модификатор интеллекта вошёл в число: 38 вместо 32. У `aley` число
    # не сдвинулось (7 и 5 дают одну и ту же единицу) — то есть вывод самого
    # теста, «лист сходится БЕЗ прибавки», на правку не опирается вовсе.
    #
    # 🔴 **Эти двое сильнее Брунны в одном месте: у них потолок ни при чём.**
    # У Брунны прибавка упиралась в вещевой пул, и расхождение было единицей;
    # здесь вещей на спасы нет вовсе, то есть с прибавкой внутри листа мы
    # печатали бы 13/14/11 и 22/21/27 — расхождение с игрой на всю прибавку
    # целиком, без всякой арифметики капа.
    #
    # ⚠️ **Это не «гроссбух» вроде четвёрки выше, но и не слабее её**: логи
    # несут экипировку, а вывод держится на том, что ВСЕ проверяемые числа
    # (AB, AC, шесть характеристик и три спаса) совпадают точка в точку —
    # чтобы объяснить совпадение иначе, экипировке пришлось бы давать ровно
    # −1 и −6 к спасам и ноль ко всему остальному.
    for {name, fort, ref, will, ranks, value, offered} <- [
          {"aley", 12, 13, 10, 5, 7, 1},
          {"frah_hall", 16, 15, 21, 32, 38, 7}
        ] do
      test "#{name}.log: спасы сходятся БЕЗ прибавки Spellcraft (замер AQ1)" do
        result = parsed(unquote(name))
        log = result.log
        stats = Rules.compute(result.build, @ruleset)

        # Билд восстановлен верно — это контроль, без которого совпадение
        # спасов ничего не значило бы.
        assert stats.abilities == log.current_abilities
        assert stats.attack_bonus == log.combat_stats.ab
        assert stats.ac_naked == log.combat_stats.ac

        # Лист: три числа игры точка в точку.
        assert {stats.fort, stats.ref, stats.will} ==
                 {log.combat_stats.fort, log.combat_stats.refl, log.combat_stats.will}

        assert {stats.fort, stats.ref, stats.will} ==
                 {unquote(fort), unquote(ref), unquote(will)}

        # Прибавка при этом настоящая и посчитана — она просто лежит рядом.
        assert Build.skill_ranks(result.build, :spellcraft, 41) == unquote(ranks)
        assert stats.skill_values.spellcraft.total == unquote(value)
        assert stats.skill_save_bonus == unquote(offered)

        assert stats.conditional_save_bonus == %{
                 fort: unquote(offered),
                 ref: unquote(offered),
                 will: unquote(offered)
               }

        # И вот что мы печатали бы, оставь её внутри листа: расхождение
        # с игрой на всю прибавку.
        assert stats.saves_conditional == %{
                 fort: unquote(fort) + unquote(offered),
                 ref: unquote(ref) + unquote(offered),
                 will: unquote(will) + unquote(offered)
               }
      end
    end
  ...
  # ------------------------------------------- 3.202: лист не сдвинулся ни у кого --

  describe "прибавка Spellcraft от значения навыка (задача 3.202)" do
    # 🔴 **ВОПРОС ЗДЕСЬ ОДИН: не сдвинулся ли ЛИСТ.** Правка 3.202 трогает
    # только условное число «против заклинаний» — прибавка как не входила
    # в базовые Fort/Ref/Will (замер `AQ1`), так и не входит. Сплошной прогон
    # по всем шестнадцати логам — единственный способ это утверждать: два лога
    # из шестнадцати меняют предложение (`brunna` 8 → 10, `frah_hall` 6 → 7),
    # один получает его ВПЕРВЫЕ (`froim` 0 → 1), и ни один — свой лист.
    #
    # ⚠️ Числа `fort/ref/will` здесь — НАША модель, снятая прогоном до правки
    # и воспроизведённая после. Игре они равны у шести логов, и там это
    # проверяют соседние тесты; остальные десять несут экипировку, которой
    # у нас нет, и их лист сверяется сам с собой — ровно тот вопрос, на который
    # отвечает эта таблица.
    #
    # ⚠️ Колонка «значение» названа рядом с рангами не для красоты: у восьми
    # логов из шестнадцати рангов нет вовсе, а значение не ноль (модификатор
    # интеллекта есть у всех), и без неё строка «0 рангов → 0» читалась бы как
    # арифметика там, где работают ВОРОТА ТРЕНИРОВКИ. ⚠️ При этом ни на одном
    # живом логе ворота ничего не решают: значения там ниже пяти, и `div` дал
    # бы ноль и без них — ворота держит синтетика в `Rules.SkillsTest`.
    for {name, ranks, value, offered, fort, ref, will} <- [
          {"aley", 5, 7, 1, 12, 13, 10},
          {"babuka", 0, 1, 0, 33, 23, 16},
          {"boido", 0, 0, 0, 17, 6, 6},
          {"brunna", 42, 54, 10, 27, 18, 24},
          {"elith", 0, 0, 0, 16, 14, 13},
          {"frah_hall", 32, 38, 7, 16, 15, 21},
          {"froim", 4, 8, 1, 27, 26, 23},
          {"hana", 0, 2, 0, 15, 10, 11},
          {"hela", 0, 3, 0, 17, 6, 17},
          {"hnyupius", 0, 2, 0, 33, 26, 24},
          {"hnyupius_alignment", 0, 2, 0, 33, 26, 24},
          {"moxie", 30, 33, 6, 24, 19, 37},
          {"nathan", 0, 2, 0, 7, 13, 4},
          {"nicha", 0, 0, 0, 10, 21, 12},
          {"timonall", 0, 1, 0, 22, 16, 12},
          {"trina", 0, 2, 0, 15, 5, 6}
        ] do
      test "#{name}.log: лист тот же, а прибавка считается от значения навыка" do
        build = parsed(unquote(name)).build
        level = Build.character_level(build)
        stats = Rules.compute(build, @ruleset)

        assert Build.skill_ranks(build, :spellcraft, level) == unquote(ranks)

        # Значение навыка — и оно же то число, от которого берётся `div`.
        # ⚠️ Спрашивается у `Skills.value/4`, а не у `stats.skill_values`:
        # панель печатает только навыки, во что билд вложился, и у восьми
        # логов из шестнадцати записи там нет вовсе — проверка вышла бы
        # пустой ровно там, где интересны ворота тренировки.
        assert Skills.value(build, @ruleset, :spellcraft, level).total == unquote(value)

        assert stats.skill_save_bonus == unquote(offered)

        # 🔴 И главное: лист персонажа, каким он был до правки.
