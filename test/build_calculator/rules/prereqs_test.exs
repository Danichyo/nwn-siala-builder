defmodule BuildCalculator.Rules.PrereqsTest do
  ...
  describe "save_bonus — голый сейв против того, что в листе" do
    ...
    test "надетое требование не закрывает, а заработанное — закрывает", %{ruleset: ruleset} do
      ...
      # То же требование, закрытое своим: фит из слота даёт +2 к Стойкости.
      #
      # ⚠️ **ЗДЕСЬ СТОЯЛИ РАНГИ Spellcraft**, и задача 3.197 их отсюда убрала:
      # с 12.09.2026 условная прибавка требование не выполняет (допущение,
      # замер `AR1`). Пример заменён, а не удалён: «заработанное закрывает» —
      # половина, без которой тест зеленел бы и на модели, не пускающей никого.
      earned =
        Build.new(levels: levels, feats: %{1 => %{{:general, 1} => :great_fortitude}})
    ...
    test "своё требование выполняет: фит и классовое умение", %{ruleset: ruleset} do
      block = %{"save_bonus" => %{"fortitude" => 8}}

      # 🔴 **ЗДЕСЬ БЫЛ ТРЕТИЙ ПРИМЕР — ранги Spellcraft**, и он переехал
      # в собственный тест ниже с обратным ответом (задача 3.197): условная
      # прибавка требование не выполняет. Остальное своё — выполняет
      # по-прежнему, и это по-прежнему половина против ложной нелегальности.
      feat =
        Build.new(
          levels: List.duplicate(:fighter, 11),
    ...
    # 🔴 А УСЛОВНАЯ ПРИБАВКА — НЕ ВЫПОЛНЯЕТ (задача 3.197). Замер `AQ1` (Dan,
    # 12.09.2026): «спасы от Spellcraft в листе персонажа не видна». До правки
    # здесь была ложная ЛЕГАЛЬНОСТЬ.
    #
    ...
    # роли»). Здесь стояло «допущение с названным направлением… отданное
    # в замер»: направление оказалось верным по факту, а не только
    # по осторожности. В данных статус поднят `assumed` → `verified`
    # (`overrides.json` → `skill_save_bonus.rules[].prerequisite`); ни одно
    # число при этом не сдвинулось — сменилась опора, а не ответ.
    #
    # ⚠️ Ранги куплены НА ДЕСЯТОМ, а не на одиннадцатом, и это ровно та точка,
    # которой не хватает замеру S7c: тот наблюдал ранги, купленные на самом
    # уровне, и его «фита нет» одинаково объясняется обоими правилами.
    test "условная прибавка Spellcraft требование НЕ выполняет", %{ruleset: ruleset} do
      ...
      stats = Rules.compute(spellcraft, ruleset)

      # Прибавка настоящая и против заклинаний работает…
      assert stats.conditional_save_bonus.fort == 1
      assert stats.saves_conditional.fort == 8

      # …но ни лист (7), ни требование её не видят.
      assert stats.fort == 7
      assert stats.saves_for_prereqs.fort == 7
      assert check(ruleset, block, spellcraft) == {:error, [{:requires_save_bonus, :fort, 8}]}
    end
    ...
    test "ответ одинаков на обоих ruleset'ах" do
      ...
      levels = List.duplicate(:fighter, 11)
      geared = Build.new(levels: levels, gear: %Gear{saves: 20})
      earned = Build.new(levels: levels, feats: %{1 => %{{:general, 1} => :great_fortitude}})

      # ⚠️ Правило про Spellcraft тоже общее: запись лежит в секции
      # `_vanilla_constants_confirmed`, которую видят оба ruleset'а, — вопрос
      # про движок, а не про баланс шарда (задача 3.197).
      conditional = Build.new(levels: levels, skills: %{10 => %{spellcraft: 5}})
      ...
      for version <- Data.versions() do
        ...
        assert check(ruleset, block, conditional) ==
                 {:error, [{:requires_save_bonus, :fort, 8}]},
               version
      end
    end
  ...
  describe "save_bonus — сейв на входе в уровень" do
    ...
    #
    # 🔴 **С 12.09.2026 ЭТОТ ЗАМЕР БОЛЬШЕ НЕ РАЗЛИЧАЕТ ДВА ПРАВИЛА, и тест
    # переписан именно поэтому** (задача 3.197). Наблюдение «фита нет»
    # объясняется и прежним чтением («ранги ЭТОГО уровня вне снимка»), и новым
    # («условная прибавка Spellcraft требование не выполняет вовсе», `AQ1`) —
    # и второе объясняет его без всякой оговорки про момент покупки.
    #
    # ⚠️ Положительной половины у S7c не было НИКОГДА: строка «воин 11 с пятью
    # рангами, купленными раньше, требование проходит» в `GAME_CHECKS.md`
    # помечена «проверено ВЫЗОВОМ при правке S2» — то есть нашим же кодом,
    # а не игрой. Поэтому здесь теперь стоит то, что модель действительно
    # утверждает: **отказ в обоих случаях**, и момент покупки на него больше
    # не влияет.
    #
    # ✅ **Замер `AR1` проведён** (Dan, 13.09.2026) — ровно на рангах, купленных
    # РАНЬШЕ, и фит всё равно недоступен. Здесь стояло «различит их замер
    # `AR1`»: он и различил, но в ту сторону, где правило S7c по-прежнему
    # ничего не двигает — наблюдение объясняется целиком тем, что условная
    # прибавка в требование не идёт. Правило «ранги этого уровня вне снимка»
    # остаётся ненаблюдаемым и остаётся на месте.
    #
    # ⚠️ Правило «ранги этого уровня вне снимка» НЕ снято: снимать измеренное
    # правило потому, что его стало нечем наблюдать, — это менять модель без
    # источника. Сегодня оно просто ничего не двигает.
    test "ранги навыка требование не закрывают — ни этого уровня, ни прошлого", %{
      ruleset: ruleset
    } do
      levels = List.duplicate(:fighter, 12)
      ...
      # Стойкость в листе у обоих одна и та же, и прибавки от рангов в ней нет
      # вовсе — она стоит рядом, «против заклинаний».
      assert Rules.compute(on_this_level, ruleset).fort ==
               Rules.compute(a_level_earlier, ruleset).fort

      assert Rules.compute(a_level_earlier, ruleset).conditional_save_bonus.fort == 1

      # ⚠️ ЗДЕСЬ СТОЯЛО `:ok` у второго билда — до задачи 3.197.
      for build <- [on_this_level, a_level_earlier] do
        assert Rules.validate_feat(build, :resist_energy, ruleset) ==
                 {:error, [{:requires_save_bonus, :fort, 8}]}
      end
    end
