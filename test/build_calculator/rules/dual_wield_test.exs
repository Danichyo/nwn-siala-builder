defmodule BuildCalculator.Rules.DualWieldTest do
  ...
  describe "бонус за тип оружия по обеим рукам" do
    ...
    # дальнобойное во вторую руку не идёт. Живая пара с двумя ВИДАМИ — вот
    # эта: клинок даёт щитовой AC, а молот — звуковой урон, получателя
    # которому мы не считаем (CLAUDE.md §9).
    ...
    # 🔴 ПОЛОЖИТЕЛЬНЫЙ КОНТРОЛЬ НА СИНТЕТИКЕ. ⚠️ Здесь стояло «и он обязателен:
    # на поставляемых данных… два ПОСЧИТАННЫХ числа от двух рук живыми данными
    # больше не достигаются» — это перестало быть верным 11.09.2026 (задача
    # 3.183): магический посох одноручный и во вторую руку идёт, а его три
    # навыка считаются, так что живая пара есть и стоит кейсом ниже.
    #
    # Синтетика при этом ОСТАЁТСЯ, и не по инерции: она держит другую пару
    # видов (щитовой AC и бонус атаки), которой живыми данными правда нет, —
    # и держит её независимо от того, что завтра случится с посохом.
    #
    ...
    # ✅ ЖИВАЯ ПАРА С ДВУМЯ ПОСЧИТАННЫМИ ЧИСЛАМИ — с задачи 3.183. Магический
    # посох `medium`, то есть одноручный, и во вторую руку идёт; клинок в
    # главной даёт щитовой AC, посох — три значения навыка. Ровно та
    # комбинация, которой три недели не существовало на поставляемых данных.
    # ⚠️ Числа сагровские (18, а не 12): билд этого файла — чистый воин.
    test "клинок и магический посох: два посчитанных числа живыми данными", %{
      ruleset: ruleset
    } do
      stats = Rules.compute(build(weapon: :katana, off_hand: :magic_staff, levels: 41), ruleset)

      assert for(e <- stats.weapon_type_bonuses, do: {e.weapon, e.kind, e.skill, e.counted}) == [
               {:katana, :shield_ac, nil, 9},
               {:magic_staff, :skill_bonus, :spellcraft, 18},
               {:magic_staff, :skill_bonus, :concentration, 18},
               {:magic_staff, :skill_bonus, :animal_empathy, 18}
             ]
    end
