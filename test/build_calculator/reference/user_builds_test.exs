  ...
  @builds [
    %{
      ...
      expect: %{
        ...
        #   fort: 22 + CON mod(+5, score 20) + 6 = 33
        #   ref:  20 + DEX mod(+1, score 13) + 0 = 21
        #   will: 24 + WIS mod(-1, score  8) + 0 = 23
        #
        # 🔴 **ЗДЕСЬ СТОЯЛО 39/27/29** — до задачи 3.197, пока прибавка
        # Spellcraft (34 ранга → +6 на все три) лежала внутри самого спаса.
        # Замер `AQ1` (Dan, 12.09.2026): «спасы от Spellcraft в листе персонажа
        # не видна». Прибавка условная, и в лист не идёт — а те самые 39/27/29
        # остались числом «против заклинаний», см. `saves_conditional` ниже.
        fort: 33,
        ref: 21,
        will: 23,
        # save_bonus — теперь только собственные термы билда (Great/Epic
        # Fortitude, fort only): fort 6, ref 0, will 0.
        save_bonus: %{fort: 6, ref: 0, will: 0},
        # 🔴 **ЗДЕСЬ СТОЯЛО 6** — до задачи 3.202, пока прибавка считалась
        # от 34 купленных рангов. Теперь она считается от полного значения
        # навыка (`fandom:Spellcraft`: «based on fully modified ranks»):
        # 34 ранга + модификатор INT +3 (интеллект 16 — базовые 14 плюс +2
        # от таблицы Ученика красного дракона на 9-м классовом уровне) = 37
        # → **+7**. Значение проверяется отдельной строкой ниже, чтобы число
        # не пришлось принимать на веру.
        spellcraft_value: 37,
        skill_save_bonus: 7,
        # Условная прибавка: +7 на все три, ничем не срезана (вещей на спасы
        # у билда нет вовсе, а собственные шесть лежат ПОВЕРХ потолка).
        # ⚠️ ЗДЕСЬ СТОЯЛО 39/27/29 — те же три числа, что печатались до 3.197;
        # после 3.202 каждое выросло на единицу вместе с прибавкой.
        saves_conditional: %{fort: 40, ref: 28, will: 30},
        conditional_save_bonus: %{fort: 7, ref: 7, will: 7},
        conditional_save_terms: [%{skill: :spellcraft, scope: "spells", bonus: 7}],
        ...
        racial_bonus: %{
          ...
          variant: nil,
          inactive?: true,
          # Какой скрипт шарда считает этот бонус (отдаётся наружу с задачи
          # 3.184): усиление мини-сетами и внутренний кап принадлежат
          # ИСПОЛНИТЕЛЮ, а не виду бонуса, и сложить наш терм с оружейным
          # можно только там, где исполнитель у обоих один.
          executor: :attack,
          # Совпадение расы с категорией оружия (задача 3.198). `:none` здесь
          # не «не совпало», а «спрашивать нечего»: руки пусты, бонус и так
          # выключен (`inactive?: true`), и удваивать нечего.
          weapon_match: :none
        },
        ...
        gaps:
          Enum.sort([
            ...
            # 🔴 ВЕРНУЛОСЬ 3.209 и УШЛО СНОВА 3.210, за один день 18.09.2026.
            # Билд берёт `epic_energy_resistance` на 32-м уровне
            # (`{:class_bonus, :pale_master} => {:epic_energy_resistance,
            # :electrical}`), и 3.209 дала поглощению стихийного урона своего
            # получателя (`damage_resistance`, `our`) — оговорка стала честной:
            # величина размечена, а складывать её с расовым/оружейным эффектом
            # и вещами было нечем. 3.210 научила (`Rules.Resistances`), и у билда
            # теперь стоит `electrical` 15 в секции «Резисты» — печатать рядом
            # «прибавку не считаем» значило бы спорить с числом на экране.
            ...
            # `WikiBuildsTest`. 🔴 И ЗДЕСЬ БЫЛА НЕВЕРНАЯ ПОСЫЛКА, А НЕ ФАКТ: у поглощения
            # появился свой получатель задачей 3.209, и строка ВЕРНУЛАСЬ выше, в список.
            # «Механика, которой калькулятор не отвечает вовсе» перестало быть правдой —
            # ей просто ещё не ответили (задача 3.210).
  ...
  for entry <- @builds do
    ...
    describe entry.title do
      ...
      test "attack, saves, attacks per round and HP", %{ruleset: ruleset} do
        ...
        # 🔴 Задача 3.202: прибавка берётся от ЗНАЧЕНИЯ навыка, а не от рангов,
        # — значит число, от которого она считается, обязано стоять в фикстуре
        # рядом. Без него «+7» читалось бы как «34 ранга дают семь», что неверно.
        assert stats.skill_values.spellcraft.total == entry.expect.spellcraft_value

        # 🔴 Задача 3.197: то же число, но рядом с листом, а не в нём.
        assert stats.saves_conditional == entry.expect.saves_conditional
        assert stats.conditional_save_bonus == entry.expect.conditional_save_bonus
        assert stats.conditional_save_terms == entry.expect.conditional_save_terms
