defmodule BuildCalculatorWeb.BuilderGearSkillsTest do
  ...
  describe "показ вписанного" do
    ...
    # ⚠️ Здесь стояло «свёрнутая сводка называет навык и число»: шапка печатала
    # `Discipline +50`. Задача 3.191 сняла перечисление целиком — с полного
    # билда оно обрывалось на второй позиции из семи, — и оставила ровно тот
    # сигнал, который не обрывался никогда: «задана ли экипировка».
    test "свёрнутая шапка не говорит «ещё не задана», когда прибавка набрана", %{
      conn: conn,
      build: build
    } do
      code = Encoding.encode(typed(build, %{discipline: 50}))
      ...
      # Само число при этом на экране есть — в своей строке, а не в шапке.
      # ⚠️ Именно поэтому его приходится открывать: строка живёт ВНУТРИ блока,
      # а шапка отвечает и свёрнутой — в этом и разница между двумя вопросами.
      open_gear(view)
      assert has_element?(view, ~s(#gear-skill-input-discipline[value="50"]))
    end
