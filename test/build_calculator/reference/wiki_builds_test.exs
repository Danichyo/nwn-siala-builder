defmodule BuildCalculator.Reference.WikiBuildsTest do
  ...
  The numbers printed on these pages are **geared** numbers: mini-set pieces and
  crafted items alone are worth up to **+105 %** HP — and **nothing at all** past
  the eleventh such item, where the shard's table cuts off to zero (task 3.186,
  `Rules.GearHitPoints`) — which is where 700–1000 HP and +71 attack come from.
  ⚠ Here stood «+15…+91 %»: the row stopped at nine items because that is where
  the wiki's own list stops; the server-script table goes to ten and then falls
  off a cliff. The fixtures state neither their pieces nor their HP, so the
  calculator now computes the percentage and still has nothing to compare it
  against — see `@not_comparable`. We model no equipment these pages describe,
  so comparing their totals is guaranteed to disagree and would teach us
  nothing. `@not_comparable` names each one and why; the test
  ...
  🔴 **`epic_energy_resistance` — ЕДИНСТВЕННОЕ из семнадцати, что ВЕРНУЛОСЬ
  и ушло снова, за один день** (задачи 3.209 и 3.210, 18.09.2026). Посылка
  «получатель эффекта не наш» перестала быть верной утром: поглощение
  стихийного урона получило СВОЙ получатель (`damage_resistance`, `our`),
  и три фикстуры ниже («Бледный Призыватель», «Гном Защитник», «Мастер оружия
  Сагровик») снова понесли `{:not_modelled, {:feat_bonus,
  :epic_energy_resistance}}` — честный ответ: величина размечена, а складывать
  её с расой, оружием и вещами было нечем. К вечеру 3.210 научила ядро считать
  (`Rules.Resistances`, `DerivedStats.resistances`), и строка ушла из всех трёх.

  ⚠️ **Оба движения — один и тот же механизм, а не качели:** оговорку
  выдаёт и гасит `Rules.FeatChoices.effect_gap/2` по двум вопросам —
  «наш ли получатель» и «посчитана ли прибавка», — и каждый раз менялся ответ
  ровно на один из них. Ни одна строка не была снята «потому что тест красный»,
  и порядок правки оба раза был прежний: сперва данные и ядро, потом фикстура.
      ...
      model: %{
        ...
        feats: %{
          ...
          caveats: [
            ...
            # 🔴 Снято задачей 3.93 (25.08.2026), ВЕРНУЛОСЬ 3.209 и СНЯТО СНОВА
            # задачей 3.210 (18.09.2026) — оба раза по одному правилу, и это
            # не качели. 3.209 дала `epic_energy_resistance` нашего получателя
            # (`damage_resistance`), но считать величину было ещё нечем, и
            # оговорка была ЧЕСТНОЙ. 3.210 её посчитала: у билда есть секция
            # «Резисты», фит взят на 23-м уровне и даёт там 15 к своей стихии
            # (`Rules.Resistances`). Признание снято тем же механизмом, каким
            # оно и появилось, — `whole_effect_counted?/2`, — а не вычёркиванием:
            # запись, чей `effect_coverage` перестанет объявлять полноту, вернёт
            # оговорку сама.
    ...
    %{
      ...
      model: %{
        ...
        gaps: [
          ...
          # 🔴 **А 18.09.2026 (задача 3.210) ВМЕСТО НЕЁ ПОЯВИЛАСЬ ДРУГАЯ, и это
          # смена посылки, а не отмена решения.** Решение Dan было «пока мы это
          # не показываем», и «пока» кончилось: у поглощения стихийного урона
          # есть секция («Резисты», `DerivedStats.resistances`), вид Гнома
          # посчитан, и билд говорит теперь ровно то же, что говорят три расы
          # с получателем, — `{:assumed, {:racial_bonus_variant, :dwarf, nil}}`,
          # «величина известна, а включить бонус нечем: оружия страница
          # не называет». Та же строка стоит у Карлика и Светлого эльфа выше,
          # и по той же причине (замер Q1/Q4: бонус включается оружием в руках).
          #
          # ⚠️ `{:missing_data, {:racial_bonus_level, :dwarf}}` тут по-прежнему
          # быть не должно — величина известна на любом уровне с задачи 3.181.
          # Под тестом в `racial_bonus_test.exs`.
          {:assumed, {:racial_bonus_variant, :dwarf, nil}}
          #
        ...
        feats: %{
          ...
          caveats: [
            ...
            # и точнее нас; видно оно в конструкторе (3.87) и на экране просмотра (3.94).
            # 🔴 Снято задачей 3.93 (25.08.2026), ВЕРНУЛОСЬ 3.209 и СНЯТО СНОВА
            # задачей 3.210 (18.09.2026): фит взят ДВАЖДЫ (36-й и 39-й уровни),
            # и оба взятия теперь ДАЮТ ЧИСЛО — 30 к своей стихии, потому что
            # потолок эффекта считает сумму взятий (`Rules.Resistances`).
            # Оговорка была честной, пока считать было нечем.
      ...
      model: %{
        ...
        feats: %{
          ...
          caveats: [
            ...
            # и точнее нас; видно оно в конструкторе (3.87) и на экране просмотра (3.94).
            # 🔴 Снято задачей 3.93 (25.08.2026), ВЕРНУЛОСЬ 3.209 и СНЯТО СНОВА
            # задачей 3.210 (18.09.2026), ровно тем же доводом, что у «Гнома
            # Защитника» выше: фит взят дважды (36-й и 39-й), и оба взятия дают
            # число.
  ...
  for entry <- @builds do
    ...
    describe "#{entry.title} (revid #{entry.revid})" do
      ...
      # (CLAUDE.md §3; с задачи 3.186 верхняя граница процента к HP — +105 %,
      # а не +91 %, и за одиннадцатой вещью он обрывается в ноль). Спелл-резист
      # на Сиале
