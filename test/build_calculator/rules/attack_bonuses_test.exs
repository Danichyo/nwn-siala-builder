defmodule BuildCalculator.Rules.AttackBonusesTest do
  ...
  describe "Weapon Finesse — формула, а не прибавка" do
    ...
    test "Zen archery переключает атаку на WIS — но только с дальнобойным в руках",
      ...
      # ⚠️ **Числа дальнобойных строк выросли на 2 задачей 3.181 (11.09.2026),
      # и это не про `Zen archery`.** Монах 10 — тир 2 по таблице роста
      # сиальской системы оружия, и дальнобойная группа даёт ему +2 к атаке
      # (монах в группу Сагры не входит, значит вариант базовый, множителя
      # группы нет). Ближний бой и пустые руки не тронуты: клинковое даёт
      # щитовой AC, а не атаку. Проверяемое здесь утверждение — какая
      # ХАРАКТЕРИСТИКА идёт в бросок, — не изменилось ни в одной строке.
      table = [
        # {что в руках, владение, характеристики, ожидаемая характеристика, AB}
        {:longbow, :siala_ranged_proficiency, %{@flat | wis: 18}, :wis, 16},
        {:dart, :siala_ranged_proficiency, %{@flat | wis: 18}, :wis, 16},
        ...
        {:longbow, :siala_ranged_proficiency, %{@flat | dex: 18}, :dex, 16}
      ]
    ...
    test "без фита лук ничего не переключает", %{ruleset: ruleset} do
      ...
      # ⚠️ 10 → 12 задачей 3.181: те же +2 дальнобойной группы на тире 2, что
      # и в таблице выше. Смысл контроля не тронут — мудрость 18 без фита
      # по-прежнему не приезжает никуда.
    ...
    test "с луком фит больше не ПОНИЖАЕТ наше AB — дырка default'а закрыта замером",
      ...
      #
      # ⚠️ Оба числа выросли на 3 задачей 3.181: воин 10 — тир 2, билд чистый
      # воинский, то есть сагровик, значит дальнобойная группа даёт
      # floor(2 × 3 / 2) = 3. Разность, ради которой кейс и написан, не
      # изменилась: терм один и тот же по обе стороны фита.
      assert {without.attack_ability, without.attack_bonus} == {:dex, 13}
      assert {with_zen.attack_ability, with_zen.attack_bonus} == {:wis, 16}
    ...
    test "замер N1: воин 3 STR 8 / DEX 14 / WIS 18 — меч 2, лук 5, лук с Zen archery 7",
      ...
      # 🔴 **ЧИСЛА ДАЛЬНОБОЙНЫХ СТРОК ВЫРОСЛИ НА 1 ЗАДАЧЕЙ 3.181, и это НЕ
      # расхождение с замером.** Замер N1 дал МЕХАНИКУ («от ловкости, а с фитом
      # от мудрости»), а не числа: заказанный сценарий на тестовом сервере
      # не прошёл — «у меня там на тестовом сервере правда оружие только
      # с 15 уровня доступно», и сам ответ кейса говорит «числа 1 / 2 / 7
      # не сверялись». Тройка 2 / 5 / 7 была нашим же выводом, снятым вызовами
      # ядра 15.08.2026.
      #
      # ⚠️ И ровно эту прибавку тот же ответ назвал причиной, по которой замер
      # на низком уровне тяжёл (Dan): «с замерами на маленьких уровнях
      # сложности из-за бонусов системы оружия: оружие дальнего боя даёт
      # на 40 уровне +6 АБ и +9 АБ для сагровиков». Воин 3 — тир 1, билд
      # чистый воинский, то есть сагровик: floor(1 × 3 / 2) = 1.
      #
      # Меч не сдвинулся: клинковое даёт щитовой AC, а не атаку.
      assert ab.(armed, :longsword) == {:str, 2}
      assert ab.(armed, :longbow) == {:dex, 6}
      assert ab.(zen, :longbow) == {:wis, 8}
  ...
  describe "характеристика броска до фитов зависит от оружия в руках" do
    ...
    test "лук считает атаку от ловкости, меч — от силы, и фитов для этого не нужно",
      ...
      # BAB воина 10, модификаторы −1 и +4, плюс +3 дальнобойной группы
      # (тир 2, воинский билд — сагровик). ⚠️ Третья строка контрольная и
      # не сдвинулась: клинковое даёт щитовой AC, а не атаку.
      assert answer.(:longbow, :siala_ranged_proficiency) == {:dex, 17, false}
      assert answer.(:dart, :siala_ranged_proficiency) == {:dex, 17, false}
