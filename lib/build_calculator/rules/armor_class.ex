defmodule BuildCalculator.Rules.ArmorClass do
  ...
  **типы** прибавок и правило столкновения однотипных (`geared/3`), оговорка
  ...
  def terms(%Build{} = build, ruleset, modifiers) do
    ...
    from_data ++
      racial_terms(build, ruleset) ++
      weapon_type_terms(build, ruleset) ++
      mini_set_terms(build, ruleset)
  end
  ...
  # И третья сторона той же системы (задача 3.184): мини-сеты умножают ОБА терма
  # выше — `B += floor(B · Nmini / 10)` — и умножают их СУММОЙ, потому что
  # в движке это один эффект. Отсюда третий терм вместо правки двух: приписать
  # усиление расе или оружию значило бы соврать в разборе, а посчитать его
  # по каждому терму отдельно — занизить число (два усечения меньше одного,
  # `Rules.MiniSets`).
  #
  # ⚠ Тип и `vs_typed` те же, что у усиливаемого: усиление — тот же бонус,
  # только больше, и со вписанным под тем же типом числом оно конкурирует
  # по тому же правилу. Свой `kind` для этого и передан.
  #
  # ⚠ Щитовой AC ДВУЛЕЗВИЙНОГО МЕЧА сюда не попадает вовсе, и не особым случаем
  # здесь, а формой своего исполнителя: линейная ветка проходит мимо всех
  # множителей («class, race, and mini-set transforms are bypassed»).
  #
  # ⚠ Никогда в `ac_naked`, по той же причине, что и два терма выше: голый
  # проход идёт по билду с пустым `%Gear{}`, а мини-сеты — надетые предметы.
  defp mini_set_terms(build, ruleset) do
    for {kind, type, ac} <- MiniSets.armor_class(build, ruleset) do
      %{
        id: :mini_sets,
        source: {:mini_sets, :mini_sets},
        type: type,
        ac: ac,
        vs_typed: vs_typed(ruleset, kind)
      }
