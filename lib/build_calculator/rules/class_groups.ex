defmodule BuildCalculator.Rules.ClassGroups do
  ...
  в группу воинов Адры», and no page of either wiki writes down whether purity is
  required.

  ⚠ **Here stood «nobody wrote it down… applying Sagra's rule to Adra is an
  assumption» — that was true until 11.09.2026 (task 3.182).** The shard's own
  server scripts name the rule: `ClassNotRestrictTorm` — whose class list is
  Adra's, four out of four — is «a Torm-flavored variant» of `ClassNotRestrict`,
  and that one «returns TRUE only when ALL levels are Barbarian + Fighter +
  WeaponMaster + DwarvenDefender». So Adra is no longer an assumption; it is a
  **reading**, carried from the sibling function onto its variant, and the record
  says so with `status: "derived"` rather than `verified`. The difference still
  must not be erased — it is now `purity_status` that carries it, and the caption
  built on it says «выведено из разбора серверных скриптов», not «не описано».
  ...
  A group as the data states it.
    ...
    * `purity_required` — `true`, `false`, or `nil` when no source says
    * `purity_status` — **how well** that rule is known, as the record itself
      marks it: `"verified"` is a quote (Sagra), `"derived"` is a reading
      (Adra, task 3.182 — the shard's scripts state the ALL-levels form for
      Sagra's predicate and call Adra's «a Torm-flavored variant» of it),
      `nil` where nobody stated the rule at all. ⚠ Never collapsed into
      `purity_required`: both readings end in `true`, and the difference is
      exactly what the flag's caption has to say out loud
  ...
  `purity_stated?` is the honest half of `purity_required?`: `true` for Sagra,
  where the rule is quoted, and `false` for a group whose rule nobody wrote down
  at all. A caller that shows the flag without that distinction is showing two
  different qualities of fact as one.

  ⚠ **And there are three qualities, not two, since task 3.182.** Here stood «for
  Adra the first is `true` because we assumed it»: true until 11.09.2026, when
  the shard's own scripts named the rule — `ClassNotRestrictTorm` is called «a
  Torm-flavored variant» of `ClassNotRestrict`, and that one «returns TRUE only
  when ALL levels are…». So Adra's rule is now **stated and still not quoted**:
  `purity_stated?` is `true` and `purity_status` is `"derived"`. Asking only the
  boolean would now print Adra exactly like Sagra, which is the very thing the
  ...
  само, потому что каждое стоит на записи в данных, а не на флаге в коде:
    ...
    * группа, чьё правило чистоты **не названо вовсе** и у которой нет `not_a_gap`
      у `purity_required`, возвращает `{:assumed, …}`. ⚠ У Адры с 11.09.2026
      (задача 3.182) молчит уже ПЕРВОЕ условие — правило названо разбором
      серверных скриптов, — а решение владельца стало запасным: верни кто-нибудь
      `purity_required` в `nil`, и молчать снова будет оно.
  ...
  # `nil` — nothing states whether purity is required — is read as "yes", and the
  # gap above says it was read that way. Two reasons, both about the direction of
  # the error: every group whose rule *is* stated requires it (Sagra by a wiki
  # quote, Adra by the shard's scripts since task 3.182), and requiring it makes
  # the claim narrower, so a wrong reading withholds a flag instead of inventing
  # one.
  ...
  defp membership(group) do
    ...
    %{
      ...
      # ⚠ Handed over as the record wrote it, for the same reason
      # `purity_not_a_gap` is: «цитата это или вывод» is a statement the data
      # makes about itself, and turning it into a boolean here would be this
      # module's reading of somebody else's record — the second copy of a rule.
      purity_status: Map.get(group, :purity_status),
  ...
  # ⚠ First silence: somebody stated the rule, so nothing was assumed about it.
  # `purity_status` deliberately does **not** matter here — a rule read off the
  # shard's scripts (`derived`) is still a rule the source states, and a gap is a
  # hole in our ANSWER (CLAUDE.md §9). Adra moves no number at all, so the only
  # thing a wrong reading costs is the flag itself, and the flag's own caption is
  # where the difference between a quote and a reading gets said.
