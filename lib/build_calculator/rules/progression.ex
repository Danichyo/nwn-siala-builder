defmodule BuildCalculator.Rules.Progression do
  ...
  alias BuildCalculator.Rules.{AttackModifiers, Build, Epic, FeatBonuses, GearHitPoints}
  ...
  **untouched and as a label**, for the same reason `progression` does above:
  ...
  `gear_bonus` is the fifth bucket and the only one this function does **not**
  fill: the shard's percentage to maximum hit points for mini-set pieces and
  named items (task 3.186, `BuildCalculator.Rules.GearHitPoints`). It is always
  `nil` here and `Rules.compute/2` puts the term in, because it is not a fact
  about the class ladder at all — it multiplies the number this function has
  already finished producing, which is exactly what `Hbase` means in the source.
  Keeping the key here rather than adding it downstream is what makes the
  invariant below one statement instead of two: every reader of a breakdown sees
  the same set of keys whatever built it.
  ...
  (gear_bonus[:amount] || 0) + floor_adjustment` equals the total **exactly**,
  always, never approximately,
  ...
  @type hp_breakdown :: %{
          ...
          gear_bonus: GearHitPoints.term_entry() | nil,
  ...
  **whole** build's hit points, feat bonuses included: one level of such a class
  ...
  defp hp_breakdown(levels, ruleset, con_modifier, feat_terms, innate, total) do
    ...
    %{
      ...
      by_feat: by_feat,
      innate: innate,
      # Всегда `nil` здесь, и это не заглушка: процент за надетое считается
      # ОТ ИТОГА этой функции (`Hbase` источника), то есть не может быть
      # посчитан внутри неё. Ключ стоит на месте, чтобы у разбора был один
      # набор полей независимо от того, кто его собрал, — см. `hp_breakdown/0`.
      gear_bonus: nil,
