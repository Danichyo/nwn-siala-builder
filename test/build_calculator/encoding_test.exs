defmodule BuildCalculator.EncodingTest do
  ...
  describe "фит с вещи (задача 3.3)" do
    ...
    # 🔴 Задача 3.204: две одинаковые записи — два взятия, и ссылка обязана
    # донести обе. До неё декодер схлопывал их `Enum.uniq/1`, то есть терял
    # ввод игрока прямо в коде билда; цена — 33 HP на билде владельца.
    test "повтор записи переживает кодирование: две строки туда, две обратно" do
      %Build{} = base = sample()
      twice = %Build{base | gear: Gear.new(feats: [:epic_toughness, :epic_toughness])}

      assert {:ok, %{build: back, dropped: []}} = Encoding.decode(Encoding.encode(twice))
      assert back.gear.feats == [:epic_toughness, :epic_toughness]
      assert back == twice

      # Положительный контроль: одна запись и две дают РАЗНЫЙ код — иначе
      # assert выше зеленел бы и на кодеке, который повтор теряет.
      once = %Build{base | gear: Gear.new(feats: [:epic_toughness])}
      refute Encoding.encode(twice) == Encoding.encode(once)

      # ⚠️ Имя фита в таблице имён по-прежнему ОДНО на две строки (`uniq: true`
      # в `common_payload/1`), то есть повтор стоит не имени, а шести байт
      # строки таблицы записей.
      assert byte_size(Encoding.encode(twice)) - byte_size(Encoding.encode(once)) < 10
    end
  ...
  describe "куски мини-сетов (задача 3.185)" do
    # Наборы безымянные и пронумерованы по порядку ввода: в `Nmini` входит
    # только мультимножество счётчиков (`Rules.MiniSets`), поэтому кодировать
    # нечего, кроме списка чисел.
    defp with_mini_sets(counts) do
      %Build{gear: %Gear{} = gear} = build = sample()
      %Build{build | gear: %Gear{gear | mini_sets: counts}}
    end

    test "куски доезжают целиком и в том же порядке" do
      build = with_mini_sets([2, 3, 2])

      assert {:ok, %{build: back, dropped: []}} = Encoding.decode(Encoding.encode(build))

      assert back.gear.mini_sets == [2, 3, 2]
      assert back == build
    end

    # 🔴 Порядок НЕ сортируется, в отличие от фитов с вещи и надетого: позиция
    # строки — это её подпись на экране («Набор 1», «Набор 2»), и сортировка
    # молча поменяла бы строки местами под курсором игрока.
    test "порядок ввода сохраняется, а не сортируется" do
      assert {:ok, %{build: back}} = Encoding.decode(Encoding.encode(with_mini_sets([3, 2])))
      assert back.gear.mini_sets == [3, 2]
    end

    # 🔴 Главный критерий задачи: билд БЕЗ мини-сетов кодируется побайтово
    # так же, как до неё. Byte-for-byte это проверяют `@frozen_v1`/`@frozen_v2`
    # выше в файле (их задача не тронула), а здесь — положительный контроль:
    # билд С кусками обязан кодироваться ИНАЧЕ, иначе assert'ы выше зеленели
    # бы у кодека, который строку просто не пишет.
    test "билд с кусками кодируется иначе, чем без них" do
      refute Encoding.encode(with_mini_sets([2, 2])) == Encoding.encode(sample())
      assert Encoding.encode(sample()) == @frozen_v2
    end

    test "старая ссылка (до задачи 3.185) открывается без мини-сетов" do
      for code <- [@frozen_v1, @frozen_v2, @frozen_v2_geared] do
        assert {:ok, %{build: %Build{} = back}} = Encoding.decode(code)
        assert back.gear.mini_sets == []
      end
    end

    # ⚠️ Нечитаемое и неположительное теряется ПОИМЁННО, строкой целиком —
    # не нулём, как битые числа оружия: там ноль значит «предмет без
    # прибавки», законное состояние, а здесь строка значит «набор надет»,
    # и «надет ноль кусков» не значит ничего вовсе.
    test "битая строка кусков названа в выпавшем, а билд открывается" do
      for bad <- ["miniset|2,x,2", "miniset|2,0,2", "miniset|2,-,2"] do
        broken =
          with_mini_sets([2, 3, 2])
          |> Encoding.encode()
          # Та же длина, иначе поедет length-prefix строки (см. `tamper/3`).
          |> tamper("miniset|2,3,2", bad)

        assert {:ok, %{build: build, dropped: dropped}} = Encoding.decode(broken)
        assert {:unknown_mini_sets, bad} in dropped
        assert build.gear.mini_sets == []

        # Остальное на месте: одна испорченная строка не роняет билд.
        assert build.levels == sample().levels
        assert build.feats == sample().feats
      end
    end

    # Псевдо-слот, которого декодер не знает, теряется своей причиной —
    # то есть игрок увидит «из ссылки выпало», а не тишину. Та же плата,
    # что у всех восьми приёмов до этого (см. moduledoc `Encoding`).
    test "опечатка в самом ключе слота названа как неизвестный слот" do
      broken =
        with_mini_sets([2, 3, 2])
        |> Encoding.encode()
        |> tamper("miniset|2,3,2", "minisxt|2,3,2")

      assert {:ok, %{build: build, dropped: dropped}} = Encoding.decode(broken)
      assert {:unknown_slot, "minisxt|2,3,2"} in dropped
      assert build.gear.mini_sets == []
    end

    # ⚠️ Границы ФОРМАТА, а не игры: слотов в игре одиннадцать, и сумму сверх
    # них клипает ядро (`Rules.MiniSets.pieces/2`), а интерфейс предупреждает,
    # не блокируя. Здесь проверяется только то, что правленая руками ссылка
    # не может попросить нас записать строку длиннее, чем формат умеет, —
    # и что круг после клипа СТАБИЛЕН (второй заход даёт тот же код).
    test "кодек клипает своими границами и остаётся стабильным на втором круге" do
      code = Encoding.encode(with_mini_sets(List.duplicate(2, 40)))

      assert {:ok, %{build: back, dropped: []}} = Encoding.decode(code)
      assert length(back.gear.mini_sets) == 32
      assert Encoding.encode(back) == code

      # Счётчик больше предела формата — срезан, а не потерян целиком.
      assert {:ok, %{build: big}} = Encoding.decode(Encoding.encode(with_mini_sets([256])))
      assert big.gear.mini_sets == [255]
    end

    # ⚠️ Сумму сверх слотов кодек НЕ режет — это дело ядра, ровно как у всех
    # остальных чисел вещей: правленая руками ссылка открывается, а во что она
    # упёрлась, говорит расчёт.
    test "сумма сверх слотов через ссылку проезжает целиком, а режет ядро" do
      assert {:ok, %{build: back}} = Encoding.decode(Encoding.encode(with_mini_sets([6, 5, 3])))
      assert back.gear.mini_sets == [6, 5, 3]

      ruleset = Data.ruleset!("siala_41")
      assert Rules.compute(back, ruleset).mini_set_pieces == 11
    end
  end

  describe "крафтовые (именные) вещи (задача 3.186)" do
    defp with_named_items(count) do
      %Build{gear: %Gear{} = gear} = build = sample()
      %Build{build | gear: %Gear{gear | named_items: count}}
    end

    test "число доезжает целиком" do
      build = with_named_items(3)

      assert {:ok, %{build: back, dropped: []}} = Encoding.decode(Encoding.encode(build))
      assert back.gear.named_items == 3
      assert back == build
    end

    # 🔴 Главный критерий задачи: билд БЕЗ именных вещей кодируется побайтово
    # так же, как до неё — `@frozen_v2` byte-for-byte это уже проверяет выше
    # в файле; здесь положительный контроль, что билд С числом кодируется
    # ИНАЧЕ, иначе assert выше зеленел бы у кодека, который строку не пишет.
    test "билд с именными вещами кодируется иначе, чем без них" do
      refute Encoding.encode(with_named_items(2)) == Encoding.encode(sample())
      assert Encoding.encode(sample()) == @frozen_v2
    end

    test "старая ссылка (до задачи 3.186) открывается без именных вещей" do
      for code <- [@frozen_v1, @frozen_v2, @frozen_v2_geared] do
        assert {:ok, %{build: %Build{} = back}} = Encoding.decode(code)
        assert back.gear.named_items == 0
      end
    end

    # ⚠️ `0` — легальное чтение здесь, в отличие от кусков мини-сетов: «вещей
    # нет» это ровно умолчание `Rules.Gear`, а не бессмысленное состояние.
    # Наш кодер такую строку не пишет (см. тест выше), но декодер её примет.
    test "правленая руками ссылка с нулём читается как ноль, не как ошибка" do
      broken =
        with_named_items(3)
        |> Encoding.encode()
        |> tamper("nameditems|3", "nameditems|0")

      assert {:ok, %{build: build, dropped: []}} = Encoding.decode(broken)
      assert build.gear.named_items == 0
    end

    # Нечитаемое и отрицательное теряется ПОИМЁННО, строкой целиком — та же
    # форма отказа, что у кусков мини-сетов (`{:unknown_mini_sets, …}`).
    # ⚠️ Длина замены обязана совпадать с длиной `"nameditems|3"` (12 байт) —
    # `tamper/3` подменяет строку внутри сжатого payload'а, не трогая
    # раскладку байт, и length-prefix остаётся верным только при равной длине.
    test "битая строка названа в выпавшем, а билд открывается" do
      for bad <- ["nameditems|x", "nameditems|-", "nameditems|."] do
        broken =
          with_named_items(3)
          |> Encoding.encode()
          |> tamper("nameditems|3", bad)

        assert {:ok, %{build: build, dropped: dropped}} = Encoding.decode(broken)
        assert {:unknown_named_items, bad} in dropped
        assert build.gear.named_items == 0
    ...
    test "опечатка в самом ключе слота названа как неизвестный слот" do
      broken =
        with_named_items(3)
        |> Encoding.encode()
        |> tamper("nameditems|3", "namedxtems|3")

      assert {:ok, %{build: build, dropped: dropped}} = Encoding.decode(broken)
      assert {:unknown_slot, "namedxtems|3"} in dropped
      assert build.gear.named_items == 0
    end

    # ⚠️ Граница ФОРМАТА, не игры: потолок слотов (10 у Сиалы) применяет
    # ядро (`Rules.GearHitPoints.named_item_slots/1`), а кодек защищает
    # только от абсурдно большого числа в правленой руками ссылке.
    test "огромное число через ссылку клипается границей формата" do
      assert {:ok, %{build: big}} = Encoding.decode(Encoding.encode(with_named_items(50_000)))
      assert big.gear.named_items == 9999
    end

    # Сумму НЕ режет кодек — режет ядро, ровно как у всех остальных чисел
    # вещей: правленая руками ссылка с «+50» открывается, а во что она
    # упёрлась, говорит расчёт.
    test "число сверх слотов через ссылку проезжает целиком, а клипает ядро" do
      assert {:ok, %{build: back}} = Encoding.decode(Encoding.encode(with_named_items(50)))
      assert back.gear.named_items == 50

      ruleset = Data.ruleset!("siala_41")
      term = Rules.GearHitPoints.term(back, ruleset, 400)
      assert term.named == Rules.GearHitPoints.named_item_slots(ruleset)
    end
  end

  describe "раздельные спасы с вещей (задача 3.187, часть B)" do
    defp with_saves_specific(specific) do
      %Build{gear: %Gear{} = gear} = build = sample()
      %Build{build | gear: %Gear{gear | saves_specific: specific}}
    end

    test "тройка доезжает целиком, включая минус" do
      build = with_saves_specific(%{fort: 12, ref: -3, will: 3})

      assert {:ok, %{build: back, dropped: []}} = Encoding.decode(Encoding.encode(build))

      assert back.gear.saves_specific == %{fort: 12, ref: -3, will: 3}
      assert back == build
    end

    # Одно поле из трёх — остальные два просто не пишутся в тройку (читаются
    # нулём, а `non_zero/1` их сразу отбрасывает), а не роняют строку.
    test "прибавка только к одному сейву" do
      assert {:ok, %{build: back}} =
               with_saves_specific(%{will: 4}) |> Encoding.encode() |> Encoding.decode()

      assert back.gear.saves_specific == %{will: 4}
    end

    # 🔴 Главный критерий задачи: билд БЕЗ раздельных спасов кодируется
    # побайтово так же, как до неё — `@frozen_v2` byte-for-byte это уже
    # проверяет выше в файле; здесь положительный контроль, что билд
    # С тройкой кодируется ИНАЧЕ.
    test "билд с раздельной прибавкой кодируется иначе, чем без неё" do
      refute Encoding.encode(with_saves_specific(%{fort: 5})) == Encoding.encode(sample())
      assert Encoding.encode(sample()) == @frozen_v2
    end

    test "старая ссылка (до задачи 3.187) открывается без раздельных спасов" do
      for code <- [@frozen_v1, @frozen_v2, @frozen_v2_geared] do
        assert {:ok, %{build: %Build{} = back}} = Encoding.decode(code)
        assert back.gear.saves_specific == %{}
      end
    end

    # Нечитаемое или неполное (не три числа) теряется ПОИМЁННО, строкой
    # целиком — та же честность, что у кусков мини-сетов: «два поля из трёх»
    # не значит ничего, а молчаливый ноль решил бы за игрока.
    #
    # ⚠️ Все три замены — ТОЙ ЖЕ длины (16 байт), что и оригинал: `tamper/3`
    # правит строку внутри уже сжатого payload'а на месте, и байт длины перед
    # ней остаётся верным только при равной длине (тот же приём, что у кусков
    # мини-сетов и именных вещей выше).
    test "битая строка названа в выпавшем, а билд открывается" do
      for bad <- ["savespec|1x,-3,3", "savespec|12,-33,", "savespec|1,2,3,4"] do
        broken =
          with_saves_specific(%{fort: 12, ref: -3, will: 3})
          |> Encoding.encode()
          |> tamper("savespec|12,-3,3", bad)

        assert {:ok, %{build: build, dropped: dropped}} = Encoding.decode(broken)
        assert {:unknown_saves_specific, bad} in dropped
        assert build.gear.saves_specific == %{}
    ...
    # `0` в отдельной позиции — легальное чтение: «этот сейв без раздельной
    # прибавки», ровно умолчание `Rules.Gear`, не ошибка формата.
    test "ноль в одной из трёх позиций читается как отсутствие прибавки" do
      broken =
        with_saves_specific(%{fort: 12, ref: -3, will: 3})
        |> Encoding.encode()
        |> tamper("savespec|12,-3,3", "savespec|12,-3,0")

      assert {:ok, %{build: build, dropped: []}} = Encoding.decode(broken)
      assert build.gear.saves_specific == %{fort: 12, ref: -3}
    end
    ...
    test "опечатка в самом ключе слота названа как неизвестный слот" do
      broken =
        with_saves_specific(%{fort: 12, ref: -3, will: 3})
        |> Encoding.encode()
        |> tamper("savespec|12,-3,3", "savesxec|12,-3,3")

      assert {:ok, %{build: build, dropped: dropped}} = Encoding.decode(broken)
      assert {:unknown_slot, "savesxec|12,-3,3"} in dropped
      assert build.gear.saves_specific == %{}
    end

    # Границы ФОРМАТА, не игры: потолок +20 применяет ядро (`Rules.Caps`),
    # правленая руками ссылка с «+999» обязана открыться, как открывается
    # такое же число в поле «ко всем» (`gear.saves`).
    test "огромное число через ссылку клипается границей формата" do
      assert {:ok, %{build: big}} =
               with_saves_specific(%{fort: 5_000}) |> Encoding.encode() |> Encoding.decode()

      assert big.gear.saves_specific == %{fort: 999}
    end

    # Живое число задачи (Хнюпиус): один клип на сумму universal + раздельной,
    # применяет ядро, не кодек.
    test "потолок на СУММУ с universal режет ядро, а не кодек" do
      %Build{gear: %Gear{} = gear} = build = sample()
      build = %Build{build | gear: %Gear{gear | saves: 16, saves_specific: %{fort: 12}}}

      assert {:ok, %{build: back}} = Encoding.decode(Encoding.encode(build))
      assert {back.gear.saves, back.gear.saves_specific} == {16, %{fort: 12}}

      ruleset = Data.ruleset!("siala_41")
      stats = Rules.compute(back, ruleset)
      assert stats.gear_save_bonuses.fort == 28
      assert stats.save_bonus.fort == 20
    end
  end

  describe "поглощение стихий с вещей (задача 3.210)" do
    defp with_resistances(map) do
      %Build{gear: %Gear{} = gear} = build = sample()
      %Build{build | gear: %Gear{gear | resistances: map}}
    end

    test "по строке на стихию, и все доезжают целиком" do
      build = with_resistances(%{fire: 15, cold: 15, negative_energy: 20})

      assert {:ok, %{build: back, dropped: []}} = Encoding.decode(Encoding.encode(build))
      assert back.gear.resistances == %{fire: 15, cold: 15, negative_energy: 20}
      assert back == build
    end

    # 🔴 Главный критерий задачи: билд БЕЗ поглощения кодируется побайтово
    # так же, как до неё (`@frozen_v2` это уже проверяет выше в файле); здесь
    # положительный контроль, что билд С числом кодируется ИНАЧЕ, иначе assert
    # выше зеленел бы у кодека, который строк не пишет вовсе.
    test "билд с поглощением кодируется иначе, чем без него" do
      refute Encoding.encode(with_resistances(%{fire: 15})) == Encoding.encode(sample())
      assert Encoding.encode(sample()) == @frozen_v2
    end

    test "старая ссылка (до задачи 3.210) открывается без поглощения" do
      for code <- [@frozen_v1, @frozen_v2, @frozen_v2_geared] do
        assert {:ok, %{build: %Build{} = back}} = Encoding.decode(code)
        assert back.gear.resistances == %{}
      end
    end

    # Нули не пишутся — «вписал и стёр» и «не вписывал» один ответ у всех
    # полей `Rules.Gear`.
    test "нулевая стихия строки не получает" do
      assert Encoding.encode(with_resistances(%{fire: 0})) == Encoding.encode(sample())
    end

    # ⚠️ `0` при ЧТЕНИИ законен, как у именных вещей: «эта стихия без
    # поглощения» — ровно умолчание `Gear.resistances`.
    test "правленая руками ссылка с нулём читается как ноль, не как ошибка" do
      broken =
        with_resistances(%{fire: 15})
        |> Encoding.encode()
        |> tamper("resist|fire|15", "resist|fire|00")

      assert {:ok, %{build: build, dropped: []}} = Encoding.decode(broken)
      assert build.gear.resistances == %{fire: 0}
    end

    # 🔴 СТИХИЯ ПРОХОДИТ БЕЛЫЙ СПИСОК, и это не формальность: `magical` — вид
    # урона, который не поглощается вовсе, `bludgeoning` — физическое
    # поглощение, которого эта секция не показывает по решению владельца.
    # Молча положить их в мапу значило бы завести в билд стихию, которой у него
    # нет.
    # ⚠️ Кодеру подсовывается билд с чужими ключами НАПРЯМУЮ, а не через
    # `tamper/3`: кодер белого списка не знает (как не знает его и `worn`),
    # то есть такая ссылка ровно та, какую получишь, собрав билд руками или
    # открыв код, выпущенный снапшотом с другим словарём. Заодно обходится
    # ограничение `tamper/3` на равную длину строк.
    test "стихия вне словаря названа в выпавшем, а билд открывается" do
      build = with_resistances(%{magical: 15, bludgeoning: 10, fire: 15})

      assert {:ok, %{build: back, dropped: dropped}} = Encoding.decode(Encoding.encode(build))

      # обе чужие названы ПОИМЁННО, строкой целиком
      assert {:unknown_resistances, "resist|magical|15"} in dropped
      assert {:unknown_resistances, "resist|bludgeoning|10"} in dropped

      # ...а своя на месте: одна чужая строка не роняет ни билд, ни соседей
      assert back.gear.resistances == %{fire: 15}
      assert back.levels == sample().levels
      assert back.feats == sample().feats
    end

    test "нечитаемое число названо в выпавшем" do
      broken =
        with_resistances(%{fire: 15})
        |> Encoding.encode()
        |> tamper("resist|fire|15", "resist|fire|xx")

      assert {:ok, %{build: build, dropped: dropped}} = Encoding.decode(broken)
      assert {:unknown_resistances, "resist|fire|xx"} in dropped
    ...
    test "опечатка в самом ключе слота названа как неизвестный слот" do
      broken =
        with_resistances(%{fire: 15})
        |> Encoding.encode()
        |> tamper("resist|fire|15", "resixt|fire|15")

      assert {:ok, %{build: build, dropped: dropped}} = Encoding.decode(broken)
      assert {:unknown_slot, "resixt|fire|15"} in dropped
      assert build.gear.resistances == %{}
    end

    # Граница ФОРМАТА, не игры: своего потолка поглощению не называет ни один
    # источник, и выдумывать его в кодеке нельзя тем более, чем в ядре.
    test "огромное число через ссылку клипается границей формата" do
      assert {:ok, %{build: big}} =
               with_resistances(%{fire: 50_000}) |> Encoding.encode() |> Encoding.decode()

      assert big.gear.resistances == %{fire: 999}
    end

    # И число доезжает до расчёта, а не только до структуры.
    test "вписанное число доезжает до секции «Резисты»" do
      assert {:ok, %{build: back}} =
               with_resistances(%{acid: 15}) |> Encoding.encode() |> Encoding.decode()

      ruleset = Data.ruleset!("siala_41")
      stats = Rules.compute(back, ruleset)

      assert Rules.Resistances.total(stats.resistances, :acid) == 15
    end
  end
  ...
  describe "все псевдо-слоты разом — детектор изменения байт (задача 3.222)" do
    # Билд, у которого в таблице фитов лежат строки ВСЕХ псевдо-слотов сразу:
    # девять уровня `0` (фит с вещи голый и парой, оружие обеих рук с числами,
    # надетое двумя категориями, куски мини-сетов, именные вещи, раздельные
    # спасы, поглощение двух стихий) плюс выбор выдачи на настоящем уровне,
    # плюс прибавки к навыкам с вещей в таблице навыков — и обычные части
    # билда рядом: раса, мировоззрение, характеристики, классы, прибавка
    # к стату, фиты в слотах (один с выбором), ранги навыков, заклинания.
    defp with_every_pseudo_slot do
      Build.new(
        ruleset_version: "siala_41",
        race: :dwarf,
        alignment: :lawful_good,
        base_abilities: %{str: 16, dex: 12, con: 15, int: 10, wis: 12, cha: 8},
        levels: [:fighter, :fighter, :fighter, :fighter, :weapon_master, :weapon_master],
        ability_increases: %{4 => :str},
        feats: %{
          1 => %{:general => :toughness, {:class_bonus, :fighter} => :power_attack},
          2 => %{{:class_bonus, :fighter} => {:weapon_focus, :scimitar}}
        },
        granted_choices: %{5 => %{weapon_of_choice: :scimitar}},
        skills: %{1 => %{discipline: 4, spot: 2}, 2 => %{discipline: 1}},
        spells: %{7 => %{{:circle, 3, 0} => :fireball, {:circle, 1, 0} => :magic_missile}},
        gear:
          Gear.new(
            abilities: %{str: 6, con: 12},
            ac: %{armor: 8, deflection: 5},
            worn: %{armor: :full_plate, shield: :large},
            saves: 12,
            saves_specific: %{fort: 3, will: -2},
            skills: %{discipline: 50, spot: 12},
            # ⚠️ `Epic toughness` ДВАЖДЫ — законное двойное взятие с вещей
            # (задача 3.204), и декодер его не схлопывает.
            feats: [:epic_toughness, :epic_toughness, {:skill_focus, :discipline}],
            weapon: :scimitar,
            weapon_attack: 5,
            off_hand_weapon: :dagger,
    ...
    # 🔴 РОЛЬ ЭТОЙ СТРОКИ ДРУГАЯ, чем у `@frozen_v1`, `@frozen_v2` и
    # `@frozen_v2_geared`: она снята СЕГОДНЯШНИМ кодером (задача 3.222), а не
    # прошлой версией модуля, и потому обратной совместимости НЕ доказывает —
    # доказать её может только код, выпущенный в прошлом.
    #
    # Она — **детектор изменения байт**. Все три настоящие замороженные
    # фикстуры сняты с билдов, где нет ни одной строки уровня `0`, поэтому
    # порядок конкатенации девяти генераторов в `Encoding.common_payload/1`
    # не закреплён больше ничем: переставь их местами — и код молча сменится
    # у КАЖДОГО одетого билда при зелёном прогоне. Строка ниже обязана
    # упасть первой; если формат меняется намеренно, её перегенерируют
    # осознанно (и версию поднимают, если сменились байты у уже расшаренных
    # ссылок — это проверяют фикстуры выше, а не эта).
    @golden_pseudo_slots "2.bVHLbtswECQpyXJdOQ8EKdB_cIHGSS79GWJDrahFKFLg0lUP_PhSjuEWRU_EzixnBjtqywQO9MtT0y8Qh88OluHktA2h31RmhG1lgr-tevzVVeTTruIU76qFuFPtQHZMGPcLwhy8noDLpEQtlaxrsbnBmYxO4WRHj8zdHBaMGlIC8_6F38k5PQRz4twTG5odefx03X68iH5sFH6iBPHrBQ2DNmMgg1dG3MkH44BZvwV_4h-XbELK1qLHCE5U6r8btagtFgHxz6vE_Yfb1SO_FvIhDMMF78FajPnbscCPS4g-Q5xCzOWATs8OEhbi_kzwSOj67CDaFdxP5Ikx5ePh-bB-7zxM2FPCifNLmW8ZfiLPaPLz4fvh7HATkYlTNqEIPb3-hQwU8Yw0rY3gE_aiUbs_V615Dkk0sjQjlRJSiKOQnVqb7dY-N6o5B9_uehwcmkTBN51Q21X4DZzbT2BLlRMxk0Oh2iIh26pE-A0"

    test "байты кода не менялись" do
      assert Encoding.encode(with_every_pseudo_slot()) == @golden_pseudo_slots
    end

    test "золотой код открывается тем же билдом, целиком и без потерь" do
      assert {:ok, %{build: back, dropped: []}} = Encoding.decode(@golden_pseudo_slots)
      assert back == with_every_pseudo_slot()
    end

    test "круг закрывается: decode(golden) |> encode == golden" do
      assert {:ok, %{build: back}} = Encoding.decode(@golden_pseudo_slots)
      assert Encoding.encode(back) == @golden_pseudo_slots
    end

    # ⚠️ Контроль к трём assert'ам выше: строка правда несёт все псевдо-слоты.
    # Без него фикстура могла бы протухнуть незаметно — потеряй билд половину
    # полей, тесты остались бы зелёными, просто проверяли бы меньше.
    test "в золотом коде лежат строки всех псевдо-слотов" do
      payload =
        @golden_pseudo_slots
        |> String.replace_prefix("2.", "")
        |> Base.url_decode64!(padding: false)
        |> :zlib.unzip()
      ...
      for key <- [
        ...
        assert String.contains?(payload, key), "в золотом коде нет строки #{key}"
      end
    end
  end

  describe "новое поле %Gear{} нельзя недовезти молча (задача 3.222)" do
    # Не-дефолтный образец на КАЖДОЕ поле `%Gear{}`. Значение — поля набора,
    # которые надо заполнить, чтобы у этого поля был смысл: у числа оружия
    # это само оружие (строку в ссылку пишет только пара — без предмета
    # числу негде ехать, и это не пробел кодека, а его правило).
    @gear_samples %{
      abilities: [abilities: %{str: 6}],
      ac: [ac: %{armor: 8}],
      worn: [worn: %{armor: :full_plate}],
      saves: [saves: 12],
      saves_specific: [saves_specific: %{ref: 4}],
      skills: [skills: %{discipline: 50}],
      feats: [feats: [{:skill_focus, :discipline}]],
      weapon: [weapon: :scimitar],
      weapon_attack: [weapon: :scimitar, weapon_attack: 5],
      off_hand_weapon: [off_hand_weapon: :dagger],
      off_hand_weapon_attack: [off_hand_weapon: :dagger, off_hand_weapon_attack: 3],
      mini_sets: [mini_sets: [2, 3]],
      named_items: [named_items: 4],
      resistances: [resistances: %{fire: 15}]
    }

    # 🔴 Смысл сторожа: поле экипировки заводится в `Rules.Gear`, а доехать
    # обязано ещё в двух местах — в кодек (иначе оно не переживает ссылку,
    # то есть в v1 просто не сохраняется) и в `Gear.any?/1` (иначе набор,
    # где введено только оно, называет себя пустым). Оба промаха молчаливые.
    test "у каждого поля есть образец — иначе таблица протухла" do
      fields = %Gear{} |> Map.from_struct() |> Map.keys() |> MapSet.new()
      samples = @gear_samples |> Map.keys() |> MapSet.new()

      assert MapSet.difference(fields, samples) |> MapSet.to_list() == [],
             "новое поле Gear: добавь образец в @gear_samples, кодек в Encoding " <>
               "и условие в Gear.any?/1"

      assert MapSet.difference(samples, fields) |> MapSet.to_list() == [],
             "в @gear_samples остался образец поля, которого у Gear больше нет"
    end

    test "каждое поле переживает ссылку и включает Gear.any?/1" do
      for {field, fields} <- @gear_samples do
        gear = Gear.new(fields)

        assert Map.fetch!(gear, field) != Map.fetch!(%Gear{}, field),
               "образец поля #{field} совпал с умолчанием — он ничего не проверяет"

        assert Gear.any?(gear),
               "поле #{field} заполнено, а Gear.any?/1 говорит «игрок ничего не вводил»"

        %Build{} = base = sample()
        build = %Build{base | gear: gear}

        assert {:ok, %{build: back, dropped: []}} = Encoding.decode(Encoding.encode(build))

        assert Map.fetch!(back.gear, field) == Map.fetch!(gear, field),
               "поле #{field} не пережило круг через ссылку — кодек о нём не знает"
