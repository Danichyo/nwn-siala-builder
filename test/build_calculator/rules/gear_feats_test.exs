defmodule BuildCalculator.Rules.GearFeatsTest do
  ...
  describe "фит с вещи не запрещает взять его же слотом" do
    ...
    test "Great strength: слот и вещь складываются, кап 10 стоит на сумме", %{ruleset: ruleset} do
      ...
      # 🔴 И ровно тот пример Dan, ради которого заведена задача 3.204 («4 фитами
      # и ещё 2 с вещи»): две записи с вещей — ДВА взятия, а не одно. Здесь
      # стояло обратное («объявить дважды нельзя, и объявленный считается ОДНИМ
      # взятием»), и это была цена прежнего решения; слово Dan 13.09.2026 её
      # сняло: «если кто-то себе введет х10 Great strength, то нам надо это
      # показать в статах. Ведь мы уже делаем это, если взять great ... при
      # лвл апе».
      twice = with_feats(four, [:great_strength, :great_strength])
      assert Build.feat_takes_owned(twice, ruleset, :great_strength, 41) == 6
      assert str.(twice) == str.(four) + 2

      # И потолок по-прежнему на СУММЕ: десять слотовых плюс две записи с вещей —
      # двенадцать взятий и всё те же +10, а не +12.
      assert Build.feat_takes_owned(
               with_feats(ten, [:great_strength, :great_strength]),
               ruleset,
               :great_strength,
               41
             ) == 12

      assert str.(with_feats(ten, [:great_strength, :great_strength])) == str.(ten)
    end

    # 🔴 Потолок ВЗЯТИЙ у вещей считает вещи — симметрично слотам (замер Dan
    # 14.08.2026, H8: слотовый потолок считает слоты). Десять объявленных
    # записей исчерпывают его, одиннадцатая отказана; фит, у которого потолка
    # в данных нет вовсе, не отказывается никогда — выводить «10» из «200 HP»
    # значило бы считать за источник.
    test "потолок объявлений: одиннадцатая запись отказана, десятая нет", %{ruleset: ruleset} do
      base = build(List.duplicate(:fighter, 41))
      nine = with_feats(base, List.duplicate(:epic_toughness, 9))
      ten = with_feats(base, List.duplicate(:epic_toughness, 10))

      assert GearFeats.add_reasons(nine.gear, ruleset, :epic_toughness) == []

      assert GearFeats.add_reasons(ten.gear, ruleset, :epic_toughness) ==
               [{:max_takes, :epic_toughness, 10}]

      # Неповторяемый — своя причина, и она про другое: объявлять второй раз
      # нечего, а не «слишком много раз».
      assert GearFeats.add_reasons(with_feats(base, [:alertness]).gear, ruleset, :alertness) ==
               [{:already_taken, :alertness}]

      # ⚠️ Потолка в данных нет — отказа нет. `Self concealment` на ванили:
      # `max_takes` там не назван никем, и билд говорит об этом гэпом
      # `{:missing_data, {:feat_max_takes, …}}`, а не отказом наугад.
      vanilla = Data.ruleset!("vanilla")
      many = with_feats(base, List.duplicate(:self_concealment, 12))
      assert GearFeats.max_takes(:self_concealment, vanilla) == nil
      assert GearFeats.add_reasons(many.gear, vanilla, :self_concealment) == []
      assert GearFeats.max_takes(:self_concealment, ruleset) == 5
    end
  ...
  describe "оговорка про значение спрашивает получателя" do
    ...
    # Живая перепись, и она же — ответ на «сколько осталось». Семь из
    # пятнадцати (было пять до задачи 3.209), и все семь стоят числа:
    #
    ...
    #   * `Resist energy` +5 и `Epic energy resistance` +10/15 за взятие —
    #     поглощение стихийного урона (`vanilla/feat_resistance_bonuses.json`,
    #     задача 3.209) — у него теперь свой получатель `damage_resistance`
    #     (`our`), а не общая метка `immunities`.
    test "на живых данных остаются ровно семь", %{ruleset: ruleset} do
      surviving =
