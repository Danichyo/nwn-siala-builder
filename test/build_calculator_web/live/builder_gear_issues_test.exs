defmodule BuildCalculatorWeb.BuilderGearIssuesTest do
  ...
  describe "нет проблем — блока нет вовсе (задача 3.88: тот же приём ворот)" do
    ...
    # Задача 3.200 (репорт Dan 12.09.2026: первое «+ Добавить набор» скроллило
    # панель «Итого» к верху). Блок появляется впервые ровно на действии, которое
    # его вызывает, и вставка НОВОГО узла между детьми `#totals-panel` заставляла
    # morphdom снять и заново вставить всех соседей ниже, включая `#gear-panel`;
    # кнопка теряла фокус, панель прыгала (`scrollTop` 1105 → 588, CDP, 1440×900).
    # Слот `#gear-issues-slot` существует ВСЕГДА, чтобы вставка шла внутрь него;
    # ворота `count > 0` остались на самом `#gear-issues`.
    test "слот под сводку есть всегда, сама сводка — только с проблемами", %{
      conn: conn,
      ruleset: ruleset
    } do
      build = dual_wield_build(ruleset, gear: Gear.new(weapon: :katana))
      ...
      assert has_element?(view, "#totals-panel > #gear-issues-slot")
      refute has_element?(view, "#gear-issues-slot #gear-issues")
      ...
      {:ok, view, _html} = open(conn, with_issue)
      assert has_element?(view, "#gear-issues-slot > #gear-issues")
    end
  ...
  describe "третий список — не дубль двух существующих (задача 3.133, граница 3)" do
    test "«Проблемы в экипировке», «N пробелов в этом билде» и данные — три разных числа", %{
      conn: conn,
      ruleset: ruleset
    } do
