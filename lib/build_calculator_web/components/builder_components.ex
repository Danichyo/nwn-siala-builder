defmodule BuildCalculatorWeb.BuilderComponents do
  ...
  # Task 3.179's designer pass (download/copy icons): the only other `<.icon>`
  # call site in the whole app (`Layouts.theme_toggle/1`) gets it for free
  # from `use BuildCalculatorWeb, :html`'s `html_helpers`. This module never
  # pulled that in — it only takes `Phoenix.Component` + `Gettext` above — so
  # a plain, narrow import beats reaching for the whole `:html` block just for
  # one function.
  import BuildCalculatorWeb.CoreComponents, only: [icon: 1]
  ...
  *into* this box rather than gaining a second copy next to it: showing both
  ...
  у Fort/Ref/Will) — ВТОРОЙ, отдельно подписанный список под тем же
  заголовком: строки, которые НЕ входят в сумму, объясняющую значение
  рядом с триггером. Печатается под разделителем, только когда
  `extra_terms` не пуст, — героем панели остаётся арифметика `terms`,
  а не эта справка.
  """
  ...
  attr :extra_title, :string, default: nil, doc: "heading of the second, unsummed block"
  attr :extra_terms, :list, default: [], doc: "[%{label:, value:}], never summed into `terms`"
  attr :class, :any, default: nil
  slot :inner_block, required: true
  ...
  def stat_pop(assigns) do
    ...
    <span
      ...
      data-pop-extra-title={@extra_title}
      data-pop-extra-terms={Jason.encode!(@extra_terms)}
    ...
    <script :type={Phoenix.LiveView.ColocatedHook} name=".StatPop">
      ...
      // Задача 3.201: тот же отступ от края вьюпорта, что у `feat_info`
      // (константа с тем же именем и значением в этом же файле) — не тайное
      // совпадение чисел, а один и тот же зазор для одной и той же роли
      // «плавающая панель у самого края окна».
      const MARGIN = 12
      let panel = null
      ...
      function renderTerms(trigger) {
        ...
        for (const term of terms) {
          p.appendChild(termRow(term, "stat-pop-term"))
        }

        // Второй, ОТДЕЛЬНО подписанный блок (задача 3.197 часть B) — «против
        // заклинаний» у Fort/Ref/Will. Свой заголовок и свой класс строк
        // (`stat-pop-term-extra`), чтобы CSS мог отличить справку от
        // арифметики выше: сумма `terms` сходится со значением триггера,
        // сумма этого списка — нет и не обязана, поэтому смешивать их одним
        // классом значило бы стереть ровно ту границу, ради которой список
        // вообще завели отдельным (`Summary.vs_spells_terms/3`'s moduledoc).
        let extraTerms = []
        try { extraTerms = JSON.parse(trigger.dataset.popExtraTerms || "[]") } catch (_e) { extraTerms = [] }

        if (extraTerms.length > 0) {
          const extraTitle = document.createElement("p")
          extraTitle.className = "stat-pop-extra-title"
          extraTitle.textContent = trigger.dataset.popExtraTitle || ""
          p.appendChild(extraTitle)

          for (const term of extraTerms) {
            p.appendChild(termRow(term, "stat-pop-term stat-pop-term-extra"))
          }
        }
      }

      function termRow(term, className) {
      ...
      //
      // 🔴 Задача 3.201 (дизайнерский проход по 3.197 части B): до этой
      // правки тут стояло безусловное «вниз от триггера», без единой
      // проверки, что снизу вообще есть место, — и это уже было на грани
      // (пустая панель Fort без второго блока на 1440×800 выступала за низ
      // окна на 24px, замер положительным контролем: временно очищали
      // `data-pop-extra-terms` перед кликом на живом сервере). Второй блок
      // («против заклинаний», часть B) добавил панели заголовок и 1–2
      // строки — и то же самое место на том же экране стало вылезать
      // на 98px, у Will на 1280×800 — на 169px: высота росла, а верх
      // по-прежнему считался только от триггера, без оглядки на низ окна.
      // Развилка — та же самая, что уже решает эту задачу у `feat_info`
      // в этом файле (задача 3.94): снизу хватает места — вниз; не хватает
      // снизу, но хватает сверху — вверх; не влезает нигде — прижать
      // к верху и отдать остаток `max-height`/`overflow-y` в CSS
      // (`.stat-pop-panel`), а не рисковать очередным ростом контента.
      // Высота панели известна ТОЧНО на этом шаге: `renderTerms(trigger)`
      // в `open()` уже отработал до вызова `place()`.
  ...
  *different* controls that happen to also carry a search box:
  ...
  # ⚠️ Задача 3.205 (дизайнерский проход по 3.204 B): `row[:chosen_label]`
  # ПЕРЕБИВАЕТ `@chosen_label`, если ключ есть и не `nil`. До этой строки
  # компонент знал только ОДИН вид «уже отмечено» на весь список — а
  # стакающийся фит с вещи (`Epic toughness`…) уже второй, у которого клик
  # ДОБАВЛЯЕТ ещё одно взятие, а не снимает («уже надет» здесь не отказ,
  # см. `GearPanel.gear_feat_picks/1`). Голое «✓ надет» у обеих строк
  # выглядело ОДИНАКОВО, и игрок не видел разницы между «сними кликом»
  # и «клик добавит третье» — сцена (`builder_live.html.heex`, чипы слотов)
  # эту развилку уже решает словом («взят ×2 · ещё раз» вместо «✓ взят»),
  # здесь той же вилки не было вовсе. `Map.get/3` на плоской map, а не
  # `row.chosen_label` — большинство строк (оружие, нестакающиеся фиты)
  # этого ключа не несут совсем, и обращение к отсутствующему полю упало бы.
  #
  # ⚠️ Задача 3.226: видов «уже отмечено» стало ТРИ. Третий — фит, объявленный
  # только ЗНАЧЕНИЯМИ (`Skill focus (Discipline)`): клик по его строке ничего
  # не снимает и взятия не добавляет, а открывает второй шаг под ЕЩЁ ОДНО
  # значение. Правило у всех трёх одно и то же: `@chosen_label` компонента
  # верен ровно там, где клик СНИМАЕТ, и переопределяется там, где клик
  # что-то добавляет. Решает это по-прежнему звавший (`GearPanel`), не компонент.
    ...
    <div :if={@open?} id={@id <> "-body"}>
      ...
      <div class="gear-picks" id={@id <> "-options"} phx-update="stream">
        ...
        >
          ...
          <span :if={row.chosen?} class="gear-pick-on">{Map.get(row, :chosen_label) || @chosen_label}</span>
        </button>
      </div>
  ...
  @doc """
  "Download as .txt" for the build's own exported text (task 3.179, Dan:
  «тот же текст, но внутри файла»).

  One component for both screens, not a hook copied into each LiveView's
  own `.heex` — CLAUDE.md §9's own lesson (two copies of one rule have
  drifted apart in this project four times in three days). A
  `phx-hook=".Name"` resolves against the module whose *template* carries
  the attribute, not wherever the matching `<script>` happens to sit
  (`stat_pop/1`'s own moduledoc spells this out), so writing
  `phx-hook=".DownloadText"` here means the hook is always
  `BuildCalculatorWeb.BuilderComponents.DownloadText`, whichever LiveView
  renders the button. Both `BuilderLive` and `BuildViewLive` already
  `import BuildCalculatorWeb.BuilderComponents`.

  `target` is a CSS selector for whatever element already holds the export
  text — the constructor's dialog reads a `<pre>` (`.textContent`), the view
  screen reads a hidden `<textarea>` (`.value`) — the hook tries `.value`
  first and falls back to `.textContent`, so either shape works through the
  one hook without the caller saying which.

  `filename` is server-computed (`Export.file_name/3`) and travels as a
  plain attribute — LiveView diffs it like any other, so it is always
  current for whatever build is loaded; there is no second, client-side
  guess at what the file should be called.

  No `pushEvent` anywhere: the click only ever needs text already sitting on
  the page. Reaching for the server here would tag the button
  `data-phx-ref-src`, the same shape that has silently eaten a build's first
  click before (CLAUDE.md §6, bug 1.11) — and there is nothing on the server
  this click needs that is not already in the DOM.

  ⚠️ **No UTF-8 BOM, on purpose.** Notepad on Windows 10 1903+ detects
  BOM-less UTF-8 on its own, and a leading BOM byte would sit inside the
  Epic Character Builders posting format other people's parsers read byte
  for byte (`Export`'s own moduledoc) — not a corner this feature gets to
  round off quietly.

  ⚠️ **The hook stays dumb, on purpose — and has to stay that way.** It reads
  whatever `target` already holds and downloads exactly that: no text
  assembled in JS, no second `data-*` attribute it stitches onto the end.

  ⚠️ **That later request landed — task 3.180, still entirely server-side,
  exactly as predicted above.** Dan, 10.09.2026: «внутрь файла на самом деле
  добавить ссылку было бы неплохо… выводить ссылку в попапе смысла нет, она
  на этот момент доступна и так». `BuildCalculatorWeb.Builder.Export.file_text/5`
  builds the extra line (`text/4`'s own block plus one line naming the
  build's link — `text/4` itself untouched), and each caller points THIS
  component's `target=` at a second hidden node that holds it:
  `BuilderLive`'s `#export-file-text` (`#export-text`'s `<pre>` stays the
  popup, unlinked), `BuildViewLive`'s `#export-source` (repurposed from
  carrying `@export_text` to carrying the file text — freeing `copy_text/1`
  there to read the dialog's own always-in-DOM `#view-export-text` instead,
  so no third node had to be added). No `data-suffix`, no text assembled in
  JS — the hook itself was not touched by that task at all.

  The label sits in its own `<span class="btn-label">` rather than as the
  button's only text node. This hook never rewrites it (unlike
  `copy_text/1`'s "Скопировано" swap below) — the two buttons keep one shape
  on purpose, so a future icon (designer's own pass) can sit next to either
  label without special-casing one of them.

  ⚠️ **That future icon landed here — designer's pass, task 3.179's second
  half.** `hero-arrow-down-tray`, sitting BEFORE `.btn-label` as its own
  sibling, never inside it — the guard above is load-bearing, not decorative:
  `.CopyText`'s "Скопировано" swap and this hook's own re-render both only
  ever touch `.btn-label`'s text, so an icon parked inside that span would
  have vanished the first time either hook wrote to it.
  ⚠️ **Sized via `class="size-4"` (Tailwind, 16px), not a `-micro`/`-mini`
  suffix.** The heroicons plugin (`assets/vendor/heroicons.js`) bakes its own
  `width`/`height: 24px` into every bare `hero-*` class regardless of suffix
  — measured live on the one other icon call site in the app
  (`Layouts.theme_toggle/1`'s `hero-moon`, no suffix): computed size is
  `16px`, not `24px`, because `size-4` sits in the same compiled stylesheet
  and wins the tie. A `-micro` name would have cost nothing and changed
  nothing — the utility already decides. `size-4` is also `icon/1`'s own
  default, so passing it explicitly here is a note to the next reader, not
  a requirement.
  """
  attr :id, :string, required: true
  attr :target, :string, required: true, doc: "CSS selector for the element holding the text"
  attr :filename, :string, required: true
  attr :label, :string, default: nil
  attr :class, :string, default: "btn"

  def download_text(assigns) do
    ~H"""
    <button
      type="button"
      class={@class}
    ...
    >
      <.icon name="hero-arrow-down-tray" class="size-4 btn-icon" />
      <span class="btn-label">{@label || gettext("Download .txt")}</span>
    </button>
      ...
      export default {
        mounted() {
          this.el.addEventListener("click", () => {
            ...
            // Аттач до клика обязателен — Firefox исторически игнорирует
            // клик по элементу, не присоединённому к документу.
            const a = document.createElement("a")
            a.href = url
            a.download = this.el.dataset.filename
            document.body.appendChild(a)
            a.click()
            a.remove()

            // Отложенно, а не сразу: синхронный отзыв успевает отменить ещё
            // не начавшуюся загрузку — сама выгрузка блоба стартует поверх
            // клика, а не внутри его обработчика.
  ...
  @doc """
  "Copy to clipboard" for the build's own exported text — the same block
  `download_text/1` above downloads (task 3.179's coordinator follow-up,
  Dan: «Кнопку "Скопировать текст" добавить стоит»; the constructor's own
  export dialog had none at all).

  Replaces what would otherwise have been a THIRD near-identical hook: the
  view screen already had its own `.CopyText` (`#copy-text`, read
  `src.value`, `.select()` only on the Clipboard-API fallback), and giving
  the constructor's new copy button a second, separate copy of the same
  fifteen lines is exactly the shape CLAUDE.md §9 warns about — two copies
  of one rule drifting apart, in this project four times in three days. One
  hook, both screens, the identical `"value" in src ? … : .textContent`
  accessor `download_text/1` uses above, for the identical reason: the
  constructor's dialog holds the text in a `<pre>`, which has no `.value` at
  all.

  ⚠️ **`.CopyLink` (`#copy-link`, `#copy-short-link`,
  `builder_live.html.heex`) stays a separate hook — a judgment call, not an
  oversight.** It always targets a *visible* `<input readonly>` holding a
  share link and calls `.select()` unconditionally, before even trying the
  Clipboard API, so the click visibly highlights the exact link text being
  copied — a real, user-facing behaviour this component's fallback (see
  below) would quietly drop on the now-universal fast path where
  `navigator.clipboard.writeText` just succeeds. Folding it in here would
  either lose that highlight or grow this component a branch for a DOM
  shape (`<input>`) that never needed the `<pre>`/`<textarea>` accessor
  below in the first place — three call sites become two, not one, and
  `.CopyLink` keeps behaving exactly as it does today.

  The label sits in its own `<span class="btn-label">`, not as the button's
  only text node — `mounted()` below swaps the label to "Скопировано" and
  back on a timer, and `textContent =` on the button itself would silently
  wipe any icon a future pass (designer's own) puts next to it, permanently,
  the first time anyone clicks.

  ⚠️ **That icon landed here — designer's pass, task 3.179's second half.**
  `hero-document-duplicate`, a sibling BEFORE `.btn-label`, never inside it,
  for the exact reason spelled out two sentences up — this button rewrites
  `.btn-label`'s `textContent` on every click AND on the 1400ms revert, so
  anything living inside that span gets clobbered twice a cycle, not once.
  Same sizing note as `download_text/1` above: `class="size-4"`
  (Tailwind, 16px) beats the heroicons plugin's own baked-in 24px — measured
  on `Layouts.theme_toggle/1`'s `hero-moon`, the one other call site in the
  app, not assumed from the plugin's suffix table.

  ⚠️ **Two defects found by review before this ever shipped, both in the
  same fifteen lines, both fixed here:**

  1. **A second click inside the 1400ms window used to freeze the label on
     "Скопировано" forever.** The old code captured `was` from a `const` in
     the click closure — click two's own `was` reads whatever click one had
     already written ("Скопировано"), and click one's timer restores the
     true label at 1400ms only for click two's *later* timer to overwrite it
     right back to "Скопировано" at 1900ms, with nothing left to undo it.
     People click "copy" twice routinely, just to be sure. Fixed by moving
     `was`/the pending timer onto the hook instance instead of the closure:
     a click only captures `was` when no timer is currently pending
     (`this.restore === null`), and always `clearTimeout`s whatever was
     pending first — so however many times the button is clicked inside the
     window, exactly one timer survives and it restores the *original*
     label, not a mid-flight one. `destroyed()` clears it too, so a removed
     button never fires a stray write into a span nobody owns any more.
  2. 🔴 **The Clipboard-API fallback used to report "Скопировано" without
     copying anything.** `document.execCommand("copy")` copies whatever is
     currently *selected on the page* — calling it bare, the way the old
     code did for a `<pre>` source (which has no `.select()` to call first),
     copies nothing, or copies whatever text the player happened to have
     highlighted, and the label lied about it either way. The branch is not
     dead code: `navigator.clipboard` only exists in a secure context, and
     testing this project over LAN from a phone against a plain-`http`
     dev machine (task 3.32) hits exactly this path. Fixed by copying
     through a throwaway, off-screen `<textarea>` seeded with `text` — the
     same string the accessor above already read — rather than touching
     `src` at all: this works identically whatever shape `src` is, which is
     also why the `typeof src.select === "function"` branch is gone
     entirely rather than merely guarded.

  ⚠️ **What a LiveView patch does while "Скопировано" is showing, checked on
  request, not guessed:** this button's own `id` never changes and neither
  dialog it lives in ever conditionally removes it (`hidden` only toggles a
  CSS attribute), so a patch that touches `.btn-label`'s text — because
  something elsewhere on the page re-rendered — resets it to whatever the
  SERVER currently thinks the label is, without calling `mounted()` or
  `destroyed()` again. That is not a bug on its own (the label briefly
  becomes honest again, ahead of schedule); the failure mode would be the
  *pending timer* later overwriting that honest text with a stale
  `this.was` — but both call sites pass a fixed default label that never
  changes across a build's lifetime, so the server's mid-flight value and
  `this.was` are always the same string, and the "overwrite" is a no-op. A
  future caller with a `label` that changes over time would need to revisit
  this; today's two do not exist.
  """
  attr :id, :string, required: true
  attr :target, :string, required: true, doc: "CSS selector for the element holding the text"
  attr :label, :string, default: nil
  ...
  def copy_text(assigns) do
    ...
    >
      <.icon name="hero-document-duplicate" class="size-4 btn-icon" />
      <span class="btn-label">{@label || gettext("Copy text")}</span>
    </button>
    <script :type={Phoenix.LiveView.ColocatedHook} name=".CopyText">
      // `execCommand("copy")` copies whatever is CURRENTLY SELECTED on the
      // page — calling it bare copies nothing, or copies whatever the
      // player happened to have highlighted, either way silently. A `<pre>`
      // source has no `.select()` at all, so the old code could not even
      // select ITSELF here. Instead: seed a throwaway, off-screen
      // `<textarea>` with `text` (already read by the accessor above,
      // whatever shape `src` was) and select THAT — works identically for
      // every `src` shape, which is also why there is no branch left on
      // `src` here at all.
      ...
      export default {
        mounted() {
          // `null` — "no revert pending". Separate from "never clicked yet"
          // vs "already reverted" on purpose: both behave the same way —
          // the NEXT click has to capture whatever the label honestly says
          // right now, not something a previous click already overwrote.
          this.restore = null
          ...
          this.el.addEventListener("click", async () => {
            ...
            // Re-entrant, not a `const was` captured in the click closure:
            // a second click inside the 1400ms window used to read the
            // FIRST click's "Скопировано" as `was`, and the two timers'
            // writes raced — the label froze on "Скопировано" for good.
            // Capturing `was` only when no timer is pending, and clearing
            // whatever WAS pending before scheduling a new one, means
            // exactly one timer survives any number of clicks, and it
            // always restores the true original label.
            if (this.restore === null) this.was = label.textContent
            clearTimeout(this.restore)
            label.textContent = this.el.dataset.labelCopied
            this.restore = setTimeout(() => {
              label.textContent = this.was
              this.restore = null
            }, 1400)
          })
        },
  ...
  @doc """
  Одна зона отчёта «применено» у импорта `.билд+`: подпись зоны и её строка.

  Семь зон разом (`abilities`, `saves`, `ac`, `skills`, `feats`, `hands`,
  `resistances`) — форма у них одна буква в букву, и ровно поэтому починку
  разделителя между пунктами пришлось делать **дважды**: 3.214 поправила одну
  копию (фиты), 3.215 тем же днём — остальные шесть, по репорту Dan.

  ⚠️ **Три соседние зоны сюда НЕ идут, и это не недосмотр:** `worn`,
  `mini_sets` и `named_items` печатают не строку через запятую, а спан
  на строку — у доспеха ещё и с оговоркой `row.note` в `<em>`. Сводить их
  сюда значило бы завести флаг «а эта зона другая», то есть вернуть в общий
  компонент ровно то различие, ради снятия которого он и заводится (правило
  `pick_list/1`: если место вынуждено выключать больше, чем включает, —
  компонент ему не подходит).

  DOM повторяет прежнюю разметку байт в байт, включая оба id (`@id`
  и `@id <> "-list"`): по ним написаны ассерты
  `builder_game_log_import_test.exs`, и они этой правкой не трогались.
  """
  attr :id, :string, required: true
  attr :label, :string, required: true, doc: "«Характеристики», «AC по типам»…"

  attr :items, :list,
    required: true,
    doc: "уже готовые строки зоны из `GameLogImportPanel`; пустой список — блока нет вовсе"

  attr :between, :string,
    default: " ",
    doc: "что стоит между подписью и значением: пробел везде, «: » у рук"

  def applied_zone(assigns) do
    ~H"""
    <div :if={@items != []} id={@id}>
      <span class="eyebrow">{@label}</span>
      <span id={@id <> "-list"}>
        {BuildCalculatorWeb.Builder.GameLogImportPanel.applied_line(@items, between: @between)}
      </span>
    </div>
    """
  end
