defmodule BuildCalculatorWeb.Builder.GearFeatRowsTest do
  ...
  test "HP: пересечься не с чем, поля в «Вещах» нет", %{ruleset: ruleset} do
    ...
    # ⚠️ И шестой раз, 11.09.2026: задача 3.184 добавила МИНИ-СЕТЫ
    # (`:mini_sets`) — список счётчиков надетых кусков. Двойного счёта у них
    # нет и быть не может по построению: поле не приносит своего числа вовсе,
    # а умножает два уже посчитанных терма (расовый бонус и бонус за тип
    # оружия) СУММОЙ, одним слагаемым `mini_set_attack_bonus` рядом с ними
    # (`Rules.MiniSets`).
    # ⚠️ И седьмой раз, 11.09.2026: задача 3.186 добавила ИМЕННЫЕ И УНИКАЛЬНЫЕ
    # вещи (`:named_items`) — второй вход той же таблицы процентов к HP, что
    # и куски. 🔴 Вот у НЕГО пересечение с этим тестом прямое: поле двигает
    # ровно то число, которое здесь разбирается. Двойного счёта нет —
    # процент считается ОДИН раз, от итога `Progression.hit_points/3`, и стоит
    # в разборе отдельным термом рядом с `Toughness`, а не внутри него; кейс
    # про это — в `gear_hit_points_test.exs` («сумма термов разбора равна hp»).
    # ⚠️ И восьмой раз, 11.09.2026: задача 3.187 добавила РАЗДЕЛЬНЫЕ СПАСЫ
    # (`:saves_specific`) — прибавку к одному сейву рядом с прибавкой ко всем
    # трём. 🔴 Пересечение с этим тестом прямое: оба поля двигают одно и то же
    # число. Двойного счёта нет по построению — их складывает ОДНА функция
    # (`Rules.Gear.save_bonus/2`), и результат идёт в клип потолка ОДНИМ
    # слагаемым, а не двумя; кейс про это — в `gear_specific_saves_test.exs`
    # («universal идёт во все три, раздельная — только в свой» и «universal
    # и раздельная режутся вместе, а не по половинке»). ⚠️ У разбора при этом
    # есть своя ловушка: печатать надо `stats.gear_save_bonuses[save]`
    # (universal + своя), а `stats.gear_save_bonus` — это по-прежнему только
    # то, что вписано в поле «все три».
    # ⚠️ И девятый раз, 18.09.2026: задача 3.210 добавила ПОГЛОЩЕНИЕ СТИХИЙНОГО
    # урона (`:resistances`) — по числу на стихию. 🔴 Пересечение с фитами
    # прямое, и оно единственное в своём роде во всём блоке «Вещи»: вписанное
    # число не ПРИБАВЛЯЕТСЯ к прибавке фита, а встречается с эффектом расы
    # и топоров и **конкурирует** с ним (побеждает наибольшее), тогда как фит
    # складывается с победителем. Двойного счёта поэтому нет по построению:
    # три слагаемых сводятся ОДНОЙ функцией (`Rules.Resistances.of/3`), и она же
    # называет, какое из двух сравниваемых не вошло (`superseded`). Кейсы —
    # в `resistances_test.exs`; живая сверка с игрой — Хнюпиус, огонь 45 при
    # вещи 15 (замер `AV1`).
