defmodule Mix.Tasks.Wiki.Fetch do
  ...
  @fandom_pages [
    "Ability score",
    # Задача 3.187. Единственная страница, называющая ТИП бонуса к классу брони
    # по СЛОТУ предмета («The type of bonus given by a particular item property
    # depends on the type of item it is on»), — без неё импорт экипировки
    # из лога `.билд+` не знает, в какой из пяти типов положить `AC Bonus`
    # с шлема, сапог или амулета.
    "Armor class",
    "Attacks per round",
    "Base attack",
    "Base attack bonus",
    ...
    # Задача 3.209. Единственная страница, называющая правило складывания
    # НЕФИТОВЫХ источников поглощения стихийного урона друг с другом
    # («only the highest resistance granted by a spell or (equipped) item is
    # used») — без неё «резист от вещи против резиста от расы/оружия» стоял бы
    # на памяти координатора, а не на цитате.
    "Damage resistance"
  ]
