  ...
  **требование по характеристике сравнивается именно с ним** — «статы с вещей
  ...
  Fields worth spelling out:
    ...
    * `fort` / `ref` / `will` — base save + governing modifier +
      ...
      🔴 **Это ЛИСТ ПЕРСОНАЖА, и с 12.09.2026 в нём нет условных прибавок**
      (задача 3.197). Замер `AQ1` (Dan): «спасы от Spellcraft в листе персонажа
      не видна» — прибавка +1 за каждые 5 рангов работает **против
      заклинаний**, и базовые Fort/Refl/Will игра считает без неё. У Брунны
      (42 ранга) лист игры печатает 50/22/28, а модель до этой правки давала
      51/30/36. Число было верным, неверно было то, **о чём** оно: два разных
      утверждения стояли под одним именем
    * `saves_conditional` — `%{fort:, ref:, will:}`, те же три сейва **при
      выполненном условии**: лист плюс всё условное, после потолка. У билда без
      условных прибавок совпадает с `fort`/`ref`/`will` точка в точку
    * `conditional_save_bonus` — `%{fort:, ref:, will:}`, сама прибавка, то есть
      разность двух чисел выше. Отдельным полем, чтобы интерфейсу не пришлось
      вычитать одно из другого в панели, в разборе, в экспорте и на экране
      просмотра порознь.

      🔴 **Потолок объясняет, почему она бывает НУЛЁМ при полных рангах.**
      Прибавка объявлена внутрикапной (`overrides.json` →
      `skill_save_bonus.rules[].counts_toward_cap`, слово Dan плюс строка
      Fandom), поэтому считается она так: `clip(всё внутрикапное вместе с ней)
      − clip(всё внутрикапное без неё)`. У Брунны вещевой пул Стойкости 19, и
      восемь рангов дают ей **+1**; у Мокси вещевые спасы 22 — пул упёрт
      в потолок и без прибавки, и она даёт **0**, ровно как её лист.
      ⚠ Само чтение капа **принятое, а не измеренное** — в игре «сколько
      выходит против заклинаний» не печатается нигде; разбор и отвергнутые
      чтения лежат в данных (`in_character_sheet.cap_reading`)
    * `conditional_save_terms` — чем условие названо: `[%{skill:, scope:,
      bonus:}]`, `scope` — строка источника (`"spells"`), `bonus` — до потолка.
      Пустой список значит «условных прибавок у билда нет», и строке
    ...
    * `save_bonus` — `%{fort:, ref:, will:}`, everything that adds to that
      **one** save, after the ruleset's ceiling — what the player typed under
      "Вещи", the Spellcraft rule, and the build's own feats/class
    ...
    * `gear_save_bonus` / `skill_save_bonus` — those two terms as they stand
      before the ceiling, still scalars: `gear_save_bonus` is what the player
      typed in the **universal** box (all three saves at once) and
      `skill_save_bonus` is Spellcraft's rule, which is save-agnostic too. The
      Spellcraft one exists because players do not count it (CLAUDE.md §3).
      ⚠ **Оно считается от ЗНАЧЕНИЯ навыка, а не от рангов** (задача 3.202,
      `fandom:Spellcraft`: «based on fully modified ranks»), поэтому подпись
      рядом с ним обязана называть значение: «42 ранга» и «+12» на пять
      не делятся
    * `gear_save_bonuses` — `%{fort:, ref:, will:}`, what equipment gives **that
      one save**: the universal line plus the save's own (task 3.187,
      `Rules.Gear.save_bonus/2`). Also before the ceiling.

      🔴 **A breakdown must read THIS one, not `gear_save_bonus`.** The two
      differ exactly when an item carries `Saving Throw Bonus: Specific
      (Fortitude)`, and printing the universal number under the caption «вещи»
      while the total carries both is the one thing a breakdown may not do —
      say something that does not add up to the value beside it. They are two
      fields rather than one because they answer two questions: «сколько игрок
      вписал в поле „все три“» and «сколько вещи дают Стойкости».

      ⚠ Today the two agree on every build that exists: nothing fills
      `Gear.saves_specific` yet — the input box and the URL code arrive with
      part B of task 3.187 — so the web layer still prints `gear_save_bonus`
      and is right to the point. It стоит переключить на эту мапу тем же
      коммитом, что заводит поле ввода
    ...
    * `resistances` — поглощение СТИХИЙНОГО урона, по стихии, с разбором
      (задача 3.210, `BuildCalculator.Rules.Resistances.entry/0`): прибавка
      фитов, эффект расы и топоров после усиления кусками, вписанное с вещей
      число и итог `фиты + max(эффект, вещь)`. ⚠ **Мапа, в которой есть только
      стихии с ненулевым источником** — не семь нулей: «здесь ничего»
      и «вообще ничего» разные утверждения, и пустая мапа значит «поглощения
      нет вовсе», что у этого стата полный ответ, а не молчание (источников
      ровно четыре, и все четыре известны). Порядок печати задаёт
      `ruleset.resistance_energy_types`, а не эта мапа.
      🔴 **Пары «голым / в экипировке» здесь НЕТ, и это не пропуск:** вещь тут
      не прибавляет, а **конкурирует** с эффектом расы, и «поглощение голым»
      значило бы «наибольшее из эффекта и нуля» — то же число, что `effect`,
      которое уже лежит в разборе отдельным полем
    ...
    * `hp` — hit points **in the equipment**, which is the number everything
      already reads: the constitution is the final score, feats off items count,
      and since task 3.186 the shard's percentage for what is worn sits on top
      (mini-set pieces and named items, up to +105 % — `Rules.GearHitPoints`)
    * `hp_naked` — the same character with **no gear at all**, computed by a
      second pass over an empty `%Gear{}` exactly like `ac_naked` and
      `saves_naked`, and for the same reason: "голым" has to mean one thing in
      all three. ⚠ **Not `hp` minus something.** Equipment reaches hit points by
      three independent roads — the constitution modifier (measured: +12 CON
      turned 760 into 1000), a feat off an item (`Toughness`), and the
      percentage above, which multiplies a total that already contains the other
      two. `nil` exactly when `hp` is
    * `hp_breakdown` — the same total as `hp`, regrouped by class rather than
      ...
      `gear_bonus` and `floor_adjustment`, sum to `hp` exactly — see
    ...
    * `mini_set_pieces` / `mini_sets` — the shard's mini-sets (task 3.184):
      `Nmini`, the number of **pieces** that count, and what they amplified,
      one entry per effect of the engine (`BuildCalculator.Rules.MiniSets`).
      ⚠ `Nmini` is a piece count, not a set count, and a lone piece counts for
      nothing at all — `[A,A,B]` is 2, `[A,B]` is 0. ⚠ `mini_set_pieces` is not
      zero just because `mini_sets` is empty: the pieces are on the character
      whether or not there was anything of ours for them to amplify
    * `mini_set_attack_bonus` — what they added to the attack roll, **before**
      the vanilla ceiling like the three terms above it. Its own line and not
      folded into `race_attack_bonus`/`weapon_type_attack_bonus`, because the
      amplification is computed off their **sum** (one effect in the engine) and
      attributing it to either one would be a lie in the breakdown. May be
      negative: the executor's own ceiling of 18 clips that same sum
  ...
  @typedoc """
  Условная прибавка к сейвам — одна запись на правило, которое её дало.

  `skill` — чей ранг (`:spellcraft`), `scope` — условие **строкой источника**
  (`"spells"`), `bonus` — сколько правило заработало **до** потолка. Перевода
  условия здесь нет и не будет: подпись «против заклинаний» — дело веб-слоя,
  ядро остаётся языконезависимым (CLAUDE.md §8).
  """
  @type conditional_save_term :: %{
          skill: atom(),
          scope: String.t() | nil,
          bonus: pos_integer()
        }
  ...
  @type t :: %__MODULE__{
          ...
          mini_set_pieces: non_neg_integer(),
          mini_sets: [MiniSets.receiver()],
          ...
          gear_save_bonuses: %{save() => integer()},
          ...
          ref: integer(),
          will: integer(),
          saves_conditional: %{save() => integer()},
          conditional_save_bonus: %{save() => integer()},
          conditional_save_terms: [conditional_save_term()],
          ...
          resistances: %{atom() => Resistances.entry()},
  ...
  defstruct character_level: 0,
            ...
            epic_save_bonus: 0,
            gear_save_bonus: 0,
            gear_save_bonuses: %{fort: 0, ref: 0, will: 0},
            skill_save_bonus: 0,
            ...
            ref: 0,
            will: 0,
            # Условной прибавки у пустого билда нет — значит итог «при
            # выполненном условии» равен листу, а сама прибавка ноль. Не `nil`:
            # «условных прибавок нет» — полный ответ, а не молчание.
            saves_conditional: %{fort: 0, ref: 0, will: 0},
            conditional_save_bonus: %{fort: 0, ref: 0, will: 0},
            ...
            # Пустая мапа — «поглощения стихийного урона нет», и это полный
            # ответ, а не молчание: источников ровно четыре (два фита, раса
            # Гнома, группа топоров), и у пустого билда не сработал ни один.
            # Стихии с нулём в мапе НЕ лежат — см. док выше.
            resistances: %{},
