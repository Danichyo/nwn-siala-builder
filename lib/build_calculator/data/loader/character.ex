defmodule BuildCalculator.Data.Loader.Character do
  ...
  # ⚠ `mini_sets` — четвёртый механизм оружейно-расовой системы (задача 3.184)
  # и единственный из всех, который НЕ ПРИНОСИТ своего бонуса, а увеличивает
  # чужой: `B += floor(B * Nmini / 10)` стоит внутри того же выражения
  # исполнителя, что и два усиливаемых терма. Сторона у него поэтому та же,
  # что у них, — но объявлена она всё равно ДАННЫМИ, а не выведена здесь:
  # «сторона усилителя равна стороне усиливаемого» звучит как правило и им
  # не является, а тихий разумный дефолт — ровно то, что запрещает §3.
  #
  # ⚠ Отдельным механизмом, а не хвостом `racial_bonus`/`weapon_bonus`, потому
  # что усиление считается от их СУММЫ (в движке это один эффект), и приписать
  # его одному из двух значило бы соврать в разборе — см. `Rules.MiniSets`.
  @cap_source_kinds ~w(gear gear_weapon mini_sets racial_bonus skill_rule weapon_bonus)
  ...
  # the 20.
  #
  # ⚠ `mini_sets` joined on 11.09.2026 with task 3.184 — the shard's mini-set
  # amplification of the two bonuses above it. It is on the **attack** and the
  # **skill** ceiling and on neither save one, because that is where the bonuses
  # it multiplies land; a save has no addend from this system at all.
  @cap_mechanisms %{
    attack_bonus: [:gear, :gear_weapon, :mini_sets, :racial_bonus, :weapon_bonus],
    saving_throw_bonus: [:gear, :skill_rule],
    skill_bonus: [:gear, :mini_sets, :racial_bonus, :weapon_bonus]
  }
