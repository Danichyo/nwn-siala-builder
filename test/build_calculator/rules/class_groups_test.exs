defmodule BuildCalculator.Rules.ClassGroupsTest do
  ...
  | Воины Адры | Fighter, Monk, Paladin, Champion of Torm | **есть**, `derived` (11.09.2026) | зелья Адры — **слово владельца**, не вики (Dan, 25.08.2026) | своей страницы нет; `Воин` 16725, `Монах` 17402, `Паладин` 19703, `Чемпион Торма` 16728 |
  ...
  страниц классов, а ни одна вики правила чистоты не формулирует.

  🔴 **С 11.09.2026 (задача 3.182) качеств ТРИ, а не два, и это главная правка
  этого файла.** Правило Адры назвали **серверные скрипты шарда**:
  `ClassNotRestrictTorm` — чей список классов ровно наш, 4 из 4 — это
  «a Torm-flavored variant» от `ClassNotRestrict`, а та «returns TRUE only when
  ALL levels are Barbarian + Fighter + WeaponMaster + DwarvenDefender»
  (`docs/research/2026-09-03-weapon-bonus-system.md`, `tob_check_inc.nss:206`).
  Значит правило **названо** (`purity_stated?` стал `true`) и **не
  процитировано** (`purity_status == "derived"`): состав функции цитата, форма
  предиката — перенос с родственной функции. Тесты обязаны различать именно это,
  потому что одним булевым `purity_stated?` Адра теперь неотличима от Сагры.
  ...
  ⚠️ **Здесь стояло: «Само допущение при этом на месте: `purity_required` у Адры
  по-прежнему `nil`… Записать `purity_required: true` было бы превращением
  допущения в прочитанный факт».** Верно до 11.09.2026 и неверно после: тогда
  источника не было вовсе, и `true` стояло бы на аналогии с Сагрой. Сегодня
  `purity_required` у Адры `true`, а «превращения вывода в цитату» не произошло —
  его держит `purity_status: "derived"` и подпись флажка, которая говорит
  «выведено из разбора серверных скриптов», а не молчит.

  ⚠️ **Решение владельца (`not_a_gap`) при этом никуда не делось и стало
  запасным:** оговорку гасит теперь более раннее условие «правило названо», а
  решение вернётся в работу, если `purity_required` когда-нибудь вернут в `nil`.
  ...
  describe "какие группы вообще есть" do
    ...
    # ⚠️ Главное различие данных, зафиксированное значением, а не словом
    # в комментарии: у Сагры правило чистоты ПРОЦИТИРОВАНО, у Адры ВЫВЕДЕНО.
    #
    # 🔴 Здесь стояло «у Адры его нет вовсе» и `purity_required == nil` —
    # устарело 11.09.2026 (задача 3.182). Само правило теперь одинаковое
    # у обеих, и различает их только провенанс: подменить одну проверку другой
    # значит перестать видеть разницу, ради которой обе и стоят.
    test "правило чистоты у Сагры процитировано, у Адры выведено", %{ruleset: ruleset} do
      by_id = Map.new(ClassGroups.all(ruleset), &{&1.id, &1})

      assert by_id[:sagra_warriors].purity_required == true
      assert by_id[:adra_warriors].purity_required == true

      assert by_id[:sagra_warriors].purity_status == "verified"
      assert by_id[:adra_warriors].purity_status == "derived"
    ...
    # у Сагры правило чистоты процитировано, поэтому решать нечего.
    #
    # 🔴 Здесь стояло «`purity_required` у Адры обязан остаться `nil`» —
    # пересмотрено 11.09.2026 (задача 3.182). Решение ОСТАЛОСЬ и стало
    # ЗАПАСНЫМ: оговорку гасит теперь первое условие `purity_gaps/2` («правило
    # названо»), а решение сработает снова, если значение вернут в `nil`.
    # Выбросить его значило бы потерять провенанс решения 25.08.2026 —
    # и уронить тест «решение про чистоту без довода роняет сборку» ниже.
  ...
  describe "честность про Адру" do
    # ⚠️ У Адры правило чистоты ВЫВЕДЕНО, а не процитировано, и это обязано быть
    # названо — иначе два разных по качеству факта («сагровик» прочитан
    # дословно, «адровец» перенесён с родственной функции скрипта) выглядят
    # на экране одинаково.
    #
    # 🔴 Здесь стояло `refute groups[:adra_warriors].purity_stated?` — с 3.182
    # булев флаг у обеих групп одинаковый, и различает их ТОЛЬКО
    # `purity_status`. Проверка, оставшаяся на одном булевом, зеленела бы
    # и у кода, который перестал различать цитату и вывод.
    test "флажок Адры помечен выводом, флажок Сагры — цитатой", %{ruleset: ruleset} do
      groups = Map.new(ClassGroups.of(build(List.duplicate(:fighter, 41)), ruleset), &{&1.id, &1})

      assert groups[:sagra_warriors].purity_stated?
      assert groups[:adra_warriors].purity_stated?

      assert groups[:sagra_warriors].purity_status == "verified"
      assert groups[:adra_warriors].purity_status == "derived"
  ...
  describe "загрузчик роняет сборку, когда три копии списка расходятся" do
    ...
    test "группа без записи в systems.json собирается из двух других копий", %{
      ...
      # 🔴 И вместе со значением копия отдаёт свой СТАТУС (задача 3.182):
      # у записи `races.json` это дословная цитата страницы «Воины Сагры»,
      # и без переноса статуса подпись флажка объявила бы её «не процитированной»
      # — то есть строгое умолчание сработало бы на настоящей цитате.
      assert group.purity_status == "verified"
