defmodule BuildCalculatorWeb.Builder.Summary do
  ...
  `extra_title` / `extra_terms` — задача 3.197 часть B, «против заклинаний»
  у Fort/Ref/Will и только у них (`nil` / `[]` у всех прочих карточек).
  🔴 **Не смешаны с `terms` НАМЕРЕННО:** сумма `terms` обязана сходиться
  со значением карточки, а условная прибавка Spellcraft в это значение
  не входит вовсе (замер `AQ1`) — второй список, а не два вида строк
  в одном, потому что смешение развязало бы «сумма терма» ровно там, где
  задача 3.197 её и завязала.

  `groups` — задача 3.216, и ровно у ОДНОЙ карточки (`resistance_section_card/2`):
  несколько стихий поглощения, сведённые в одну секцию «Резисты», а не семь
  карточек на стихию (было в 3.211/3.212). У карточки с `groups` `value` —
  пустая строка и `terms`/`from` — `nil` (общего итога у резистов нет,
  выдумывать агрегат нельзя), а разметка вместо одного `.v` печатает список
  пар «стихия + её итог» со СВОИМ разбором слагаемых у каждой — не второй вид
  строки внутри `terms`, а отдельное поле, потому что здесь несколько сумм
  на одной карточке, а не одна.
  """
  @type card :: %{
          key: String.t(),
          label: String.t(),
          ...
          extra_title: String.t() | nil,
          extra_terms: [%{label: String.t(), value: String.t()}],
          groups: [group()] | nil,
          hero?: boolean(),
          unknown?: boolean()
        }
  ...
  @typedoc """
  Один пункт `card().groups` (задача 3.216) — стихия с итогом и своим разбором.

  `terms` — тот же формат `%{label:, value:}`, что у `card().terms`; отдаёт его
  `resistance_terms/3` — только слагаемые, дающие итог, без проигравшей стороны
  сравнения эффект/вещь (см. её собственный moduledoc).
  """
  @type group :: %{
          id: atom(),
          label: String.t(),
          value: String.t(),
          terms: [%{label: String.t(), value: String.t()}]
        }
  ...
  def stat_cards(ruleset, stats) do
    ...
    # «Резисты» — В САМОМ КОНЦЕ сетки, после «Скилл-поинты» (задача 3.216,
    # слово Dan 18.09.2026: «в самый конец её за скилл поинты поставить»).
    # До этой задачи семь карточек резистов стояли между AC и BAB (3.211) —
    # ближе к остальной «живучести» персонажа, но и ближе к началу сетки,
    # где страница ещё короткая; теперь резисты — самая длинная секция
    # экрана (до семи стихий, у каждой свой разбор), и подвал ей подходит
    # больше, чем середина.
    [
      # Пара «HP голым / HP в экипировке» (задача 3.186) — тем же доводом, что
      # у AC два ряда ниже: мини-сеты и крафтовые вещи домножают итог до
      # +105 %, и одно число «HP» перестало отвечать сразу на «сколько без
      # вещей» и «сколько в бою». ⚠️ Ключ `"hp"` у карточки «в экипировке» не
      # тронут — это тот же ключ, что был здесь ДО задачи, названо только
      # точнее; `"hp_naked"` — новый, вторым проходом (`stats.hp_naked`,
      # `Rules.compute/2`), без собственного разбора: `hp_breakdown` ядро
      # считает только для «в экипировке», а второй, независимый расчёт
      # разбора здесь заводить незачем — «голое» число и так читается прямо.
      card("hp_naked", "HP голым", number(stats.hp_naked), nil),
      card("hp", "HP в экипировке", number(stats.hp), hp_card_terms(ruleset, stats)),
      ...
      [
        card(
          ...
          vs_spells_card_opts(ruleset, stats, :fort)
        ),
        card(
          "ref",
          "Ref",
          ...
          vs_spells_card_opts(ruleset, stats, :ref)
        ),
        card(
          "will",
          "Will",
          ...
          vs_spells_card_opts(ruleset, stats, :will)
        ),
        card(
          "skill_points",
          "Скилл-поинты",
      ...
      resistance_section_card(ruleset, stats)
  end
  ...
  defp build_card(key, label, value, from, terms, opts) do
    %{
      ...
      from: from,
      terms: terms,
      # Всегда `nil` здесь — «Резисты» (задача 3.216) единственная карточка
      # с `groups`, и она собирается своей отдельной функцией
      # (`resistance_section_card/2`), минуя `card/5`/`build_card/6` вовсе:
      # у неё нет ни `value`, ни суммы термов в обычном смысле этой карточки.
      # Ключ обязан стоять здесь у ВСЕХ карточек без исключения: heex
      # спрашивает `card.groups` у каждой без разбора, и голая карта без
      # ключа упала бы `KeyError`, а не отрисовала пустоту.
      groups: nil,
      # «Против заклинаний» — только у Fort/Ref/Will, задача 3.197 часть B.
      # `[]`, не `nil`, у прочих карточек: heex спрашивает `card.extra_terms
      # != []` без отдельной проверки на `nil` (тот же приём, что у `terms`
      # выше, но со своим дефолтом — здесь пустой список честен, а `nil`
      # был бы «неизвестно», которого для остальных карточек не бывает).
      extra_title: Keyword.get(opts, :extra_title),
      extra_terms: Keyword.get(opts, :extra_terms, []),
  ...
  #
  # Spellcraft по-прежнему save-agnostic (бьёт во все три сейва одинаково) —
  # в отличие от собственных термов билда (задача 1.12a), у каждого из
  # которых свой save, поэтому им нужен свой список, а не запись в этой паре.
  #
  # ⚠️ «Вещи» с задачи 3.187 (часть B) — БОЛЬШЕ НЕ save-agnostic: раздельные
  # спасы с экипировки (`Gear.saves_specific`) бьют в СВОЙ сейв, и Хнюпиус —
  # живой пример (`test/fixtures/game_logs_plus/hnyupius.log`, universal +16
  # и Fortitude +12 разом). Значение для строки «вещи» читается не полем
  # `:gear_save_bonus` (тот по-прежнему universal-only), а картой
  # `stats.gear_save_bonuses[save]` — `save_extras/2` ниже.
  ...
  # 🔴 **ЗДЕСЬ БЫЛА ВТОРАЯ СТРОКА — `{:skill_save_bonus, "Spellcraft",
  # :skill_rule}`, и задача 3.197 (часть A) её убрала.** Замер `AQ1` (Dan,
  # 12.09.2026: «спасы от Spellcraft в листе персонажа не видна») вынес эту
  # прибавку ИЗ самого спаса: ядро считает `fort`/`ref`/`will` без неё и отдаёт
  # её отдельно (`stats.conditional_save_bonus`, `saves_conditional`,
  # `conditional_save_terms`). Оставить её здесь значило бы печатать слагаемое,
  # которого в объясняемом числе нет, — единственное, чего разбор делать
  # не имеет права.
  #
  # 📋 **Часть B задачи 3.197 печатает её своей строкой «против заклинаний
  # +N»** — рядом с числом, а не внутри него. До неё прибавка видна игроку
  # только в панели «Вещи» (подпись про общий потолок) и нигде больше.
  @save_extras [
  ...
  def save_terms(ruleset, stats, save) do
    ...
    extras =
      for {value, label, source} <- save_extras(stats, save),
  ...
  def save_extras_terms(stats) do
    # `nil` — «вещи» здесь по-прежнему читает `gear_save_bonus`, СВОЙ старый
    # save-agnostic смысл (см. `save_extras/2`'s own note): группа не про один
    # сейв, и печатать в ней разошедшиеся по сейвам числа было бы неправдой
    # похуже, чем печатать только universal-часть.
    for {value, label, _source} <- save_extras(stats, nil),
  ...
  # спрашивает её сам.
  #
  # `save` — `nil` для группового note (universal-only, как было до 3.187),
  # или один из `:fort`/`:ref`/`:will` для разбора КОНКРЕТНОГО сейва, где
  # «вещи» обязаны включать и раздельную прибавку этого сейва, а не только
  # universal — иначе сумма термов под числом не сходится с самим числом
  # (`stats.save_bonus[save]`), как только у билда есть хоть одна раздельная
  # прибавка.
  defp save_extras(stats, save) do
    for {field, label, source} <- @save_extras,
        value = save_extra_value(stats, field, save),
        value != 0,
        do: {value, label, source}
  end

  defp save_extra_value(stats, :gear_save_bonus, save) when not is_nil(save) do
    stats |> Map.get(:gear_save_bonuses, %{}) |> Map.get(save, 0) |> Kernel.||(0)
  end

  defp save_extra_value(stats, field, _save), do: extra_value(stats, field, 0)
  ...
  # `opts` для `card/5` — подраздел «против заклинаний» этого спаса (задача
  # 3.197 часть B), только когда билду есть что в нём сказать: пустой список
  # у `Keyword.get(opts, :extra_terms, [])` в `build_card/5` и без явного
  # `[]` здесь означал бы то же самое, но заголовок («против заклинаний»)
  # печатался бы и без единой строки под ним — heex спрашивает `extra_terms`,
  # а не заголовок, поэтому оба поля гасятся вместе.
  defp vs_spells_card_opts(ruleset, stats, save) do
    case vs_spells_terms(ruleset, stats, save) do
      [] -> []
      terms -> [extra_title: vs_spells_title(), extra_terms: terms]
    end
  end
  ...
  #
  # ⚠️ Четвёртая строка с 11.09.2026 (задача 3.185) — куски мини-сетов. Она
  # НЕ бонус сама по себе: мини-сеты усиливают две строки выше
  # (`B += floor(B · Nmini / 10)`), и прибавка считается от их СУММЫ, потому
  # что в движке это один эффект (`Rules.MiniSets`). Отдельным термом она стоит
  # именно поэтому — приписать её расе или типу оружия было бы неправдой
  # в разборе, а не сложить вовсе значило бы напечатать сумму термов, которая
  # НЕ РАВНА своему итогу: проверено прогоном 11.09.2026, у Светлого эльфа 40
  # с луком и четырьмя кусками разбор давал 46 при AB 50.
  #
  # ⚠️ Сторона капа читается у ядра тем же `Caps.covers_source?/3`, что
  # и у трёх соседей: усиление внутри ванильного +20, как и оба усиливаемых
  # терма (`stat_caps.attack_bonus.applies_to_sources.mini_sets` в данных).
  @ab_extras [
    {:gear_attack_bonus, "вещи", :gear},
    {:race_attack_bonus, "раса", :racial_bonus},
    {:weapon_type_attack_bonus, "тип оружия", :weapon_bonus},
    {:mini_set_attack_bonus, :mini_sets_name, :mini_sets}
  ]
    ...
    extras =
      own ++
        ...
        for {field, label, cap_source} <- @ab_extras,
            ...
            value != 0,
            do:
              {value, ab_extra_label(label),
               Caps.covers_source?(ruleset, :attack_bonus, cap_source)}
  ...
  # Подпись механизма — либо готовая строка (три старых терма: их русский текст
  # никогда и не шёл через `gettext`), либо атом, который разрешается ЗДЕСЬ,
  # в рантайме. Атом нужен ровно потому, что `@ab_extras` — модульный атрибут,
  # то есть вычисляется на компиляции: позови в нём `Labels.mini_sets_name/0`
  # — и перевод вмёрзнет в сборку, а не выберется по локали запроса.
  #
  # ⚠️ Имя берётся у `Labels`, а не пишется здесь второй раз: то же число
  # печатают разбор AB, разбор значения навыка и разбор AC в шмоте — три места
  # на одно слагаемое, и расходиться в подписи они не имеют права.
  defp ab_extra_label(:mini_sets_name), do: Labels.mini_sets_name()
  defp ab_extra_label(label) when is_binary(label), do: label
  ...
  # И третья сторона той же системы (задача 3.184): усиление мини-сетами.
  # ⚠️ Своя ветка, а не хвост общей, по той же причине, что у расы строкой
  # выше: `id` здесь не фит, и `feat_name/2` напечатал бы для него `mini_sets`
  # — неизвестный id этот справочник отдаёт как есть. ⚠️ Поля ввода у мини-сетов
  # ещё нет (это задача 3.185), а строка разбора нужна уже сейчас: билд,
  # открытый по ссылке с кусками, обязан объяснить, откуда взялся лишний AC.
  defp own_ac_label(_ruleset, %{source: {:mini_sets, _id}}), do: Labels.mini_sets_name()
  ...
  **geared** ability scores rather than reused from the naked list: `Monk AC
  ...
  ⚠️ **Mini-sets and named items are counted since task 3.186** — up to
  +105 %, with a hard break to +0 % past the tenth item
  (`BuildCalculator.Rules.GearHitPoints`). `gear_term/2` prints the term next
  to `Toughness` and «Дух Сиалы», not inside either, and names the break-off
  out loud when it fires (see its own doc). Comparing this breakdown's total
  against a wiki build's HP still only ever works against the "голым" number
  (`stats.hp_naked`) — the wiki's own numbers already include mini-sets, and
  a shared build rarely carries the same pieces this tool was told about.
  ...
  def hp_terms(ruleset, %{hp_breakdown: breakdown} = stats) do
    ...
    by_class ++
      ...
      feat_terms(ruleset, breakdown) ++
      gear_term(ruleset, breakdown.gear_bonus) ++ floor_term(breakdown.floor_adjustment)
  end

  # Мини-сеты и крафтовые (именные) вещи (задача 3.186) — рядом с `Toughness`
  # и «Дух Сиалы», НЕ внутри них: это третий независимый источник прибавки
  # к HP, и CLAUDE.md §6 требует называть его отдельной строкой, а не топить
  # в чужом слагаемом.
  #
  # ⚠️ `nil` у `gear_bonus` — билд без единого куска и без единой именной
  # вещи (`Rules.GearHitPoints.term/3`: `Ctotal == 0`), и здесь это ЗАКОННОЕ
  # «строки нет», как у `floor_term(0)`: печатать «+0 мини-сетов» о билде,
  # который их не носит, было бы шумом на каждой карточке HP в приложении.
  #
  # 🔴 При обрыве (`cut_off?`) строка ОСТАЁТСЯ и называет причину нуля —
  # молчание здесь выглядело бы как потерянные сотни HP, а не как правило
  # игры («на одиннадцатой вещи прибавка обрывается в ноль», не опечатка
  # источника). `signed(0)` — не «терма нет», а «терм есть и равен нулю»:
  # разбор обязан сходиться со своим итогом (CLAUDE.md §6), и без явного нуля
  # сумма термов не сошлась бы с `hp`, который этот же ноль уже учёл.
  defp gear_term(_ruleset, nil), do: []

  defp gear_term(ruleset, %{cut_off?: true, count: count}) do
    [
      %{
        label: Labels.mini_set_hp_bonus_cut_off(count, GearHitPoints.cut_off_at(ruleset)),
        value: signed(0)
      }
    ]
  end

  defp gear_term(_ruleset, %{count: count, percent: percent, amount: amount}),
    do: [%{label: Labels.mini_set_hp_bonus(count, percent), value: signed(amount)}]
  ...
  @doc """
  Отфильтрованные записи `stats.resistances`, ГОТОВЫЕ к показу — по стихии,
  в порядке `ruleset.resistance_energy_types`, только с ненулевым итогом.

  Общий источник для `resistance_section_card/2` ниже (экран просмотра) и
  `BuildCalculatorWeb.Builder.TotalsPanel`'s `resistance_rows_with_ghost/3`
  (конструктор): фильтр и порядок посчитаны здесь ОДИН раз, а не дважды —
  два независимых прохода по одному и тому же условию рано или поздно
  разойдутся, ровно та ловушка, о которой предупреждает `terms_caption/1`
  ниже.

  `stats.resistances` несёт только стихии, у которых что-то есть
  (`Rules.Resistances.of/3`'s own moduledoc, «стихия без единого источника
  ОТСУТСТВУЕТ»), но не обещает порядка печати — он читается из ruleset'а,
  чтобы два билда с разным набором стихий показывали их в одном порядке.
  """
  @spec resistance_entries(map(), map()) :: [%{id: atom(), en: String.t(), entry: map()}]
  def resistance_entries(ruleset, stats) do
    for %{id: id, en: en} <- Map.get(ruleset, :resistance_energy_types) || [],
        entry = Map.get(stats.resistances, id),
        not is_nil(entry),
        entry.counted != 0 do
      %{id: id, en: en, entry: entry}
    end
  end

  @doc """
  Поглощение стихийного урона — ОДНА карточка «Резисты» на ВСЕ стихии сразу
  (задача 3.216; было семь отдельных карточек, по одной на стихию, — 3.211).

  Слово Dan 18.09.2026, которым секция и переписана: «все резисты в одну
  секцию надо объединить… слагаемые было бы здорово получить, давай оставим
  их». То есть меняется КОМПОНОВКА (семь карточек → одна с внутренним
  списком), а не содержание: разбор у каждой стихии остаётся — не прячем
  его, как одно время предлагала более ранняя редакция этой же задачи.

  Пустой список, если у билда вообще нет поглощения (`resistance_entries/2`
  дал `[]`) — секции на экране тогда нет вовсе, а не карточка-заглушка.

  `card.value` — пустая строка, `card.terms`/`card.from` — `nil`: общего
  итога у резистов нет (никто не складывает поглощение семи разных стихий
  в одно число), сумма живёт ВНУТРИ каждой группы `card.groups[].value`.
  """
  @spec resistance_section_card(map(), map()) :: [card()]
  def resistance_section_card(ruleset, stats) do
    case resistance_entries(ruleset, stats) do
      [] ->
        []

      entries ->
        groups =
          for %{id: id, en: en, entry: entry} <- entries do
            %{
              id: id,
              label: en,
              value: number(entry.counted),
              terms: resistance_terms(ruleset, stats, entry)
            }
          end
        ...
        [
          %{
            key: "resistances",
            label: Labels.resistances_zone_title(),
            value: "",
            from: nil,
            terms: nil,
            extra_title: nil,
  ...
  @doc """
  Разбор ОДНОЙ стихии по слагаемым, ДАЮЩИМ её итог, — общий список конструктора
  (поп-ап `.StatPop`, `#totals-resistances`) и экрана просмотра
  (`resistance_section_card/2`): оба читают его как есть, и его сумма обязана
  сходиться со значением стихии (правило `card()`).

  Три возможных слагаемых, ровно в порядке движка (Сиала, `Rules.Resistances`
  moduledoc, «фит СКЛАДЫВАЕТСЯ… эффект расы/оружия КОНКУРИРУЕТ с вещью»):

    * фиты (`Resist energy`, `Epic energy resistance`) — по записи, с `×N`
      взятий и `(потолок)` фита, той же грамматикой, что у `spell_resistance_terms/2`;
    * эффект расы/оружия/мини-сетов — ОДНИМ термом, потому что в движке это
      один эффект (`Rules.MiniSets`'s own moduledoc: «эффект один на все семь
      стихий, и считается он один раз»), сложенный ядром ДО усиления
      кусками — разбить его обратно на «расу» и «тип оружия» здесь означало
      бы второе, независимое описание той же суммы (см. `terms_caption/1`'s
      own warning). Состав термина читаем оттуда же, откуда он собран:
      `stats.racial_bonus`/`stats.weapon_type_bonuses` для ИМЕНИ вклада,
      `entry.effect_term` — для самого ЧИСЛА;
    * вещь — что вписано в «Вещах» по этой стихии.

  ⚠️ **Проигравшей стороны сравнения эффект/вещь здесь НЕТ** — она в итог
  не входит и живёт своим списком, `resistance_lost_terms/3`. История в три
  шага: 3.211 печатала её в ЭТОМ списке с суффиксом «— не вошла: меньше
  эффекта»; 3.216 сняла её с экрана просмотра (Dan: «не вошедшее я бы убрал,
  чтобы не загромождать UI»); 3.218 вынесла и из основного списка поп-апа
  (Dan 18.09.2026, скриншот `Fire 45` с тремя строками `+15 / +30 / +15`:
  «вместо прямого перечисления сделаем отступ и заголовок, чтобы было понятно,
  что дальше перечислено то что не вошло»). Довод сильнее вкуса: под «45»
  стояли три числа, дающие в сумме 60, — разбор спорил со своим итогом.
  """
  @spec resistance_terms(map(), map(), map()) :: [map()]
  def resistance_terms(ruleset, stats, entry) do
    resistance_feat_terms(ruleset, entry.feat_terms) ++
      resistance_effect_terms(ruleset, stats, entry) ++
      resistance_gear_term(entry)
  end

  @doc """
  Проигравшая сторона сравнения эффект/вещь (`entry.superseded`) — ОТДЕЛЬНЫМ
  списком, для второго, не суммируемого блока поп-апа (`stat_pop/1`'s
  `extra_terms`, тот же механизм, каким у спасов печатается «против
  заклинаний», задача 3.197 часть B). Задача 3.218.

  Симметрично для обеих сторон: вещь меньше эффекта → `вещь`; эффект меньше
  вещи → тот же ярлык эффекта, каким он назывался бы в основном списке
  (`resistance_effect_label/3`). Причину называет заголовок блока
  (`resistance_lost_title/0`), а не суффикс у каждой строки. Пустой список —
  сравнивать было нечего или некому проигрывать; блока тогда нет вовсе.

  ⚠️ Экран просмотра этот список НЕ печатает (решение Dan, 3.216) — зовёт его
  только панель итогов конструктора.
  """
  @spec resistance_lost_terms(map(), map(), map()) :: [map()]
  def resistance_lost_terms(_ruleset, _stats, %{superseded: :gear, gear: gear}) when gear != 0,
    do: [%{label: "вещь", value: signed(gear)}]

  def resistance_lost_terms(ruleset, stats, %{superseded: :effect, effect_term: %{} = effect}),
    do: [%{label: resistance_effect_label(ruleset, stats, effect), value: signed(effect.total)}]

  def resistance_lost_terms(_ruleset, _stats, _entry), do: []

  @doc """
  Заголовок блока `resistance_lost_terms/3` в поп-апе. Строчными, как
  «против заклинаний» у спасов: капитель даёт CSS (`.stat-pop-extra-title`).
  Называет и факт, и правило — эффект расы/оружия и вещь не складываются,
  засчитывается большее из двух (`Rules.Resistances`, замер `AV1`).
  """
  @spec resistance_lost_title() :: String.t()
  def resistance_lost_title, do: "не вошло — берётся большее"

  defp resistance_feat_terms(ruleset, terms) do
    for term <- terms do
      takes = if term.takes > 1, do: " ×#{term.takes}", else: ""
      cap = if term.capped?, do: " (потолок)", else: ""
      %{label: "#{Labels.feat_name(ruleset, term.id)}#{takes}#{cap}", value: signed(term.bonus)}
    end
  end

  # `nil` — ни раса, ни оружие не поглощают ничего у этого билда, и термина
  # об эффекте нет вовсе (не «эффект 0»): `Rules.MiniSets.damage_resistance/2`
  # отдаёт `[]`, когда обеим сторонам нечего сказать.
  defp resistance_effect_terms(_ruleset, _stats, %{effect_term: nil}), do: []

  # Эффект проиграл вещи — в итог не входит, значит и в этом списке его нет;
  # он назван в `resistance_lost_terms/3` (задача 3.218).
  defp resistance_effect_terms(_ruleset, _stats, %{superseded: :effect}), do: []

  defp resistance_effect_terms(ruleset, stats, %{effect_term: effect}),
    do: [%{label: resistance_effect_label(ruleset, stats, effect), value: signed(effect.total)}]

  # Ярлык эффекта — один на оба списка (основной и «не вошло»): одно число
  # не должно называться на экране двумя способами.
  defp resistance_effect_label(ruleset, stats, effect) do
    parts =
      [
        resistance_race_part(ruleset, stats),
        resistance_weapon_type_part(stats),
        resistance_mini_set_part(effect)
      ]
      |> Enum.reject(&is_nil/1)

    # Пустой список частей — билд, у которого сам эффект посчитан (значит
    # хоть кто-то дал ненулевой вклад), но чтение состава не сошлось ни
    # с одной из трёх причин (нулевой на грани округления). Своя фраза,
    # а не молчаливое склеивание "" + "" — так строка не выглядит забытой.
    label = if parts == [], do: "усиление шарда", else: Enum.join(parts, " + ")
    cap = if effect.capped?, do: " (потолок)", else: ""

    label <> cap
  end

  # Имя расы, только если раса этого билда действительно даёт
  # `:damage_resistance` (не любая раса — Гоблин и Тёмный эльф не поглощают
  # ничего, а Карлик даёт щитовой AC, не резист) И вклад не нулевой (раса
  # активна, но эффект не включён без оружия в руках — `RacialBonus`'s
  # `inactive?`, тот же `counted: nil`/`0`).
  defp resistance_race_part(ruleset, %{racial_bonus: %{kind: :damage_resistance} = bonus}) do
    case bonus.counted do
      c when is_integer(c) and c != 0 -> Labels.race_ru(ruleset, bonus.race)
      _zero_or_unmodelled -> nil
    end
  end

  defp resistance_race_part(_ruleset, _stats), do: nil

  # «Тип оружия» — та же подпись, что у этого же терма в разборе AB и навыка
  # (`weapon_type_term/2`, эта же функция обслуживает оба): один источник
  # поглощения, названный одним словом везде на экране, а не именем
  # предмета — оно ничего не объясняет, кроме случая, где игрок ждёт увидеть
  # своё оружие в списке причин.
  defp resistance_weapon_type_part(%{weapon_type_bonuses: bonuses}) do
    total =
      bonuses
      |> Enum.filter(&(&1.kind == :damage_resistance))
      |> Enum.reduce(0, fn %{counted: c}, acc -> acc + (c || 0) end)

    if total != 0, do: "тип оружия"
  end

  # Мини-сеты — то же слово, что и везде на этой панели (`Labels.
  # mini_sets_name/0`), не строкой «усиление»: одно число не должно называться
  # на экране двумя способами.
  defp resistance_mini_set_part(%{added: added}) when added != 0, do: Labels.mini_sets_name()
  defp resistance_mini_set_part(_effect), do: nil

  defp resistance_gear_term(%{gear: 0}), do: []

  # Вещь проиграла эффекту — в итог не входит; названа в
  # `resistance_lost_terms/3` (задача 3.218).
  defp resistance_gear_term(%{superseded: :gear}), do: []

  defp resistance_gear_term(%{gear: gear}), do: [%{label: "вещь", value: signed(gear)}]
  ...
  Отдаёт `%{id:, name:, title:, assumed?:}`:
    ...
    * `assumed?` — правило чистоты у этой группы **не процитировано**: либо его
      не написал никто, либо оно выведено. ⚠️ Обязано доезжать до экрана:
      «сагровик» прочитан дословно и `verified`, «адровец» с 11.09.2026
      (задача 3.182) выведен из разбора серверных скриптов шарда и помечен
      `derived`, и показать два разных по качеству факта одинаково значило бы
      соврать про второй.

      ⚠️ **Здесь стояло «не написано никем», и это было верно ровно до 3.182.**
      Перестало: скрипты шарда правило называют, просто не про саму Адру —
      `ClassNotRestrictTorm` названа «a Torm-flavored variant» от
      `ClassNotRestrict`, у которой «returns TRUE only when ALL levels are…».
      Значит меняется ТЕКСТ подписи, а не решение её показывать: флажок
      по-прежнему помечен, потому что вывод цитатой не является.

      🔴 **Флажок и подпись считает ОДНА функция** (`purity_caveat/1`): две
      записи об одном правиле расходятся молча, и в этом файле это уже ловили
      (задача 3.85). `assumed?` — это ровно «подпись не пуста», а не второе
      чтение тех же полей.
  ...
  def class_group_flags(ruleset, stats) do
    for group <- Map.get(stats, :class_groups, []) do
      caveat = purity_caveat(group)

      %{
        id: group.id,
        name: group.name || to_string(group.id),
        title: class_group_title(ruleset, group, caveat),
        # ⚠️ Ровно «подпись не пуста», а не второе чтение `purity_stated?`:
        # значок и текст, объясняющий значок, обязаны появляться вместе.
        assumed?: caveat != ""
      }
    end
  end
  ...
  defp class_group_title(ruleset, group, caveat) do
  ...
  # Три состояния провенанса правила чистоты, и у каждого своя фраза — потому
  # что это три разных утверждения, а не оттенки одного:
  #
  #   * правила не назвал никто → мы применили правило Сагры, это допущение;
  #   * правило названо, но ВЫВЕДЕНО (`derived`) → у Адры с 11.09.2026: разбор
  #     серверных скриптов шарда называет форму предиката у родственной функции
  #     и саму `ClassNotRestrictTorm` её «вариантом», то есть цитаты именно про
  #     Адру по-прежнему нет;
  #   * правило процитировано (`verified`, Сагра) → подписи нет вовсе.
  #
  # ⚠️ Вывод не выдаём за цитату и не прячем за молчанием: подпись называет
  # ИСТОЧНИК вывода, чтобы игрок мог взвесить его сам.
  defp purity_caveat(%{purity_stated?: false}),
    do: " ⚠ Правило чистоты у этой группы на вики не описано — применяем правило Сагры."

  defp purity_caveat(%{purity_status: "verified"}), do: ""

  defp purity_caveat(%{purity_status: "derived"}),
    do:
      " ⚠ Правило чистоты у этой группы на вики не описано — выведено из разбора" <>
        " серверных скриптов шарда, а не процитировано."

  # 🔴 Умолчание направлено В СТОРОНУ ПОКАЗА, и это не осторожность ради
  # осторожности: молчание здесь читается игроком как «процитировано». Правило
  # с любым другим статусом (или вовсе без него) цитатой не является, назвать
  # его источник мы не можем — значит говорим ровно то, что знаем.
  defp purity_caveat(_group), do: " ⚠ Правило чистоты у этой группы не процитировано."
  ...
  # ---- «против заклинаний» — задача 3.197, часть B ----
  #
  # Часть A (замер `AQ1`, Dan 12.09.2026: «спасы от Spellcraft в листе
  # персонажа не видна») вынесла условную прибавку Spellcraft ИЗ самого
  # спаса: `stats.fort`/`ref`/`will` — лист персонажа, `saves_conditional`
  # — тот же сейв с прибавкой внутри пула, `conditional_save_bonus` —
  # сама прибавка после потолка, `conditional_save_terms` — чем условие
  # названо, до потолка. Часть B даёт этим числам экран.

  @vs_spells_title "против заклинаний"

  @doc "Заголовок — одно слово на панель, экран просмотра и подраздел разбора."
  @spec vs_spells_title() :: String.t()
  def vs_spells_title, do: @vs_spells_title

  @doc """
  Компактная строка под тройкой Fort/Ref/Will — панель итогов и экран
  просмотра, одной формой (`note/3`'s `%{text:, group:}`, тот же приём, что
  у `racial_bonus_note/2` выше). `nil`, пока `stats.conditional_save_terms`
  пуст: строке взяться неоткуда, а печатать «против заклинаний: +0» у билда
  без единого ранга значило бы придумать предупреждение там, где вопрос
  не встаёт (CLAUDE.md §6 — недоступное показываем с причиной, а отсутствующее
  не показываем вовсе).

  Печатает САМУ ПРИБАВКУ (`+N`), а не второй раз `saves_conditional`: строка
  стоит буквально под уже напечатанными Fort/Ref/Will, и печатать там ещё
  одну тройку абсолютных чисел заставило бы читателя вычитать одно из
  другого самому — калькулятор не разрешает себе этого ни в одном другом
  месте («Из этого следует» у вещей, призрачный `+N` превью левелапа).

  🔴 **Ноль под капом печатается СЛОВОМ, а не молчанием.** Раз в списке
  условная прибавка ЕСТЬ (билд вложился в Spellcraft), тихий `+0` рядом
  читался бы как «прибавка потерялась», а не как честный итог клипа —
  билд с вещевым пулом уже в потолке (кейс «Мокси») обязан увидеть слово
  «потолок», а не решить, что калькулятор забыл про его ранги.
  """
  @spec saves_vs_spells_note(map()) :: %{text: String.t(), group: String.t()} | nil
  def saves_vs_spells_note(stats) do
    case saves_vs_spells_rows(stats) do
      nil ->
        nil
        ...
        text =
          @vs_spells_title <>
            ...
            Enum.map_join(rows, " · ", fn
              %{label: label, value: value, capped?: true} -> "#{label} #{value} (потолок)"
              %{label: label, value: value} -> "#{label} #{value}"
            end)

        %{text: text, group: "save"}
    end
  end

  defp saves_vs_spells_rows(stats) do
    case Map.get(stats, :conditional_save_terms, []) do
      [] ->
        nil

      _terms ->
        for save <- [:fort, :ref, :will] do
          bonus = stats |> Map.get(:conditional_save_bonus, %{}) |> Map.get(save, 0)
          %{label: save_short_label(save), value: signed(bonus), capped?: bonus == 0}
        end
    end
  end

  defp save_short_label(:fort), do: "Fort"
  defp save_short_label(:ref), do: "Ref"
  defp save_short_label(:will), do: "Will"

  @doc """
  Подраздел «против заклинаний» разбора ОДНОГО спаса — попап панели итогов
  (`stat_pop/1`'s `extra_terms`) и список экрана просмотра, задача 3.197
  часть B.

  🔴 **НЕ входит в `save_terms/3`, и это не забывчивость.** Сумма термов
  разбора обязана сходиться со значением рядом (`stats.fort`/`ref`/`will`,
  тест на это уже есть) — а условная прибавка в эти три числа не идёт
  вовсе, замер `AQ1`. Смешать её со списком, который проверяют суммой,
  значило бы вернуть ровно ту ошибку, ради которой делалась часть A: строка
  внутри «сколько это» перестала бы сходиться со стоящим рядом числом.

  Один терм на правило — «сколько предложено» — всегда, и «до потолка
  доехало» — только когда потолок откусил хоть очко: полное совпадение
  печатать незачем, разбор не повторяет число дважды без причины.
  """
  @spec vs_spells_terms(map(), map(), DerivedStats.save()) :: [
          %{label: String.t(), value: String.t()}
        ]
  def vs_spells_terms(ruleset, stats, save) do
    landed = stats |> Map.get(:conditional_save_bonus, %{}) |> Map.get(save, 0)

    Enum.flat_map(Map.get(stats, :conditional_save_terms, []), fn term ->
      offered = %{label: vs_spells_label(ruleset, stats, term), value: signed(term.bonus)}

      if landed == term.bonus,
        do: [offered],
        else: [offered, %{label: "до потолка доехало", value: signed(landed)}]
    end)
  end

  # Имя навыка плюс ЗНАЧЕНИЕ, от которого считается прибавка, и ранги в скобках
  # (`stats.skill_values` может не нести запись, если её вычистили из вывода
  # вручную — синтетический stats в тестах, например) — тем же приёмом, что
  # у `own_save_label/2` выше: число рядом с именем источника, а не безымянная
  # строка.
  #
  # ⚠️ С 3.202 прибавка считается от ПОЛНОГО значения навыка (ранги +
  # модификатор INT + вещи + фиты, `fandom:Spellcraft`: «fully modified ranks»),
  # и печатать здесь одни ранги значило бы показать «43 ранга → +9» — число,
  # которое на пять не делится и объяснить которое читатель не может. Значение
  # стоит первым, потому что именно оно делится на пять; ранги — в скобках,
  # потому что игрок покупает ранги, а не значение.
  defp vs_spells_label(ruleset, stats, %{skill: skill}) do
    name = Labels.skill_name(ruleset, skill)

    case stats |> Map.get(:skill_values, %{}) |> Map.get(skill) do
      %{ranks: ranks, total: total} when is_integer(total) and is_integer(ranks) and ranks > 0 ->
        "#{name} #{total} (#{ranks} #{ranks_word(ranks)})"

      %{ranks: ranks} when is_integer(ranks) and ranks > 0 ->
        "#{name} (#{ranks} #{ranks_word(ranks)})"
  ...
  # Русское склонение «ранг/ранга/рангов» — самостоятельная функция, а не
  # инвариантная форма: строка стоит в разборе, который выше в этом же файле
  # называет себя «дословной цитатой ниоткуда не выдуманного числа», и число
  # с неверным словом рядом читается как небрежность в тексте, который
  # других мест как раз учит точности.
  defp ranks_word(n) do
    cond do
  ...
  **`ranks` and `value` are two different numbers and the row keeps them apart.**
  ...
  класс(ы) → правила шарда» — для поп-апа панели итогов.

  🔴 **Это ЕДИНСТВЕННЫЙ список слагаемых значения навыка во всём слое
  итогов:** подпись экрана просмотра (`skill_from/3`) с задачи 3.220
  собирается из него же, а не описывает ту же сумму вторым разом. Здесь
  стояло «тот же каскад, что `skill_from/3` уже печатает строкой (оба
  читают ровно те поля, что собрал `Skills.value/4`)» — и это было верно
  ровно до первого нового терма: «читают одни поля» не мешает одному
  из двух списков молча отстать, что и случилось дважды подряд
  (`weapon_type_bonus`, 3.35, и `mini_set_bonus`, 3.185). Та же причина,
  по которой `ab_terms/2` и «AB» на экране просмотра читают один список,
  а не два похожих.
  ...
  def skill_value_terms(ruleset, value) do
    [
      ...
      # ⚠️ И третьей строкой — усиление кусками мини-сетов (задача 3.185).
      # Тем же доводом, что у этого терма в разборе AB: мини-сеты усиливают
      # две строки выше от их СУММЫ (в движке это один эффект), поэтому
      # приписать прибавку расе или типу оружия нельзя, а не напечатать
      # вовсе — значит напечатать сумму термов, не равную своему итогу
      # (прогон 11.09.2026: Дисциплина 42 при сумме термов 30).
      mini_set_term(value),
  ...
  # Усиление кусками мини-сетов (задача 3.185). Подпись — та же, что у этого
  # терма в разборе AB и в блоке «Вещей» (`Labels.mini_sets_name/0`), чтобы
  # одно и то же число не называлось на экране двумя словами. Терм с нулём
  # не печатается — то же правило, что у всех соседей, и именно оно оставляет
  # разбор билда без мини-сетов ровно таким, каким он был.
  defp mini_set_term(%{mini_set_bonus: 0}), do: nil

  defp mini_set_term(value),
    do: %{label: Labels.mini_sets_name(), value: signed(value.mini_set_bonus)}
  ...
  # 🔴 Собирается ИЗ `skill_value_terms/2`, а не пишется вторым списком рядом
  # (задача 3.220). Здесь стоял свой перечень из десяти слагаемых, и он дважды
  # отстал от первого молча: `weapon_type_bonus` (задача 3.35) и `mini_set_bonus`
  # (3.185) добавили в поп-ап и не добавили сюда. Живой промах — Дисциплина
  # у Человека с древковым и двумя кусками: значение 40, подпись «4 р. · STR +0 ·
  # Человек +12» на 16, и это на ШАРЕННОЙ ссылке, то есть у чужого читателя.
  # Правило файла было записано у `card/5` («одна сумма — одно описание») ещё
  # до обоих промахов — просто эта подпись ему не следовала.
  #
  # Два слагаемых сверх того списка — и оба не слагаемые суммы вовсе:
  #
  #   * «характеристика не названа ни на одной вики» — печать отказа. В поп-апе
  #     её нет намеренно (`skill_ability_term/1` отдаёт `nil`), потому что там
  #     отсутствие строки читается вместе с «?» рядом; здесь же подпись это
  #     единственное место, где сказано, почему числа нет;
  #   * `feats_term/2` — признание в непосчитанном, у строки поп-апа оно едет
  #     отдельным `note` (см. `skill_value_terms/2`), а в одну строку подписи
  #     складывать больше некуда.
  defp skill_from(ruleset, value, _ranks) do
    # Терм рангов у `skill_value_terms/2` всегда первый и никогда не `nil` —
    # на нём и держится склейка: подпись печатает его «4 р.», а не «ранги 4»,
    # и это единственное расхождение двух форм одного списка.
    [ranks | rest] = skill_value_terms(ruleset, value)

    ([
       "#{ranks.value} р.",
       unnamed_ability_term(value)
     ] ++
       Enum.map(rest, &skill_sentence_term/1) ++
       [feats_term(ruleset, value.unmodelled_feats)])
    |> Enum.reject(&is_nil/1)
    |> Enum.join(" · ")
  end

  # Терм поп-апа куском строки. Подпись и число те же самые — расходиться им
  # теперь негде, список один.
  defp skill_sentence_term(%{label: label, value: value}), do: "#{label} #{value}"
  ...
  #
  # ⚠️ Названной характеристике здесь пары НЕТ, и это не забытая клауза: её
  # печатает `skill_ability_term/1` общим списком. Второй клаузой она была бы
  # ровно тем вторым описанием одной суммы, ради устранения которого задача
  # 3.220 и переписала `skill_from/3`.
  defp unnamed_ability_term(%{ability: nil}), do: "характеристика не названа ни на одной вики"
  defp unnamed_ability_term(_value), do: nil
