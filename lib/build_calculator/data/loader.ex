defmodule BuildCalculator.Data.Loader do
  ...
  def source_files do
    [
      ...
      # Восьмой файл этой формы (задача 3.209) — поглощение СТИХИЙНОГО урона:
      # `Resist energy` и `Epic energy resistance`. Тем же двум причинам, что
      # у семи соседей: правка файла обязана пересобирать ruleset, и
      # неучтённый `vanilla/*.json` читался бы как словарь значений выбора
      # (`domain_files/1`), то есть без второй записи появился бы домен
      # `feat_resistance_bonuses`, чьи «значения» — записи разметки.
      "vanilla/feat_resistance_bonuses.json",
  ...
  def load!(root) do
    raw = %{
      ...
      # И что даёт ПОГЛОЩЕНИЕ СТИХИЙНОГО УРОНА (задача 3.209) — `Resist energy`
      # и `Epic energy resistance`, восьмой ручной файл подряд и по восьмому
      # разу потому, что поля в корпусе нет. ⚠ Величина шарда (15/150) отличается
      # от ванильной (10/100) — единственный из восьми файлов разметки, где
      # сиальское число не совпадает с ванильным дословно, поэтому у него есть
      # двойник-поле `siala_*`, тем же приёмом, что у `gear.worn.siala_max_dex`.
      # См. `build_resistance_bonuses/2`.
      feat_resistance_bonuses: read_json(root, "vanilla/feat_resistance_bonuses.json"),
  ...
  defp ruleset(version, raw, shard) do
    ...
    # Рост обоих сиальских бонусов по уровню персонажа (задача 3.181). Таблица
    # тиров и арифметика исполнителей лежат ОДИН раз, в `systems.json`, и
    # раздаются обоим слоям: расовый бонус и бонус за тип оружия исполняет один
    # диспетчер движка, то есть это одна система, а не две похожие.
    shard_bonus_scaling = Races.shard_bonus_scaling(systems)

    # Третья часть той же системы мини-сетов, и у неё СВОЙ вход: процент к HP
    # считается от `Cgear + Nmini`, где второе слагаемое — именные и уникальные
    # вещи, которых усиление бонусов не знает вовсе (задача 3.186). Поэтому
    # отдельным ключом ruleset'а, а не полем внутри `shard_bonus_scaling`.
    gear_hit_points = Systems.gear_hit_points(systems)

    racial_bonuses =
      Races.racial_bonuses(shard.races, %{
        scaling: shard_bonus_scaling,
        races: races,
        classes: classes,
        skills: skills,
        weapons: weapons,
    ...
    # Два написания одного числа, сверенные там, где живёт второе — тот же
    # приём, что строкой выше. Бонус магического посоха перенесён дважды:
    # фактом-переносом предложения (`magic_staff_skill_bonus`) и применяемой
    # записью в таблице по оружию, и разойтись им нельзя (задача 3.183).
    Races.verify_weapon_transcription_agrees!(weapon_type_bonuses, systems)

    # 🔴 И сверка самой формулы роста со всеми 56 числами обоих файлов: число
    # 40-го уровня осталось в данных и стало **сверкой**, а не источником
    # значения. Расхождение роняет сборку — тот же приём, что у
    # `bonus_spell_slots` и у таблицы прогрессии ПДК.
    Races.verify_shard_bonus_scaling!(racial_bonuses, weapon_type_bonuses)
    ...
    # И для ПОГЛОЩЕНИЯ СТИХИЙНОГО УРОНА (задача 3.209) — восьмой и последний
    # файл этой формы. Единственный дополнительный ключ среди восьми: `version`,
    # потому что это ЕДИНСТВЕННЫЙ из восьми файлов, чьё число у Сиалы РЕАЛЬНО
    # отличается от ванильного (15/150 против 10/100) — `build_resistance_bonuses/2`
    # разрешает twin-поля `siala_*` в момент чтения, тем же приёмом, что
    # `Gear.worn!/3` разрешает `siala_max_dex`, и поэтому должен знать, для
    # какого ruleset'а он сейчас строится.
    resistance_bonuses =
      Bonuses.build_resistance_bonuses(Map.get(raw, :feat_resistance_bonuses, :missing), %{
        feats: feats,
        known_receivers: known_receivers,
        version: version
      })

    # Семь стихий секции «Резисты» (задача 3.209, п. 5) — шире домена выбора
    # фитов на два (негативная/позитивная энергия расы и оружия). Читает
    # `raw.domain_files` напрямую, а не резолвленный `choice_domains/3`: тот
    # ещё не построен на этом шаге, а сверять здесь нужно ровно то же самое
    # содержимое `energy_types.json`.
    resistance_energy_types =
      Bonuses.resistance_energy_types(
        Map.get(raw, :feat_resistance_bonuses, :missing),
        Map.get(raw, :domain_files, %{})
      )

    # И то, чего секция «Резисты» НЕ показывает, вместе с именами, которыми это
    # печатает игровой лог (задача 3.210, п. 5). Не «остальное»: у трёх записей
    # разные причины, и свод обязан сказать игроку, какая именно — решение
    # владельца (физическое поглощение) или сама игра (магический, божественный).
    resistance_excluded_types =
      Bonuses.resistance_excluded_types(Map.get(raw, :feat_resistance_bonuses, :missing))
    ...
    %{
      ...
      # И что даёт ПОГЛОЩЕНИЕ СТИХИЙНОГО УРОНА — `Resist energy` (+5, одно
      # число на оба ruleset'а) и `Epic energy resistance` (+10 за взятие,
      # потолок 100 на ваниле; +15/150 на Сиале — задача 3.209). Та же форма,
      # что у семи выше; см. `build_resistance_bonuses/2`.
      # ⚠ `resistance_bonuses[id].amount` даёт величину ОДНОГО слагаемого —
      # «фиты» в формуле `резист(стихия) = фиты + max(эффект расы/оружия
      # с мини-сетами, вещь)`. Второе слагаемое считает `Rules.RacialBonus` /
      # `Rules.WeaponTypeBonus` / `Rules.MiniSets` из `shard_bonus_scaling`
      # ниже, а не этот словарь — `feat_resistance_bonuses.json` → `_stacking_
      # decision.note_for_3_210` разбирает это разделение подробно. Пока нет
      # читателя (задача 3.210, `DerivedStats.resistances`), билд с этими
      # фитами несёт {:not_modelled, {:feat_bonus, id}} — оговорка снимется
      # сама, тем же механизмом, каким это уже случилось у шестнадцати других
      # записей `vanilla/feat_effect_receivers.json` (задача 3.93).
      resistance_bonuses: resistance_bonuses,
      # Семь стихий, которые печатает секция «Резисты» (задача 3.209, п. 5) —
      # пять из `vanilla/energy_types.json` (то, чем параметризуются два фита
      # выше) плюс негативная/позитивная энергия, которые фитом не выбираются,
      # но поглощаются расой Гнома и группой топоров. Читает
      # `feat_resistance_bonuses.json` → `totals_energy_types`; см.
      # `Bonuses.resistance_energy_types/2`. Одинаков у обоих ruleset'ов —
      # список того, ЧТО ПОКАЗЫВАЕТ секция, а не сиальское число.
      resistance_energy_types: resistance_energy_types,
      # Чего секция «Резисты» не показывает, и почему — с именами игрового
      # лога (задача 3.210). Читает `Rules.GearImport`: строка про физическое
      # поглощение обязана попасть в «решением не считаем», а не в «не сложить
      # без сервера», иначе она уедет в список доработок администрации.
      resistance_excluded_types: resistance_excluded_types,
      # Как НЕфитовые источники поглощения складываются между собой:
      # эффект расы/топоров против вещи и вещь против вещи (задача 3.210).
      # `nil` у ванили — там эффекта нет вовсе. Читают `Rules.Resistances`
      # (первая половина) и `Rules.GearImport` (вторая).
      resistance_stacking: Systems.resistance_stacking(systems),
      ...
      # Машинерия, общая обоим бонусам выше: таблица тиров по уровню персонажа,
      # арифметика восьми исполнителей движка с их ВНУТРЕННИМИ капами и правило
      # мини-сетов (задачи 3.181, 3.184). Раздаётся обоим слоям под ключом
      # `scaling` и лежит здесь же целиком, потому что мини-сеты — свойство
      # ПЕРСОНАЖА, а не расы и не оружия: усиление считается от суммы обоих
      # термов, и спросить его у одного из двух слоёв было бы неверно.
      # `nil` у ванили, у которой этой системы нет вовсе. Читает
      # `Rules.MiniSets`; сами тиры — `Rules.RacialBonus.variants_at/3`.
      shard_bonus_scaling: shard_bonus_scaling,
      # Процент к максимуму HP за надетое — нелинейная таблица мини-сетов
      # с ОБРЫВОМ В НОЛЬ на одиннадцатой вещи (задача 3.186). Отдельно от
      # `shard_bonus_scaling` выше, потому что вход у неё свой: `Cgear + Nmini`,
      # где `Cgear` — именные и уникальные вещи, которых усиление бонусов
      # не знает вовсе. `nil` у ванили. Читает `Rules.GearHitPoints`.
      gear_hit_points: gear_hit_points,
