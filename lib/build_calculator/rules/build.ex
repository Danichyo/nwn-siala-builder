  ...
  *which*, and two picks of one id are then indistinguishable — the calculator
  ...
  same place, because it is the same measured line (H7, 14.08.2026):
    ...
    * **how many takes went on it** — neither: `feat_takes/3` counts slots and
      `feat_takes_owned/4` adds what the items lend
      (`Rules.GearFeats.takes/3`). Two **values** off two items are two bonuses
      and still one take — a feat that names a value does not stack off items
      at all, and only the ones that name nothing count per declaration
      (task 3.204).
  ...
  @doc """
  How many times `feat` was taken, counting what an **item** lends.

  `feat_takes/3` counts slots; this counts what the character has. A declaration
  under «Вещи» is a take, so `Epic toughness` picked once and lent by an amulet
  is **two** takes and twenty hit points twice over (Dan, 09.08.2026: «Это
  будет +2») — and two declarations of it are two takes of their own since task
  3.204. How many the items are worth is `Rules.GearFeats.takes/3`'s answer,
  never a `1` assumed here.
  ...
  ⚠ **The reason stopped being true on 13.09.2026** (task 3.204), and the cost
  was measured on his own build: the game prints 1503 hit points, the model
  printed 1437. Слово Dan: «на фитах с вещи нам необходимо добавить возможность
  настакивать те фиты, которые можно взять несколько раз… Там не только epic
  toughness». So the gear half is now a **count** of declarations
  (`Rules.GearFeats.takes/3`) wherever the data says the feat repeats with no
  value to name, and the numbered-copies question stays exactly where Dan left
  it: numbers are not distinguished, the sum is what is capped. Boots lending
  `Epic Toughness II` and `III` are two takes; a belt lending `II` beside them
  is not a third, and that is the game's own arithmetic (1503 = 906 × 1.66).
  ...
  ⚠ **Never attributed to a value**, and for a feat whose takes must differ
  (`Favored enemy`) still one take at most: a declaration cannot say *which*
  school or enemy it was granted with, so two of them are one — the build says
  so through `{:not_modelled, {:gear_feat_choice, id}}`, and the item's take
  collides with nothing and blocks nothing. `feat_choices/3` stays slots-only on
  purpose. Here stood «one take, never more» flat, which was true of every feat
  until task 3.204 and is now true of exactly the ones that name a value.
  """
  ...
  @doc """
  How many times `feat` was taken **with this value**, items included.

  The pair-shaped twin of `feat_takes_owned/4`, and a separate function rather
  than a fifth argument with a default because the two count different units:
  `/4` counts takes of a **name** (`Epic toughness` has no value to name),
  `/5` counts takes of a **pair** (`Epic energy resistance (Fire)` twice is
  thirty points of fire resistance and nothing at all to cold). Задача 3.210.

  ⚠ `choice: nil` is a value like any other here: it counts the picks and the
  declarations that recorded **nothing**, which is what every link written
  before task 3.97 carries. A caller pricing an effect off a value therefore
  gets `0` for every value the build never named, and the build owes a caveat
  about the silence rather than an invented element.

  ⚠ Both halves are read the same way `/4` reads them — slots by
  `feat_takes_by_choice/3`, items by `Rules.GearFeats.takes/4` — so the split
  between «what counts as a take» and «how many the items lend» stays in one
  place.
  """
  @spec feat_takes_owned(t(), map(), atom(), non_neg_integer(), feat_choice() | nil) ::
          non_neg_integer()
  def feat_takes_owned(%Build{} = build, ruleset, feat, level, choice) do
    picked = build |> feat_takes_by_choice(feat, level) |> Map.get(choice, 0)

    picked + GearFeats.takes(build.gear, ruleset, feat, choice)
  end

  # Сколько взятий лендят вещи — один вопрос, один читатель
  # (`GearFeats.takes/3`). Здесь стоял `if held?, do: 1`, то есть «предмет
  # одалживает фит, а не число его копий», и задача 3.204 это сняла словом Dan
  # 13.09.2026: повторяемый фит без выбора можно объявить несколько раз, и число
  # записей есть число взятий. Правило про то, КАКИЕ фиты так умеют, живёт
  # в данных (`repeatable.choice`), а не здесь.
  defp gear_takes(%Build{gear: gear}, ruleset, feat), do: GearFeats.takes(gear, ruleset, feat)
