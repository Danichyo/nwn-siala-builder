defmodule BuildCalculator.Rules do
  ...
  `abilities_naked`, `saves_naked` and — since task 3.186 — `hp_naked` are
  computed with no gear at all so the interface can show `было → стало`.

  ⚠ `hp_naked` joined that list the day equipment gained a **multiplier** on hit
  points: the shard's percentage for worn mini-set pieces and crafted items
  reaches +105 % (`Rules.GearHitPoints`), so one number stopped answering both
  questions the way it had while gear only moved the constitution modifier.
  ...
  def compute(%Build{} = build, ruleset) do
    ...
    # HP «в экипировке» — то самое число, которое читают все: телосложение здесь
    # итоговое, фиты с вещи посчитаны, а с 11.09.2026 (задача 3.186) сверху
    # ложится процент шарда за надетое — мини-сеты и именные вещи.
    #
    # ⚠ Прибавка считается ОТ ИТОГА `hit_points/3`, а не внутри него: `Hbase`
    # источника — это «the value returned by `GetMaxHitPoints(oPC)`», то есть
    # готовое число со всеми костями, телосложением, фитами и «Духом Сиалы».
    # Терм ложится в разбор РЯДОМ с `Toughness` и `innate`, а не внутрь них,
    # иначе сумма частей перестала бы сходиться со своим итогом.
    {hp, hp_breakdown, hp_gaps} =
      case Progression.hit_points(build, ruleset, modifiers.con) do
        {:ok, hp, breakdown} ->
          gear_hp = GearHitPoints.term(build, ruleset, hp)
          {hp + GearHitPoints.amount(gear_hp), %{breakdown | gear_bonus: gear_hp}, []}

        {:error, reason} ->
          {nil, nil, [reason]}
      end

    # И то же HP у персонажа БЕЗ ВЕЩЕЙ ВООБЩЕ — второй проход по билду с пустой
    # экипировкой, ровно как `ac_naked` и `saves_naked`, и ровно по тем же двум
    # причинам. Первая: «голым» обязано значить одно и то же во всех трёх
    # числах, а телосложение с вещей двигает HP ретроактивно на всех уровнях
    # (измерено 16.08.2026: +12 CON превратили 760 в 1000). Вторая: вычитанием
    # это не считается — процент за надетое умножает ИТОГ, в который входит
    # и вещевое телосложение, и фит с вещи, так что «минус вещи» пришлось бы
    # разбирать на три независимых дороги.
    hp_naked =
      case Progression.hit_points(naked_build, ruleset, naked_modifiers.con) do
        {:ok, hp, _breakdown} -> hp
        {:error, _reason} -> nil
      end
    ...
    # ⚠ **И пятый механизм с 11.09.2026 (задача 3.184) — мини-сеты**, и он
    # единственный, который не приносит своего бонуса, а увеличивает два терма
    # выше: `B += floor(B · Nmini / 10)`. Прибавка считается от их СУММЫ,
    # потому что в движке это один эффект, — поэтому она и стоит отдельным
    # слагаемым, а не внутри одного из двух (`Rules.MiniSets`). Число может
    # быть отрицательным: внутренний кап исполнителя (18) режет ту же сумму.
    mini_set_attack = MiniSets.attack_bonus(build, ruleset)
    ...
    off_hand =
      off_hand_attack(build, ruleset, dual_wield, %{
        ...
        mini_set_attack: mini_set_attack,
    ...
    #
    # 🔴 **И одно из трёх слагаемых в лист персонажа не попадает вовсе**
    # (задача 3.197, замер `AQ1`, Dan 12.09.2026: «спасы от Spellcraft в листе
    # персонажа не видна»). Прибавка от рангов условная — она работает против
    # заклинаний, — поэтому базовые Fort/Ref/Will считаются **без** неё, а она
    # отдаётся отдельным числом (`conditional_save_bonus`, `saves_conditional`).
    # Куда попадает чьё число, решают данные, а не эта строка:
    # `Skills.save_bonus_terms/3` несёт признак у каждого правила.
    #
    # ⚠ **И считается прибавка от ПОЛНОГО значения навыка, а не от купленных
    # рангов** (задача 3.202, `fandom:Spellcraft`: «based on fully modified
    # ranks»). Поэтому `skill_values` считается ВЫШЕ и передаётся правилу:
    # число на строке навыка и число под спасами обязаны быть одним и тем же
    # числом, а не двумя сложениями, которые однажды разойдутся.
    skill_values = Skills.values(build, ruleset, character_level)
    skill_save_terms = Skills.save_bonus_terms(build, ruleset, character_level, skill_values)
    save_skills = Enum.map(skill_save_terms, & &1.skill)
    skill_save = skill_save_total(skill_save_terms)
    sheet_skill_save = skill_save_total(skill_save_terms, & &1.in_character_sheet?)

    # ⚠ **Одно число на сейв, а не одно на персонажа** (задача 3.187): у вещей
    # с этого дня две строки — `Saving Throw Bonus (Universal)` и прибавка
    # к одному сейву, — и складывает их `Rules.Gear.save_bonus/2`, а не каждый
    # читатель по-своему. Под потолок они идут ВМЕСТЕ, одним клипом: «общее
    # правило то же — +20 — это кап» (Dan 11.09.2026). Два клипа по +20 — это
    # ровно та ошибка, ради которой потолок и заведён (CLAUDE.md §9).
    gear_saves = Map.new([:fort, :ref, :will], &{&1, Gear.save_bonus(build.gear, &1)})
    ...
    %{total: save_bonus, clipped: save_clipped, capped: saves_capped} =
      clip_saves(ruleset, gear_saves, sheet_skill_save, own_save_terms)

    # То же ещё раз, с условной прибавкой внутри пула, — и разность двух клипов
    # и есть «сколько она добавляет». Чтение капа **принятое**, а не измеренное,
    # и записано в данных (`overrides.json` → `skill_save_bonus.rules[].
    # in_character_sheet.cap_reading`): прибавка объявлена внутрикапной, значит
    # она конкурирует с вещами за тот же +20, и под потолком бывает нулём.
    #
    # ⚠ Живые числа, на которых это видно: у Брунны вещевой пул Стойкости 19,
    # и восемь рангов дают ей **+1**, а не +8; у Мокси вещевые спасы 22, пул
    # упёрт в потолок и без прибавки — она даёт **0**, ровно как в её листе.
    # Вычитанием этого не получить: клип стоит внутри формулы (тот же довод,
    # что у `saves_naked`).
    %{total: save_bonus_conditional} =
      clip_saves(ruleset, gear_saves, skill_save, own_save_terms)
    ...
    #
    # ✅ **И этот день настал 13.09.2026** (задача 3.202): прибавка считается
    # от значения навыка, а в значении сидят и вещевой модификатор
    # характеристики, и вписанная игроком прибавка к самому навыку. Голый
    # проход зовёт `Skills.save_bonus_terms/3` по голому билду и получает
    # другое число — у Брунны 54 против 60, то есть 10 против 12, — и оно
    # выпало отсюда само, ровно как здесь и было записано. Сегодня до листа
    # это число всё равно не доезжает (правило условное, `in_character_sheet?`
    # отсеивает его в обоих фильтрах ниже), но считается честно: вернут
    # условность в лист — и «голым» окажется голым без правки этого места.
    own_save_terms_naked = SaveBonuses.terms(naked_build, ruleset, naked_modifiers)
    skill_save_terms_naked = Skills.save_bonus_terms(naked_build, ruleset, character_level)

    %{total: save_bonus_naked} =
      clip_saves(
        ruleset,
        %{},
        skill_save_total(skill_save_terms_naked, & &1.in_character_sheet?),
        own_save_terms_naked
      )
    ...
    #
    # ⚠ И с 12.09.2026 из него вычтено третье — **условная прибавка навыка**
    # (задача 3.197). Довод тот же, которым S2 вычла вещи, только в другую
    # сторону: числа нет в листе персонажа, значит требование его не видит.
    # Направление было выбрано сознательно: до правки здесь стояла ложная
    # ЛЕГАЛЬНОСТЬ — калькулятор предлагал `Resist energy` персонажу, которому
    # игра его не даёт.
    #
    # ✅ **И это больше не допущение: замер `AR1` проведён** (Dan, 13.09.2026:
    # «AR1 провел, resist energy не доступен, 5 очков spellcraft не играют
    # роли») — воин со Стойкостью 7 и пятью рангами, купленными РАНЬШЕ
    # 11-го уровня, фита не видит. Здесь стояло «**допущение**, названное
    # допущением в данных… и отданное в замер»; в данных статус поднят
    # `assumed` → `verified` (`overrides.json` → `skill_save_bonus.rules[].
    # prerequisite`), число не сдвинулось ни на очко — сменилась опора.
    %{total: save_bonus_for_prereqs} =
      clip_saves(
        ruleset,
        %{},
        skill_save_total(
          skill_save_terms_naked,
          &(&1.in_character_sheet? and &1.counts_for_prereqs?)
        ),
    ...
    # «Против заклинаний» — тот же сейв, посчитанный с условной прибавкой
    # внутри пула, и отдельно сама прибавка, чтобы интерфейсу не пришлось
    # вычитать одно из другого в трёх местах. Ноль у обеих величин — честный
    # ответ «условных прибавок у этого билда нет», а не молчание.
    saves_conditional = saves(base, epic_save, modifiers, save_bonus_conditional)

    conditional_save_bonus =
      Map.new(saves, fn {save, value} ->
        {save, Map.fetch!(saves_conditional, save) - value}
      end)

    # Чем условие названо. ⚠ `bonus` здесь — ДО потолка, как и у соседей
    # (`gear_save_bonus`, `skill_save_bonus`), а сколько из него дошло до числа,
    # говорит `conditional_save_bonus`: у Брунны это 8 и +1 соответственно, и
    # оба числа правдивы, просто отвечают на разные вопросы.
    conditional_save_terms =
      for term <- skill_save_terms,
          not term.in_character_sheet?,
          do: %{skill: term.skill, scope: term.scope, bonus: term.bonus}
    ...
    # Поглощение СТИХИЙНОГО урона (задача 3.210) — стат с самым разнородным
    # набором источников во всём файле: два фита складываются с наибольшим
    # из «эффект расы и топоров, усиленный кусками» и «число, вписанное
    # в „Вещах“». ⚠ Не сумма трёх слагаемых: фит СКЛАДЫВАЕТСЯ (три независимые
    # цитаты), а эффект с вещью КОНКУРИРУЕТ (слово Dan плюс замер `AV1`, где
    # у Хнюпиуса холод 30 при вещи 15). См. `Rules.Resistances`.
    resistances = Resistances.of(build, ruleset, character_level)
    ...
    %DerivedStats{
      ...
      hp: hp,
      # ⚠ Пара «голым / в экипировке», а не одно число с оговоркой: у HP она
      # с 11.09.2026 значит ровно то же, что у AC, — процент за надетое доходит
      # до +105 %, и одно число на два вопроса отвечать перестало.
      hp_naked: hp_naked,
      ...
      # Что к этой сумме добавили мини-сеты (задача 3.184) — тоже ДО потолка,
      # и тоже отдельной строкой: разбор обязан называть усиление, а не
      # прятать его внутрь усиленного.
      mini_set_attack_bonus: mini_set_attack,
      ...
      # И то же самое про мини-сеты: сколько кусков пошло в счёт и что они
      # прибавили каждому получателю. ⚠ `pieces` отдаётся отдельно от списка,
      # потому что счёт кусков — свойство ПЕРСОНАЖА и он не ноль даже тогда,
      # когда усиливать оказалось нечего (пустые руки, раса без бонуса).
      mini_set_pieces: MiniSets.pieces(build, ruleset),
      mini_sets: MiniSets.receivers(build, ruleset),
      ...
      # То же самое по сейвам (задача 3.187): universal плюс собственная строка
      # этого сейва. ⚠ Второе поле, а не замена первому: «сколько игрок вписал
      # в поле „все три“» и «сколько вещи дают Стойкости» — разные вопросы,
      # и разбор обязан печатать ВТОРОЕ, иначе слагаемые не сойдутся в число.
      gear_save_bonuses: gear_saves,
      ...
      ref: saves.ref,
      will: saves.will,
      # 🔴 Три числа выше — **лист персонажа**, и с 12.09.2026 это сказано
      # вслух: условной прибавки в них нет (задача 3.197, замер `AQ1`).
      # Она живёт в следующих трёх полях, потому что «Стойкость 50» и
      # «Стойкость 51 против заклинаний» — два разных утверждения, и печатать
      # второе под именем первого значило называть игроку число, которого
      # он в своём листе не найдёт.
      #
      # ⚠ `saves_conditional` — ИТОГ при выполненном условии (лист + прибавка),
      # `conditional_save_bonus` — сама прибавка после потолка. Оба, а не одно:
      # сумму иначе пришлось бы складывать в панели, в разборе, в экспорте и на
      # экране просмотра по отдельности, а четыре копии одного сложения — это
      # четыре возможности разойтись.
      saves_conditional: saves_conditional,
      conditional_save_bonus: conditional_save_bonus,
      # Чем именно условие названо — строкой источника (`"spells"`), а не нашим
      # переводом: подпись «против заклинаний» рисует веб-слой, ядро остаётся
      # языконезависимым. Пустой список = условных прибавок нет, и строке
      # в интерфейсе взяться неоткуда.
      conditional_save_terms: conditional_save_terms,
      ...
      # Поглощение по стихиям с разбором (задача 3.210). ⚠ Мапа, в которой
      # лежат только стихии с ненулевым источником: пустая значит «поглощения
      # нет», а не «мы не знаем». Порядок печати задаёт словарь ruleset'а.
      ...
      gaps:
        hp_gaps ++
          ...
          MiniSets.gaps(build, ruleset) ++
          GearHitPoints.gaps(build, ruleset) ++
          ...
          Resistances.gaps(build, ruleset, character_level, resistances) ++
  ...
  defp off_hand_attack(build, ruleset, dual_wield, shared) do
    ...
    mechanisms =
      for {source, value} <- [
            ...
            {:mini_sets, shared.mini_set_attack}
          ],
          do: {Caps.covers_source?(ruleset, :attack_bonus, source), value}
    ...
    %{
      ...
      # ⚠ То же число, что у главной руки, и намеренно то же: усиление висит
      # на персонаже, а не на предмете, и считается от суммы термов, которые
      # у рук общие. Своё у рук только число с предмета.
      mini_set_attack_bonus: shared.mini_set_attack,
      own_attack_bonus: own_attack,
  ...
  # Прибавка Spellcraft условна дважды: ваниль даёт её только против
  # заклинаний, Сиала отнимает ещё и против AOE (кроме имплозии).
  #
  # ⚠ **Первое условие с 12.09.2026 МОДЕЛИРУЕТСЯ** (задача 3.197, замер `AQ1`):
  # прибавки нет в самом сейве, она отдана отдельным числом «против
  # заклинаний». Не моделируется второе — сиальское сужение, — и оговорка
  # теперь ровно про него. Форма гэпа та же: сама оговорка не исчезла,
  # изменилось то, о чём она.
  ...
  # Сумма прибавок навыков — всех или только тех, что проходят признак.
  # Признак приходит предикатом, а не булевым флагом: вопросов у зовущего три
  # («сколько всего», «сколько в листе», «что засчитает требование»), и все три
  # читаются с терма, который собрал загрузчик.
  defp skill_save_total(terms, keep? \\ fn _term -> true end),
    do:
      Enum.reduce(terms, 0, fn term, sum -> if keep?.(term), do: sum + term.bonus, else: sum end)
  ...
  # ⚠ `gear_saves` — мапа «сейв → сколько дали вещи», а не одно число: с задачи
  # 3.187 предмет может прибавить и ко всем трём сразу, и к одному
  # (`Rules.Gear.save_bonus/2`). Ключ отсутствует — ноль, и именно так сюда
  # приходит «голый» проход: у него вещей нет вовсе.
  defp clip_saves(ruleset, gear_saves, skill_save, own_terms) do
    Enum.reduce([:fort, :ref, :will], %{total: %{}, clipped: %{}, capped: []}, fn save, acc ->
      mechanisms =
        for {source, value} <- [
              {:gear, Map.get(gear_saves, save, 0)},
              {:skill_rule, skill_save}
            ],
