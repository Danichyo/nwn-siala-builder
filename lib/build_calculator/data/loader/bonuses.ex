defmodule BuildCalculator.Data.Loader.Bonuses do
  ...
  # Восьмой файл этой формы (задача 3.209, «Резисты») — поглощение СТИХИЙНОГО
  # урона. `priv/rules/vanilla/feat_resistance_bonuses.json`'s own `_note`
  # carries the sweep: exactly two feats grant it in the whole nine-file corpus,
  # `resist_energy` and `epic_energy_resistance`, cross-checked against the
  # spell-resistance reader's own wide pass (which already listed the same two
  # while looking for something else entirely).
  #
  # ## One source in the schema, and that is the sweep's finding, not a limit
  #
  # Only `feat` occurs — no class table, no skill and no race grants this
  # directly (the shard's own racial bonus and its weapon-type twin are a
  # different mechanism entirely, `Rules.RacialBonus`/`Rules.WeaponTypeBonus`,
  # and combine with what this file states by the "highest wins" rule the data
  # states in `siala_41/systems.json`, not by addition — see the file's own
  # `_stacking_decision.note_for_3_210`).
  #
  # ## `energy_type: "chosen"` — a domain flag, not a value
  #
  # Both feats are `repeatable` with `choice: "energy_type"` (`vanilla/feats.json`
  # / `siala_41/feats.json`), so the amount below applies to whichever element
  # the feat was taken *for*, not to a fixed one. The field states that in the
  # data rather than leaving a reader to infer it from the feat's name.
  #
  # ## Two kinds, and the shard doubles one of them
  #
  # `flat` is `Resist energy`: one number, does not grow with a second take on
  # the **same** element (it is not even legal — `distinct: true`, measured,
  # `GAME_CHECKS.md` `AA1`). `per_take` is `Epic energy resistance`: the same
  # shape and the same names (`bonus`/`max_total`) `feat_hp_bonuses.json`'s
  # `epic_toughness` and `feat_spell_resistance.json`'s `improved_spell_resistance`
  # already carry — except the shard actually *changed* this one's magnitude
  # (15/150 instead of Fandom's 10/100), which neither of those two siblings'
  # numbers ever needed. The twin fields (`siala_bonus`/`siala_max_total` plus
  # `siala_values_apply_to_ruleset`) are the same device `gear.worn` uses for
  # `siala_max_dex` — see `resistance_apply_siala_twin/4` — because this file, like
  # `gear.worn`, is read once from `raw` and built once per ruleset version
  # (`ruleset/3` calls `build_resistance_bonuses/2` twice, with a different
  # `dictionaries.version` each time).
  #
  # ## `superseded_by` — a fact neither `amount` nor `cap` can carry
  #
  # `Epic energy resistance` supersedes `Resist energy` on the **same** element
  # rather than stacking with it (both feats' own pages say so, verbatim and
  # mirrored) — a relationship between two records, not a property of either
  # one alone. Recorded on `resist_energy`'s own record because that is the one
  # a build could otherwise wrongly keep adding once the epic version is also
  # owned; `nil` on the feat nothing supersedes.
  #
  # ## What raises at compile time
  #
  # An unknown feat name; an `applied` record with no amount or an amount kind
  # this loader does not compute; a `cap` on any record (no ruleset states a
  # ceiling on this stat — same guard as HP and spell resistance); a siala twin
  # stated without `siala_values_apply_to_ruleset`, or with only one of
  # `siala_bonus`/`siala_max_total`; `superseded_by` naming a feat the
  # dictionary does not know.
  @applied_resistance_kinds ~w(flat per_take)

  @resistance_file "feat_resistance_bonuses.json"
  @resistance_sources ~w(feat)

  @resistance_verdicts %{
    "applied" => :applied,
    "counted_elsewhere" => :counted_elsewhere,
    "not_modelled" => :not_modelled,
    "not_a_resistance_bonus" => :not_a_resistance_bonus
  }

  @resistance_reader %{
    file: @resistance_file,
    keys: @resistance_sources,
    verdicts: @resistance_verdicts,
    guard: &__MODULE__.resistance_cap_forbidden!/2
  }

  def build_resistance_bonuses(shape, dictionaries),
    do: BonusMarkup.build(shape, dictionaries, &resistance_bonus_record/2)

  defp resistance_bonus_record(entry, dictionaries) do
    BonusMarkup.record(@resistance_reader, entry, dictionaries, fn entry, verdict, name ->
      %{
        energy_type: resistance_energy_type!(entry, verdict, name),
        amount: resistance_amount(entry, verdict, name, dictionaries),
        superseded_by: resistance_superseded_by!(entry, name, dictionaries),
        # Same contract as six of its seven siblings (`ac_bonuses.json` is the
        # one exception): the claim is the data's, never inferred from the
        # bonus being applied.
        covers_feat?: entry["effect_coverage"] == "whole_feat"
      }
    end)
  ...
  # No ruleset states a ceiling on elemental damage resistance, so a side of
  # one is a statement about nothing — same guard and same reason as HP and
  # spell resistance.
  @doc false
  def resistance_cap_forbidden!(entry, name),
    do: BonusMarkup.cap_forbidden!(@resistance_file, name, entry, "elemental damage resistance")

  defp resistance_energy_type!(entry, :applied, name) do
    case entry["energy_type"] do
      "chosen" ->
        :chosen

      other ->
        raise "#{@resistance_file}: #{name} states energy_type #{inspect(other)}, expected " <>
                "\"chosen\" — both feats here apply to whichever element they were taken for"
    end
  end

  defp resistance_energy_type!(_entry, _verdict, _name), do: nil

  defp resistance_amount(entry, verdict, name, dictionaries) do
    BonusMarkup.amount!(
      @resistance_file,
      name,
      verdict,
      @applied_resistance_kinds,
      resistance_amount_shape(entry, name, dictionaries)
    )
  end

  defp resistance_amount_shape(%{"amount" => %{} = amount}, name, dictionaries) do
    case amount["kind"] do
      "flat" ->
        %{kind: :flat, bonus: resistance_number!(name, amount["bonus"])}

      "per_take" ->
        %{
          kind: :per_take,
          bonus: resistance_number!(name, amount["bonus"]),
          max_total: resistance_number!(name, amount["max_total"])
        }
        |> resistance_apply_siala_twin(amount, name, dictionaries)

      other ->
        raise "#{@resistance_file}: #{name} states amount kind #{inspect(other)}, which this " <>
                "loader does not know"
    end
  end

  defp resistance_amount_shape(_entry, _name, _dictionaries), do: nil

  defp resistance_number!(name, value),
    do: BonusMarkup.number!(@resistance_file, name, value, "a whole number")

  # Ванильное поле и его сиальский двойник, тем же приёмом, что у
  # `gear.worn` → `siala_max_dex` (`Loader.Gear.worn!/3`): версия, которой
  # двойник принадлежит, называется самой записью, а не выводится кодом.
  @resistance_siala_bonus "siala_bonus"
  @resistance_siala_max_total "siala_max_total"
  @resistance_siala_layer_field "siala_values_apply_to_ruleset"

  defp resistance_apply_siala_twin(base, amount, name, dictionaries) do
    has_bonus? = Map.has_key?(amount, @resistance_siala_bonus)
    has_max_total? = Map.has_key?(amount, @resistance_siala_max_total)
    declared = amount[@resistance_siala_layer_field]

    cond do
      (has_bonus? or has_max_total?) and not is_binary(declared) ->
        raise "#{@resistance_file}: #{name} states #{@resistance_siala_bonus}/" <>
                "#{@resistance_siala_max_total} and no `#{@resistance_siala_layer_field}` — " <>
                "values no ruleset will ever read look exactly like values that are being read"

      has_bonus? != has_max_total? ->
        raise "#{@resistance_file}: #{name} states one of #{@resistance_siala_bonus}/" <>
                "#{@resistance_siala_max_total} without the other — half a twin is worse than " <>
                "none of it"

      has_bonus? and Map.get(dictionaries, :version) == declared ->
        %{
          base
          | bonus: resistance_number!(name, amount[@resistance_siala_bonus]),
            max_total: resistance_number!(name, amount[@resistance_siala_max_total])
        }
  ...
  # ⚠ The two statuses are resolved **here**, not by the caller, and the record
  # says which ruleset the second one belongs to — the same device the amount
  # twin uses (`siala_values_apply_to_ruleset`) and for the same reason: without
  # it the rules core would have to know a ruleset's name to choose between two
  # statuses, and a shard's name in `rules/` is exactly what CLAUDE.md §5
  # forbids. `assumed?` is what the build prints a caveat off
  # (`Rules.Resistances.gaps/4`): verbatim on both Fandom pages for vanilla,
  # silence on all five Siala pages for `siala_41`.
  defp resistance_superseded_by!(entry, name, dictionaries) do
    case entry["superseded_by"] do
      nil ->
        nil

      %{"feat" => feat} = m ->
        %{
          feat: BonusMarkup.id!(@resistance_file, name, feat, dictionaries, :feat),
          status: m["status"],
          status_siala: m["status_siala"],
          assumed?: resistance_supersede_status!(m, name, dictionaries) != "verified"
        }

      other ->
        raise "#{@resistance_file}: #{name} states superseded_by #{inspect(other)}, expected a " <>
                "map with a `feat`"
    end
  end

  @resistance_siala_status "status_siala"
  @resistance_siala_status_layer "status_siala_applies_to_ruleset"

  defp resistance_supersede_status!(m, name, dictionaries) do
    twin = Map.get(m, @resistance_siala_status)
    declared = m[@resistance_siala_status_layer]

    cond do
      not is_nil(twin) and not is_binary(declared) ->
        raise "#{@resistance_file}: #{name} states #{@resistance_siala_status} and no " <>
                "`#{@resistance_siala_status_layer}` — a status no ruleset will ever read " <>
                "looks exactly like a status that is being read"

      not is_nil(twin) and Map.get(dictionaries, :version) == declared ->
        twin
  ...
  @doc """
  The seven elements the «Резисты» totals section prints (задача 3.209, п. 5).

  Wider than the feats' own choice domain (`vanilla/energy_types.json`, five
  values — `resist_energy`/`epic_energy_resistance` are parametrised by it):
  the racial and weapon-type effect (Dwarf, axes) additionally absorbs negative
  and positive energy, which no feat lets a player pick. Magical and divine
  damage are named excluded, not merely unmentioned — see the file's own
  `totals_energy_types.excluded`.

  `domain_files` is `raw.domain_files` — read once at the top of `load!/1` and
  passed through here rather than the resolved `choice_domains/3` output,
  because this runs well before that table exists and the two describe the
  same five ids either way.
  """
  @spec resistance_energy_types(map() | :missing, map()) :: [map()]
  def resistance_energy_types(:missing, _domain_files), do: []

  def resistance_energy_types(%{"totals_energy_types" => %{"types" => types}}, domain_files)
      when is_list(types) do
    records = Enum.map(types, &resistance_energy_type_entry!/1)
    verify_resistance_energy_types_cover_domain!(records, domain_files)
    records
  end

  def resistance_energy_types(_other, _domain_files), do: []

  @doc """
  The damage types the «Резисты» section deliberately does **not** show, with
  the names the game log prints for each (задача 3.210, п. 5).

  Three entries, and the point of the list is that they are **not** the same
  answer as «мы такого имени не знаем»: physical absorption has a receiver and
  is left out by the owner's decision («поглощение стихийного урона, без
  физического»), magical and divine are not absorbed by the game at all. So a
  `Damage Resistance (Bludgeoning) 10` line off `.билд+` belongs in `not_ours`
  with a sentence, while a name nobody has ever seen stays `unresolved` — the
  server's to-do list rather than ours (`Rules.GearImport`).

  ⚠ `verdict` is what the two cases are told apart by, and the word is closed:
  `decided` (ours by receiver, out by decision) and `not_absorbed` (out by the
  game). A third word fails the build rather than quietly reading as one of
  the two.

  ⚠ `log_names` is normalised the same way `Rules.GearImport` normalises every
  other printed name (trim, downcase, collapse whitespace), so the caller
  compares like with like and no second normaliser exists.
  """
  @resistance_excluded_verdicts %{"decided" => :decided, "not_absorbed" => :not_absorbed}

  @spec resistance_excluded_types(map() | :missing) :: [map()]
  def resistance_excluded_types(:missing), do: []

  def resistance_excluded_types(%{"totals_energy_types" => %{"excluded" => excluded}})
      when is_list(excluded) do
    Enum.map(excluded, &resistance_excluded_entry!/1)
  end

  def resistance_excluded_types(_other), do: []

  defp resistance_excluded_entry!(entry) do
    id =
      case entry["id"] do
        nil -> raise "#{@resistance_file}: totals_energy_types.excluded has an entry with no id"
        value -> atom(value)
      end

    verdict =
      Map.get(@resistance_excluded_verdicts, entry["verdict"]) ||
        raise "#{@resistance_file}: totals_energy_types.excluded entry #{id} states verdict " <>
                "#{inspect(entry["verdict"])}, and the rules core knows only " <>
                "#{inspect(Map.keys(@resistance_excluded_verdicts))}"

    names = entry["log_names"] || []

    unless is_list(names) and Enum.all?(names, &is_binary/1) do
      raise "#{@resistance_file}: totals_energy_types.excluded entry #{id} states log_names " <>
              "#{inspect(names)}, expected a list of strings (empty means «the log never " <>
              "prints this», which is a statement and not an omission)"
    end

    %{id: id, verdict: verdict, log_names: Enum.map(names, &normalize_printed_name/1)}
  end

  # Одна нормализация на весь проект (`Rules.GearImport` сверяет базовый тип
  # предмета ровно так же): обрезать, опустить регистр, свернуть пробелы.
  defp normalize_printed_name(text),
    do: text |> String.trim() |> String.downcase() |> String.replace(~r/\s+/u, " ")

  defp resistance_energy_type_entry!(entry) do
    id =
      case entry["id"] do
        nil -> raise "#{@resistance_file}: totals_energy_types has an entry with no id"
        value -> atom(value)
      end

    en =
      entry["en"] ||
        raise "#{@resistance_file}: totals_energy_types entry #{id} has no `en`"

    %{id: id, en: en, granted_by_feats: entry["granted_by_feats"] == true}
  end

  # Sторож: the five elements marked `granted_by_feats: true` here must be
  # EXACTLY the five `vanilla/energy_types.json` names, so a sixth Fandom
  # damage type would fail the build instead of the two lists silently
  # disagreeing about what a player can pick a feat for.
  defp verify_resistance_energy_types_cover_domain!(records, domain_files) do
    ours = records |> Enum.filter(& &1.granted_by_feats) |> MapSet.new(& &1.id)

    domain =
      domain_files
      |> Map.get(:energy_types, %{})
      |> Map.get("types", [])
      |> MapSet.new(&atom(&1["id"]))

    if domain != MapSet.new() and ours != domain do
      raise "#{@resistance_file}: totals_energy_types marks #{inspect(MapSet.to_list(ours))} as " <>
              "granted_by_feats, and vanilla/energy_types.json names #{inspect(MapSet.to_list(domain))} " <>
              "— the two must name the same five, or a new energy type would miss one list silently"
    end
