defmodule BuildCalculator.GameLog do
  ...
  **without** gear. Neither is the point buy: Dan measured that a level-40
  ...
  @typedoc """
  One property line off `=== Equipped: <name> ===` (task 3.187), already
  classified — same shape `BuildCalculator.Rules.GearImport.property()`
  wants, minus the one thing this module cannot finish on its own: a `Bonus
  Feat`'s own parenthesised argument (`Epic Spell Focus (Evocation)`) needs
  `ruleset.choice_domains`, which only `BuildCalculatorWeb.Builder.ChoiceIndex`
  (a web-layer module) knows how to search by free text — the exact same
  split `resolve_feats/4` above already draws for the `FEATS:` ladder, one
  layer over. Such an entry's `param` carries a `{:pending_choice, feat_id,
  argument}` marker instead of the final `{feat_id, choice}` pair;
  `BuildCalculatorWeb.Builder.GameLogImport` is the one place that resolves
  it, right before handing the whole list to `GearImport.sum/2`.

  ⚠ `rank` is there only where the game printed one — `Bonus Feat (Epic
  Toughness II)` — and it is what tells one take from another off a repeatable
  feat (task 3.204). Kept as the text the tokenizer captured, never as a number.
  """
  @type equip_property :: %{
          required(:kind) => atom(),
  ...
  @typedoc """
  One item off `=== Equipped: <name> ===` — a slot, its name, its properties.

  Four OPTIONAL keys arrived with the server-side additions the calculator
  asked for in `docs/bild_plus_for_admins.md` (13.09.2026 task 3.206, then
  18.09.2026 task 3.213), and each is present only when the log printed it,
  so an older log parses to exactly the map it did before:

    * `base_type` — the item's base type as the server prints it in parens
      after the name (`Ург Шак (Warhammer)`, `Нарукавник Ратника (Bracer)`),
      text as printed; and `base_item` — the `baseitems.2da` row the server
      prints beside it (`[BaseItem:5]`, `[BaseItem:78]`). Both on the two hand
      slots since 13.09.2026 and on `ARMS` since 18.09.2026;
    * `base_ac` — the armour's own base armour class, which the server prints
      on `CHEST` as `[BaseAC:8]` since 18.09.2026 (`NWNX_Item_
      GetBaseArmorClass`). A NUMBER, not a type name: which armour record it
      names is `Rules.GearImport`'s business, off `gear.worn`;
    * `craft?: true` — the name carried the `[CRAFT]` mark, i.e. the server
      calls this item a craft (personalised) one. Absent, not `false`, when
      there is no mark — so a log that predates the mark says nothing, rather
      than "not craft", about every item.

  ⚠ `base_ac: 0` is a LEGAL value and means «this is clothing / a robe», the
  `gear.worn` row whose base is zero — not «the server said nothing». Only the
  absence of the key means that, and the two answers differ in the number
  on the player's screen: zero base leaves a monk's AC bonuses alone, an
  unstated base leaves the armour unset altogether.
  """
  @type equip_item :: %{
          required(:slot) => atom(),
          required(:name) => String.t(),
          required(:properties) => [equip_property()],
  ...
  @typedoc """
  The counts the second-generation `=== Equipped` section prints in its own
  header (task 3.206): `MINI SET PIECES: 7` and `CRAFT ITEMS: 7`. Read as
  printed, never derived from the items below them — `Rules.GearImport` is
  what says what each means and reconciles it with the per-item marks.
  Empty on a first-generation log.
  """
  @type equipment_counts :: %{
  ...
  defp empty_scan do
    %{
      ...
      current_level: nil,
      level_entries: [],
      # `.билд+` (task 3.187) is the same dump plus one trailing section,
      # `=== Equipped: <name> ===` — the marker line itself still carries the
      # `[CHAT WINDOW TEXT]` prefix every other client echo does (so it would
      # vanish into the boilerplate-drop clause below if not caught first),
      # but every line *after* it — a slot header, a property — does not.
      # `equipping?` is what tells the two apart once the marker has been seen.
      equipping?: false,
      equipment_raw: [],
      problems: []
    }
  ...
  defp scan_line(scan, raw_line) do
    ...
    cond do
      ...
      # Caught ahead of the boilerplate-drop clause on purpose — this marker
      # line starts with the same `[CHAT WINDOW TEXT] [timestamp]` prefix
      # every other client echo does, and would silently disappear into that
      # clause otherwise.
      Regex.match?(~r/===\s*Equipped:.*===\s*$/u, trimmed) ->
        %{scan | equipping?: true}

      # Everything from here to the end of the dump is the equipped section's
      # own body — a slot header or a property line, never anything this scan
      # otherwise knows how to name. Read raw here, classified in
      # `put_equipment/3` once the whole dump (and its ruleset) is in hand.
      scan.equipping? ->
        %{scan | equipment_raw: scan.equipment_raw ++ [line]}
  ...
  # --------------------------------------------------------------- equipment --
  #
  # `=== Equipped: <name> ===` (task 3.187) — the `.билд+` command's own extra
  # section, eleven slots of `[SLOT] Item name` followed by that item's own
  # `[n] Property (param) value` lines. Read into `equip_item()`s that are
  # ALREADY the shape `Rules.GearImport.sum/2` wants for every kind but one —
  # see `equip_property()`'s own typedoc for the one exception (`Bonus Feat`'s
  # own argument) and why it cannot be finished here.

  defp put_equipment(struct, %{equipment_raw: []}, _ruleset), do: struct

  defp put_equipment(struct, %{equipment_raw: lines}, ruleset) do
    {blocks, counts, block_problems} = equipment_blocks(lines)
    slot_table = equip_slot_dictionary(ruleset)
    feat_dictionary = feat_dictionary(ruleset)
    skill_dictionary = skill_dictionary(ruleset)

    {items, item_problems} =
      Enum.reduce(blocks, {[], []}, fn block, {items, problems} ->
        case Map.fetch(slot_table, block.token) do
          {:ok, slot} ->
            properties =
              for {raw, body} <- block.properties do
                body
                |> classify_equipment_property(ruleset, feat_dictionary, skill_dictionary)
                |> Map.put(:raw, raw)
              end

            item =
              %{slot: slot, name: block.name, properties: properties}
              |> Map.merge(block.extras)

            {items ++ [item], problems}

          :error ->
            {items, problems ++ [{:unresolved_equipment_slot, block.token}]}
        end
      end)

    struct
    |> Map.put(:equipment, items)
    |> Map.put(:equipment_counts, counts)
    |> add_problems(block_problems ++ item_problems)
  end

  # Slot header (`[HEAD] Шлем …`) and property (`  [1] Cast Spell … 13`) lines
  # look alike — both open with a bracketed run — so the two are told apart by
  # what is *inside* the brackets: digits mean a property, letters mean a new
  # item. Nothing here reads a ruleset yet; that happens once every line of
  # the dump is already grouped into items, in `put_equipment/3` above.
  #
  # The second generation of the command (13.09.2026, task 3.206) adds a
  # third shape ABOVE the first slot — `MINI SET PIECES: 7` / `CRAFT ITEMS: 7`,
  # the server's own two counts — and it is told apart from the other two by
  # opening with a bare capitalised word rather than a bracket.
  @equip_slot_line ~r/^\[([A-ZА-Я]+)\]\s+(.+)$/u
  @equip_property_line ~r/^\[\d+\]\s+(.+)$/u
  @equip_count_line ~r/^([A-Z][A-Z ]*[A-Z]):\s*(\d+)$/u

  # The two header counts this reader knows, by the exact label the server
  # prints. A label outside this table is NAMED (`{:unknown_equipment_count,
  # label}`) rather than dropped as an unrecognised line: a count the server
  # starts printing tomorrow is a fact about the character, and the player
  # should hear that we saw it and did not read it.
  @equip_count_labels %{
    "MINI SET PIECES" => :mini_set_pieces,
    "CRAFT ITEMS" => :craft_items
  }

  defp equipment_blocks(lines) do
    {blocks, counts, problems} =
      Enum.reduce(lines, {[], %{}, []}, fn raw, {blocks, counts, problems} ->
        ...
        cond do
          ...
          m = Regex.run(@equip_property_line, trimmed) ->
            [_, body] = m

            case blocks do
              [] ->
                {blocks, counts, problems ++ [{:equipment_property_before_slot, trimmed}]}

              _ ->
                block = List.last(blocks)
                block = %{block | properties: block.properties ++ [{trimmed, body}]}
                {List.replace_at(blocks, -1, block), counts, problems}
            end

          m = Regex.run(@equip_slot_line, trimmed) ->
            [_, token, rest] = m
            {name, extras} = equip_split_item_name(rest)

            {blocks ++ [%{token: token, name: name, properties: [], extras: extras}], counts,
             problems}

          m = Regex.run(@equip_count_line, trimmed) ->
            [_, label, digits] = m

            case Map.fetch(@equip_count_labels, label) do
              {:ok, key} ->
                {blocks, Map.put(counts, key, String.to_integer(digits)), problems}

              :error ->
                {blocks, counts, problems ++ [{:unknown_equipment_count, label}]}
            end

          true ->
            {blocks, counts, problems ++ [{:unrecognized_equipment_line, trimmed}]}
        end
      end)
  ...
  # What trails an item's name on a slot line, peeled off the END one token at
  # a time so the marks may come in any order:
  #
  #     Ург Шак (Warhammer) [BaseItem:5]           → base_type + base_item
  #     Перчатки (Gauntlet) [BaseItem:36] [CRAFT]  → all three at once
  #     Нагрудник Призрака [BaseAC:8]              → base_ac
  #     Сапоги [CRAFT]                             → craft?
  #
  # ⚠ The parenthesised base type is read ONLY when a `[BaseItem:n]` stood
  # beside it: an item name may carry parens of its own, and without the row
  # number next to it there is nothing that says the group is a type and not
  # part of the name. `base_item` is kept as printed for the same reason
  # `rank` is kept on a feat — provenance, never a lookup key: the calculator
  # matches the printed TYPE NAME against its own tables (`Rules.GearImport`),
  # and the row number rides along so a reader can check it against
  # `priv/hak/2da/baseitems.2da` by hand. ⚠ The two do NOT always agree: row 36
  # is labelled `gloves` in the hak and printed `Gauntlet` by the server, so the
  # name is the key and the row is the receipt, never the other way round.
  @equip_craft_mark ~r/\s*\[CRAFT\]$/u
  @equip_base_item_mark ~r/\s*\[BaseItem:(\d+)\]$/u
  @equip_base_ac_mark ~r/\s*\[BaseAC:(\d+)\]$/u
  @equip_base_type ~r/^(.*\S)\s+\(([^()]+)\)$/u

  defp equip_split_item_name(text), do: equip_peel_marks(String.trim(text), %{})

  defp equip_peel_marks(text, extras) do
    cond do
      Regex.match?(@equip_craft_mark, text) ->
        text
        |> String.replace(@equip_craft_mark, "")
        |> equip_peel_marks(Map.put(extras, :craft?, true))

      m = Regex.run(@equip_base_item_mark, text) ->
        [_, digits] = m

        text
        |> String.replace(@equip_base_item_mark, "")
        |> equip_peel_marks(Map.put(extras, :base_item, String.to_integer(digits)))

      m = Regex.run(@equip_base_ac_mark, text) ->
        [_, digits] = m

        text
        |> String.replace(@equip_base_ac_mark, "")
        |> equip_peel_marks(Map.put(extras, :base_ac, String.to_integer(digits)))

      Map.has_key?(extras, :base_item) and not Map.has_key?(extras, :base_type) ->
        case Regex.run(@equip_base_type, text) do
          [_, name, type] -> {name, Map.put(extras, :base_type, String.trim(type))}
          nil -> {text, extras}
        end
  ...
  # `HEAD` → `:head` &c, off the same table `Rules.GearImport` reads AC types
  # from (`ruleset.gear.item_slot_ac_types`, `log` field) — one slot list, not
  # a second one typed out for this reader.
  defp equip_slot_dictionary(ruleset) do
    for row <- ruleset.gear.item_slot_ac_types, into: %{}, do: {row.log, row.id}
  end
  ...
  @equip_save_names %{"fortitude" => :fort, "reflex" => :ref, "will" => :will}

  # Names this reader knows are not ours at all (CLAUDE.md §9 — a gap is a
  # hole in our ANSWER, not in our knowledge): kept as a closed table purely
  # so `not_ours`'s own reason can name the property rather than fall back to
  # the raw line, exactly the same courtesy `name_of/1` in `Rules.GearImport`
  # already extends a resolved ability or skill. A name outside this table
  # still reaches `not_ours` — `Rules.GearImport.read_property/4`'s `:other`
  # clause reads `raw` when `param` is absent — it just does so unlabelled.
  @equip_other_names %{
    "Cast Spell" => :cast_spell,
    "Damage Bonus" => :damage_bonus,
    # ⚠️ `"Damage Resistance"` уехало отсюда 18.09.2026 (задача 3.210): у него
    # теперь СВОЙ вид (`:damage_resistance` с разобранной стихией в `param`),
    # и `classify_named_property/4` ловит имя раньше этой таблицы. Запись
    # оставлять было нельзя — мёртвый ключ в таблице «эти механики не наши»
    # утверждал бы обратное тому, что делает код.
    # ⚠️ `"Damage Reduction"` рядом — ДРУГАЯ механика (физическая, варварская),
    # и она не наша по-прежнему.
    "Damage Reduction" => :damage_reduction,
    "Damage Vulnerability" => :damage_vulnerability,
    "Regeneration" => :regeneration,
    "On Hit Cast Spell" => :on_hit_cast_spell,
    "Keen" => :keen,
    "Light" => :light,
    "Visual Effect" => :visual_effect,
    "Arcane Spell Failure" => :arcane_spell_failure,
    "Bonus Spell Slot of Level" => :bonus_spell_slot_of_level,
    "Base Item Weight Reduction" => :base_item_weight_reduction,
    "Immunity" => :immunity,
    "Use Limitation" => :use_limitation,
    "Use Item" => :use_item
  }

  # 🔴 Третье поколение печати (18.09.2026, задача 3.213): `AC Bonus` больше
  # НЕ печатает параметра — `AC Bonus 5` там, где прежде стояло
  # `AC Bonus (0) 5` или `AC Bonus (65535) 5`. Обе формы читает ОДНА регулярка,
  # потому что сохранённые логи прежних поколений у игроков остались, а импорт
  # обязан открывать их так же (форма под тестом фрагментами текста).
  #
  # 🔴 Цена промаха измерена: без этой строки `AC Bonus 5` не совпадал ни с одним
  # именем таблицы и уходил в `not_ours` — свод по четырём логам 18.09.2026 давал
  # 128 / 148 / 2 вместо 142 / 134 / 2, а AC Хнюпиуса 38 вместо 58. Формально
  # «ни одна строка не потеряна» держалось (строка лежала в ведре), по сути
  # игрок получал импорт без вещевого AC и без единой оговорки.
  #
  # ⚠️ ОБОБЩАТЬ НЕЛЬЗЯ: остальные свойства параметр сохранили (`Attack Bonus (0)
  # 5`, `Regeneration (65535) 8`, `Quality (65535) 10`), и «имя без скобок»
  # у них означало бы форму, которой сервер не печатает. Поэтому исключение
  # названо одним свойством, а не механизмом.
  @equip_ac_bonus ~r/^AC Bonus\s*(?:\(\s*\d+\s*\)\s*)?(?<value>-?\d+)\s*$/u

  # One property line's body (everything after `[n] `) → the shape
  # `Rules.GearImport.property()` wants. Every branch is a closed, ATTESTED
  # name off `test/fixtures/game_logs_plus/README.md`'s own list of "our"
  # and "not our" property names — nothing here is guessed at, and a name
  # this table does not carry falls through to the generic `:other` clause
  # at the bottom, never lost.
  defp classify_equipment_property(body, ruleset, feat_dictionary, skill_dictionary) do
    case Regex.named_captures(@equip_ac_bonus, String.trim(body)) do
      %{"value" => digits} ->
        %{kind: :ac_bonus, value: String.to_integer(digits)}

      nil ->
        classify_named_property(body, ruleset, feat_dictionary, skill_dictionary)
    end
  end

  defp classify_named_property(body, ruleset, feat_dictionary, skill_dictionary) do
    {name, rest} = equip_split_name(body)
    {param_text, tail} = equip_take_param(rest)

    case name do
      "Ability Bonus" ->
        %{
          kind: :ability_bonus,
          param: equip_atom(@equip_ability_names, param_text),
          value: equip_value(tail)
        }

      # Ветка ОСТАЁТСЯ сверх регулярки выше: та читает две наблюдённые формы,
      # а эта — любую третью (`AC Bonus (Foo) 5`). Параметр здесь не читается
      # ни в одной из форм: он ни разу не оказался типом AC — один и тот же
      # слот встречался и с `(0)`, и с `(65535)`, — а с 18.09.2026 сервер его
      # и не печатает вовсе.
      "AC Bonus" ->
        %{kind: :ac_bonus, value: equip_value(tail)}

      "Attack Bonus" ->
        %{kind: :attack_bonus, value: equip_value(tail)}

      "Enhancement Bonus" ->
        %{kind: :enhancement_bonus, value: equip_value(tail)}

      "Skill Bonus" ->
        skill = equip_atom(skill_dictionary, param_text)
        %{kind: :skill_bonus, param: skill, value: equip_value(tail)}

      "Saving Throw Bonus: Specific" ->
        save = equip_atom(@equip_save_names, param_text)
        %{kind: :saving_throw_specific, param: save, value: equip_value(tail)}

      "Saving Throw Bonus" ->
        if normalize(param_text || "") == "universal" do
          %{kind: :saving_throw_universal, value: equip_value(tail)}
        else
          %{kind: :other, param: :saving_throw_bonus}
        end

      "Bonus Feat" ->
        equip_feat_property(param_text, ruleset, feat_dictionary)

      # `Use Item (Mini Set)` — a piece of SOME set; since 13.09.2026 the
      # server prints WHICH one beside it, `[SetID:22]` (task 3.206), and the
      # group id travels as `param`. Absent on a first-generation line, and
      # that absence is what `Rules.GearImport` reads as «набор не назван».
      "Use Item" ->
        if normalize(param_text || "") == "mini set" do
          case Regex.run(~r/\[SetID:(\d+)\]/u, tail || "") do
            [_, digits] -> %{kind: :mini_set, param: String.to_integer(digits)}
            nil -> %{kind: :mini_set}
          end
        else
          %{kind: :other, param: :use_item}
        end

      "Quality" ->
        %{kind: :quality, value: equip_value(tail)}

      "Spell Resistance" ->
        %{kind: :spell_resistance, value: equip_value(tail)}

      # `Damage Resistance (Fire) 15` — поглощение стихийного урона (задача
      # 3.210). Стихия разрешается против ТОГО ЖЕ словаря, который называет
      # стихии секции «Резисты» (`ruleset.resistance_energy_types`), а не против
      # второго списка здесь: имена в логе — те же английские, что в словаре
      # (`Fire` = `Fire`, `Positive Energy` = `Positive energy`, регистр
      # снимает `normalize/1`).
      #
      # ⚠️ Имя, которого в словаре нет, едет как `{:unresolved, text}` — ровно
      # как неразобранная характеристика или навык, — и решает его судьбу свод
      # (`Rules.GearImport`): физическое поглощение он назовёт решением
      # владельца, божественное — не нашей механикой, а незнакомое оставит
      # в списке доработок сервера. Здесь об этом знать нечего.
      "Damage Resistance" ->
        %{
          kind: :damage_resistance,
          param: equip_atom(resistance_dictionary(ruleset), param_text),
          value: equip_value(tail)
        }

      other ->
        equip_other_property(other)
    end
  end

  # `"Immunity: Damage Type"` and `"Use Limitation: Class"` both name a
  # mechanic by their part before the colon — the part after is the SHAPE of
  # what got granted (a damage type, a class), not a second mechanic.
  defp equip_other_property(name) do
    base = name |> String.split(":") |> List.first() |> String.trim()

    case Map.fetch(@equip_other_names, base) do
      {:ok, atom} -> %{kind: :other, param: atom}
      :error -> %{kind: :other}
    end
  end

  # `"Ability Bonus (Constitution) 2"` → `{"Ability Bonus", "(Constitution) 2"}`.
  # Every property this reader knows carries at least one paren (checked
  # against all four fixtures), so a body with none at all is not a shape any
  # of the branches above expect — it falls through to `equip_other_property/1`
  # with an empty `rest`, same as any other unread name.
  defp equip_split_name(body) do
    case String.split(body, "(", parts: 2) do
      [name, tail] -> {String.trim(name), "(" <> tail}
      [name] -> {String.trim(name), ""}
    end
  end

  # Balanced-paren read of the FIRST parenthesised group in `rest`, plus
  # whatever trails it — `"(Epic Spell Focus (Evocation)) "` →
  # `{"Epic Spell Focus (Evocation)", " "}`. A single-level reader
  # (`Regex.run(~r/^\(([^)]*)\)/u, …)`, the way `FeatListTokenizer.
  # read_argument/1` reads a feat's OWN parenthesised argument) is not enough
  # here — `Bonus Feat (Epic Spell Focus (Evocation))` nests one level deeper
  # than that, in the wild, on `moxie.log`.
  defp equip_take_param("(" <> rest), do: equip_paren(rest, 1, [])
  defp equip_take_param(rest), do: {nil, rest}

  defp equip_paren(<<")", rest::binary>>, 1, acc),
    do: {acc |> Enum.reverse() |> IO.iodata_to_binary(), rest}

  defp equip_paren(<<")", rest::binary>>, depth, acc),
    do: equip_paren(rest, depth - 1, [")" | acc])

  defp equip_paren(<<"(", rest::binary>>, depth, acc),
    do: equip_paren(rest, depth + 1, ["(" | acc])

  defp equip_paren(<<ch::utf8, rest::binary>>, depth, acc),
    do: equip_paren(rest, depth, [<<ch::utf8>> | acc])

  defp equip_paren(<<>>, _depth, acc), do: {acc |> Enum.reverse() |> IO.iodata_to_binary(), ""}

  # Leading integer off whatever trails a property's own `(…)` — `" 6"` → `6`,
  # `" 4 [3]"` → `4` (`Light`'s own trailing bracket, never read past), `"
  # 50%"` → `50` (kept for symmetry, unused: no `:other` kind ever reads
  # `value`). `nil` when the property prints no number at all — never a
  # silent `0`, the same distinction `Rules.GearImport.number/1` draws.
  defp equip_value(tail) do
    case Regex.run(~r/(-?\d+)/u, tail || "") do
      [_, digits] -> String.to_integer(digits)
      nil -> nil
    end
  end

  # An ability, skill or save name → its atom, off a closed table — or
  # `{:unresolved, text}`, the same pass-through shape a feat name that
  # matches nothing in its own dictionary gets below. `Rules.GearImport`'s own
  # `known/3` already has a branch for a non-atom `param`
  # (`{:error, {refusal, other}}`), so this need not raise or guess.
  defp equip_atom(table, text) do
    case Map.get(table, normalize(text || "")) do
      nil -> {:unresolved, text}
      atom -> atom
    end
  end

  # A `Bonus Feat`'s own inner text, through the SAME tokenizer and the SAME
  # dictionary the `FEATS:` ladder line already reads — one word list, not a
  # second one for this section (CLAUDE.md §9). `Epic Toughness II` and
  # `Epic Toughness III` both read to the bare id, and the roman numeral travels
  # **beside** it as `rank`.
  #
  # 🔴 Задача 3.204, и это пересмотр: здесь стояло «the roman-numeral rank the
  # tokenizer captures is never carried into `param` — CLAUDE.md §6 already
  # decided instance numbers of a repeatable feat are not distinguished». Решение
  # §6 не отменено — номера по-прежнему не различаются В ЧИСЛАХ, складываются
  # просто взятия, — но чтобы сложить ДВА взятия, надо сначала увидеть, что
  # строки две, а не одна повторённая. Ровно это и печатает ступень: сапоги
  # Хнюпиуса несут `II` и `III` (два взятия), пояс — ещё раз `II` (тот же фит,
  # третьим взятием не становится; игра печатает 1503 HP = ровно два).
  #
  # ⚠️ Ключ `rank` появляется ТОЛЬКО когда ступень напечатана: свойство без неё
  # кодируется ровно так же, как до этой задачи, — то есть ни одна фикстура
  # части A, кроме двух строк `Epic Toughness`, не сдвинулась ни на байт.
  # ⚠️ Строкой, а не числом: `Uncanny Dodge VI+` — тоже законная ступень
  # (`FeatListTokenizer`), и превращать её в 6 значило бы терять «+».
  defp equip_feat_property(nil, _ruleset, _feat_dictionary),
    do: %{kind: :bonus_feat, param: {:unresolved, ""}}

  defp equip_feat_property(text, ruleset, feat_dictionary) do
    normalized = text |> normalize() |> String.replace(~r/\s+/u, " ")

    entry =
      case FeatListTokenizer.tokenize(normalized, feat_dictionary) do
        [first | _] -> first
        [] -> %{value: nil, raw: normalized, argument: nil, rank: nil}
      end

    case classify_feat_entry(entry) do
      %{kind: :feat, id: id, argument: argument, rank: rank} ->
        id |> equip_feat_with_id(argument, text, ruleset) |> put_rank(rank)

      _not_a_feat ->
        %{kind: :bonus_feat, param: {:unresolved, String.trim(text)}}
    end
  end

  defp put_rank(property, nil), do: property
  defp put_rank(property, rank), do: Map.put(property, :rank, rank)

  # `Rules.feat_choice_domain/2` — a pure ruleset read, safe to call straight
  # from this core module — is what decides the shape, in three ways:
  #
  #   * no domain, no argument (`Cleave`) — a bare grant, the ordinary case;
  #   * a domain (`Epic Spell Focus (Evocation)`) — matching free text against
  #     it needs `BuildCalculatorWeb.Builder.ChoiceIndex`, a web-layer module
  #     this core one may not call, so the id and the raw argument travel on
  #     as `{:pending_choice, …}` for `GameLogImport` to finish;
  #   * no domain, BUT an argument (`Sneak Attack (+4d6)`, real players' own
  #     rings — `test/fixtures/game_logs_plus/hnyupius.log`) — the one case
  #     that is NOT the same as the `FEATS:` ladder's own reading of the same
  #     text. There, a class-grant line with no domain drops its argument
  #     silently on purpose (`GameLogImport.grant/8`'s own comment: the class
  #     table already re-derives the grant, so the printed "(+4d6)" carries
  #     nothing the build needs). A GEAR line is not that: the item is the
  #     ONLY source naming this feat, `sneak_attack`'s own domain is `nil`
  #     because the number is a damage die rather than a choice, and turning
  #     it into a bare `gear.feats` entry would silently claim the wearer
  #     owns the Rogue class feature outright — true of nobody in
  #     `hnyupius.log` (10 Fighter / 23 Dwarven Defender / 7 Weapon Master).
  #     Kept unresolved instead, same as a name the dictionary never matched
  #     at all — CLAUDE.md §9's own rule that a gap is a hole in the ANSWER,
  #     not license to guess a plausible one.
  defp equip_feat_with_id(id, nil, _text, _ruleset), do: %{kind: :bonus_feat, param: id}

  defp equip_feat_with_id(id, argument, text, ruleset) do
    case BuildCalculator.Rules.feat_choice_domain(id, ruleset) do
      nil -> %{kind: :bonus_feat, param: {:unresolved, String.trim(text)}}
      _domain -> %{kind: :bonus_feat, param: {:pending_choice, id, argument}}
    end
  end
  ...
  # Стихии, которые печатает секция «Резисты», по их англоязычным именам
  # (задача 3.210). Один словарь на разбор лога и на расчёт: список лежит
  # в `priv/rules/vanilla/feat_resistance_bonuses.json` → `totals_energy_types`
  # и приезжает в ruleset готовым.
  defp resistance_dictionary(ruleset) do
