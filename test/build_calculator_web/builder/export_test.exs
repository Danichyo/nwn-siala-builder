defmodule BuildCalculatorWeb.Builder.ExportTest do
  ...
  describe "мини-сеты в текст НЕ идут — решение, а не умолчание (задача 3.185)" do
    defp with_mini_sets(ruleset, counts) do
      build =
        Build.new(
          ruleset_version: ruleset.version,
          race: :half_elf,
          ...
          gear:
            BuildCalculator.Rules.Gear.new(
              weapon: :longbow,
              feats: [:siala_ranged_proficiency],
              mini_sets: counts
            )
        )
    ...
    # 🔴 Формат гильдии — соглашение комьюнити, и поля под экипировку в нём
    # нет НИ ОДНОГО: ни оружие, ни надетое, ни фиты с вещи не печатались
    # здесь никогда. Куски — такой же ввод про экипировку, и первой строкой
    # про вещи в этом формате им становиться незачем (см. moduledoc
    # `Export`). Числа, которые они двигают, уже стоят в `AB`/`AC`/`SKILLS`.
    #
    # ⚠️ Здесь стояло `assert with_sets == without`, потом (3.186) — «отличается
    # ТОЛЬКО строкой Hitpoints», потому что `stats.hp` понёс процент к HP
    # за куски. С 12.09.2026 снова `==`, и уже не совпадением капов, а правилом
    # Dan: «если мини-сетов в экспорте вообще нет, то ХП без их воздействия» —
    # `Export.hitpoints/1` вычитает терм `gear_bonus`. Билд подобран так, что
    # оба оружейных бонуса упёрты в кап и без кусков — иначе `AB` разошёлся бы
    # по своей причине, и проверка перестала бы быть про HP.
    test "канонический блок билда С кусками и без них ОДИНАКОВ — HP без процента за куски" do
      ruleset = Data.ruleset!("siala_41")
      {plain, plain_stats} = with_mini_sets(ruleset, [])
      {sets, sets_stats} = with_mini_sets(ruleset, [5, 5])

      # Положительный контроль: куски у этого билда действительно записаны
      # и действительно двигают HP — иначе проверка ниже зеленела бы
      # на пустом месте.
      assert sets.gear.mini_sets == [5, 5]
      assert sets_stats.mini_set_pieces == 10
      assert sets_stats.hp == plain_stats.hp + div(plain_stats.hp * 105, 100)

      [with_sets, _] =
        String.split(Export.text(sets, ruleset, sets_stats, title: "Test"), "\n---\n")

      [without, _] =
        String.split(Export.text(plain, ruleset, plain_stats, title: "Test"), "\n---\n")

      assert with_sets == without
      assert with_sets =~ "Hitpoints: #{plain_stats.hp}"
      refute with_sets =~ "Hitpoints: #{sets_stats.hp}"
      refute with_sets =~ "ини-сет"
    end

    # ⚠️ И ПОДВАЛ у двух билдов теперь ОДИНАКОВ, хотя до 11.09.2026 расходился.
    # Здесь стояло: «подвал честно называет пробел, которого у билда без кусков
    # нет» — куски давали оговорку `{:not_modelled, :mini_set_hp_bonus}`, потому
    # что процент к HP мы не считали. Задача 3.186 его посчитала, и признание
    # ушло: подвал — наше признание в НЕпосчитанном, а не список всего, что
    # система делает. Печатать «Пробелов: 1» над посчитанным числом значило бы
    # ложную неопределённость наоборот (CLAUDE.md §6).
    #
    # 🔴 Тест оставлен, а не удалён, и проверяет теперь ОБРАТНОЕ утверждение
    # с положительным контролем: оговорки нет, а число — есть.
    test "подвал больше не признаётся в HP: процент посчитан" do
      ruleset = Data.ruleset!("siala_41")
      {plain, plain_stats} = with_mini_sets(ruleset, [])
      {sets, sets_stats} = with_mini_sets(ruleset, [5, 5])
      ...
      assert Enum.filter(sets_stats.gaps, &match?({_, :mini_set_hp_bonus}, &1)) == []
      assert sets_stats.gaps == plain_stats.gaps

      # Положительный контроль: считать было что — 105 % это больше, чем
      # удвоение «голого» числа.
      assert sets_stats.hp > plain_stats.hp

      refute with_sets =~ "Пробелов в этом билде"
      refute without =~ "Пробелов в этом билде"
      assert with_sets == without
    end

    # ⚠️ И это НЕ значит, что кусками нельзя поделиться: их переносит
    # ССЫЛКА (`BuildCalculator.Encoding`, та же задача), а ссылка стоит
    # в самом низу скачанного `.txt` (задача 3.180). Игрок, отдавший файл,
    # отдаёт вместе с ним и куски — просто кодом билда, а не строкой формата.
    test "но скачанный файл несёт их — ссылкой внизу" do
      ruleset = Data.ruleset!("siala_41")
      {build, stats} = with_mini_sets(ruleset, [5, 5])

      url = "https://example.test/b/" <> BuildCalculator.Encoding.encode(build)
      text = Export.file_text(build, ruleset, stats, url, title: "Test")

      assert text =~ url

      assert {:ok, %{build: back}} =
               BuildCalculator.Encoding.decode(
                 String.replace_prefix(url, "https://example.test/b/", "")
               )

      assert back.gear.mini_sets == [5, 5]
    end
  end
  ...
  describe "file_name/3 — имя для кнопки «скачать .txt» (задача 3.179)" do
    test "одноклассовый билд", ctx do
      build = Build.new(levels: List.duplicate(:fighter, 5))
      stats = %{class_levels: %{fighter: 5}}

      assert Export.file_name(build, ctx.ruleset, stats) == "lvl5_fighter_5.txt"
    end

    test "референсный билд Dan (CLAUDE.md §3): Fighter 10 / Dwarven Defender 23 / Weapon Master 7",
         ctx do
      build =
        Build.new(
          levels:
            List.duplicate(:fighter, 10) ++
              List.duplicate(:dwarven_defender, 23) ++
              List.duplicate(:weapon_master, 7)
        )

      stats = %{class_levels: %{fighter: 10, dwarven_defender: 23, weapon_master: 7}}

      assert Export.file_name(build, ctx.ruleset, stats) ==
               "lvl40_fighter_10_dwarven_defender_23_weapon_master_7.txt"
    end

    # "wizard" алфавитно позже "fighter" — если бы порядок был алфавитным,
    # имя выглядело бы "…_fighter_2_wizard_3…", а не так, как билд взят.
    # CLAUDE.md §3: порядок взятия классов необратимо меняет билд после
    # 20-го уровня, значит имя файла обязано быть верным тем же порядком,
    # каким собран заголовок (`header/4`), а не отсортированным.
    test "порядок классов — порядок ВЗЯТИЯ, а не алфавит", ctx do
      build = Build.new(levels: List.duplicate(:wizard, 3) ++ List.duplicate(:fighter, 2))
      stats = %{class_levels: %{wizard: 3, fighter: 2}}

      assert Export.file_name(build, ctx.ruleset, stats) == "lvl5_wizard_3_fighter_2.txt"
    end

    test "пустой билд — без класса, и без lvl0", ctx do
      build = Build.new(levels: [])
      stats = %{class_levels: %{}}

      assert Export.file_name(build, ctx.ruleset, stats) == "build.txt"
    end

    test "не зависит от ruleset — id класса общий для vanilla и siala_41" do
      build =
        Build.new(levels: List.duplicate(:fighter, 10) ++ List.duplicate(:dwarven_defender, 5))

      stats = %{class_levels: %{fighter: 10, dwarven_defender: 5}}

      vanilla = Export.file_name(build, Data.ruleset!("vanilla"), stats)
      siala = Export.file_name(build, Data.ruleset!("siala_41"), stats)

      assert vanilla == siala
      assert vanilla == "lvl15_fighter_10_dwarven_defender_5.txt"
    end

    test "санитайзер — id класса вне [a-z0-9_] никогда не даёт `/` или `..` в имени файла",
         ctx do
      # `String.to_atom/1` здесь безопасно: это ровно один фиксированный
      # атом, порождённый тестовым кодом, а не пользовательским вводом —
      # AGENTS.md запрещает именно utf8/URL-данные без границы, не горстку
      # своих же атомов теста.
      evil = String.to_atom("weird/../class")
      build = Build.new(levels: [evil, evil, :fighter])
      stats = %{class_levels: %{evil => 2, :fighter => 1}}

      name = Export.file_name(build, ctx.ruleset, stats)

      refute name =~ "/"
      refute name =~ ".."
      assert String.ends_with?(name, ".txt")
    end

    test "потолок длины — стем длиннее 120 символов уходит в короткое имя", ctx do
      # "lvl1_" (5) + 114 + "_1" (2) = 121 символ — на единицу больше потолка.
      long_id = String.to_atom(String.duplicate("a", 114))
      build = Build.new(levels: [long_id])
      stats = %{class_levels: %{long_id => 1}}

      assert Export.file_name(build, ctx.ruleset, stats) == "lvl1_build.txt"
    end

    test "потолок длины — ровно 120 символов ещё НЕ срезается", ctx do
      # "lvl1_" (5) + 113 + "_1" (2) = 120 — ровно на границе, полное имя.
      id_str = String.duplicate("a", 113)
      ok_id = String.to_atom(id_str)
      build = Build.new(levels: [ok_id])
      stats = %{class_levels: %{ok_id => 1}}

      name = Export.file_name(build, ctx.ruleset, stats)

      assert name == "lvl1_#{id_str}_1.txt"
      assert String.length(String.trim_trailing(name, ".txt")) == 120
    end

    test "реальные четыре класса умещаются далеко до потолка (~78 символов)", ctx do
      build =
        Build.new(
          levels:
            List.duplicate(:fighter, 4) ++
              List.duplicate(:purple_dragon_knight, 10) ++
              List.duplicate(:champion_of_torm, 10) ++
              List.duplicate(:arcane_archer, 11)
        )

      stats = %{
        class_levels: %{
          fighter: 4,
          purple_dragon_knight: 10,
          champion_of_torm: 10,
          arcane_archer: 11
        }
      }
      ...
      refute name == "lvl35_build.txt"
      assert String.length(String.trim_trailing(name, ".txt")) < 120
    end
  end

  # Задача 3.180 (Dan, 10.09.2026): скачиваемый файл несёт ссылку на билд
  # одной строкой внизу, попап и «скопировать» — нет. `text/4` сам по себе
  # не тронут ни одной строкой этой задачи — каждый тест ниже сверяет
  # `file_text/5`'s вывод С НИМ напрямую, а не с ожидаемой строкой,
  # переписанной руками, чтобы снимок `text/4` не мог протухнуть незаметно.
  describe "file_text/5 — ссылка на билд внизу скачиваемого файла (задача 3.180)" do
    @url "https://builder.dondryanich.ru/b/1.AbC-dEf_02"

    test "text/4's own block, byte for byte, is a PREFIX of file_text/5's output", ctx do
      popup = Export.text(ctx.build, ctx.ruleset, ctx.stats)
      file = Export.file_text(ctx.build, ctx.ruleset, ctx.stats, @url)

      assert String.starts_with?(file, popup)
    end

    test "adds exactly one blank line, then one line naming the url", ctx do
      popup = Export.text(ctx.build, ctx.ruleset, ctx.stats)
      file = Export.file_text(ctx.build, ctx.ruleset, ctx.stats, @url)

      assert file == popup <> "\n\nСобран в конструкторе: #{@url}"
    end

    # Три текста, два разных (постановка 3.180): видимый `<pre>` и
    # «скопировать» оба читают `text/4`'s вывод как есть — если оно вдруг
    # понесёт ссылку само, оба канала тихо получат то, что просил НЕ
    # показывать Dan («выводить ссылку в попапе смысла нет»).
    #
    # ⚠️ Не голое `refute popup =~ "конструкторе"` — подвал уже легально
    # называет слово «конструкторе» своим (нарушения правил отправляют
    # «открой его в конструкторе», строкой выше `footer/3`); нужна именно
    # НАША новая фраза, а не любое её вхождение в текст.
    test "text/4 itself never learns about the link — no url, no new line naming it", ctx do
      popup = Export.text(ctx.build, ctx.ruleset, ctx.stats)

      refute popup =~ @url
      refute popup =~ "Собран в конструкторе"
    end

    test "opts thread straight through to text/4, exactly as text/4 itself would read them",
         ctx do
      hidden_popup = Export.text(ctx.build, ctx.ruleset, ctx.stats, show_granted_feats: false)

      hidden_file =
        Export.file_text(ctx.build, ctx.ruleset, ctx.stats, @url, show_granted_feats: false)

      assert String.starts_with?(hidden_file, hidden_popup)
      refute hidden_file =~ "○"

      shown_file = Export.file_text(ctx.build, ctx.ruleset, ctx.stats, @url)
      assert shown_file =~ "○ Armor proficiency (heavy), Armor proficiency (light)"
    end

    # Подвал (и теперь ссылка под ним) не переключается по ruleset'у —
    # `describe "vanilla — формат гильдии не меняется"` уже сверяет это для
    # `text/4`'s собственного подвала; здесь та же гарантия для новой строки.
    test "vanilla builds get the same trailing link line" do
      ruleset = Data.ruleset!("vanilla")
      build = Build.new(ruleset_version: ruleset.version, levels: [:fighter])
      stats = Rules.compute(build, ruleset)

      file = Export.file_text(build, ruleset, stats, @url)

      assert String.ends_with?(file, "\n\nСобран в конструкторе: #{@url}")
    end

    test "an empty build still gets a file — no class taken is not an error" do
      ruleset = Data.ruleset!()
      ...
      assert file =~ "Собран в конструкторе: #{@url}"
    end
  end
end
