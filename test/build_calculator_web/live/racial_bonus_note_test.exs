defmodule BuildCalculatorWeb.RacialBonusNoteTest do
  ...
  describe "то, что переезд обязан был сохранить" do
    ...
    # 🔴 **Здесь стоял кейс «ниже 40-го — пробел про уровень, и справки нет» —
    # переписан 11.09.2026 (задача 3.181).** Довод был верен, пока величины
    # ниже 40-го не знал никто; теперь она известна ступенями (разбор серверных
    # скриптов шарда), бонус ПОСЧИТАН на любом уровне, и справка обязана быть —
    # иначе игрок не узнает, какой из четырёх вариантов взят.
    #
    # ⚠️ И число в справке — ЭТОГО уровня: 39-й это тир 5, воинский билд
    # (сагровик) получает floor(5 × 3 / 2) = 7. Напечатать здесь +9 было бы
    # обещанием, которого билд не получит.
    test "ниже 40-го справка есть, и число в ней — своего уровня", %{
      conn: conn,
      ruleset: ruleset
    } do
      ...
      note = render(element(view, "#stat-note-racial-bonus"))

      assert note =~ "+7"
      refute note =~ "+9"

      stats = Rules.compute(below, ruleset)

      assert stats.racial_bonus.counted == 7
      refute {:missing_data, {:racial_bonus_level, :half_elf}} in stats.gaps
    end
