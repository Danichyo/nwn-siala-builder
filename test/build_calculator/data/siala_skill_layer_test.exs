defmodule BuildCalculator.Data.SialaSkillLayerTest do
  ...
  describe "what reaches a build's own gap list" do
    ...
    # 🔴 Задача 3.197: у правила С ОБЛАСТЬЮ ДЕЙСТВИЯ обязан быть ответ, где
    # печатается его число. Умолчания у этого вопроса нет сознательно — молчание
    # выбрало бы между двумя разными числами на экране, и ровно так прибавка
    # Spellcraft полтора месяца стояла внутри спаса, которого игра не печатает
    # (`GAME_CHECKS.md` `AQ1`).
    test "правило с областью действия без `in_character_sheet` роняет сборку" do
      root = BuildCalculator.TmpDir.unique_path!()
      File.cp_r!("priv/rules", root)
      on_exit(fn -> File.rm_rf!(root) end)
      ...
      stripped =
        update_in(
          overrides["_vanilla_constants_confirmed"]["skill_save_bonus"]["rules"],
          fn rules -> Enum.map(rules, &Map.delete(&1, "in_character_sheet")) end
        )

      File.write!(path, Jason.encode!(stripped))

      assert_raise RuntimeError, ~r/in_character_sheet/, fn -> Loader.load!(root) end
    end

    # 🔴 Задача 3.202: тот же сторож у второго вопроса — ОТ ЧЕГО считать.
    # «Ранги» и «полное значение навыка» — разные числа у любого билда
    # с ненулевым модификатором характеристики (у Брунны 8 против 12),
    # и выбрать молча значит с равной вероятностью показать не то.
    test "правило без `based_on` роняет сборку" do
      root = BuildCalculator.TmpDir.unique_path!()
      File.cp_r!("priv/rules", root)
      on_exit(fn -> File.rm_rf!(root) end)
      ...
      stripped =
        update_in(
          overrides["_vanilla_constants_confirmed"]["skill_save_bonus"]["rules"],
          fn rules -> Enum.map(rules, &Map.delete(&1, "based_on")) end
        )

      File.write!(path, Jason.encode!(stripped))

      assert_raise RuntimeError, ~r/based_on/, fn -> Loader.load!(root) end
    end

    # ⚠️ И у ворот тренировки свой сторож — но только у правила, считающего
    # ОТ ЗНАЧЕНИЯ: у рангового вопрос не встаёт вовсе (без рангов там и числа
    # нет). Вещь, поднявшая значение нетренированного навыка, либо даёт
    # прибавку, либо нет, и это ответ источника, а не наш выбор.
    test "правило от значения без `requires_training` роняет сборку" do
      root = BuildCalculator.TmpDir.unique_path!()
      File.cp_r!("priv/rules", root)
      on_exit(fn -> File.rm_rf!(root) end)
      ...
      stripped =
        update_in(
          ...
          fn rules ->
            Enum.map(
              rules,
              &update_in(&1, ["based_on"], fn b ->
                Map.delete(b, "requires_training")
              end)
            )
          end
        )
      ...
      assert_raise RuntimeError, ~r/requires_training/, fn -> Loader.load!(root) end
    end

    # ⚠️ И обратная сторона того же ключа: снимут признак условности — прибавка
    # вернётся в лист сама, без правки кода. Это и есть проверка на то, что
    # правило читается из данных, а не зашито в ядре (ответ измерен: `AQ1` —
    # прибавки нет в листе, `AR1` 13.09.2026 — её не видит и требование).
    test "`in_character_sheet: true` возвращает прибавку в сам спас" do
      root = BuildCalculator.TmpDir.unique_path!()
      File.cp_r!("priv/rules", root)
      on_exit(fn -> File.rm_rf!(root) end)
        ...
        update_in(
          ...
          fn rules ->
            Enum.map(rules, fn rule ->
              rule
              |> put_in(["in_character_sheet", "value"], true)
              |> put_in(["prerequisite", "counts"], true)
            end)
          end
        )

      File.write!(path, Jason.encode!(flipped))
      ...
      stats = BuildCalculator.Rules.compute(build, ruleset)
      shipped = BuildCalculator.Rules.compute(build, BuildCalculator.Data.ruleset!("siala_41"))

      # Прибавка +4 (20 рангов) снова внутри числа — и ровно на неё же
      # расходится с тем, что считает поставляемый ruleset.
      assert stats.fort - shipped.fort == 4
      assert stats.conditional_save_bonus == %{fort: 0, ref: 0, will: 0}
      assert stats.conditional_save_terms == []
      assert stats.saves_for_prereqs.fort == shipped.saves_for_prereqs.fort + 4

      # ⚠️ Оговорка про сиальское сужение (AOE) остаётся в обоих случаях: она
      # про другую половину области действия.
      assert {:not_modelled, {:save_bonus_scope, :spellcraft}} in stats.gaps
    end
  end
  ...
  describe "the shard's custom systems" do
    ...
    test "all ten arrive with their verdicts", %{siala: s} do
      ...
      # 🔴 Был `"no"` до 11.09.2026 (задача 3.184), и сменился он не правкой
      # вердикта, а правкой РАСЧЁТА: усиление оружейных и расовых бонусов
      # мини-сетами теперь доезжает до чисел (`Rules.MiniSets`). Довод прежнего
      # вердикта («чистая экипировка… территория армори целиком») снялся вместе
      # со своей посылкой: арифметику прислал разбор серверных скриптов, а
      # надетое количество игрок и так вводит руками. Из трёх частей системы
      # не доезжают две — процент к HP (задача 3.186) и прибавка к скорости.
      assert by_id[:mini_sets] == "partial"
    ...
    # ⚠️ Здесь стояло «mini-sets are worth +15…+91 % hit points in game and the
    # calculator does not model them»: половина устарела 11.09.2026 (задача
    # 3.186) — проценты считаются, и верхняя граница у них не 91, а **105**,
    # с обрывом в ноль на одиннадцатой вещи. Ценой решения остался один
    # получатель из трёх, `movement_speed`, и список это по-прежнему называет.
    test "a system that is out of scope still says what it touches in game", %{siala: s} do
      ...
      # ⚠️ Приписано 11.09.2026 вместе с задачей 3.184: усиление падает и
      # на значения навыков (Дисциплина, три навыка магического посоха),
      # а в списке цены решения этого не стояло.
      assert "skill_values" in mini_sets.derived_stats_touched
    end

    # И та половина вердикта, которая ПЕРЕСТАЛА быть ценой решения: усиление
    # оружейных и расовых бонусов считается, и это проверяется вызовом, а не
    # прозой (задача 3.184). ⚠️ Тот же приём, что у `partial` у Сагры строкой
    # выше: билд без кусков и он же с десятью дают разные числа.
    test "the verdict `partial` of mini_sets is a call, not prose", %{siala: s} do
      armed = fn sets ->
      ...
      without = Rules.compute(Build.new(race: :half_elf, levels: levels, gear: armed.([])), s)
      with_sets = Rules.compute(Build.new(race: :half_elf, levels: levels, gear: armed.([10])), s)

      assert without.mini_set_pieces == 0
      assert without.mini_set_attack_bonus == 0
      assert with_sets.mini_set_pieces == 10
      assert with_sets.mini_set_attack_bonus == 6
      assert with_sets.attack_bonus == without.attack_bonus + 6

      # ...и ВТОРАЯ доехавшая часть — процент к HP (задача 3.186): десять
      # кусков это `Ctotal = 10`, то есть +105 %, и число ровно удваивается
      # с точностью до целочисленного усечения.
      assert with_sets.hp == without.hp + div(without.hp * 105, 100)
      assert with_sets.hp_naked == without.hp

      # ⚠️ И ни одной оговорки про HP при этом нет: считаем — молчим. Здесь
      # стояло `assert {:not_modelled, :mini_set_hp_bonus} in with_sets.gaps`
      # («цена решения»), и это была правда ровно до задачи 3.186.
      assert Enum.filter(with_sets.gaps, &match?({_, :mini_set_hp_bonus}, &1)) == []
      assert Enum.filter(with_sets.gaps, &match?({_, :mini_set_hp_table}, &1)) == []
    end
