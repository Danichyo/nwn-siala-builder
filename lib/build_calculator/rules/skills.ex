defmodule BuildCalculator.Rules.Skills do
  ...
  and both are read from `ruleset.skill_rules` rather than written here:
    ...
    * `save_bonus/3` — Spellcraft adds +1 to **every** saving throw per 5 points
      of the skill. Players do not count it, which is why CLAUDE.md §3 asks for
      it to be shown. ⚠ Shown **beside** the saves rather than inside them since
      task 3.197: the bonus is conditional (against spells) and the game's
      character sheet does not carry it at all, so `save_bonus_terms/3` says of
      each rule where its number lands and the summing is the caller's.
      ⚠ **Per 5 points, not per 5 ranks, since task 3.202** («based on fully
      modified ranks (not base ranks)», `fandom:Spellcraft` revid 68572) — which
      is why this rule is the one skill rule that reads `value/4` rather than
      `Build.skill_ranks/3`.
  ...
  @doc """
  What the build's skills add to every saving throw.
  ...
  **that** narrowing is deliberately not modelled (CLAUDE.md §3, Дан's
  decision), so the number is the one for spells at large and the caveat
  travels beside it.

  ⚠ Само же условие «против заклинаний» с 12.09.2026 моделируется: число
  не идёт в лист персонажа, а отдаётся рядом (`Rules.compute/2` →
  `conditional_save_bonus`), потому что игра считает лист без него — замер
  `AQ1`.

  A ruleset with no such rule returns `{0, []}` — absence of a rule is never
  treated as a rule.

  ⚠ **Сумма ВСЕХ правил, и листовых, и условных** (задача 3.197). Кто считает
  сейв, зовёт `save_bonus_terms/3` и спрашивает у каждого терма, идёт ли он
  в лист персонажа: с 12.09.2026 прибавка Spellcraft в базовые Fort/Ref/Will
  листа не входит вовсе, и сложить два разных числа в одно здесь значило бы
  вернуть ровно ту ошибку, ради которой задача делалась.

  🔴 **Считается от ПОЛНОГО ЗНАЧЕНИЯ навыка, а не от купленных рангов** (задача
  3.202, 13.09.2026) — и от какого именно числа, говорит ruleset
  (`rule.based_on`), а не эта функция. «Полное значение» здесь — ровно то,
  что печатает строка навыка (`value/4`): ранги, модификатор характеристики
  (вещевой), прибавка с вещей, `Skill focus` и `Epic skill focus`, расовая
  склонность, расовый бонус шарда, бонус за тип оружия и мини-сеты — после
  капа пула (+50) и капа итога (127), минус штраф надетого.

  ⚠ **ЗДЕСЬ СТОЯЛО обратное** — «считается от КУПЛЕННЫХ РАНГОВ… прибавка,
  вписанная тому же Spellcraft в „Вещах“, на сейвы не влияет **вообще**», —
  и держалось это на словах Dan 09.08.2026 («30 рангов дают +6, значит
  с вещей до потолка сейвов добирается ровно +14»). Посылка не сошлась:
  по первоисточнику (`GAME_CHECKS.md`, кейс F6) та фраза отвечала на вопрос
  про **общий кап +20** — что прибавка навыка и прибавка к сейвам с вещей
  делят один потолок, — и про состав числа, от которого берётся `div`,
  не говорила ничего. `fandom:Spellcraft` (revid 68572, Notes) говорит прямо
  противоположное прежнему чтению: «the saving throw bonus is based on **fully
  modified ranks (not base ranks)**. Hence epic skill focus in spellcraft grants
  a +2 bonus to all saving throws against spells». Плюс слово Dan 13.09.2026:
  «вроде spellcraft с вещей работает» — **неуверенное**, и записано оно так
  же неуверенно (`overrides.json` → `based_on.shard`, `status: unclear`):
  на Сиале правило не замерено и дёшево не замеряется, величины нет ни
  на одном экране игры (кейс `AQ1`).

  ⚠ **Тренировка**: без единого КУПЛЕННОГО ранга прибавки нет вовсе, даже
  если вещь подняла значение навыка до пятидесяти («since spellcraft requires
  training, the penalty will only occur if the player has at least one rank»).
  Ворота стоят там, где сам навык требует тренировки (`trained_only?`
  в `ruleset.skills`), и включает их запись правила (`training_gate`) —
  ни одного имени навыка здесь нет.

  ⚠ **Отрицательное значение источник тоже описывает** («-1 for each -5
  points»), и мы его **не реализуем отдельно**: единственный названный
  источник минуса — песня проклятия, то есть бафф, а баффы не считаются
  вовсе (CLAUDE.md §9). Арифметика при этом уже ведёт себя как цитата —
  `div/2` усекает к нулю, и −6 дало бы −1, — но терм с неположительным
  числом отбрасывается: печатать штраф, источника которого в модели нет,
  значит показать число, которого у игрока не будет.
  """
  @spec save_bonus(Build.t(), map(), non_neg_integer()) :: {integer(), [atom()]}
  def save_bonus(%Build{} = build, ruleset, level) do
    terms = save_bonus_terms(build, ruleset, level)

    {Enum.reduce(terms, 0, &(&1.bonus + &2)), Enum.map(terms, & &1.skill)}
  end

  @typedoc """
  Одно правило навыка, отработавшее на этом билде: сколько дало и **куда это
  число попадает**.

    * `skill` / `bonus` — чей навык и сколько (до потолка, как и всё остальное,
      что приходит в `clip`). ⚠ **Чей навык, а не чьи ранги**: с задачи 3.202
      число берётся от полного значения навыка, и «42 ранга» с «прибавка 12»
      рядом больше не делятся друг на друга — подпись обязана называть
      значение, а не ранги;
    * `in_character_sheet?` — идёт ли прибавка в базовые Fort/Ref/Will, которые
      печатает лист персонажа. У Spellcraft **нет** (замер `AQ1`, Dan
      12.09.2026: «спасы от Spellcraft в листе персонажа не видна»);
    * `scope` — условие, при котором прибавка работает, дословной строкой
      источника (`"spells"`). `nil` значит «безусловна», и это разные вещи:
      правило без условия обязано быть в листе, потому что больше ему быть
      негде. Имени условия ядро не переводит — подпись рисует веб-слой;
    * `counts_for_prereqs?` — засчитывает ли эту прибавку **требование** фита
      по сейву (`Rules.Prereqs`, ключ `save_bonus`).
  """
  @type save_bonus_term :: %{
          skill: atom(),
          bonus: pos_integer(),
          scope: String.t() | nil,
          in_character_sheet?: boolean(),
          counts_for_prereqs?: boolean()
        }

  @doc """
  То же самое по правилам, а не одним числом — задача 3.197.

  Одна прибавка навыка перестала быть одним слагаемым в тот день, когда
  выяснилось, что в лист персонажа она не попадает: «сколько Spellcraft даёт»
  и «сколько из этого видно в Стойкости» — два разных числа, и складывать их
  снова нельзя.

  Правила без прибавки (нет рангов, значения меньше `per_ranks`) не возвращаются
  вовсе: «здесь ноль» и «такого правила нет» отвечают на разные вопросы, а
  читателю здесь нужен первый ответ только затем, чтобы его пропустить.

  ⚠ **Значение навыка берётся у `value/4`, а не складывается здесь второй
  раз** (задача 3.202). Слагаемых у него двенадцать и два потолка, и вторая
  копия этого сложения разошлась бы с первой молча — ровно тем способом,
  каким однажды разошлись два клипа капа (CLAUDE.md §9). Прямое следствие:
  строка навыка на экране и прибавка к спасам не могут показать разные числа,
  потому что число одно.

  ⚠ **Значение может быть НЕИЗВЕСТНО** (`value/4` вернул `total: nil` —
  не назван ключевой характеристики или штраф надетого). Тогда терма нет:
  подставить вместо неизвестного ранги значило бы напечатать правдоподобное
  число вместо честного молчания. Причина при этом не теряется — её уже несёт
  `values/3` тем же билдом (`{:missing_data, {:skill_key_ability, …}}`),
  и `Rules.compute/2` кладёт её в `stats.gaps`.
  """
  @spec save_bonus_terms(Build.t(), map(), non_neg_integer()) :: [save_bonus_term()]
  def save_bonus_terms(%Build{} = build, ruleset, level),
    do: save_bonus_terms(build, ruleset, level, %{})

  @doc """
  То же с уже посчитанными значениями навыков — задача 3.202.

  `Rules.compute/2` считает `values/3` для панели навыков и тем же проходом
  кормит это правило: `%{}` значит «не считал», и тогда значение берётся
  поштучно у `value/4`. Ответ у обеих дорог один и тот же — отличается
  только цена.

  ⚠ Навык, которого в мапе нет, спрашивается у `value/4` **отдельно**, а не
  считается нулём: `values/3` печатает то, во что билд **вложился**
  (см. её доку), а прибавка к спасам — вопрос не про вложение.
  """
  @spec save_bonus_terms(Build.t(), map(), non_neg_integer(), %{atom() => value()}) :: [
          save_bonus_term()
        ]
  def save_bonus_terms(%Build{} = build, ruleset, level, values) do
    for rule <- rules(ruleset).save_bonus,
        trained?(build, ruleset, rule, level),
        amount = rule_amount(build, ruleset, level, rule, values),
        not is_nil(amount),
        bonus = div(amount, rule.per_ranks) * rule.bonus,
        bonus > 0 do
      %{
        skill: rule.skill,
        bonus: bonus,
        scope: Map.get(rule, :scope),
        # Умолчания у обоих признаков решает загрузчик, а не эта строка:
        # `Map.get/3` здесь только для ruleset'а, собранного без слоя навыков
        # (`@no_rules`), у которого правил нет вовсе.
        in_character_sheet?: Map.get(rule, :in_character_sheet, true),
        counts_for_prereqs?: Map.get(rule, :counts_toward_prerequisites, true)
      }
    end
  end

  # От чего считать — говорит правило, а не эта функция. Оба варианта
  # процитированы в данных, и загрузчик не пускает правило без ответа
  # (`Loader.Skills.based_on!/1`): «ранги» и «значение» — разные числа
  # у любого билда с ненулевым модификатором характеристики.
  defp rule_amount(build, _ruleset, level, %{based_on: :ranks} = rule, _values),
    do: Build.skill_ranks(build, rule.skill, level)

  defp rule_amount(build, ruleset, level, %{based_on: :skill_value} = rule, values) do
    case Map.fetch(values, rule.skill) do
      {:ok, %{total: total}} -> total
      :error -> value(build, ruleset, rule.skill, level).total
    end
  end

  # «Нетренированным навыком персонаж не пользуется, значит и прибавки от него
  # не получает»: `fandom:Spellcraft` выводит это из свойства САМОГО НАВЫКА
  # («since spellcraft requires training…»), поэтому признак спрашивается
  # у навыка, а запись правила только говорит, спрашивать ли его вообще.
  #
  # ⚠ Навык без записи в справочнике ворот не получает: «про тренировку никто
  # не высказался» — не то же самое, что «тренировка не нужна», но у такого
  # навыка есть своя причина в `ruleset.gaps`, и называть её второй раз
  # отсюда значило бы отвечать чужим числом на чужой вопрос.
  defp trained?(build, ruleset, %{training_gate: :skill_trained_only} = rule, level) do
    not trained_only?(ruleset, rule.skill) or Build.skill_ranks(build, rule.skill, level) > 0
  end

  defp trained?(_build, _ruleset, _rule, _level), do: true

  defp trained_only?(ruleset, skill) do
    case ruleset.skills do
      %{^skill => %{trained_only?: flag}} -> flag == true
      _ -> false
    end
  end
  ...
  * **запись разметки с `amount.kind: :class_level_sum`** — «пока фит у
  ...
  ## Три поля про потолки, и они говорят разное
    ...
    * `gear_bonus`, `shard_race_bonus`, `weapon_type_bonus` и
      `mini_set_bonus` — **до** потолка, как есть. ⚠ Третье — прибавка за ТИП
      оружия в руках (задача 3.35): своим полем, а не внутри
      `shard_race_bonus`, хотя число у них одно и то же и источник тоже шард.
      Это два независимых терма, они складываются (замер Dan,
      `GAME_CHECKS.md` Q1), и Человек-сагровик с древковым оружием получает
      к Дисциплине +18 и ещё +18. ⚠ Четвёртое — усиление мини-сетами (задача
      3.184), и оно не приносит своего бонуса, а увеличивает два предыдущих
      СУММОЙ: в движке это один эффект, и приписать усиление одному из двух
      значило бы соврать в разборе (`Rules.MiniSets`). Может быть
      отрицательным — тогда это внутренний кап исполнителя, срезавший сумму;
  ...
  **вложился** (CLAUDE.md §6: «Все 28 на каждом из 41 уровня — шум»), а
  ...
  defp sum(known, ruleset) do
    raw =
      known.ranks + known.ability_modifier + known.race_bonus + known.shard_race_bonus +
        known.weapon_type_bonus + known.mini_set_bonus + known.gear_bonus +
  ...
  defp bonus_pool(build, ruleset, skill) do
    ...
    # ⚠ **Четвёртое слагаемое с 11.09.2026 (задача 3.184)** — мини-сеты,
    # и оно единственное, которое не приносит своего бонуса, а увеличивает два
    # первых: `B += floor(B · Nmini / 10)` от их СУММЫ, потому что в движке это
    # один эффект. Отдельным слагаемым, а не внутри них, — иначе разбор назвал
    # бы чужим то, что прибавили сеты, а по термам порознь число вышло бы
    # меньше игрового (`Rules.MiniSets`).
    #
    # 🔴 И тот же клип, что на остальные три: у Дисциплины усиление ограничено
    # сперва СВОИМ капом исполнителя (36), а у трёх навыков магического посоха
    # своего капа нет вовсе, и +50 остаётся единственным потолком — там он
    # кусает по-настоящему (12 без сетов, 24 с десятью кусками, 36 у сагровика).
    mini_sets = MiniSets.skill_bonus(build, ruleset, skill)
    ...
    clip =
      Bonuses.clip(ruleset, :skill_bonus, [
        ...
        {Caps.covers_source?(ruleset, :skill_bonus, :mini_sets), mini_sets},
