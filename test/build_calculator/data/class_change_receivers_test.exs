defmodule BuildCalculator.Data.ClassChangeReceiversTest do
  ...
  **89 фактов производили `{:not_modelled, {:class_change, …}}` до фильтра**.
  ...
  describe "the closed vocabulary in _receivers" do
    ...
    # class group flags, elemental damage resistance. Sixteen names, not
    # fifteen or seventeen — this is the one place that vocabulary is allowed
    # to be invented, and it must match what the constructor and the build
    # page actually print, not a superset or a subset of it.
    #
    # ⚠ `damage_resistance` joined 3.209 (was fifteen names before): the
    # totals section reads it, even though not one fact of THIS FILE
    # (`siala_41/classes.json`) uses it yet — `receivers_in_feat_effects/0`
    # below is the reason a receiver may be real and still absent from
    # `facts()` here.
    test "`our` matches exactly what the constructor and the build page print" do
      assert receivers_our() ==
               MapSet.new(~w(
                 ...
                 class_availability class_group damage_resistance
               ))
    end
  ...
  describe "receiver counts (snapshot — a silent drift here is the bug)" do
    ...
    #
    # ⚠️ `damage_resistance` стал ДЕВЯТЫМ 3.209 — тем же способом: `resist_energy`
    # и `epic_energy_resistance` называют его в `vanilla/feat_effect_receivers.json`,
    # ни один факт `siala_41/classes.json` его не носит, и это законно ровно
    # по тому же доводу, что у предыдущих четырёх.
    test "every declared receiver is either used, or one of the nine known-unused ones" do
      used =
        for {_class, _what, affects} <- facts(),
            receiver <- affects,
            into: MapSet.new(),
    ...
    # Обратная половина: пять имён, которых нет ни на одном факте класса,
    ...
    test "the five receivers no class fact names are the feat-effect layer's" do
